...
local function BuildPagePartyCDs(cat)
    ...
    -- Visible only when at least one mode uses ATTACH
    if posInit and posInit.AddShownPredicate then
        posInit:AddShownPredicate(function()
    ...
    if tooltipInit and tooltipInit.AddShownPredicate then tooltipInit:AddShownPredicate(isNotBarsMode) end
    G(tooltipInit)
    ...
    if rowGapInit and rowGapInit.AddShownPredicate then rowGapInit:AddShownPredicate(isAttachMode) end
    G(rowGapInit)
    ...
    if attachOffXInit and attachOffXInit.AddShownPredicate then attachOffXInit:AddShownPredicate(isAttachMode) end
    G(attachOffXInit)
    ...
    if attachOffYInit and attachOffYInit.AddShownPredicate then attachOffYInit:AddShownPredicate(isAttachMode) end
    G(attachOffYInit)
    ...
    G(MakeCheckbox(cat, "BIT_syncCdShowCC",
        L["CB_SHOW_CC"]  or "Show Crowd Controls",
        function() return BIT.db.syncCdShowCC  ~= false end,
        function(v) BIT.db.syncCdShowCC  = v; bumpCatVer() end))
    ...
    -- ── Category Row Order (Attach mode) ──────────────────────────────
    ...
    G(MakeDropdown(cat, "BIT_syncCdCatRowDMG", L["DD_CAT_ROW_DMG"] or "Offensives — Row", "1",
        function() return tostring(BIT.db.syncCdCatRowDMG or "1") end,
        function(v) BIT.db.syncCdCatRowDMG = v; bumpCatVer() end,
        rowOpts))

    G(MakeDropdown(cat, "BIT_syncCdCatRowDEF", L["DD_CAT_ROW_DEF"] or "Defensives — Row", "2",
        function() return tostring(BIT.db.syncCdCatRowDEF or "2") end,
        function(v) BIT.db.syncCdCatRowDEF = v; bumpCatVer() end,
        rowOpts))

    G(MakeDropdown(cat, "BIT_syncCdCatRowCC", L["DD_CAT_ROW_CC"] or "Crowd Controls — Row", "3",
        function() return tostring(BIT.db.syncCdCatRowCC or "3") end,
        function(v) BIT.db.syncCdCatRowCC = v; bumpCatVer() end,
        rowOpts))
    ...
    local SYNC_CATEGORIES = {
        { key = "DMG", label = "Damage CDs"   },
        { key = "CC",  label = "CC"            },
        { key = "DEF", label = "Defensive CDs" },
    }

    for _, catEntry in ipairs(SYNC_CATEGORIES) do
        local clsMap = catData[catEntry.key]
        if clsMap then
            local innerKey  = "syncSpells_" .. catEntry.key
            local innerSect = CreateSection(layout, catEntry.label, innerKey, false)
            ...
            for _, cls in ipairs(CLASS_ORDER) do
                ...
                if spells and #spells > 0 then
                    ...
                    if clsInit.AddShownPredicate then
                        clsInit:AddShownPredicate(function()
                            return innerSect:IsExpanded()
                        end)
                    end
                    ...
                    for _, s in ipairs(spells) do
                        ...
                        if spellInit and spellInit.AddShownPredicate then
                            spellInit:AddShownPredicate(function()
                                return innerSect:IsExpanded()
                            end)
                        end
                    end
...
local function BuildPageSizeFont(cat)
    ...
    InSection(frameSect, MakeSliderEdit(cat, "BIT_barW", L["SL_WIDTH"], 180, 60, 500, 1,
...
------------------------------------------------------------
-- Register all settings on one main page
------------------------------------------------------------
function BIT.Config:RegisterBlizzardOptions()
    if not Settings or not Settings.RegisterVerticalLayoutCategory then
        return
    end

    local L      = BIT.L
    local cat    = Settings.RegisterVerticalLayoutCategory("BliZzi Interrupts")
    local layout = SettingsPanel:GetLayout(cat)

    -- ── Slash Commands (collapsed) ────────────────────────────────────
    local slashSect = CreateSection(layout, "Slash Commands", "slashCmds", false)
    local function SI(text) InSection(slashSect, AddInfo(layout, text)) end
    SI("|cFFFFD700/blizzi|r  |cFFAAAAAA·|r  |cFFFFD700/bitset|r  |cFFAAAAAA·|r  |cFFFFD700/bliset|r  |cFFAAAAAA·|r  |cFFFFD700/interrupts|r  — opens settings")
    SI("|cFFFFD700/bitrotation|r — opens the kick rotation editor")
    SI("|cFFFFD700/bitprofile|r — opens the import/export panel")
    SI("|cFFFFD700/bittest|r   — toggles test mode")
    SI("|cFFFFD700/bitdebug|r  — toggles debug logging")

    -- ── General (expanded) ────────────────────────────────────────────
    local genSect = CreateSection(layout, "General", "general", true)
    local function G(init) InSection(genSect, init) end
    ...
    G(MakeDropdown(cat, "BIT_colorMode", L["SEC_BAR_COLOR"], "class",
        ...
        function(v) BIT.db.useClassColors = (v == "class"); BIT.UI:UpdateDisplay(); if BIT.SyncCD and BIT.SyncCD.UpdateColors then BIT.SyncCD:UpdateColors() end end,
        function()
            local c = Settings.CreateControlTextContainer()
    ...
    G(MakeSlider(cat, "BIT_alpha", L["SL_OPACITY"], 100,
        10, 100, 5,
        function(v) return v .. "%" end,
    ...
    G(MakeDropdown(cat, "BIT_language", L["SEC_LANGUAGE"], "auto",
        function() return BIT.db.language or "auto" end,
        function(v)
            BIT.db.language = v
            BIT:ApplyLocale()
            print(BIT.L["MSG_LANG_CHANGED"])
        end,
        function()
            local c = Settings.CreateControlTextContainer()
    ...
    -- ── Click Announce (collapsed) ────────────────────────────────────
    local annSect = CreateSection(layout, "Click Announce", "clickAnnounce", false)
    InSection(annSect, MakeCheckbox(cat, "BIT_clickAnnounce", L["CB_CLICK_ANNOUNCE"],
        function() return BIT.db.clickToAnnounce end,
        function(v) BIT.db.clickToAnnounce = v; BIT.UI:ApplyClickThrough() end))
    InSection(annSect, MakeCheckbox(cat, "BIT_syncCdClickAnnounce", L["CB_SYNC_CD_CLICK_ANNOUNCE"],
        function() return BIT.db.syncCdClickAnnounce end,
        function(v) BIT.db.syncCdClickAnnounce = v end))
    InSection(annSect, MakeDropdown(cat, "BIT_channel", L["SEC_ANNOUNCE_CHANNEL"], "PARTY",
        function() return BIT.db.announceChannel or "PARTY" end,
        function(v) BIT.db.announceChannel = v end,
        function()
            local c = Settings.CreateControlTextContainer()
            c:Add("PARTY", L["ANNOUNCE_PARTY"])
            c:Add("SAY",   L["ANNOUNCE_SAY"])
            c:Add("YELL",  L["ANNOUNCE_YELL"])
            return c:GetData()
        end))
    InSection(annSect, MakeCheckbox(cat, "BIT_antiSpam", L["CB_ANTI_SPAM"],
        function() return BIT.db.antiSpam end,
        function(v) BIT.db.antiSpam = v end))

    -- ── Visibility (collapsed) ────────────────────────────────────────
    local visSect = CreateSection(layout, "Visibility", "visibility", false)
    ...
    -- ── Sounds (collapsed) ────────────────────────────────────────────
    local sndSect = CreateSection(layout, L["PANEL_SOUNDS"], "sounds", false)
    ...
    -- ── Spell Filters (collapsed) ─────────────────────────────────────
    local spellSect = CreateSection(layout, "Spell Filters", "spellFilters", false)
    local function Sp(init) InSection(spellSect, init) end
    ...
    local INTERRUPT_BY_CLASS = {
        ...
        { class = "DEMONHUNTER", name = "Demon Hunter", spells = {
            { id = 183752, label = "Disrupt"                },
            { id = 202137, label = "Sigil of Silence"       },
        }},
        { class = "DRUID",       name = "Druid",        spells = {
            { id = 106839, label = "Skull Bash"             },
            { id = 78675,  label = "Solar Beam"             },
        }},
        { class = "EVOKER",      name = "Evoker",       spells = {
            { id = 351338, label = "Quell"                  },
        }},
        { class = "HUNTER",      name = "Hunter",       spells = {
            { id = 147362, label = "Counter Shot"           },
            { id = 187707, label = "Muzzle"                 },
        }},
        { class = "MAGE",        name = "Mage",         spells = {
            { id = 2139,   label = "Counterspell"           },
        }},
        { class = "MONK",        name = "Monk",         spells = {
            { id = 116705, label = "Spear Hand Strike"      },
        }},
        { class = "PALADIN",     name = "Paladin",      spells = {
            { id = 96231,  label = "Rebuke"                 },
        }},
        { class = "PRIEST",      name = "Priest",       spells = {
            { id = 15487,  label = "Silence"                },
        }},
        { class = "ROGUE",       name = "Rogue",        spells = {
            { id = 1766,   label = "Kick"                   },
        }},
        { class = "SHAMAN",      name = "Shaman",       spells = {
            { id = 57994,  label = "Wind Shear"             },
        }},
        { class = "WARLOCK",     name = "Warlock",      spells = {
            { id = 19647,  label = "Spell Lock"             },
            { id = 132409, label = "Spell Lock (Felhunter)" },
            { id = 119914, label = "Axe Toss (Felguard)"   },
        }},
        { class = "WARRIOR",     name = "Warrior",      spells = {
            { id = 6552,   label = "Pummel"                 },
        }},
    }

    Sp(AddInfo(layout, "|cFF00DDDD" .. (L["SEC_INTERRUPTS"] or "Interrupts") .. "|r"))
    for _, group in ipairs(INTERRUPT_BY_CLASS) do
        local cc  = BIT.CLASS_COLORS and BIT.CLASS_COLORS[group.class]
        local r   = cc and cc[1] or 1
        local g   = cc and cc[2] or 1
        local b   = cc and cc[3] or 1
        local hex = string.format("%02x%02x%02x",
            math.floor(r * 255), math.floor(g * 255), math.floor(b * 255))
        Sp(AddInfo(layout, "|cff" .. hex .. group.name .. "|r"))
        for _, entry in ipairs(group.spells) do
            local icon = C_Spell.GetSpellTexture(entry.id)
            local displayLabel = icon
                and ("|T" .. icon .. ":16:16:0:0:64:64:5:59:5:59|t " .. entry.label)
                or entry.label
            local setting = Settings.RegisterProxySetting(cat,
                "BIT_spell_" .. entry.id,
                Settings.VarType.Boolean,
                displayLabel, true,
                function()
                    return not (BIT.db.disabledSpells and BIT.db.disabledSpells[entry.id])
                end,
                function(v)
                    if not BIT.db.disabledSpells then BIT.db.disabledSpells = {} end
                    if v then
                        BIT.db.disabledSpells[entry.id] = nil
                    else
                        BIT.db.disabledSpells[entry.id] = true
                    end
                    BIT.UI:RebuildBars()
                end)
            Sp(Settings.CreateCheckbox(cat, setting, nil))
        end
    end

    -- ── Links (collapsed) ─────────────────────────────────────────────
    local linksSect = CreateSection(layout, "Links", "links", false)
    InSection(linksSect, AddInfo(layout,
        "|cFF00DDDD" .. (L["LINK_TWITCH_HEADER"] or "Check Out Twitch") .. "|r"))
    InSection(linksSect, AddInfo(layout,
        "|TInterface\\AddOns\\BliZzi_Interrupts\\Media\\twitch:20:20:0:0|t"
        .. "  |cFF9146FFtwitch.tv/BliZzi1337|r"))
    InSection(linksSect, AddInfo(layout,
        "|cFF00DDDD" .. (L["LINK_DISCORD_HEADER"] or "For Support and Community join Discord") .. "|r"))
    InSection(linksSect, AddInfo(layout,
        "|TInterface\\AddOns\\BliZzi_Interrupts\\Media\\discord:20:20:0:0|t"
        .. "  |cFF5865F2discord.gg/BliZzi1337|r"))

    -- ── Size & Font — eigener Sidebar-Eintrag ─────────────────────────
    local sizeCat = Settings.RegisterVerticalLayoutSubcategory(cat, "Size & Font")
    BuildPageSizeFont(sizeCat)

    -- ── Colors — eigener Sidebar-Eintrag ──────────────────────────────
    local colorsCat = Settings.RegisterVerticalLayoutSubcategory(cat, "Colors")
    BuildPageColors(colorsCat)

    -- ── Party CDs — eigener Sidebar-Eintrag ───────────────────────────
    local partyCat = Settings.RegisterVerticalLayoutSubcategory(cat, "Party CDs")
    BuildPagePartyCDs(partyCat)

    Settings.RegisterAddOnCategory(cat)
    mainCatRef = cat
    BIT.Config.mainCat = cat
end

function BIT.Config:Open()
    if mainCatRef then
        Settings.OpenToCategory(mainCatRef:GetID())
    end
end
