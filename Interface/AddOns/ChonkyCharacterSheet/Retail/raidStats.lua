...
local function CreateHeaderRow(raidData, anchor, parent, rowHeight)
	...
	ccsrf_rx:SetSize(840, rowHeight)
	ccsrf_rx:Show()
...
local function CreateBossRow(bossData, anchor, parent, rowHeight, rowCount)
	...
	ccsrf_bx:SetSize(840, rowHeight)
	ccsrf_bx:Show()
...
function module:Initialize()
    if option("showraidprogress") ~= true then return end
	if InCombatLockdown() then CCS.raidupdatedisabled = true return end
    local ccsr_btn = _G["ccsr_btn"] or CreateFrame("Frame", "ccsr_btn", CharacterHandsSlot)
    ...
    if C_AddOns.IsAddOnLoaded("DejaCharacterStats") then
		ccsrf_sf:SetPoint("TOPLEFT", CharacterFrame, "TOPRIGHT", option("hpad"), 0);
		ccsrf_sf:SetPoint("BOTTOMLEFT", CharacterFrame, "BOTTOMRIGHT", option("hpad"), 0);   
	else
		ccsrf_sf:SetPoint("TOPLEFT", CharacterFrame, "TOPRIGHT", 63+option("hpad"), 0);
		ccsrf_sf:SetPoint("BOTTOMLEFT", CharacterFrame, "BOTTOMRIGHT", 63+option("hpad"), 0);   
	end
    ...
    for i, entry in ipairs(layoutChain) do
        ...
        if i == 1 and entry.isHeader then
            ...
            frame:SetPoint("TOPLEFT", ccsrf_sf, "TOPLEFT", 28, -20)
        else
...
function CCS.RaidProgressEventHandler(event, ...)
    ...
    if event == "CCS_EVENT_OPTIONS" and option("showm_sp") == false then
        --ccsrf_sf:Show()
    end
