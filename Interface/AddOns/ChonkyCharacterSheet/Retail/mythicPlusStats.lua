...
local function initializeframes()
	...
	--ccsm_af:SetPoint("TOPLEFT", CharacterFrame, "TOPRIGHT", (60+option("hpad")) * (option("sheetscale") or 1), 0);
	--ccsm_af:SetPoint("BOTTOMLEFT", CharacterFrame, "BOTTOMRIGHT", (60+option("hpad")) * (option("sheetscale") or 1), 0);
	--ccsm_af:SetSize(1, 1)
...
-- 884 x 640 CharacterFrameBg
	--ccsm_sf:ClearAllPoints()
	--ccsm_af:ClearAllPoints()  ---- option("sheetscale") or 1
	--ccsm_sf:SetPoint("BOTTOMLEFT", ccsm_af, "BOTTOMRIGHT", 0, 0); 
...
----
	...
	-- Add default icon texture
	local icon = btn2:CreateTexture(nil, "ARTWORK")
	icon:SetTexture("Interface\\AddOns\\ChonkyCharacterSheet\\Media\\Textures\\rightarrow.png")
	icon:SetAllPoints(btn2)
	btn2.icon = icon -- store reference for swapping

	-- Tooltip and hover behavior
	btn2:SetScript("OnEnter", function(self)
		GameTooltip:SetOwner(self, "ANCHOR_RIGHT", -7, -11)
		GameTooltip_SetTitle(GameTooltip, format(CHALLENGE_COMPLETE_DUNGEON_SCORE, ""))
		GameTooltip_AddNormalLine(GameTooltip, CLICK_HERE_FOR_MORE_INFO)
		GameTooltip_AddNormalLine(GameTooltip, "\n"..L["CONTROL_CLICK"])
		GameTooltip:Show()

		-- Swap to hover texture
		self.icon:SetTexture("Interface\\AddOns\\ChonkyCharacterSheet\\Media\\Textures\\rightarrow_hi.png")
	end)
