...
-- 0.38 by Jim-Bim
-- - TOC Updates
-- - Updated for new Retail UI
-- 0.37 by Jim-Bim
-- - TOC Updates
-- - Fixed issues with BC
-- 0.36 by Jim-Bim
-- - TOC Updates
-- 0.35 by Jim-Bim
-- - TOC Updates
...
function MapCoordsPlayer_OnUpdate()
	...
	if (_G.WOW_PROJECT_ID == _G.WOW_PROJECT_MAINLINE) then
		for memberFrame in PartyFrame.PartyMemberFramePool:EnumerateActive() do
			...
			if (MapCoords2["portrait party" .. i] == true and memberFrame:IsVisible()) then
				_G["MapCoordsParty" .. i .. "PortraitCoords"]:ClearAllPoints()
				_G["MapCoordsParty" .. i .. "PortraitCoords"]:SetPoint("BOTTOMLEFT", memberFrame, "BOTTOMLEFT", 3, -3)
				local posX, posY = MapPositionToXY("party" .. i)
				if ( posX == 0 and posY == 0 ) then
					_G["MapCoordsParty" .. i .. "PortraitCoords"]:SetText("n/a")
				else
					_G["MapCoordsParty" .. i .. "PortraitCoords"]:SetText(shwcrd(posX).." / "..shwcrd(posY))
				end
			else
				_G["MapCoordsParty" .. i .. "PortraitCoords"]:SetText("")
			end
		end
	else
		...
		for i = 1, 4 do 
			if (MapCoords2["portrait party" .. i] == true and currentPlayers >= i and _G["PartyMemberFrame" .. i]:IsVisible()) then
				_G["MapCoordsParty" .. i .. "PortraitCoords"]:ClearAllPoints()
				_G["MapCoordsParty" .. i .. "PortraitCoords"]:SetPoint("LEFT", _G["PartyMemberFrame" .. i], "BOTTOM", -57, 0)
				local posX, posY = MapPositionToXY("party" .. i)
				if ( posX == 0 and posY == 0 ) then
					_G["MapCoordsParty" .. i .. "PortraitCoords"]:SetText("n/a")
				else
...
function MapCoordsWorldMap_OnUpdate()
	...
	elseif (_G.WOW_PROJECT_ID == _G.WOW_PROJECT_CLASSIC or _G.WOW_PROJECT_BURNING_CRUSADE_CLASSIC) then
		MapCoordsWorldMap:SetPoint("CENTER", WorldMapFrame.BorderFrame, "BOTTOM", 0, 15)
		MapCoordsWorldMap:SetTextColor(GameFontNormal:GetTextColor())
	else
        local offset = 0
        if (C_AddOns.IsAddOnLoaded("WorldQuestTracker") ~= nil) then
...
MapCoordsOptions.name = C_AddOns.GetAddOnMetadata("MapCoords", "Title")
...
local category = Settings.RegisterCanvasLayoutCategory(MapCoordsOptions, MapCoordsOptions.name)
...
local MapCoordsCheckButton1 = CreateFrame("CheckButton", "MapCoordsCheckButton1", MapCoordsOptions, helperFrame)
...
local MapCoordsCheckButton2 = CreateFrame("CheckButton", "MapCoordsCheckButton2", MapCoordsOptions, helperFrame)
...
local MapCoordsCheckButton3 = CreateFrame("CheckButton", "MapCoordsCheckButton3", MapCoordsOptions, helperFrame)
...
local MapCoordsCheckButton4 = CreateFrame("CheckButton", "MapCoordsCheckButton4", MapCoordsOptions, helperFrame)
...
local MapCoordsCheckButton5 = CreateFrame("CheckButton", "MapCoordsCheckButton5", MapCoordsOptions, helperFrame)
...
local MapCoordsCheckButton6= CreateFrame("CheckButton", "MapCoordsCheckButton6", MapCoordsOptions, helperFrame)
...
local MapCoordsCheckButton7 = CreateFrame("CheckButton", "MapCoordsCheckButton7", MapCoordsOptions, helperFrame)
...
local MapCoordsCheckButton8 = CreateFrame("CheckButton", "MapCoordsCheckButton8", MapCoordsOptions, helperFrame)
...
local MapCoordsCheckButton9 = CreateFrame("CheckButton", "MapCoordsCheckButton9", MapCoordsOptions, helperFrame)
