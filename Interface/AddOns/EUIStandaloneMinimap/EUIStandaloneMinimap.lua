...
-------------------------------------------------------------------------------
--  EUIStandaloneMinimap.lua
--  Custom minimap skin and layout for EUICoreStandaloneMinimap.
-------------------------------------------------------------------------------
local ADDON_NAME = ...
if not (EUICoreStandaloneMinimap and EUICoreStandaloneMinimap._ModuleNS) then EUI_CLIENT_BLOCKED = true; return end -- stale-parent guard: a partially updated install (old parent, new child) goes dormant via the line-1 failsafe instead of erroring
EUICoreStandaloneMinimap._ModuleNS[ADDON_NAME] = select(2, ...)  -- LOD options files read this module ns via the registry

local EBS = EUICoreStandaloneMinimap.Lite.NewAddon("EUIStandaloneMinimap")
...
do
    ...
    EUICoreStandaloneMinimap.RegisterBorderDefaults("minimap", {
        ["glow"] = {
            defaultSize = 1,
...
local function GetBorderStyleColor(cfg)
    if cfg.borderUseClassColor and EUICoreStandaloneMinimap.GetClassColor then
        local cc = EUICoreStandaloneMinimap.GetClassColor(select(2, UnitClass("player")))
...
local function LayoutFlyoutButtons()
    ...
    for i, btn in ipairs(buttons) do
        ...
        if icon then
            ...
            if EUICoreStandaloneMinimap.PP then EUICoreStandaloneMinimap.PP.DisablePixelSnap(icon) end
        end
...
local function CreateFlyoutToggle()
    if flyoutToggle then
        ...
        local EG2 = EUICoreStandaloneMinimap.ELLESMERE_GREEN
    ...
    EUICoreStandaloneMinimap.RegAccent({ type = "vertex", obj = norm })
    EUICoreStandaloneMinimap.RegAccent({ type = "vertex", obj = pushed })
    EUICoreStandaloneMinimap.RegAccent({ type = "vertex", obj = hl })
    ...
    btn:SetScript("OnClick", function(self)
        ...
        if EUICoreStandaloneMinimap.HideWidgetTooltip then EUICoreStandaloneMinimap.HideWidgetTooltip(true) end
        ToggleFlyoutPanel()
    end)
    btn:SetScript("OnEnter", function(self)
        if flyoutPanel and flyoutPanel:IsShown() then return end
        if not GetFFD(self).freeMoveJustDragged and EUICoreStandaloneMinimap.ShowWidgetTooltip then
            EUICoreStandaloneMinimap.ShowWidgetTooltip(self, "Addon Buttons", { anchor = EBS._Grow.TT(), scale = GetCustomTooltipScale() })
        end
    end)
...
local function GetMinimapFont()
    local path = EUICoreStandaloneMinimap.GetFontPath and EUICoreStandaloneMinimap.GetFontPath("minimap") or STANDARD_TEXT_FONT
    local flag = EUICoreStandaloneMinimap.GetFontOutlineFlag and EUICoreStandaloneMinimap.GetFontOutlineFlag("minimap") or "OUTLINE, SLUG"
    return path, flag
end

local function ApplyMinimapFont(fs, size)
    local path, flag = GetMinimapFont()
    if EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.PrimeFontShadow then EUICoreStandaloneMinimap.PrimeFontShadow(fs, EUICoreStandaloneMinimap.GetFontUseShadow and EUICoreStandaloneMinimap.GetFontUseShadow("minimap")) end
...
local function RegisterVaultEscClose()
    ...
    if not wrf or not EUICoreStandaloneMinimap.RegisterEscapeClose then return end
    EUICoreStandaloneMinimap.RegisterEscapeClose(wrf)
end
...
local function GetVaultTooltip()
    ...
    title:SetText(EUICoreStandaloneMinimap.L("Great Vault"))
    f._title = title
...
local function ShowVaultTooltip(anchor)
    ...
    local rows = {
        BuildVaultRowData(EUICoreStandaloneMinimap.L("Raids"), raidType, true),
        BuildVaultRowData(EUICoreStandaloneMinimap.L("Mythic+"), dungeonType, false),
        BuildVaultRowData(EUICoreStandaloneMinimap.L("World"), worldType, false),
    }
    ...
    local fontPath = (EUICoreStandaloneMinimap.GetFontPath and EUICoreStandaloneMinimap.GetFontPath("minimap")) or "Fonts\\FRIZQT__.TTF"
    local fontFlags = (EUICoreStandaloneMinimap.GetFontOutlineFlag and EUICoreStandaloneMinimap.GetFontOutlineFlag("minimap")) or ""
...
-------------------------------------------------------------------------------
-- Built from the shared season list (EUICoreStandaloneMinimap.SEASON_PORTALS): one place to update.
local PORTAL_SPELLS, PORTAL_SHORT = {}, {}
for _, e in ipairs(EUICoreStandaloneMinimap.SEASON_PORTALS) do
...
local function CreateMinimapPortalFlyout()
    ...
    for i, spellID in ipairs(PORTAL_SPELLS) do
        ...
        if short then
            local fontPath = (EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.GetFontPath and EUICoreStandaloneMinimap.GetFontPath("minimap")) or "Fonts\\FRIZQT__.TTF"
            ...
            if EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.PrimeFontShadow then EUICoreStandaloneMinimap.PrimeFontShadow(label, true) end
            label:SetFont(fontPath, 8, "OUTLINE")
            label:SetPoint("BOTTOM", btn, "BOTTOM", 0, 2)
            label:SetTextColor(1, 1, 1, 0.9)
            label:SetText((EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.L and EUICoreStandaloneMinimap.L(short)) or short)
        end
    ...
    for i = 1, HS_COUNT do
        ...
        btn:SetScript("OnEnter", function(self)
            ...
            elseif self._hsType == "housing" then
                GameTooltip:AddLine(EUICoreStandaloneMinimap.L("Housing Dashboard"))
            end
            GameTooltip:Show()
        end)
        btn:SetScript("OnLeave", function() GameTooltip:Hide() end)
    ...
    local function ResolveHearthButtons()
        if InCombatLockdown() then return end
        local EUI = EUICoreStandaloneMinimap
        local resolvers = {
    ...
    EUICoreStandaloneMinimap.RegisterEscapeClose(flyout)
...
local function CreatePortalBtn(parent)
    ...
    btn:SetScript("OnEnter", function(self)
        ...
        if EUICoreStandaloneMinimap.ShowWidgetTooltip then EUICoreStandaloneMinimap.ShowWidgetTooltip(self, "M+ Portals", { anchor = EBS._Grow.TT(), scale = GetCustomTooltipScale() }) end
    end)
    btn:SetScript("OnLeave", function(self)
...
local function FTT_FONT()
    return (EUICoreStandaloneMinimap.GetFontPath and EUICoreStandaloneMinimap.GetFontPath("minimap")) or EUICoreStandaloneMinimap.EXPRESSWAY or "Fonts\\FRIZQT__.TTF"
end
...
-- EUICoreStandaloneMinimap.InProtectedInstance(), the canonical EUI guard for taint-sensitive ops.
-- Whispering is suppressed entirely there; invites stay allowed.
local function FTTInProtectedContent()
    return EUICoreStandaloneMinimap.InProtectedInstance and EUICoreStandaloneMinimap.InProtectedInstance() or false
end
...
local function FTTOpenWhisper(charName, bnetName)
    ...
    local blocked
    if EUICoreStandaloneMinimap.IsDevModeActive and EUICoreStandaloneMinimap.IsDevModeActive() then
...
local function GetFTTMenu()
    ...
    EUICoreStandaloneMinimap.MakeBorder(m, 1, 1, 1, 0.15, EUICoreStandaloneMinimap.PanelPP)
...
local function GetFriendsTT()
    ...
    EUICoreStandaloneMinimap.MakeBorder(f, 1, 1, 1, 0.15, EUICoreStandaloneMinimap.PanelPP)
...
function ShowFriendsTooltip(anchor)
    ...
    for si, sec in ipairs(sections) do
        ...
        hdr:SetFont(font, 12, "")
        local ac = EUICoreStandaloneMinimap.ELLESMERE_GREEN or EUICoreStandaloneMinimap._accentColor
        local acHex = ac and format("%02x%02x%02x", (ac.r or 0.05) * 255, (ac.g or 0.82) * 255, (ac.b or 0.62) * 255) or "0cd29f"
        hdr:SetText(EUICoreStandaloneMinimap.L(sec.title) .. " (|cff" .. acHex .. #sec.list .. "|r)")
        hdr:ClearAllPoints()
...
local function ShowCalendarTooltip(anchor, lockoutEntries)
    ...
    title:SetText(EUICoreStandaloneMinimap.L("Calendar"))

    local footerData = {
        { left = EUICoreStandaloneMinimap.L("Server Time"), right = GetServerTimeText() },
...
local function BuildCustomIndicators(minimap)
    ...
    _customIndicators.tracking:SetScript("OnEnter", function(self)
        ...
        if not GetFFD(self).freeMoveJustDragged and EUICoreStandaloneMinimap.ShowWidgetTooltip then
            EUICoreStandaloneMinimap.ShowWidgetTooltip(self, "Tracking", { anchor = EBS._Grow.TT(), scale = GetCustomTooltipScale() })
        end
    end)
    ...
    _customIndicators.calendar:SetScript("OnEnter", function(self)
        ...
        if not (EUICoreStandaloneMinimap.InProtectedInstance and EUICoreStandaloneMinimap.InProtectedInstance()) then
            lockoutEntries = GetCalendarLockoutEntries()
        end
        if lockoutEntries then
            ShowCalendarTooltip(self, lockoutEntries)
        elseif EUICoreStandaloneMinimap.ShowWidgetTooltip then
            EUICoreStandaloneMinimap.ShowWidgetTooltip(self, "Calendar", { anchor = EBS._Grow.TT(), scale = GetCustomTooltipScale() })
        end
    end)
    ...
    _customIndicators.mail:SetScript("OnEnter", function(self)
        ...
        if not GetFFD(self).freeMoveJustDragged and EUICoreStandaloneMinimap.ShowWidgetTooltip then
            EUICoreStandaloneMinimap.ShowWidgetTooltip(self, HAVE_MAIL or "New Mail", { anchor = EBS._Grow.TT(), scale = GetCustomTooltipScale() })
        end
    end)
    ...
    _customIndicators.crafting:SetScript("OnEnter", function(self)
        ...
        if not GetFFD(self).freeMoveJustDragged and EUICoreStandaloneMinimap.ShowWidgetTooltip then
            ...
            EUICoreStandaloneMinimap.ShowWidgetTooltip(self, label, { anchor = EBS._Grow.TT(), scale = GetCustomTooltipScale() })
        end
    end)
    ...
    if _customIndicators.friends._icon then
        ...
        EUICoreStandaloneMinimap.RegAccent({ type = "vertex", obj = _customIndicators.friends._icon })
        -- Apply current accent immediately (initial accent pass already ran)
        local g = EUICoreStandaloneMinimap.ELLESMERE_GREEN
        if g then
...
local function ApplyMinimap()
    ...
    do
        local vis = EUICoreStandaloneMinimap.EvalVisibility and p and EUICoreStandaloneMinimap.EvalVisibility(p)
        if not EUICoreStandaloneMinimap.EvalVisibility or vis == true then
            minimap:SetAlpha(1)
            minimap:Show()
    ...
    if p.shape == "square" then
        ...
        host:SetAlpha(1)
        EUICoreStandaloneMinimap.ApplyBorderStyle(host, bs, r, g, b, borderA,
    ...
    if p.useClassColor and not p.borderUseClassColor then
        if not GetFFD(minimap).accentBorderCB then
            GetFFD(minimap).accentBorderCB = function(ar, ag, ab)
                ...
                if host and host:IsShown() then
                    EUICoreStandaloneMinimap.SetBorderStyleColor(host, ar, ag, ab, 1)
                end
        ...
        EUICoreStandaloneMinimap.RegAccent({ type = "callback", fn = GetFFD(minimap).accentBorderCB })
    end
            ...
            local function CheckHousing()
                ...
                if housingAtlas then
                    if frame._isCircle then
                        ...
                        tex:SetTexture("Interface\\AddOns\\EUIStandaloneMinimap\\media\\housing-minimap.png")
                    end
                    frame:Show()
                else
                    frame:Hide()
        ...
        if not clockBg._euiHoverHooked then
            ...
            clockBg:SetScript("OnEnter", function(self)
                ...
                if mode == "lockouts" then
                    if EUICoreStandaloneMinimap.InProtectedInstance and EUICoreStandaloneMinimap.InProtectedInstance() then return end
...
-------------------------------------------------------------------------------
--  Visibility (registered with the shared EUICoreStandaloneMinimap visibility dispatcher)
-------------------------------------------------------------------------------
...
local function MinimapDriverString(p, vm)
    if vm then
        return EUICoreStandaloneMinimap.BuildVisibilityDriverString
            and EUICoreStandaloneMinimap.BuildVisibilityDriverString("", vm)
    end
...
local function UpdateMinimapVisibility()
    ...
    local vm = EUICoreStandaloneMinimap.GetActiveVisibilityModes
        and EUICoreStandaloneMinimap.GetActiveVisibilityModes(p, "visibility")
    ...
    local want = EUICoreStandaloneMinimap.VisDependsOnCombat
        and EUICoreStandaloneMinimap.VisDependsOnCombat(p, "visibility")
    ...
    if InCombatLockdown() then return end
    local vis = EUICoreStandaloneMinimap.EvalVisibility(p)
...
ApplyAll = function()
    ApplyMinimap()
    if EUICoreStandaloneMinimap.RequestVisibilityUpdate then
        C_Timer.After(0, EUICoreStandaloneMinimap.RequestVisibilityUpdate)
    end
end
...
do
    ...
    local function CreateMenuFrame()
        menuFrame = CreateFrame("Frame", "EUICoreStandaloneMinimapMicroMenu", UIParent, "BackdropTemplate")
        ...
        for _, item in ipairs(menuItems) do
            ...
            elseif item.fn then
                ...
                if EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.PrimeFontShadow then EUICoreStandaloneMinimap.PrimeFontShadow(label, true) end
                label:SetFont((EUICoreStandaloneMinimap.GetFontPath and EUICoreStandaloneMinimap.GetFontPath("minimap"))
                    or "Fonts\\FRIZQT__.TTF", 11, "")
                label:SetPoint("LEFT", btn, "LEFT", 10, 0)
                label:SetTextColor(0.9, 0.9, 0.9)
                label:SetText(EUICoreStandaloneMinimap.L(item.text))
...
function EBS:OnInitialize()
    EBS.db = EUICoreStandaloneMinimap.Lite.NewDB("EUIStandaloneMinimapDB", defaults)
    ...
    if EUICoreStandaloneMinimap.RegisterVisibilityUpdater then
        EUICoreStandaloneMinimap.RegisterVisibilityUpdater(UpdateMinimapVisibility)
    end
    if EUICoreStandaloneMinimap.RegisterMouseoverTarget and Minimap then
        EUICoreStandaloneMinimap.RegisterMouseoverTarget(Minimap, function()
            ...
            return EUICoreStandaloneMinimap.VisWantsMouseover(p, "visibility")
        end)
    end
end
...
function EBS:OnEnable()
    ...
    if EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.RegisterUnlockElements then
        local MK = EUICoreStandaloneMinimap.MakeUnlockElement
        local function MDB() return EBS.db and EBS.db.profile.minimap end
        EUICoreStandaloneMinimap:RegisterUnlockElements({
            MK({
                key   = "EBS_Minimap",
                label = "Minimap",
                group = "Minimap",
                ...
                savePos = function(_, point, relPoint, x, y)
                    ...
                    if not EUICoreStandaloneMinimap._unlockActive then
                        ApplyMinimap()
                    end
                end,
                loadPos = function()
