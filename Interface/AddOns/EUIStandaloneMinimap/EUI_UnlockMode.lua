...
--  register via EUICoreStandaloneMinimap:RegisterUnlockElements().
-------------------------------------------------------------------------------
local ADDON_NAME, ns = ...
local EAB = ns.EAB  -- may be nil if loaded by a non-ActionBars addon

-------------------------------------------------------------------------------
--  Registration API -- on the EUICoreStandaloneMinimap global so ALL addons share one
--  table regardless of which copy of this file runs.
-------------------------------------------------------------------------------
if not EUICoreStandaloneMinimap._unlockRegisteredElements then
    EUICoreStandaloneMinimap._unlockRegisteredElements = {}
    EUICoreStandaloneMinimap._unlockRegisteredOrder    = {}
    EUICoreStandaloneMinimap._unlockRegistrationDirty  = true
end

if not EUICoreStandaloneMinimap.RegisterUnlockElements then
    ...
    function EUICoreStandaloneMinimap:RegisterUnlockElements(elements, folder)
        ...
        if EUICoreStandaloneMinimap.SpecOverrides_UnlockPokeFlush then
            EUICoreStandaloneMinimap.SpecOverrides_UnlockPokeFlush()
        end
    end
end

if not EUICoreStandaloneMinimap.UnregisterUnlockElement then
    function EUICoreStandaloneMinimap:UnregisterUnlockElement(key)
...
if not EUICoreStandaloneMinimap._unlockModeListeners then
    EUICoreStandaloneMinimap._unlockModeListeners = {}
end

if not EUICoreStandaloneMinimap.RegisterUnlockModeListener then
    function EUICoreStandaloneMinimap:RegisterUnlockModeListener(owner, listener)
    ...
    function EUICoreStandaloneMinimap:UnregisterUnlockModeListener(owner)
        self._unlockModeListeners[owner] = nil
    end

    function EUICoreStandaloneMinimap:IsUnlockModeActive()
        return self._unlockModeSessionActive == true
    end

    function EUICoreStandaloneMinimap:_NotifyUnlockModeListeners(active, closeAction)
...
if EUICoreStandaloneMinimap._unlockModeLoaded then return end
EUICoreStandaloneMinimap._unlockModeLoaded = true
...
-------------------------------------------------------------------------------
if not EUICoreStandaloneMinimap.ReapplyOwnAnchor then
    EUICoreStandaloneMinimap.ReapplyOwnAnchor = function(key)
        if not EUICoreStandaloneMinimapDB or not EUICoreStandaloneMinimapDB.unlockAnchors then return end
        local info = EUICoreStandaloneMinimapDB.unlockAnchors[key]
        if not info or not info.target then return end

        local elems = EUICoreStandaloneMinimap._unlockRegisteredElements
...
-------------------------------------------------------------------------------
if not EUICoreStandaloneMinimap.NotifyElementResized then
    EUICoreStandaloneMinimap.NotifyElementResized = function(key)
        if not EUICoreStandaloneMinimapDB then return end
        -- Skip if anchored (early ReapplyOwnAnchor handles those)
        local anchors = EUICoreStandaloneMinimapDB.unlockAnchors
        ...
        elseif key:sub(1, 4) == "CDM_" then
            ...
            local cdm = EUICoreStandaloneMinimap.Lite and EUICoreStandaloneMinimap.Lite.GetAddon and EUICoreStandaloneMinimap.Lite.GetAddon("EUICoreStandaloneMinimapCooldownManager", true)
            ...
            local eab = EUICoreStandaloneMinimap.Lite and EUICoreStandaloneMinimap.Lite.GetAddon and EUICoreStandaloneMinimap.Lite.GetAddon("EUICoreStandaloneMinimapActionBars", true)
...
if not EUICoreStandaloneMinimap.IsUnlockAnchored then
    EUICoreStandaloneMinimap.IsUnlockAnchored = function(unlockKey)
        if not EUICoreStandaloneMinimapDB or not EUICoreStandaloneMinimapDB.unlockAnchors then return false end
        local ai = EUICoreStandaloneMinimapDB.unlockAnchors[unlockKey]
...
do
    ...
    f:SetScript("OnEvent", function(self)
        ...
        C_Timer.After(1.5, function()
            ...
            if EUICoreStandaloneMinimap._applySavedPositions then return end
            ...
            if EUICoreStandaloneMinimap._applySavedPositions then
                EUICoreStandaloneMinimap._applySavedPositions()
            end
            if EUICoreStandaloneMinimap.ReapplyAllUnlockAnchorsForced then
                EUICoreStandaloneMinimap.ReapplyAllUnlockAnchorsForced()
            end
        end)
    end)
end
...
EUICoreStandaloneMinimap._unlockCoreInit = function()
...
local PP = EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.PP
...
-- On EUICoreStandaloneMinimap to avoid an upvalue in CreateMover (Lua 5.1 limit: 60).
function EUICoreStandaloneMinimap.RepositionBarToMover(barKey)
...
--  Constants
-------------------------------------------------------------------------------
local FONT_PATH   = (EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.GetFontPath and EUICoreStandaloneMinimap.GetFontPath("extras"))
    or "Interface\\AddOns\\EUIStandaloneMinimap\\media\\fonts\\Expressway.TTF"
...
local LOCK_INNER  = "Interface\\AddOns\\EUIStandaloneMinimap\\media\\eui-unlocked-inner-2.png"
local LOCK_OUTER  = "Interface\\AddOns\\EUIStandaloneMinimap\\media\\eui-unlocked-outer-2.png"
local LOCK_TOP    = "Interface\\AddOns\\EUIStandaloneMinimap\\media\\eui-unlocked-top-2.png"
...
local registeredElements = EUICoreStandaloneMinimap._unlockRegisteredElements
local registeredOrder    = EUICoreStandaloneMinimap._unlockRegisteredOrder

local function RebuildRegisteredOrder()
    if not EUICoreStandaloneMinimap._unlockRegistrationDirty then return end
    ...
    EUICoreStandaloneMinimap._unlockRegistrationDirty = false
end
...
function EUICoreStandaloneMinimap.IsUnlockModeActive() return isUnlocked end
...
-- Spec-override banner refresh = EUICoreStandaloneMinimap._unlockRefreshSpecOvMarks (namespace
...
-- the page builds. On EUICoreStandaloneMinimap to dodge CreateMover's 60-upvalue cap.
local function SelectActionBar(key)
    return function()
        ...
        if EUICoreStandaloneMinimap._setActionBarKey then EUICoreStandaloneMinimap._setActionBarKey(key) end
        EUICoreStandaloneMinimap._pendingActionBarSelect = key
    end
end
local function SelectUnitFrame(unit)
    return function()
        -- Direct setter (if init already ran) + pending flag (consumed at page build)
        if EUICoreStandaloneMinimap._setUnitFrameUnit then EUICoreStandaloneMinimap._setUnitFrameUnit(unit) end
        EUICoreStandaloneMinimap._pendingUnitSelect = unit
    end
end
-- Mini-frame pre-select factory (ToT/FoT/Pet/Boss live on "Mini Frames" with
-- their own dropdown). On EUICoreStandaloneMinimap, not file-local, to avoid a local in this limit-sensitive deferred function.
EUICoreStandaloneMinimap._SelectMiniUnit = function(unit)
    return function()
        if EUICoreStandaloneMinimap._setMiniUnit then EUICoreStandaloneMinimap._setMiniUnit(unit) end
        EUICoreStandaloneMinimap._pendingMiniSelect = unit
    end
end
...
EUICoreStandaloneMinimap._elemMapPre = EUICoreStandaloneMinimap._ELEMENT_SETTINGS_MAP
EUICoreStandaloneMinimap._ELEMENT_SETTINGS_MAP = {
    -- Main frames: "Main Frames" page; dropdown pre-selected to the unit.
    ["player"]       = { module = "EUICoreStandaloneMinimapUnitFrames",       page = "Main Frames",   sectionName = "HEALTH BAR",       preSelectFn = SelectUnitFrame("player"),                   highlightText = "Bar Height" },
    ["target"]       = { module = "EUICoreStandaloneMinimapUnitFrames",       page = "Main Frames",   sectionName = "HEALTH BAR",       preSelectFn = SelectUnitFrame("target"),                   highlightText = "Bar Height" },
    ["focus"]        = { module = "EUICoreStandaloneMinimapUnitFrames",       page = "Main Frames",   sectionName = "HEALTH BAR",       preSelectFn = SelectUnitFrame("focus"),                    highlightText = "Bar Height" },
    -- Mini frames: "Mini Frames" page; mini dropdown pre-selected to the unit.
    ["pet"]          = { module = "EUICoreStandaloneMinimapUnitFrames",       page = "Mini Frames",   sectionName = "HEALTH BAR",       preSelectFn = EUICoreStandaloneMinimap._SelectMiniUnit("pet"),          highlightText = "Bar Height" },
    ["targettarget"] = { module = "EUICoreStandaloneMinimapUnitFrames",       page = "Mini Frames",   sectionName = "HEALTH BAR",       preSelectFn = EUICoreStandaloneMinimap._SelectMiniUnit("targettarget"), highlightText = "Bar Height" },
    ["focustarget"]  = { module = "EUICoreStandaloneMinimapUnitFrames",       page = "Mini Frames",   sectionName = "HEALTH BAR",       preSelectFn = EUICoreStandaloneMinimap._SelectMiniUnit("focustarget"),  highlightText = "Bar Height" },
    -- Boss frames live on their own "Boss Frames" page (no unit dropdown).
    ["boss"]         = { module = "EUICoreStandaloneMinimapUnitFrames",       page = "Boss Frames",   sectionName = "HEALTH BAR",       highlightText = "Bar Height" },
    ["classPower"]   = { module = "EUICoreStandaloneMinimapUnitFrames",       page = "Main Frames",   sectionName = "CLASS RESOURCE",   preSelectFn = SelectUnitFrame("player"),                   highlightText = "Enable Class Resource" },

    -- Unit-frame cast bars (configured on the Main Frames page, CAST BAR section, per selected unit)
    ["playerCastbar"] = { module = "EUICoreStandaloneMinimapUnitFrames",      page = "Main Frames",   sectionName = "CAST BAR",         preSelectFn = SelectUnitFrame("player"),                   highlightText = "Show Cast Bar" },
    ["targetCastbar"] = { module = "EUICoreStandaloneMinimapUnitFrames",      page = "Main Frames",   sectionName = "CAST BAR",         preSelectFn = SelectUnitFrame("target"),                   highlightText = "Show Cast Bar" },
    ["focusCastbar"]  = { module = "EUICoreStandaloneMinimapUnitFrames",      page = "Main Frames",   sectionName = "CAST BAR",         preSelectFn = SelectUnitFrame("focus"),                    highlightText = "Show Cast Bar" },

    -- Resource Bars (no dropdown -- each bar has its own section)
    ["ERB_Health"]        = { module = "EUICoreStandaloneMinimapResourceBars",       page = "Class, Power and Health Bars", sectionName = "HEALTH BAR",           highlightText = "Bar Height" },
    ["ERB_Power"]         = { module = "EUICoreStandaloneMinimapResourceBars",       page = "Class, Power and Health Bars", sectionName = "POWER BAR",            highlightText = "Bar Height" },
    ["ERB_ClassResource"] = { module = "EUICoreStandaloneMinimapResourceBars",       page = "Class, Power and Health Bars", sectionName = "CLASS RESOURCE BAR",   highlightText = "Bar Height" },
    ["ERB_CastBar"]       = { module = "EUICoreStandaloneMinimapResourceBars",       page = "Cast Bar",                     sectionName = "BAR DISPLAY",          highlightText = "Bar Height" },

    -- Action Bars (all share "Bar Display" page; dropdown pre-selected to correct bar)
    ["MainBar"]   = { module = "EUICoreStandaloneMinimapActionBars",          page = "Bar Display",                  sectionName = "LAYOUT",  preSelectFn = SelectActionBar("MainBar"),   highlightText = "Icon Size" },
    ["Bar2"]      = { module = "EUICoreStandaloneMinimapActionBars",          page = "Bar Display",                  sectionName = "LAYOUT",  preSelectFn = SelectActionBar("Bar2"),      highlightText = "Icon Size" },
    ["Bar3"]      = { module = "EUICoreStandaloneMinimapActionBars",          page = "Bar Display",                  sectionName = "LAYOUT",  preSelectFn = SelectActionBar("Bar3"),      highlightText = "Icon Size" },
    ["Bar4"]      = { module = "EUICoreStandaloneMinimapActionBars",          page = "Bar Display",                  sectionName = "LAYOUT",  preSelectFn = SelectActionBar("Bar4"),      highlightText = "Icon Size" },
    ["Bar5"]      = { module = "EUICoreStandaloneMinimapActionBars",          page = "Bar Display",                  sectionName = "LAYOUT",  preSelectFn = SelectActionBar("Bar5"),      highlightText = "Icon Size" },
    ["Bar6"]      = { module = "EUICoreStandaloneMinimapActionBars",          page = "Bar Display",                  sectionName = "LAYOUT",  preSelectFn = SelectActionBar("Bar6"),      highlightText = "Icon Size" },
    ["Bar7"]      = { module = "EUICoreStandaloneMinimapActionBars",          page = "Bar Display",                  sectionName = "LAYOUT",  preSelectFn = SelectActionBar("Bar7"),      highlightText = "Icon Size" },
    ["Bar8"]      = { module = "EUICoreStandaloneMinimapActionBars",          page = "Bar Display",                  sectionName = "LAYOUT",  preSelectFn = SelectActionBar("Bar8"),      highlightText = "Icon Size" },
    ["Bar9"]      = { module = "EUICoreStandaloneMinimapActionBars",          page = "Bar Display",                  sectionName = "LAYOUT",  preSelectFn = SelectActionBar("Bar9"),      highlightText = "Icon Size" },
    ["Bar10"]     = { module = "EUICoreStandaloneMinimapActionBars",          page = "Bar Display",                  sectionName = "LAYOUT",  preSelectFn = SelectActionBar("Bar10"),     highlightText = "Icon Size" },
    ["StanceBar"] = { module = "EUICoreStandaloneMinimapActionBars",          page = "Bar Display",                  sectionName = "LAYOUT",  preSelectFn = SelectActionBar("StanceBar"), highlightText = "Icon Size" },
    ["PetBar"]    = { module = "EUICoreStandaloneMinimapActionBars",          page = "Bar Display",                  sectionName = "LAYOUT",  preSelectFn = SelectActionBar("PetBar"),    highlightText = "Icon Size" },
    ["XPBar"]     = { module = "EUICoreStandaloneMinimapActionBars",          page = "Bar Display",                  sectionName = "LAYOUT",  preSelectFn = SelectActionBar("XPBar"),     highlightText = "Icon Size" },
    ["RepBar"]    = { module = "EUICoreStandaloneMinimapActionBars",          page = "Bar Display",                  sectionName = "LAYOUT",  preSelectFn = SelectActionBar("RepBar"),    highlightText = "Icon Size" },

    -- Action Bars -- visibility-only (dropdown pre-selected, scroll to top)
    ["MicroBar"] = { module = "EUICoreStandaloneMinimapActionBars",          page = "Bar Display",                  sectionName = "GENERAL", preSelectFn = SelectActionBar("MicroBagBars") },
    ["BagBar"]   = { module = "EUICoreStandaloneMinimapActionBars",          page = "Bar Display",                  sectionName = "GENERAL", preSelectFn = SelectActionBar("MicroBagBars") },

    -- Aura Buff Reminders
    ["EABR_Reminders"] = { module = "EUICoreStandaloneMinimapAuraBuffReminders", page = "Auras, Buffs & Consumables", sectionName = "DISPLAY" },

    -- Quality of Life (FPS + Secondary Stats live on the QoL page's EXTRAS section)
    ["EUI_FPS"]            = { module = "EUICoreStandaloneMinimapQoL", page = "QoL", sectionName = "EXTRAS", highlightText = "Show FPS Counter" },
    ["EUI_SecondaryStats"] = { module = "EUICoreStandaloneMinimapQoL", page = "QoL", sectionName = "EXTRAS", highlightText = "Secondary Stat Display" },

    -- Battle Res + Bloodlust (bottom of the Quality of Life page)
    ["EUI_BattleRes"]      = { module = "EUICoreStandaloneMinimapQoL",             page = "QoL",   sectionName = "BATTLE RES",        highlightText = "Enable BattleRes Icon" },
    ["EUI_Bloodlust"]      = { module = "EUICoreStandaloneMinimapQoL",             page = "QoL",   sectionName = "BLOODLUST TRACKER", highlightText = "Enable Bloodlust Icon" },

    -- Mythic+ Tools
    ["EMT_MythicTimer"]    = { module = "EUICoreStandaloneMinimapMythicTimer",     page = "Mythic+ Timer",     sectionName = "DISPLAY",           highlightText = "Scale" },
    ["EMT_TargetedSpellBars"] = { module = "EUICoreStandaloneMinimapMythicTimer",  page = "Targeted Spell Bars", sectionName = "TARGETED SPELL BARS", highlightText = "Enable Targeted Spell Bars" },
    ["EMT_TargetCastBar"]  = { module = "EUICoreStandaloneMinimapMythicTimer",     page = "Target/Focus Bars", sectionName = "TARGET CAST BAR",   highlightText = "Enable Target Cast Bar" },
    ["EMT_FocusCastBar"]   = { module = "EUICoreStandaloneMinimapMythicTimer",     page = "Target/Focus Bars", sectionName = "FOCUS CAST BAR",    highlightText = "Enable Focus Cast Bar" },

    -- Dragon Riding HUD (Blizz UI Enhanced > Dragon Riding page)
    ["EDR_Cluster"]        = { module = "EUICoreStandaloneMinimapBlizzardSkin",    page = "Dragon Riding",     sectionName = "GENERAL",           highlightText = "Enable Dragon Riding Bar" },

    -- Minimap
    ["EBS_Minimap"]        = { module = "EUIStandaloneMinimap",         page = "Minimap",           sectionName = "DISPLAY",           highlightText = "Size" },

    -- Raid + Party Frames (separate registered pages/tabs)
    ["RF_RaidFrames"]      = { module = "EUICoreStandaloneMinimapRaidFrames",      page = "Raid",              sectionName = "FRAME SIZES",       highlightText = "20 Man Frame Width" },
    ["RF_PartyFrames"]     = { module = "EUICoreStandaloneMinimapRaidFrames",      page = "Party",             sectionName = "FRAMES",            highlightText = "Frame Width" },
    ...
    ["CDM_"]               = { module = "EUICoreStandaloneMinimapCooldownManager", page = "CDM Bars" },
    ["TBB_"]               = { module = "EUICoreStandaloneMinimapCooldownManager", page = "Tracking Bars" },
}
if EUICoreStandaloneMinimap._elemMapPre then
    for k, v in pairs(EUICoreStandaloneMinimap._elemMapPre) do
        if EUICoreStandaloneMinimap._ELEMENT_SETTINGS_MAP[k] == nil then
            EUICoreStandaloneMinimap._ELEMENT_SETTINGS_MAP[k] = v
        end
    end
    EUICoreStandaloneMinimap._elemMapPre = nil
end
...
-- Cursor speed tracking for hover intent detection (stored on EUICoreStandaloneMinimap to avoid upvalue pressure)
EUICoreStandaloneMinimap._unlockCursorX     = 0
EUICoreStandaloneMinimap._unlockCursorY     = 0
EUICoreStandaloneMinimap._unlockCursorSpeed = 0   -- pixels/sec at UIParent scale
EUICoreStandaloneMinimap._unlockHoverSpeedThresh = 80 * 80 -- squared px/sec threshold (avoids sqrt each frame)
EUICoreStandaloneMinimap._unlockHoverIntentDelay = 0.12 -- seconds to wait after settling before expanding

-------------------------------------------------------------------------------
--  Anchor / Match DB helpers
--  EUICoreStandaloneMinimapDB.unlockAnchors = { [childKey] = { target=key, side="LEFT"|"RIGHT"|"TOP"|"BOTTOM" } }
...
local function GetBarGrowDirActual(barKey)
    ...
    if barKey == "ERB_TotemBar" then
        if EUICoreStandaloneMinimap.GetTotemGrowDir then return (EUICoreStandaloneMinimap.GetTotemGrowDir()) end
        return "RIGHT"
    end
    if barKey:sub(1, 4) == "CDM_" then
        local rawKey = barKey:sub(5)
        local cdm = EUICoreStandaloneMinimap.Lite.GetAddon("EUICoreStandaloneMinimapCooldownManager", true)
        local cdmBars = cdm and cdm.db and cdm.db.profile and cdm.db.profile.cdmBars
        if cdmBars and cdmBars.bars then
            for _, bar in ipairs(cdmBars.bars) do
                if bar.key == rawKey then
        ...
        local eab = EUICoreStandaloneMinimap.Lite.GetAddon("EUICoreStandaloneMinimapActionBars", true)
...
local function GetBarGrowDir(barKey)
    if barKey == "EQT_Tracker" then return "DOWN" end
    if barKey == "ERB_TotemBar" then
        if not EUICoreStandaloneMinimap.GetTotemGrowDir then return "RIGHT" end
        local g = EUICoreStandaloneMinimap.GetTotemGrowDir()
...
local function GetAnchorDB()
    if not EUICoreStandaloneMinimapDB then return nil end
    if not EUICoreStandaloneMinimapDB.unlockAnchors then
        EUICoreStandaloneMinimapDB.unlockAnchors = {}
    end
    return EUICoreStandaloneMinimapDB.unlockAnchors
end
...
function EUICoreStandaloneMinimap.RecenterBarAnchor(barKey)
    if not isUnlocked then return end
    ...
    local PPr = PP or (EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.PP)
    if PPr then
...
local function SetAnchorInfo(childKey, targetKey, side, offsetX, offsetY)
    ...
    EUICoreStandaloneMinimap._anchorLinksStamp = (EUICoreStandaloneMinimap._anchorLinksStamp or 0) + 1
end
...
function MatchH.GetWidthMatchDB()
    if not EUICoreStandaloneMinimapDB then return nil end
    if not EUICoreStandaloneMinimapDB.unlockWidthMatch then
        EUICoreStandaloneMinimapDB.unlockWidthMatch = {}
    end
    return EUICoreStandaloneMinimapDB.unlockWidthMatch
end


function MatchH.GetHeightMatchDB()
    if not EUICoreStandaloneMinimapDB then return nil end
    if not EUICoreStandaloneMinimapDB.unlockHeightMatch then
        EUICoreStandaloneMinimapDB.unlockHeightMatch = {}
    end
    return EUICoreStandaloneMinimapDB.unlockHeightMatch
end
...
function MatchH.ChainDrivenToBar(barKey, axis)
    ...
    local abKeys = EUICoreStandaloneMinimap._abBarKeys
    local visited = {}
    ...
    if elem and elem.setWidth then
        ...
        if frame then
            ...
            if curW and curW > 0 then
                local PPm = EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.PP
...
function EUICoreStandaloneMinimap._unlockFrameSize(key)
    local f = GetBarFrame(key)
    if not f then return nil end
    return f:GetWidth(), f:GetHeight(), f:GetEffectiveScale()
end

function EUICoreStandaloneMinimap.GetWidthMatchTarget(barKey)
    local db = MatchH.GetWidthMatchDB()
    return db and db[barKey] or nil
end

function EUICoreStandaloneMinimap.GetHeightMatchTarget(barKey)
    local db = MatchH.GetHeightMatchDB()
    return db and db[barKey] or nil
end
...
function EUICoreStandaloneMinimap.MatchGuard(barKey, axis, existingDisabled, existingTooltip)
    local isWidth = (axis == "Width" or axis == "width")
    local getFn = isWidth and EUICoreStandaloneMinimap.GetWidthMatchTarget or EUICoreStandaloneMinimap.GetHeightMatchTarget
    ...
    local tooltip = function()
        ...
        if target then
            local name = (EUICoreStandaloneMinimap.GetBarLabel and EUICoreStandaloneMinimap.GetBarLabel(target)) or target
...
local function PruneStaleLinks(key)
    ...
    local wm = EUICoreStandaloneMinimapDB.unlockWidthMatch
    if wm then
        wm[key] = nil
    ...
    local hm = EUICoreStandaloneMinimapDB.unlockHeightMatch
    if hm then
        hm[key] = nil
...
EUICoreStandaloneMinimap.PruneStaleLinks = PruneStaleLinks
...
-------------------------------------------------------------------------------
function EUICoreStandaloneMinimap.ShiftIndexedAnchorKeys(prefix, removedIdx, oldCount)
    if not EUICoreStandaloneMinimapDB then return end
    ...
    ShiftMatchStore(EUICoreStandaloneMinimapDB.unlockWidthMatch)
    ShiftMatchStore(EUICoreStandaloneMinimapDB.unlockHeightMatch)
end
...
local function ValidateStoredLinks()
    ...
    local activeFn = EUICoreStandaloneMinimap.SpecOverrides_UnlockActive
...
EUICoreStandaloneMinimap._propagatingMatch = false
    ...
    if targetW and targetW > 0 then
        ...
        if sourceElem and sourceElem.setWidth then
            if isUnlocked then
                ...
                local savedAlpha = sb and EUICoreStandaloneMinimap._GetFFD(sb).restoreAlpha
                if sb and not savedAlpha then sb:SetAlpha(0) end
                _propagatingMatch = true; EUICoreStandaloneMinimap._propagatingMatch = true
                pcall(sourceElem.setWidth, sourceKey, targetW)
                _propagatingMatch = false; EUICoreStandaloneMinimap._propagatingMatch = false
                EUICoreStandaloneMinimap.RecenterBarAnchor(sourceKey)
...
local function ScheduleAnchorBatch()
    ...
    C_Timer.After(0, function()
        ...
        local abSkip = EUICoreStandaloneMinimap._abAnchorSuppressed and EUICoreStandaloneMinimap._abBarKeys
        for k, axis in pairs(keys) do
            ...
            if anchorDB then
                ...
                if ownInfo and ownInfo.target then
                    ...
                    local isAB = EUICoreStandaloneMinimap._abBarKeys and EUICoreStandaloneMinimap._abBarKeys[k]
                    if not isAB then
                ...
                local aliasK = EUICoreStandaloneMinimap._unlockKeyAliases and EUICoreStandaloneMinimap._unlockKeyAliases[k]
...
do
    local function MigrateOne(childKey, info)
        ...
        if childKey:sub(1, 4) == "CDM_" then
            local t = EUICoreStandaloneMinimap._cdmBarPositions
            savedEdge = t and t[childKey:sub(5)]
        elseif EUICoreStandaloneMinimap._abBarKeys and EUICoreStandaloneMinimap._abBarKeys[childKey] then
            local t = EUICoreStandaloneMinimap._abBarPositions
    ...
    function EUICoreStandaloneMinimap._MigrateAnchorFollowBaselines()
        ...
        for childKey, info in pairs(adb) do
            if info.target and (childKey:sub(1, 4) == "CDM_"
               or (EUICoreStandaloneMinimap._abBarKeys and EUICoreStandaloneMinimap._abBarKeys[childKey])) then
...
function EUICoreStandaloneMinimap.ScheduleSettleReapply()
    if isUnlocked then return end                            -- unlock owns positioning
    if EUICoreStandaloneMinimap._settleReapplyInProgress then return end  -- never re-arm from our own pass
    ...
    local su = EUICoreStandaloneMinimap._settleSuppressUntil
    if su and GetTime() < su then return end
    if EUICoreStandaloneMinimap._settleTimer then EUICoreStandaloneMinimap._settleTimer:Cancel() end
    EUICoreStandaloneMinimap._settleTimer = C_Timer.NewTimer(0.25, function()
        EUICoreStandaloneMinimap._settleTimer = nil
        ...
        EUICoreStandaloneMinimap._anchorFollowReady = true
        -- One-shot follow-baseline migration for anchored grow bars; no-op once every candidate has its tgt* baseline.
        if EUICoreStandaloneMinimap._MigrateAnchorFollowBaselines then
            pcall(EUICoreStandaloneMinimap._MigrateAnchorFollowBaselines)
        end
        ...
        local wdb = EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.unlockWidthMatch
        local hdb = EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.unlockHeightMatch
        if ((wdb and next(wdb)) or (hdb and next(hdb)))
           and EUICoreStandaloneMinimap.ApplyAllWidthHeightMatches then
            EUICoreStandaloneMinimap._settleReapplyInProgress = true
            EUICoreStandaloneMinimap._settleSuppressUntil = GetTime() + 0.75
            pcall(EUICoreStandaloneMinimap.ApplyAllWidthHeightMatches)
            EUICoreStandaloneMinimap._settleReapplyInProgress = false
        end
        if EUICoreStandaloneMinimap.ReapplyAllUnlockAnchorsForced then
            EUICoreStandaloneMinimap._settleReapplyInProgress = true
            EUICoreStandaloneMinimap._settleSuppressUntil = GetTime() + 0.75
            pcall(EUICoreStandaloneMinimap.ReapplyAllUnlockAnchorsForced)
            EUICoreStandaloneMinimap._settleReapplyInProgress = false
        end
    end)
end

function EUICoreStandaloneMinimap.PropagateWidthMatch(key)
    local db = MatchH.GetWidthMatchDB()
    if not db then return end
...
function EUICoreStandaloneMinimap.PropagateHeightMatch(key)
    local db = MatchH.GetHeightMatchDB()
    if not db then return end
    local visited = { [key] = true }
    local function pushChildren(parentKey)
...
-------------------------------------------------------------------------------
--  Centralized resize notification: EUICoreStandaloneMinimap.NotifyElementResized(key), called
...
-- EUICoreStandaloneMinimap rather than in locals). pending[key] = a re-run is queued;
...
EUICoreStandaloneMinimap._resizeTrail = { pending = {}, at = {} }
...
EUICoreStandaloneMinimap._layoutBarResizing = nil

function EUICoreStandaloneMinimap.NotifyElementResized(key)
    ...
    local layoutBarHandled = (EUICoreStandaloneMinimap._layoutBarResizing == key)
    ...
    local suppressMatchProp = EUICoreStandaloneMinimap._specProfileSwitching
                           or EUICoreStandaloneMinimap._zoneTransitionActive
    ...
    if lastNotify and (now - lastNotify) < RESIZE_THROTTLE_SEC then
        local trail = EUICoreStandaloneMinimap._resizeTrail
        ...
        if not trail.pending[key]
           and (not lastTrail or (now - lastTrail) >= 0.5) then
            ...
            C_Timer.After(RESIZE_THROTTLE_SEC - (now - lastNotify), function()
                ...
                EUICoreStandaloneMinimap.NotifyElementResized(key)
            end)
        end
        return
    end
    ...
    local abSwapSkip = EUICoreStandaloneMinimap._abAnchorSuppressed
        and EUICoreStandaloneMinimap._abBarKeys and EUICoreStandaloneMinimap._abBarKeys[key]
    if not layoutBarHandled and not abSwapSkip then
        ...
        if ownAnchor and ownAnchor.target then
            if EUICoreStandaloneMinimap.ReapplyOwnAnchor then
                EUICoreStandaloneMinimap.ReapplyOwnAnchor(key)
            end
        else
    ...
    if not suppressMatchProp then
        ...
        if wdb then
            ...
            if hasChildren then
                EUICoreStandaloneMinimap.PropagateWidthMatch(key)
            end
        ...
        if hdb then
            ...
            if hasChildren then
                EUICoreStandaloneMinimap.PropagateHeightMatch(key)
            end
    ...
    if axis then
        ...
        if EUICoreStandaloneMinimap.ScheduleSettleReapply then EUICoreStandaloneMinimap.ScheduleSettleReapply() end
    end
end
...
-- EUICoreStandaloneMinimap.PP, shared with EUICoreStandaloneMinimap.lua's PP.SetUIScale trigger, so one
-- listener cancels/replaces the other's pending timer instead of both running a full pass.
do
    local f = CreateFrame("Frame")
    f:RegisterEvent("UI_SCALE_CHANGED")
    f:SetScript("OnEvent", function()
        local PPu = EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.PP
...
local function NotifyElementMoved(key)
    ...
    C_Timer.After(0, function()
        ...
        if EUICoreStandaloneMinimap.PropagateAnchorChain then
            EUICoreStandaloneMinimap.PropagateAnchorChain(key, "all")
        end
    end)
end
...
do
    local origRegister = EUICoreStandaloneMinimap.RegisterUnlockElements
    function EUICoreStandaloneMinimap:RegisterUnlockElements(elements, folder)
...
function EUICoreStandaloneMinimap._BeginFallbackAnchorPick(mover)
...
local function FlashRedBorder(m)
    ...
    if not m._redFlashBrd then
        m._redFlashBrd = EUICoreStandaloneMinimap.MakeBorder(m, 1, 0.2, 0.2, 0)
...
function RejectH.ShowTooltip(text)
    ...
    EUICoreStandaloneMinimap.ShowWidgetTooltip(RejectH._anchor, text, {})
    local elapsed = 0
...
do
    local AnchorPark = {}
    EUICoreStandaloneMinimap._AnchorPark = AnchorPark
        ...
        if not f then
            ...
            f:SetScript("OnEvent", function()
                ...
                if anchorKeys then
                    ...
                    if EUICoreStandaloneMinimap.ReapplyAllUnlockAnchors then
                        EUICoreStandaloneMinimap.ReapplyAllUnlockAnchors()
                    end
                end
            end)
        end
...
do
    ...
    function EUICoreStandaloneMinimap.EligibleFallbackTarget(targetKey)
        return EligibleTarget(targetKey)
    end

    function EUICoreStandaloneMinimap.HasAnchorFallback(childKey)
        local db = GetAnchorDB()
    ...
    EUICoreStandaloneMinimap._FallbackGrowShift = function(childKey, side, cW, cH)
    ...
    function EUICoreStandaloneMinimap._TryFallbackAnchor(childKey, targetKey, childBar, targetBar)
        ...
        if InCombatLockdown() and childBar:IsProtected() then
            EUICoreStandaloneMinimap._AnchorPark.Park(childKey)
            return true
        end
        ...
        if EUICoreStandaloneMinimap._GetAnchorTargetExtent then
            local ext = EUICoreStandaloneMinimap._GetAnchorTargetExtent(fb.target, side)
            if ext then
        ...
        local gsx, gsy = EUICoreStandaloneMinimap._FallbackGrowShift(childKey, side, cW, cH)
        cx = cx + gsx
        cy = cy + gsy
        ...
        if not isUnlocked and EUICoreStandaloneMinimap._GetAnchorTargetShiftDir then
            local dir, extraY = EUICoreStandaloneMinimap._GetAnchorTargetShiftDir(fb.target, childKey)
        ...
        local PPa = PP or (EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.PP)
    ...
    function EUICoreStandaloneMinimap.SetAnchorFallback(childKey, targetKey, side)
        local db = GetAnchorDB()
        local info = db and db[childKey]
        ...
        EUICoreStandaloneMinimap._anchorLinksStamp = (EUICoreStandaloneMinimap._anchorLinksStamp or 0) + 1
        EUICoreStandaloneMinimap._EnsureFallbackWatchers()
        if EUICoreStandaloneMinimap._RefreshFallbackGhosts then EUICoreStandaloneMinimap._RefreshFallbackGhosts() end
        return true
    end

    function EUICoreStandaloneMinimap.ClearAnchorFallback(childKey)
        local db = GetAnchorDB()
        local info = db and db[childKey]
    ...
    function EUICoreStandaloneMinimap._EnsureFallbackWatchers()
        local db = GetAnchorDB()
        if not db then return end
        local needPet = false
    ...
    function EUICoreStandaloneMinimap.NotifyFallbackTargetsChanged()
        local db = GetAnchorDB()
        if not db then return end
        local any = false
        for _, info in pairs(db) do
...
do
    ...
    local function SyncGhost(g)
        ...
        local gsx, gsy = EUICoreStandaloneMinimap._FallbackGrowShift(childKey, fb.side, w or 0, h or 0)
        cx = cx + gsx
        cy = cy + gsy
        ...
        if childBar then
            ...
            local PPg = PP or (EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.PP)
    ...
    local function CreateGhost(childKey)
        ...
        if EUICoreStandaloneMinimap.GetAccentColor then ar, ag, ab = EUICoreStandaloneMinimap.GetAccentColor() end
        ...
        g._brd = EUICoreStandaloneMinimap.MakeBorder(g, wr, wg, wb, 0.6)
        ...
        if EUICoreStandaloneMinimap.PrimeFontShadow then EUICoreStandaloneMinimap.PrimeFontShadow(fs, true) end
        ...
        fs:SetText(EUICoreStandaloneMinimap.L("Fallback") .. ": " .. (GetBarLabel(childKey) or childKey))
        ...
        g:SetScript("OnMouseDown", function(self, btn)
            ...
            if EUICoreStandaloneMinimap._DeselectSelectedMover then EUICoreStandaloneMinimap._DeselectSelectedMover() end
            if EUICoreStandaloneMinimap._DeselectOverrideGhosts then EUICoreStandaloneMinimap._DeselectOverrideGhosts() end
        ...
        g:SetScript("OnMouseUp", function(self, btn)
            ...
            if cx and gl then
                ...
                local dw = (gr - gl) * gs
                local dh = (gt - gb) * gs
                local gsx, gsy = EUICoreStandaloneMinimap._FallbackGrowShift(self._childKey, fb.side, dw, dh)
    ...
    function EUICoreStandaloneMinimap._HideFallbackGhosts()
    ...
    function EUICoreStandaloneMinimap._SetFallbackGhostsAlpha(mult)
    ...
    function EUICoreStandaloneMinimap._DeselectFallbackGhosts()
    ...
    -- namespaced helper.
    function EUICoreStandaloneMinimap._ClearFallbackGhostTempHides()
    ...
    function EUICoreStandaloneMinimap._NudgeSelectedFallbackGhost(dx, dy)
    ...
    function EUICoreStandaloneMinimap._RefreshFallbackGhosts()
        if not isUnlocked or not unlockFrame then
            EUICoreStandaloneMinimap._HideFallbackGhosts()
            return
        end
        local db = GetAnchorDB()
...
do
    ...
    local function Store(create)
        local prof = EUICoreStandaloneMinimap.GetActiveProfileData and EUICoreStandaloneMinimap.GetActiveProfileData()
    ...
    function EUICoreStandaloneMinimap._OverrideAnchorEligible(childKey)
    ...
    function EUICoreStandaloneMinimap._OverrideAnchorGroups()
        local groups = Groups()
        if not groups or #groups == 0 then return nil end
        local prof = EUICoreStandaloneMinimap.GetActiveProfileData()
    ...
    function EUICoreStandaloneMinimap._HasOverrideAnchor(childKey, gid)
        local store = Store()
    ...
    local function ActiveGidFor(childKey)
        ...
        if EUICoreStandaloneMinimap.SpecOverrides_UnlockActive
           and EUICoreStandaloneMinimap.SpecOverrides_UnlockActive() ~= nil then
            return nil
        end
        local specID = EUICoreStandaloneMinimap._specID
        if not specID or specID == 0 then
            if EUICoreStandaloneMinimap._RefreshSpecID then EUICoreStandaloneMinimap._RefreshSpecID() end
    ...
    function EUICoreStandaloneMinimap._TryOverrideAnchor(childKey, childBar)
        if isUnlocked then return false end
        ...
        if EUICoreStandaloneMinimap._GetAnchorTargetExtent then
            local ext = EUICoreStandaloneMinimap._GetAnchorTargetExtent(ov.target, side)
            if ext then
                if side == "TOP" then tT = ext
                elseif side == "BOTTOM" then tB = ext
                elseif side == "LEFT" then tL = ext
        ...
        if EUICoreStandaloneMinimap._GetAnchorTargetShiftDir then
            local dir, extraY = EUICoreStandaloneMinimap._GetAnchorTargetShiftDir(ov.target, childKey)
            if dir ~= 0 then cy = cy + dir * ((tT - tB) + (extraY or 0)) end
        end
        local acRatio = uiS / cS
        local bCenterX = (cx - UIParent:GetWidth() / 2) * acRatio
    ...
    local function RepaintStandard(childKey)
        ...
        if EUICoreStandaloneMinimap.IsUnlockAnchored and EUICoreStandaloneMinimap.IsUnlockAnchored(childKey)
           and EUICoreStandaloneMinimap.ReapplyUnlockAnchor then
            EUICoreStandaloneMinimap.ReapplyUnlockAnchor(childKey)
        end
    end
    ...
    function EUICoreStandaloneMinimap._ReapplyOverrideAnchors()
        local store = Store()
        local released
        ...
        if store then
            for childKey in pairs(store) do
                if EUICoreStandaloneMinimap._TryOverrideAnchor(childKey) and released then
    ...
    function EUICoreStandaloneMinimap._SetOverrideAnchor(childKey, gid, targetKey, side)
        ...
        hasChanges = true
        if EUICoreStandaloneMinimap._RefreshOverrideGhosts then EUICoreStandaloneMinimap._RefreshOverrideGhosts() end
    end
    ...
    function EUICoreStandaloneMinimap._BeginOverrideAnchorPick(mover, gid)
    ...
    function EUICoreStandaloneMinimap._OverridePickGid()
        return ovPickGid
    end

    -- Every stored override target for an element (any group) -- the pick
    -- handler's cycle walk expands through these edges.
    function EUICoreStandaloneMinimap._OverrideAnchorTargets(childKey)
        local store = Store()
        local ent = store and store[childKey]
        if not ent then return nil end
        local out
    ...
    function EUICoreStandaloneMinimap._OverrideAnchorRiders(parentKey)
    ...
    function EUICoreStandaloneMinimap._ClearOverrideAnchor(childKey, gid)
        local store = Store()
        local ent = store and store[childKey]
        if ent then
            ent[gid] = nil
    ...
    local function SyncGhost(g)
        ...
        local gsx, gsy = EUICoreStandaloneMinimap._FallbackGrowShift(g._childKey, ov.side, w or 0, h or 0)
        cx = cx + gsx
        cy = cy + gsy
        -- Run the exact runtime pipeline (child-local conversion + dim-aware
        -- pixel snap) so the ghost previews the landed position to the pixel.
    ...
    local function CreateGhost(childKey, gid)
        ...
        g._brd = EUICoreStandaloneMinimap.MakeBorder(g, OV_R, OV_G, OV_B, 0.6)
        ...
        g:SetScript("OnMouseDown", function(self, btn)
            ...
            if EUICoreStandaloneMinimap._DeselectSelectedMover then EUICoreStandaloneMinimap._DeselectSelectedMover() end
            if EUICoreStandaloneMinimap._DeselectFallbackGhosts then EUICoreStandaloneMinimap._DeselectFallbackGhosts() end
            if selectedGhost and selectedGhost ~= self then
                SetGhostSelected(selectedGhost, false)
            end
            selectedGhost = self
        ...
        g:SetScript("OnMouseUp", function(self, btn)
            ...
            if cx and gl then
                ...
                local dw = (gr - gl) * gs
                local dh = (gt - gb) * gs
                local gsx, gsy = EUICoreStandaloneMinimap._FallbackGrowShift(self._childKey, ov.side, dw, dh)
    ...
    function EUICoreStandaloneMinimap._HideOverrideGhosts()
        for _, g in pairs(ghosts) do HideGhost(g) end
    end

    function EUICoreStandaloneMinimap._SetOverrideGhostsAlpha(mult)
        for _, g in pairs(ghosts) do
            if g:IsShown() then
                g:SetAlpha(GHOST_ALPHA * (mult or 1))
            end
    ...
    function EUICoreStandaloneMinimap._DeselectOverrideGhosts()
        if selectedGhost then
            SetGhostSelected(selectedGhost, false)
            selectedGhost = nil
        end
    ...
    -- ghosts visible again. The ghosts table is a do-block local, hence the
    -- namespaced helper.
    function EUICoreStandaloneMinimap._ClearOverrideGhostTempHides()
        for _, g in pairs(ghosts) do g._tempHidden = nil end
    end
    ...
    function EUICoreStandaloneMinimap._NudgeSelectedOverrideGhost(dx, dy)
        local g = selectedGhost
        if not g or not g:IsShown() then return false end
        local ov = GhostPos(g)
    ...
    function EUICoreStandaloneMinimap._RefreshOverrideGhosts()
        if not isUnlocked or not unlockFrame then
            EUICoreStandaloneMinimap._HideOverrideGhosts()
            return
        end
        local store = Store()
...
EUICoreStandaloneMinimap._unlockCaptureGrowPin = function(childKey, ai, side)
    ...
    local cS = childBar:GetEffectiveScale()
    local tS = targetBar:GetEffectiveScale()
    local snapF = (EUICoreStandaloneMinimap.PP and EUICoreStandaloneMinimap.PP.Snap)
    ...
    local tB = (targetBar:GetBottom() or 0) * tS / uiS
    if EUICoreStandaloneMinimap._GetAnchorTargetExtent then
        local ext = EUICoreStandaloneMinimap._GetAnchorTargetExtent(ai.target, side)
        if ext then
            if side == "TOP" then tT = ext
            elseif side == "BOTTOM" then tB = ext
            elseif side == "LEFT" then tL = ext
...
local function ExtraAnchorOffset(childKey)
    local t = EUICoreStandaloneMinimap._anchorExtraOffset
...
-- EUICoreStandaloneMinimap._GetAnchorTargetShiftDir the apply paths consult dispatches to
...
EUICoreStandaloneMinimap._anchorShiftProviders = EUICoreStandaloneMinimap._anchorShiftProviders or {}
-- pcall-isolated: one provider erroring must not take the OTHER module's
-- anchoring (or unlock entry) down with it.
EUICoreStandaloneMinimap._GetAnchorTargetShiftDir = function(targetKey, childKey)
    local t = EUICoreStandaloneMinimap._anchorShiftProviders
    for i = 1, #t do
...
function EUICoreStandaloneMinimap.AnchorShiftWantsApply()
    local t = EUICoreStandaloneMinimap._anchorShiftProviders
    for i = 1, #t do
        local w = t[i].wants
        if w then
...
function EUICoreStandaloneMinimap.RestoreAnchorShifts()
    local t = EUICoreStandaloneMinimap._anchorShiftProviders
    for i = 1, #t do
        local r = t[i].restore
...
function EUICoreStandaloneMinimap.RunAnchorShiftEnters()
    local t = EUICoreStandaloneMinimap._anchorShiftProviders
    for i = 1, #t do
        local e = t[i].enter
...
ApplyAnchorPosition = function(childKey, targetKey, side, noMark, noMove, fromCascade)
    ...
    if EUICoreStandaloneMinimap._TryOverrideAnchor
       and EUICoreStandaloneMinimap._TryOverrideAnchor(childKey, childBar) then
        return
    end
    ...
    if EUICoreStandaloneMinimap._TryFallbackAnchor
       and EUICoreStandaloneMinimap._TryFallbackAnchor(childKey, targetKey, childBar, targetBar) then
        return
    end
    ...
    if EUICoreStandaloneMinimap._GetAnchorTargetExtent then
        local ext = EUICoreStandaloneMinimap._GetAnchorTargetExtent(targetKey, side)
        if ext then
            if side == "TOP" then tT = ext
            elseif side == "BOTTOM" then tB = ext
            elseif side == "LEFT" then tL = ext
    ...
    if ai and ai.offsetX ~= nil and ai.offsetY ~= nil then
        ...
        if ai and (ai.offsetX == nil or ai.offsetY == nil) then
            local snap = (EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.PP and EUICoreStandaloneMinimap.PP.Snap) or function(v) return math.floor(v + 0.5) end
            local edgeX, edgeY
            if side == "LEFT" then
                edgeX = tL; edgeY = tCY
    ...
    local isCdmOrAB = childKey:sub(1, 4) == "CDM_"
        or (EUICoreStandaloneMinimap._abBarKeys and EUICoreStandaloneMinimap._abBarKeys[childKey])
    ...
    if isCdmOrAB and ai and ai.target and ai.offsetX ~= nil and ai.offsetY ~= nil then
        ...
        if gd and gd ~= "CENTER" then
            ...
            if ai.refFor ~= gd and EUICoreStandaloneMinimap._unlockCaptureGrowPin
               and (EUICoreStandaloneMinimap._settleReapplyInProgress or isUnlocked) then
                EUICoreStandaloneMinimap._unlockCaptureGrowPin(childKey, ai, side)
            end
    ...
    if not growPinned and isCdmOrAB
       and (not isUnlocked or EUICoreStandaloneMinimap._reapplyForceEdgePreserve)
       and (isCDM or childKey == "StanceBar" or not EUICoreStandaloneMinimap._applyingSavedPositions) then
        local growDir = GetBarGrowDirActual(childKey)
        if growDir and growDir ~= "CENTER" then
            ...
            local shiftActive = not isUnlocked
                and EUICoreStandaloneMinimap._GetAnchorTargetShiftDir
                and EUICoreStandaloneMinimap._GetAnchorTargetShiftDir(targetKey, childKey) ~= 0
            ...
            if not skipEdgePreserve then
                ...
                local savedEdge
                if isCDM and EUICoreStandaloneMinimap._cdmBarPositions then
                    local sp = EUICoreStandaloneMinimap._cdmBarPositions[childKey:sub(5)]
                    if sp then savedEdge = sp end
                elseif childKey == "StanceBar" and EUICoreStandaloneMinimap._abBarPositions then
                    local sp = EUICoreStandaloneMinimap._abBarPositions[childKey]
                    if sp then savedEdge = sp end
                end
                ...
                if (isCDM or childKey == "StanceBar") and not shiftActive
                   and not isUnlocked and EUICoreStandaloneMinimap._anchorFollowReady and savedEdge then
                    ...
                    if bTgtx == nil and bTgty == nil then
                        local rt = EUICoreStandaloneMinimap._anchorRtBaseline
                        ...
                        if not b and EUICoreStandaloneMinimap._settleReapplyInProgress then
                            b = { sx = savedEdge.x, sy = savedEdge.y, tgt = targetKey,
                                  tgtx = tCX, tgty = tCY,
                                  tgtL = tL, tgtR = tR, tgtT = tT, tgtB = tB }
                            if not rt then rt = {}; EUICoreStandaloneMinimap._anchorRtBaseline = rt end
                            rt[childKey] = b
                        end
                        if b then
            ...
            elseif hasAnchorTarget and not shiftActive
                   and EUICoreStandaloneMinimap._abBarKeys and EUICoreStandaloneMinimap._abBarKeys[childKey]
                   and childKey ~= "StanceBar" and EUICoreStandaloneMinimap._abBarPositions then
    ...
    if not isUnlocked and EUICoreStandaloneMinimap._GetAnchorTargetShiftDir then
        ...
        local dir, extraY = EUICoreStandaloneMinimap._GetAnchorTargetShiftDir(targetKey, childKey)
        if dir ~= 0 then cy = cy + dir * ((tT - tB) + (extraY or 0)) end
    end
    ...
    if not noMove then
        ...
        if cdmEdgeAnchor then
            ...
            local PPa = EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.PP
    ...
    if m then
        ...
        local PPp = EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.PP
    ...
    if not noMove and EUICoreStandaloneMinimap._unlockActive then
...
PropagateAnchorChain = function(parentKey, visited, changedAxis)
    ...
    local aliasKey = EUICoreStandaloneMinimap._unlockKeyAliases and EUICoreStandaloneMinimap._unlockKeyAliases[parentKey]
            ...
            if primaryMatch and changedAxis == "width" then
                ...
                if dominated then
                    if parentKey:sub(1, 4) == "CDM_"
                       or (EUICoreStandaloneMinimap._abBarKeys and EUICoreStandaloneMinimap._abBarKeys[parentKey]) then
                    ...
                    if dominated and EUICoreStandaloneMinimap._abBarKeys
                       and EUICoreStandaloneMinimap._abBarKeys[childKey] and childKey ~= "StanceBar"
                       and MatchH.ChainDrivenToBar(parentKey, "width") then
                        dominated = false
                    end
                end
    ...
    local ovRiders = EUICoreStandaloneMinimap._OverrideAnchorRiders
        and EUICoreStandaloneMinimap._OverrideAnchorRiders(parentKey)
    if ovRiders then
        for i = 1, #ovRiders do
            local rk = ovRiders[i]
            if not visited[rk] then
                visited[rk] = true
                EUICoreStandaloneMinimap._TryOverrideAnchor(rk)
            end
        end
    end
end
...
EUICoreStandaloneMinimap.PropagateAnchorChain = function(key, changedAxis)
...
EUICoreStandaloneMinimap.IsAnchored = function(key)
    local adb = GetAnchorDB()
...
EUICoreStandaloneMinimap.ReapplyAllUnlockAnchors = function()
    local adb = GetAnchorDB()
    if not adb then return end
...
EUICoreStandaloneMinimap.ReapplyAllUnlockAnchorsForced = function()
    local adb = GetAnchorDB()
    if not adb then return end
        ...
        if info and info.target then
            ...
            if childBar and (targetBar or info.fallback ~= nil)
               ...
               and not (rcElem and rcElem.isAnchored and rcElem.isAnchored(childKey)) then
                ...
                if EUICoreStandaloneMinimap._abBarKeys and EUICoreStandaloneMinimap._abBarKeys[childKey]
...
function EUICoreStandaloneMinimap.GetAnchorTargetCenterUI(childKey)
    local adb = GetAnchorDB()
...
function EUICoreStandaloneMinimap.GetAnchorTargetEdgesUI(childKey)
    local adb = GetAnchorDB()
    local info = adb and adb[childKey]
    if not info or not info.target then return nil end
    local targetBar = GetBarFrame(info.target)
...
EUICoreStandaloneMinimap.ResyncAnchorOffsets = function()
    local adb = GetAnchorDB()
    if not adb then return end
    for childKey, info in pairs(adb) do
...
ApplyCenterPosition = function(barKey, pos)
    ...
    local PPap = EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.PP
    ...
    pcall(function()
        if InCombatLockdown() and frame:IsProtected() then
            ...
            local ffd = EUICoreStandaloneMinimap._GetFFD(frame)
...
-- Expose on EUICoreStandaloneMinimap for child addons
EUICoreStandaloneMinimap.ConvertToCenterPos = ConvertToCenterPos
EUICoreStandaloneMinimap.ApplyCenterPosition = ApplyCenterPosition
...
EUICoreStandaloneMinimap.SaveBarPosition = SaveBarPosition
...
EUICoreStandaloneMinimap.GetBarLabel = GetBarLabel
...
function EUICoreStandaloneMinimap.RefreshUnlockElementLabel(key)
    local m = movers[key]
...
local function ApplySavedPositions()
    EUICoreStandaloneMinimap._applyingSavedPositions = true
    local inCombat = InCombatLockdown()
    ...
    for _, key in ipairs(registeredOrder) do
        ...
        if elem then
            ...
            if EUICoreStandaloneMinimap._TryOverrideAnchor
               and EUICoreStandaloneMinimap._TryOverrideAnchor(key, GetBarFrame(key)) then
    ...
    EUICoreStandaloneMinimap._applyingSavedPositions = false
    EUICoreStandaloneMinimap._abAnchorSuppressed = false
    ...
    if EUICoreStandaloneMinimap._EnsureFallbackWatchers then
        EUICoreStandaloneMinimap._EnsureFallbackWatchers()
    end
...
-- Expose for profile import/switch (called from EUICoreStandaloneMinimap_Profiles.lua)
EUICoreStandaloneMinimap._applySavedPositions = ApplySavedPositions

-- Expose for unlock spec-overrides (position override removal / NIL apply)
EUICoreStandaloneMinimap._UnlockClearSavedPosition = ClearBarPosition
...
EUICoreStandaloneMinimap.ApplyAllWidthHeightMatches = ApplyAllWidthHeightMatches
...
function EUICoreStandaloneMinimap.IsUnlockAnchored(unlockKey)
    local adb = GetAnchorDB()
...
function EUICoreStandaloneMinimap.ReapplyUnlockAnchor(unlockKey)
    local adb = GetAnchorDB()
    local ai = adb and adb[unlockKey]
        ...
        if db and db[barKey] and db[barKey].point then
            ...
            C_Timer.After(0, function()
                if InCombatLockdown() then
                    ...
                    EUICoreStandaloneMinimap._AnchorPark.ParkBarPos(barKey)
                    return
                end
...
if EAB then
    ...
    -- Called by EUICoreStandaloneMinimapActionBars when Blizzard's Edit Mode saves or exits.
...
do
    ...
    ztFrame:SetScript("OnEvent", function(_, event)
        if event == "PLAYER_LEAVING_WORLD" then
            EUICoreStandaloneMinimap._zoneTransitionActive = true
        elseif event == "PLAYER_ENTERING_WORLD" then
            -- Hold the guard 2s after zone-in so CDM/ERB finish rebuilding
            -- with final icon counts.
            C_Timer.After(2, function()
                EUICoreStandaloneMinimap._zoneTransitionActive = false
            end)
        end
    end)
end
...
-------------------------------------------------------------------------------
--  Accent color helper (reads live from EUICoreStandaloneMinimap)
-------------------------------------------------------------------------------
local function GetAccent()
    local eg = EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.ELLESMERE_GREEN
...
local function CreateGrid(parent)
    ...
    local NUM_SEGS       = 5
    local FLASH_PATH = "Interface\\AddOns\\EUIStandaloneMinimap\\media\\unlock-flash.png"
...
local function GetOrCreateSnapBorder(m)
    ...
    local brd = EUICoreStandaloneMinimap.MakeBorder(m, 1, 1, 1, 0)
...
local function ShowVerticalMeasure(idx, xPos, yBot, yTop, dist)
    ...
    f._label:SetText(EUICoreStandaloneMinimap.L(text))
...
EUICoreStandaloneMinimap._snapAxisLocked = function() return lastSnapInfo.lockX, lastSnapInfo.lockY end
...
local function ShowAlignmentGuides(dragKey)
    ...
    local PPg = EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.PP
...
EUICoreStandaloneMinimap._DeselectSelectedMover = DeselectMover
...
EUICoreStandaloneMinimap._unlockNudge = NudgeMover
...
local function SetupArrowKeyFrame()
    ...
    arrowKeyFrame:SetScript("OnKeyDown", function(self, key)
        ...
        if not selectedMover and EUICoreStandaloneMinimap._NudgeSelectedFallbackGhost then
            local PPg = EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.PP
            local gStep = PPg and PPg.mult or 1
            local gs = IsShiftKeyDown() and (100 * gStep) or gStep
            if EUICoreStandaloneMinimap._NudgeSelectedFallbackGhost(dir[1] * gs, dir[2] * gs) then
                self:SetPropagateKeyboardInput(false)
                return
            end
        end
        -- A selected override-anchor ghost answers the same way.
        if not selectedMover and EUICoreStandaloneMinimap._NudgeSelectedOverrideGhost then
            local PPo = EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.PP
            local oStep = PPo and PPo.mult or 1
            local os = IsShiftKeyDown() and (100 * oStep) or oStep
            if EUICoreStandaloneMinimap._NudgeSelectedOverrideGhost(dir[1] * os, dir[2] * os) then
                self:SetPropagateKeyboardInput(false)
                return
            end
        end
        ...
        local PPn = EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.PP
...
local BLIZZ_OWNED_OVERLAY_DEFS = {
    ...
    -- EUICoreStandaloneMinimapChat (position genesis-captured from Edit Mode's last spot,
    ...
    -- tooltip position (fixed anchor in EUICoreStandaloneMinimapBlizzardSkin, a real draggable
    ...
    { label = "Tooltip",       frame = function()
          if not (EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.customTooltips == false) then return nil end
...
local function CreateBlizzOwnedOverlay(def, parent)
    ...
    local brd = EUICoreStandaloneMinimap.MakeBorder(ov, ar, ag, ab, 0.6)
    ov._brd = brd
    -- Label (always visible, same style as mover labels)
    local nameFs = ov:CreateFontString(nil, "OVERLAY")
    if EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.PrimeFontShadow then EUICoreStandaloneMinimap.PrimeFontShadow(nameFs, true) end
    ...
    nameFs:SetText(EUICoreStandaloneMinimap.L(def.label))
    nameFs:SetWordWrap(false)
    ov._nameFs = nameFs
    -- Action text (hidden at idle, fades in on hover)
    local actionFs = ov:CreateFontString(nil, "OVERLAY")
    if EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.PrimeFontShadow then EUICoreStandaloneMinimap.PrimeFontShadow(actionFs, true) end
    ...
    actionFs:SetText(EUICoreStandaloneMinimap.L("Move via Blizz Edit Mode"))
    actionFs:SetAlpha(0)
...
local function CreateMover(barKey)
    ...
    local brd = EUICoreStandaloneMinimap.MakeBorder(mover, ar, ag, ab, 0.6)
    mover._brd = brd
    ...
    if EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.PrimeFontShadow then EUICoreStandaloneMinimap.PrimeFontShadow(nameFS, true) end
    nameFS:SetFont(FONT_PATH, 10 + (UIParent:GetEffectiveScale() < 0.6 and 1 or 0), "")
    nameFS:SetText(EUICoreStandaloneMinimap.L(label))
    nameFS:SetTextColor(1, 1, 1, 0.75)
    nameFS:SetWordWrap(false)
    ...
    if regElem and regElem.subtitle then
        ...
        if EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.PrimeFontShadow then EUICoreStandaloneMinimap.PrimeFontShadow(subFS, true) end
        subFS:SetFont(FONT_PATH, 8 + (UIParent:GetEffectiveScale() < 0.6 and 1 or 0), "")
        subFS:SetText(EUICoreStandaloneMinimap.L(regElem.subtitle))
    ...
    if EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.PrimeFontShadow then EUICoreStandaloneMinimap.PrimeFontShadow(coordFS, true) end
    ...
    if EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.PrimeFontShadow then EUICoreStandaloneMinimap.PrimeFontShadow(wmFS, true) end
    wmFS:SetFont(FONT_PATH, 9, "")
    wmFS:SetTextColor(ar, ag, ab, 0.85)
    wmFS:SetText(EUICoreStandaloneMinimap.L(WM_TEXT))
    wmFS:SetPoint("CENTER")

    local hmFS = hmBtn:CreateFontString(nil, "OVERLAY")
    if EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.PrimeFontShadow then EUICoreStandaloneMinimap.PrimeFontShadow(hmFS, true) end
    hmFS:SetFont(FONT_PATH, 9, "")
    hmFS:SetTextColor(ar, ag, ab, 0.85)
    hmFS:SetText(EUICoreStandaloneMinimap.L(HM_TEXT))
    hmFS:SetPoint("CENTER")

    local atFS = atBtn:CreateFontString(nil, "OVERLAY")
    if EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.PrimeFontShadow then EUICoreStandaloneMinimap.PrimeFontShadow(atFS, true) end
    atFS:SetFont(FONT_PATH, 9, "")
    atFS:SetTextColor(ar, ag, ab, 0.85)
    atFS:SetText(EUICoreStandaloneMinimap.L(AT_TEXT))
    atFS:SetPoint("CENTER")

    local gdFS = gdBtn:CreateFontString(nil, "OVERLAY")
    if EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.PrimeFontShadow then EUICoreStandaloneMinimap.PrimeFontShadow(gdFS, true) end
    gdFS:SetFont(FONT_PATH, 9, "")
    gdFS:SetTextColor(ar, ag, ab, 0.85)
    gdFS:SetText(EUICoreStandaloneMinimap.L(GD_TEXT))
    ...
    if EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.PrimeFontShadow then EUICoreStandaloneMinimap.PrimeFontShadow(pickFS, true) end
    ...
    local function RefreshLinkStates()
        ...
        if wm then
            wmFS:SetText(EUICoreStandaloneMinimap.L("W Matched"))
            wmFS:SetTextColor(1, 0.7, 0.3, 0.85)
        elseif wmBlocked then
            wmFS:SetText(EUICoreStandaloneMinimap.L("W Match"))
        ...
        if hm then
            hmFS:SetText(EUICoreStandaloneMinimap.L("H Matched"))
            hmFS:SetTextColor(1, 0.7, 0.3, 0.85)
        elseif hmBlocked then
            hmFS:SetText(EUICoreStandaloneMinimap.L("H Match"))
        ...
        if ai then
            atFS:SetText(EUICoreStandaloneMinimap.L("Anchored"))
            atFS:SetTextColor(1, 0.7, 0.3, 0.85)
        else
            atFS:SetText(EUICoreStandaloneMinimap.L("Anchor"))
            atFS:SetTextColor(ar, ag, ab, 0.85)
        end
        gdFS:SetText(EUICoreStandaloneMinimap.L("Grow"))
    ...
    local function ShowPickText(text)
        ...
        nameFS:SetAlpha(0)
        pickFS:SetText(EUICoreStandaloneMinimap.L(text))
        pickFS:Show()
    end
    ...
    local function BtnEnter(btn, fs, matchType)
        ...
        if isBlocked then
            EUICoreStandaloneMinimap.ShowWidgetTooltip(btn, "This element doesn't support both Height and Width matching")
            return
        end
        fs:SetTextColor(1, 1, 1, 1)
        ...
        if tipText then
            EUICoreStandaloneMinimap.ShowWidgetTooltip(btn, tipText)
        end
    end
        ...
        if elem and elem.matchUnavailable then
            ...
            if why then
                if EUICoreStandaloneMinimap.ShowWidgetTooltip then
                    EUICoreStandaloneMinimap.ShowWidgetTooltip(wmBtn, why)
                end
                return
            end
        end
    ...
    hmBtn:SetScript("OnClick", function()
        ...
        if elem and elem.matchUnavailable then
            ...
            if why then
                if EUICoreStandaloneMinimap.ShowWidgetTooltip then
                    EUICoreStandaloneMinimap.ShowWidgetTooltip(hmBtn, why)
                end
                return
            end
        end
    ...
    gdBtn:SetScript("OnClick", function()
        ...
        EUICoreStandaloneMinimap.MakeBorder(growDropdownFrame, 1, 1, 1, 0.20)

        local ddY = -4
        local titleFS = growDropdownFrame:CreateFontString(nil, "OVERLAY")
        if EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.PrimeFontShadow then EUICoreStandaloneMinimap.PrimeFontShadow(titleFS, true) end
        ...
        titleFS:SetText(EUICoreStandaloneMinimap.L("Grow Direction"))
        ddY = ddY - 18
        ...
        local isVert = false
        if barKey:sub(1, 4) == "CDM_" then
            local cdm3 = EUICoreStandaloneMinimap.Lite.GetAddon("EUICoreStandaloneMinimapCooldownManager", true)
        ...
        elseif barKey == "ERB_TotemBar" then
            if EUICoreStandaloneMinimap.GetTotemGrowDir then
                local _, v3 = EUICoreStandaloneMinimap.GetTotemGrowDir()
                isVert = v3
            end
        elseif barKey:sub(1, 4) == "PAB_" then
            ...
            local euf3 = EUICoreStandaloneMinimap.Lite.GetAddon("EUICoreStandaloneMinimapUnitFrames", true)
            ...
            local eab3 = EUICoreStandaloneMinimap.Lite.GetAddon("EUICoreStandaloneMinimapActionBars", true)
        ...
        if barKey:sub(1, 4) == "CDM_" then
            local cdm4 = EUICoreStandaloneMinimap.Lite.GetAddon("EUICoreStandaloneMinimapCooldownManager", true)
        ...
        elseif barKey == "ERB_TotemBar" then
            ...
            currentVal = EUICoreStandaloneMinimap.GetTotemGrowDir and EUICoreStandaloneMinimap.GetTotemGrowDir()
                or (isVert and "DOWN" or "RIGHT")
        elseif barKey:sub(1, 4) == "PAB_" then
            local euf4 = EUICoreStandaloneMinimap.Lite.GetAddon("EUICoreStandaloneMinimapUnitFrames", true)
            currentVal = (euf4 and euf4.GetGrowDirectionForBar and euf4:GetGrowDirectionForBar(barKey)) or "LEFT"
        else
            local eab4 = EUICoreStandaloneMinimap.Lite.GetAddon("EUICoreStandaloneMinimapActionBars", true)
        ...
        for _, entry in ipairs(growDirs) do
            ...
            if EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.PrimeFontShadow then EUICoreStandaloneMinimap.PrimeFontShadow(lbl, true) end
            lbl:SetFont(FONT_PATH, 11, "")
            lbl:SetJustifyH("LEFT")
            lbl:SetPoint("LEFT", item, "LEFT", 10, 0)
            lbl:SetText(EUICoreStandaloneMinimap.L(entry.label))
            if isDisabled then
                lbl:SetTextColor(0.4, 0.4, 0.4, 0.5)
                local tipText = "Deselect a grow direction to return to centered"
                item:SetScript("OnEnter", function()
                    EUICoreStandaloneMinimap.ShowWidgetTooltip(item, tipText)
                end)
                ...
                item:SetScript("OnClick", function()
                    ...
                    if barKey:sub(1, 4) == "CDM_" then
                        ...
                        if EUICoreStandaloneMinimap.LayoutCDMBar then
                            EUICoreStandaloneMinimap.LayoutCDMBar(rawKey)
                        end
                        EUICoreStandaloneMinimap.RecenterBarAnchor(barKey)
                    elseif barKey == "ERB_TotemBar" then
                        local erb = EUICoreStandaloneMinimap.Lite.GetAddon("EUICoreStandaloneMinimapResourceBars", true)
                        local tb = erb and erb.db and erb.db.profile and erb.db.profile.totemBar
                        if tb then tb.growDirection = sideVal end
                        if EUICoreStandaloneMinimap.LayoutTotemBar then EUICoreStandaloneMinimap.LayoutTotemBar() end
                        EUICoreStandaloneMinimap.RecenterBarAnchor(barKey)
                    elseif barKey:sub(1, 4) == "PAB_" then
                        local euf = EUICoreStandaloneMinimap.Lite.GetAddon("EUICoreStandaloneMinimapUnitFrames", true)
    ...
    function mover:UpdateCoordText()
        ...
        local bk = self._barKey
        local PPi = EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.PP
    ...
    mover:SetScript("OnDragStart", function(self)
        ...
        if bar0 and not InCombatLockdown() then
            ...
            local PPd = EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.PP
            if PPd and PPd.SnapForES then
                local lockX, lockY = EUICoreStandaloneMinimap._snapAxisLocked()
        ...
        self:SetScript("OnUpdate", function(s)
            ...
            if s._coordFS and s:GetHeight() >= 12 then
                local PPc = EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.PP
    ...
    mover:SetScript("OnDragStop", function(self)
        ...
        if ai then
            ...
            if EUICoreStandaloneMinimap._unlockCaptureGrowPin then
                EUICoreStandaloneMinimap._unlockCaptureGrowPin(self._barKey, ai, ai.side)
            end
        end
    ...
    mover:SetScript("OnEnter", function(self)
        if not self._dragging then
            ...
            if pickModeMover == self and pickMode then
                ...
                local m = self
                C_Timer.After(EUICoreStandaloneMinimap._unlockHoverIntentDelay, function()
                    ...
                    if EUICoreStandaloneMinimap._unlockCursorSpeed > EUICoreStandaloneMinimap._unlockHoverSpeedThresh then
        ...
        if button == "LeftButton" then
            ...
            if pickModeMover and pickModeMover ~= self and pickMode then
                ...
                if pickMode == "widthMatch" then
                    ...
                    EUICoreStandaloneMinimap.PropagateWidthMatch(sourceKey)
                ...
                elseif pickMode == "heightMatch" then
                    ...
                    local ai = GetAnchorInfo(sourceKey)
                    if ai then ApplyAnchorPosition(sourceKey, ai.target, ai.side, true) end
                    EUICoreStandaloneMinimap.PropagateHeightMatch(sourceKey)
                    PropagateAnchorChain(sourceKey)
                    return
                ...
                elseif pickMode == "anchorTo" or pickMode == "fallbackAnchor"
                    ...
                    local ovGid = ovPick and EUICoreStandaloneMinimap._OverridePickGid
                        and EUICoreStandaloneMinimap._OverridePickGid() or nil
                    ...
                    if ovPick then
                        ...
                        while #stack > 0 do
                            ...
                            if not seen[k] then
                                ...
                                local ovT = EUICoreStandaloneMinimap._OverrideAnchorTargets
                                    and EUICoreStandaloneMinimap._OverrideAnchorTargets(k)
                                if ovT then
                    ...
                    ddBg:SetAllPoints()
                    ddBg:SetColorTexture(0.075, 0.113, 0.141, 0.95)
                    EUICoreStandaloneMinimap.MakeBorder(anchorDropdownFrame, 1, 1, 1, 0.20)
                    ...
                    if fbPick then
                        titleFS:SetTextColor(1, 1, 1, 0.40)
                        titleFS:SetText(EUICoreStandaloneMinimap.L("Fallback Direction"))
                        ddY = ddY - 18
                    elseif ovPick then
                        titleFS:SetTextColor(1, 1, 1, 0.40)
                        titleFS:SetText(EUICoreStandaloneMinimap.L("Override Anchor Direction"))
                        ddY = ddY - 18
                    else
                        titleFS:SetTextColor(0.75, 0.75, 0.75, 0.9)
                        titleFS:SetWidth(DD_WIDTH - 20)
                        titleFS:SetWordWrap(true)
                        titleFS:SetText(EUICoreStandaloneMinimap.L("After anchoring, drag to an edge and choose a grow direction to maintain its corner spot as bars change size."))
                    ...
                    for _, sideName in ipairs(sides) do
                        ...
                        lbl:SetJustifyH("LEFT")
                        lbl:SetPoint("LEFT", item, "LEFT", 10, 0)
                        lbl:SetText(EUICoreStandaloneMinimap.Lf(fbPick and "Fallback to %1$s" or "Anchor to %1$s", EUICoreStandaloneMinimap.L(sideName)))
                        item:SetScript("OnEnter", function()
                            hl:SetColorTexture(1, 1, 1, 0.08)
                            lbl:SetTextColor(1, 1, 1, 1)
                        end)
                        ...
                        item:SetScript("OnClick", function()
                            ...
                            if ovPick then
                                ...
                                if EUICoreStandaloneMinimap._SetOverrideAnchor then
                                    EUICoreStandaloneMinimap._SetOverrideAnchor(pmKey, ovGid, targetKey, sideVal)
                                end
                                hasChanges = true
                                return
                            end
                            if fbPick then
                                ...
                                if EUICoreStandaloneMinimap.SetAnchorFallback then
                                    EUICoreStandaloneMinimap.SetAnchorFallback(pmKey, targetKey, sideVal)
                                end
                                hasChanges = true
                                return
                            end
                            ...
                            if pmKey:sub(1, 4) == "CDM_" then
                                ...
                                local cdmAddon = EUICoreStandaloneMinimap.Lite.GetAddon("EUICoreStandaloneMinimapCooldownManager", true)
                    ...
                    if not fbPick and IsAnchored(pmKey) then
                        ...
                        rLbl:SetText(EUICoreStandaloneMinimap.L("Remove Anchor"))
    ...
    ---------------------------------------------------------------------------
    local ICON_PATH = "Interface\\AddOns\\EUIStandaloneMinimap\\media\\icons\\"
    local ARROW_ICON  = ICON_PATH .. "eui-arrow.png"
    local ARROW_RIGHT_ICON = ICON_PATH .. "right-arrow.png"
    local COGS_ICON   = EUICoreStandaloneMinimap.COGS_ICON or (ICON_PATH .. "cogs-3.png")
    ...
    do
        ...
        cogBtn._bg = bg
        local brd = EUICoreStandaloneMinimap.MakeBorder(cogBtn, 1, 1, 1, 0.20)
        cogBtn._brd = brd
    ...
    snapDD._bg = snapDDBg
    local snapDDBrd = EUICoreStandaloneMinimap.MakeBorder(snapDD, 1, 1, 1, 0.20)
    ...
    snapDDLbl:SetText(EUICoreStandaloneMinimap.L("Snap to: Auto"))
    local snapDDArrow = EUICoreStandaloneMinimap.MakeDropdownArrow(snapDD, 12)
    ...
    snapDD:SetScript("OnEnter", function(self)
        if not snapEnabled then
            ...
            EUICoreStandaloneMinimap.ShowWidgetTooltip(self, "This feature requires Snap Elements to be enabled")
            return
        end
        self._bg:SetColorTexture(0.075, 0.113, 0.141, 0.98)
        self._brd:SetColor(1, 1, 1, 0.30)
    ...
    local function UpdateSnapLabel()
        ...
        if tgt == "_disable_" then
            snapDDLbl:SetText(EUICoreStandaloneMinimap.L("Snap to: None"))
        elseif tgt == "_select_" then
            snapDDLbl:SetText(EUICoreStandaloneMinimap.L("Snap to: Select Element"))
        elseif tgt then
            local lbl = GetBarLabel(tgt)
            snapDDLbl:SetText(EUICoreStandaloneMinimap.Lf("Snap to: %1$s", lbl or tgt))
        else
            snapDDLbl:SetText(EUICoreStandaloneMinimap.L("Snap to: All Elements"))
        end
    ...
    local function BuildSnapMenu()
        ...
        EUICoreStandaloneMinimap.MakeBorder(snapMenu, 1, 1, 1, 0.20)
        ...
        titleLbl:SetText(EUICoreStandaloneMinimap.L("Snap Target"))
        yOff = yOff - 18
        ...
        for _, gName in ipairs(regGroupOrder) do
            ...
            rgLbl:SetText(EUICoreStandaloneMinimap.L(gName))
            ...
            local function ShowRegSub()
                ...
                EUICoreStandaloneMinimap.MakeBorder(regSub, 1, 1, 1, 0.20)
                local rsYOff = -4
                for _, eInfo in ipairs(gElems) do
                    ...
                    sLbl:SetText(EUICoreStandaloneMinimap.L(eLbl))
                ...
                local rsMaxW = DD_W
                for _, eInfo in ipairs(gElems) do
                    local tw = (EUICoreStandaloneMinimap.MeasureText and EUICoreStandaloneMinimap.MeasureText(eInfo.label, FONT_PATH, 11)) or 0
    ...
    local function BuildCogMenu()
        ...
        menuBg:SetAllPoints()
        menuBg:SetColorTexture(0.075, 0.113, 0.141, 0.95)
        EUICoreStandaloneMinimap.MakeBorder(cogMenu, 1, 1, 1, 0.20)
        ...
        do
            ...
            if EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.PrimeFontShadow then EUICoreStandaloneMinimap.PrimeFontShadow(hintFS, true) end
            ...
            hintFS:SetText(EUICoreStandaloneMinimap.L("Use arrow keys to move selected element 1px any direction")
                .. ". " .. EUICoreStandaloneMinimap.L("Shift+Right Click to temporarily hide overlay"))
        ...
        local settingsMapping = EUICoreStandaloneMinimap._ELEMENT_SETTINGS_MAP[barKey]
        ...
        if not settingsMapping then
            if barKey:sub(1, 4) == "CDM_" then
                settingsMapping = EUICoreStandaloneMinimap._ELEMENT_SETTINGS_MAP["CDM_"]
            elseif barKey:sub(1, 4) == "TBB_" or barKey:sub(1, 5) == "TBBG_" then
                settingsMapping = EUICoreStandaloneMinimap._ELEMENT_SETTINGS_MAP["TBB_"]
            end
        end
        ...
        if settingsMapping or opensEditMode then
            ...
            if EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.PrimeFontShadow then EUICoreStandaloneMinimap.PrimeFontShadow(optLbl, true) end
            ...
            optLbl:SetText(EUICoreStandaloneMinimap.L("Element Options"))
            ...
            optItem:SetScript("OnClick", function()
                ...
                if opensEditMode then
                    ...
                    ns.RequestClose(true, function()
                        EUICoreStandaloneMinimap:NavigateToElementSettings(
        ...
        if canResize and elem then
            ...
            local function MakeSizeRow(axis, initVal)
                ...
                lbl:SetText((EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.L and EUICoreStandaloneMinimap.L(axis)) or axis)
                ...
                local matchTarget = isWidth and EUICoreStandaloneMinimap.GetWidthMatchTarget(barKey) or (not isWidth and EUICoreStandaloneMinimap.GetHeightMatchTarget(barKey))
                if matchTarget then
                    box:Disable()
                    box:SetTextColor(0.4, 0.4, 0.4, 0.7)
                    local targetName = GetBarLabel(matchTarget) or matchTarget
                    box:SetScript("OnEnter", function()
                        EUICoreStandaloneMinimap.ShowWidgetTooltip(box,
                            axis .. " matched to " .. targetName .. ". Unmatch to edit.")
                    end)
                    box:SetScript("OnLeave", function() EUICoreStandaloneMinimap.HideWidgetTooltip() end)
                end
            ...
            if not isCDMBar and not EUICoreStandaloneMinimap._specialUnlockGroup then
            ...
            do
                ...
                local function MakePosRow(axis, initVal)
                    ...
                    lbl:SetJustifyH("LEFT")
                    lbl:SetPoint("LEFT", rowFrame, "LEFT", 10, 0)
                    lbl:SetText(axis == "X" and EUICoreStandaloneMinimap.L("X Position") or EUICoreStandaloneMinimap.L("Y Position"))
                    ...
                    box:SetScript("OnEnterPressed", function(self)
                        ...
                        if deltaPx ~= 0 then
                            ...
                            if axis == "X" then
                                EUICoreStandaloneMinimap._unlockNudge(stepUnits, 0, mover, true)
                            else
                                EUICoreStandaloneMinimap._unlockNudge(0, stepUnits, mover, true)
                            end
                        end
                        if mover.ReanchorToBar then mover:ReanchorToBar() end
        ...
        if EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.PrimeFontShadow then EUICoreStandaloneMinimap.PrimeFontShadow(selElemLbl, true) end
        ...
        if hasTarget then
            ...
            selElemLbl:SetText(EUICoreStandaloneMinimap.Lf("Snap Target: %1$s", "|cFF0CD29D" .. tgtName .. "|r"))
            selElemLbl:SetTextColor(0.75, 0.75, 0.75, 0.9)
        else
            selElemLbl:SetText(EUICoreStandaloneMinimap.L("Select Snap Target"))
            selElemLbl:SetTextColor(0.75, 0.75, 0.75, 0.9)
        end
        ...
        local function MakeActionItem(text, onClick, hoverFullText)
            ...
            item:SetScript("OnEnter", function()
                ...
                if hoverFullText and EUICoreStandaloneMinimap.ShowWidgetTooltip
                   and lbl:GetStringWidth() > lbl:GetWidth() + 0.5 then
                    EUICoreStandaloneMinimap.ShowWidgetTooltip(item, hoverFullText)
                end
            end)
            item:SetScript("OnLeave", function()
                hl:SetColorTexture(1, 1, 1, 0)
                lbl:SetTextColor(0.75, 0.75, 0.75, 0.9)
                if hoverFullText and EUICoreStandaloneMinimap.HideWidgetTooltip then
                    EUICoreStandaloneMinimap.HideWidgetTooltip()
                end
            end)
            item:SetScript("OnClick", function()
        ...
        MakeActionItem("Center on Screen", function()
            ...
            if curPx and curPx ~= 0 then
                EUICoreStandaloneMinimap._unlockNudge(PPc.FromPixels(-curPx), 0, mover, true)
            end
            if mover.ReanchorToBar then mover:ReanchorToBar() end
        ...
        do
            ...
            local fbEligible = aiF and aiF.target
                and EUICoreStandaloneMinimap.EligibleFallbackTarget
                and EUICoreStandaloneMinimap.EligibleFallbackTarget(aiF.target)
            if fbEligible then
                ...
                MakeActionItem(hasFb and "Fallback Anchor: Change" or "Fallback Anchor: Select", function()
                    if EUICoreStandaloneMinimap._BeginFallbackAnchorPick then
                        EUICoreStandaloneMinimap._BeginFallbackAnchorPick(mover)
                    end
                end)
                if hasFb then
                    MakeActionItem("Fallback Anchor: Clear", function()
                        if EUICoreStandaloneMinimap.ClearAnchorFallback then
                            EUICoreStandaloneMinimap.ClearAnchorFallback(barKey)
                        end
                    end)
                end
            end
        ...
        do
            local ovGroups = EUICoreStandaloneMinimap._OverrideAnchorEligible
                and EUICoreStandaloneMinimap._OverrideAnchorEligible(barKey)
                and EUICoreStandaloneMinimap._OverrideAnchorGroups
                and EUICoreStandaloneMinimap._OverrideAnchorGroups() or nil
            if ovGroups then
                local OV_ARROW = "Interface\\AddOns\\EUIStandaloneMinimap\\media\\icons\\right-arrow.png"
                ...
                local function OvSubnavItem(text, buildEntries)
                    ...
                    local function ShowSub()
                        ...
                        EUICoreStandaloneMinimap.MakeBorder(sub, 1, 1, 1, 0.20)
                        ...
                        for _, e in ipairs(entries) do
                            ...
                            local tw = (EUICoreStandaloneMinimap.MeasureText
                                and EUICoreStandaloneMinimap.MeasureText(e.text, FONT_PATH, e.title and 10 or 11)) or 0
                            local needed = 10 + tw + 10 + 2
                ...
                for _, og in ipairs(ovGroups) do
                    if not (EUICoreStandaloneMinimap._HasOverrideAnchor and EUICoreStandaloneMinimap._HasOverrideAnchor(barKey, og.id)) then
                    ...
                    OvSubnavItem("Override Anchor", function()
                        ...
                        for _, og in ipairs(addable) do
                            ...
                            entries[#entries + 1] = {
                                ...
                                fn = function()
                                    if EUICoreStandaloneMinimap._BeginOverrideAnchorPick then
                                        EUICoreStandaloneMinimap._BeginOverrideAnchorPick(mover, gid)
                                    end
                                end,
                            }
                        end
                ...
                for _, og in ipairs(ovGroups) do
                    if EUICoreStandaloneMinimap._HasOverrideAnchor and EUICoreStandaloneMinimap._HasOverrideAnchor(barKey, og.id) then
                        local gid = og.id
                        local gname = og.name or ("Group " .. tostring(gid))
                        OvSubnavItem("Edit Override: " .. gname, function()
                            return {
                                { text = gname, title = true },
                                { text = EUICoreStandaloneMinimap.L("Edit Anchor"), fn = function()
                                    if EUICoreStandaloneMinimap._BeginOverrideAnchorPick then
                                        EUICoreStandaloneMinimap._BeginOverrideAnchorPick(mover, gid)
                                    end
                                end },
                                { text = EUICoreStandaloneMinimap.L("Delete Override Anchor"), fn = function()
                                    if EUICoreStandaloneMinimap._ClearOverrideAnchor then
                                        EUICoreStandaloneMinimap._ClearOverrideAnchor(barKey, gid)
                                    end
                                end },
                            }
                        end)
    ...
    mover:HookScript("OnEnter", function()
        ...
        if tip and EUICoreStandaloneMinimap.ShowWidgetTooltip then
            EUICoreStandaloneMinimap.ShowWidgetTooltip(mover, tip)
        end
    end)
...
do
    local _origRegister = EUICoreStandaloneMinimap.RegisterUnlockElements
    function EUICoreStandaloneMinimap:RegisterUnlockElements(elements, folder)
...
-------------------------------------------------------------------------------
local GRID_ICON       = "Interface\\AddOns\\EUIStandaloneMinimap\\media\\icons\\grid.png"
local MAGNET_ICON     = "Interface\\AddOns\\EUIStandaloneMinimap\\media\\icons\\magnet.png"
local FLASHLIGHT_ICON = "Interface\\AddOns\\EUIStandaloneMinimap\\media\\icons\\flashlight.png"
local HOVER_ICON      = "Interface\\AddOns\\EUIStandaloneMinimap\\media\\icons\\hover.png"
local DARK_OVERLAY_ICON = "Interface\\AddOns\\EUIStandaloneMinimap\\media\\icons\\dark-overlay.png"
local COORD_ICON      = "Interface\\AddOns\\EUIStandaloneMinimap\\media\\icons\\coordinates.png"
local BANNER_TEX      = "Interface\\AddOns\\EUIStandaloneMinimap\\media\\eui-unlocked-banner-2.png"
...
function EUICoreStandaloneMinimap:GetUnlockModeTopBarAnchor()
...
local function CreateHUD(parent)
    ...
    if EUICoreStandaloneMinimapDB then
        if EUICoreStandaloneMinimapDB.unlockGridMode == nil then EUICoreStandaloneMinimapDB.unlockGridMode = "dimmed" end
        if EUICoreStandaloneMinimapDB.unlockSnapEnabled == nil then EUICoreStandaloneMinimapDB.unlockSnapEnabled = true end
    end
    gridMode = (EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.unlockGridMode) or "dimmed"
    snapEnabled = (EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.unlockSnapEnabled ~= false) or true
    ...
    gridLabel:SetText(EUICoreStandaloneMinimap.L(GridLabelText()))
    ...
    gridBtn:SetScript("OnClick", function()
        CycleGridMode()
        if EUICoreStandaloneMinimapDB then EUICoreStandaloneMinimapDB.unlockGridMode = gridMode end
    ...
    darkOverlayLabel:SetText(darkOverlaysEnabled and EUICoreStandaloneMinimap.L("Dark Overlays\nEnabled") or EUICoreStandaloneMinimap.L("Dark Overlays\nDisabled"))
    ...
    flashLabel:SetText(flashlightEnabled and EUICoreStandaloneMinimap.L("Cursor Light\nEnabled") or EUICoreStandaloneMinimap.L("Cursor Light\nDisabled"))
    ...
    magnetLabel:SetText(snapEnabled and EUICoreStandaloneMinimap.L("Snap Elements\nEnabled") or EUICoreStandaloneMinimap.L("Snap Elements\nDisabled"))
    ...
    SetupToggleBtn(magnetBtn, magnetTex, magnetLabel,
        ...
        function()
            ...
            if EUICoreStandaloneMinimapDB then EUICoreStandaloneMinimapDB.unlockSnapEnabled = snapEnabled end
            magnetTex:SetAlpha(snapEnabled and HUD_ON_ALPHA or HUD_OFF_ALPHA)
            magnetLabel:SetTextColor(1, 1, 1, snapEnabled and HUD_ON_ALPHA or HUD_OFF_ALPHA)
            magnetLabel:SetText(snapEnabled and EUICoreStandaloneMinimap.L("Snap Elements\nEnabled") or EUICoreStandaloneMinimap.L("Snap Elements\nDisabled"))
    ...
    coordLabel:SetText(coordsEnabled and EUICoreStandaloneMinimap.L("Coordinates\nEnabled") or EUICoreStandaloneMinimap.L("Coordinates\nDisabled"))
    ...
    hoverLabel:SetText(hoverBarEnabled and EUICoreStandaloneMinimap.L("Hover Top Bar\nEnabled") or EUICoreStandaloneMinimap.L("Hover Top Bar\nDisabled"))
    ...
    EUICoreStandaloneMinimap.MakeStyledButton(exitBtn, "Exit", BTN_FONT,
        EUICoreStandaloneMinimap.RB_COLOURS, function() ns.RequestClose(false) end)
    ...
    do
        ...
        local eg = EUICoreStandaloneMinimap.ELLESMERE_GREEN or { r = 12/255, g = 210/255, b = 157/255 }
        EUICoreStandaloneMinimap.MakeBorder(btn, eg.r, eg.g, eg.b, 0.7)
        ...
        lbl:SetPoint("CENTER")
        lbl:SetText(EUICoreStandaloneMinimap.L("Save & Exit"))
        ...
        local function Apply(t)
            local c = EUICoreStandaloneMinimap.ELLESMERE_GREEN or eg
    ...
    --  Scale range 100%-150% in 10% steps. Saved to EUICoreStandaloneMinimapDB.unlockBannerScale.
    ---------------------------------------------------------------
    do
        local SCALE_MIN = 1.0
        local SCALE_MAX = 1.5
        ...
        if EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.unlockBannerScale then
            bannerUserScale = EUICoreStandaloneMinimapDB.unlockBannerScale
        ...
        local function ApplyBannerScale(newScale)
            ...
            if EUICoreStandaloneMinimapDB then EUICoreStandaloneMinimapDB.unlockBannerScale = newScale end
        ...
        local function MakeScaleBtn(text, anchorPoint, anchorTo, anchorRel, xOff, yOff)
            ...
            shadow:SetText(EUICoreStandaloneMinimap.L(text))
            ...
            label:SetText(EUICoreStandaloneMinimap.L(text))
...
local function SnapshotPositions()
    ...
    local rawSnap = {}
    EUICoreStandaloneMinimap._unlockSnapRawPos = rawSnap
    ...
    if abPos and EUICoreStandaloneMinimap._abBarKeys then
        for bk in pairs(EUICoreStandaloneMinimap._abBarKeys) do
            local e = abPos[bk]
...
do
    ...
    local function UpdateSpecOvBanner()
        local g = EUICoreStandaloneMinimap._specialUnlockGroup
        ...
        if not specOvBanner then
            ...
            EUICoreStandaloneMinimap.MakeBorder(specOvBanner, 199/255, 166/255, 90/255, 0.7)
        ...
        specOvBanner._text:SetText(string.format(
            EUICoreStandaloneMinimap.L("Customizing Unlock Mode: %s (changes apply only to this group's specs)"),
            g.name or "?"))
    ...
    EUICoreStandaloneMinimap._unlockRefreshSpecOvMarks = function()
        UpdateSpecOvBanner()
        for _, m in pairs(movers) do
...
local function CommitPositions()
    ...
    EUICoreStandaloneMinimap._propagatingSave = true
    ...
    EUICoreStandaloneMinimap._propagatingSave = false
    ...
    EUICoreStandaloneMinimap._reapplyForceEdgePreserve = true
    ...
    EUICoreStandaloneMinimap._reapplyForceEdgePreserve = false

    -- Persist unlock layout into the active profile so it survives reloads
    -- without requiring a manual profile switch.
    if EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimap.GetProfilesDB then
        ...
        if profileData then
            local snap = {
                anchors       = CopyTable(EUICoreStandaloneMinimapDB.unlockAnchors     or {}),
                widthMatch    = CopyTable(EUICoreStandaloneMinimapDB.unlockWidthMatch  or {}),
                heightMatch   = CopyTable(EUICoreStandaloneMinimapDB.unlockHeightMatch or {}),
                phantomBounds = CopyTable(EUICoreStandaloneMinimapDB.phantomBounds     or {}),
            }
            ...
            if EUICoreStandaloneMinimap.SpecOverrides_UnlockBaselineLinks then
                local ba, bw, bh = EUICoreStandaloneMinimap.SpecOverrides_UnlockBaselineLinks()
                if ba then
    ...
    if EUICoreStandaloneMinimap.SpecOverrides_UnlockValueSnapCommit then
        pcall(EUICoreStandaloneMinimap.SpecOverrides_UnlockValueSnapCommit)
    end
    ...
    if EUICoreStandaloneMinimap.SpecOverrides_HarvestUnlockLayout then
        local okH, errH = pcall(EUICoreStandaloneMinimap.SpecOverrides_HarvestUnlockLayout, true)
        if not okH then
            print("|cffff6060[EUICoreStandaloneMinimap]|r Unlock layer harvest failed: "
                .. tostring(errH))
        end
    end
end
...
local function RevertPositions()
    ...
    if EUICoreStandaloneMinimap.SpecOverrides_UnlockValueSnapDiscard then
        EUICoreStandaloneMinimap.SpecOverrides_UnlockValueSnapDiscard()
    end
    ...
    if db then
        for barKey, _ in pairs(pendingPositions) do
            if not registeredElements[barKey] then
                ...
                local rawSnap = EUICoreStandaloneMinimap._unlockSnapRawPos
                    and EUICoreStandaloneMinimap._unlockSnapRawPos[barKey]
                local snap = snapshotPositions[barKey]
                if rawSnap then
    ...
    for key, snapGrow in pairs(snapshotGrowDirs) do
        ...
        if key:sub(1, 4) == "CDM_" then
            local rawKey = key:sub(5)
            local cdmA = EUICoreStandaloneMinimap.Lite.GetAddon("EUICoreStandaloneMinimapCooldownManager", true)
            ...
            local eabA = EUICoreStandaloneMinimap.Lite.GetAddon("EUICoreStandaloneMinimapActionBars", true)
...
local function DoClose(closeAction)
    if not isUnlocked then return end
    isUnlocked = false
    EUICoreStandaloneMinimap._unlockActive = false
    EUICoreStandaloneMinimap._unlockModeActive = false
    EUICoreStandaloneMinimap:_NotifyUnlockModeListeners(false, closeAction or "exit")
    if EUICoreStandaloneMinimap._HideFallbackGhosts then EUICoreStandaloneMinimap._HideFallbackGhosts() end
    if EUICoreStandaloneMinimap._HideOverrideGhosts then EUICoreStandaloneMinimap._HideOverrideGhosts() end
    ...
    if EUICoreStandaloneMinimap._ReapplyOverrideAnchors then EUICoreStandaloneMinimap._ReapplyOverrideAnchors() end
    ...
    -- None = no work.
    EUICoreStandaloneMinimap.RestoreAnchorShifts()

    -- Restore unit frame buffs/debuffs
    local UF_FRAME_NAMES = {
        "EUICoreStandaloneMinimapUnitFrames_Player", "EUICoreStandaloneMinimapUnitFrames_Target",
        "EUICoreStandaloneMinimapUnitFrames_Focus", "EUICoreStandaloneMinimapUnitFrames_Pet",
        "EUICoreStandaloneMinimapUnitFrames_TargetTarget", "EUICoreStandaloneMinimapUnitFrames_FocusTarget",
    }
    for i = 1, 8 do UF_FRAME_NAMES[#UF_FRAME_NAMES + 1] = "EUICoreStandaloneMinimapUnitFrames_Boss" .. i end
    ...
    -- Restore EUICoreStandaloneMinimap QT background
    local qtBg = _G.EUICoreStandaloneMinimapQTBackground
    if qtBg then qtBg:SetAlpha(1) end
    -- Re-apply user visibility setting (handles "never" mode for both tracker + bg)
    if _G.EUICoreStandaloneMinimapQuestTracker and _G.EUICoreStandaloneMinimapQuestTracker.UpdateVisibility then
        _G.EUICoreStandaloneMinimapQuestTracker.UpdateVisibility()
    end
    ...
    hoveredMover    = nil
    cogHoveredMover = nil
    EUICoreStandaloneMinimap._unlockCursorSpeed = 0
    for _, m in pairs(movers) do
        m._snapTarget   = nil
        m._dragging     = false
        m._shiftAxis    = nil
    ...
    EUICoreStandaloneMinimap._specialUnlockGroup = nil
    for _, m in pairs(movers) do
        if m._specOvBrd then m._specOvBrd:Hide() end
        if m._specOvLock then m._specOvLock:Hide() end
    end
    ...
    local panel = EUICoreStandaloneMinimap and EUICoreStandaloneMinimap._mainFrame
    ...
    if not pendingAfterClose then
        if EUICoreStandaloneMinimap then
            -- Restore the module + page active before unlock mode opened (captured
            -- by SelectPage("Unlock Mode") in EUICoreStandaloneMinimap.lua). Do NOT show the panel
            ...
            local restoreModule = EUICoreStandaloneMinimap._unlockReturnModule
            local restorePage   = EUICoreStandaloneMinimap._unlockReturnPage
            EUICoreStandaloneMinimap._unlockReturnPage = nil
            EUICoreStandaloneMinimap._unlockReturnModule = nil
            if restoreModule then
                if EUICoreStandaloneMinimap.SelectModule then
                    EUICoreStandaloneMinimap:SelectModule(restoreModule)
                end
                if restorePage and EUICoreStandaloneMinimap.SelectPage then
                    local currentPage = EUICoreStandaloneMinimap.GetActivePage and EUICoreStandaloneMinimap:GetActivePage()
                    if currentPage ~= restorePage then
                        EUICoreStandaloneMinimap:SelectPage(restorePage)
                    end
                end
                -- NOW show the panel — one clean Show, no prior cycling.
                if EUICoreStandaloneMinimap.Toggle then EUICoreStandaloneMinimap:Toggle() end
            end
        end
    end
...
function EUICoreStandaloneMinimap.ForceCloseUnlockDiscard()
    if not isUnlocked then return end
    print("|cffff6060[EUICoreStandaloneMinimap]|r Spec changed: Unlock Mode closed, unsaved layout changes discarded.")
    pendingAfterClose = nil
...
local function CreateUnlockFrame()
    ...
    local function GetAnchorLine(idx)
        ...
        line:SetTexture("Interface\\AddOns\\EUIStandaloneMinimap\\media\\textures\\soft-line")
...
--  Saved to EUICoreStandaloneMinimapDB.unlockTipSeen so it never shows again.
-------------------------------------------------------------------------------

function ns.ShowUnlockTip()
    if EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.unlockTipSeen then return end
    ...
    if not unlockTipFrame then
        ...
        EUICoreStandaloneMinimap.MakeBorder(tip, ar, ag, ab, 0.25)
...
function ns.OpenUnlockMode()
    if isUnlocked then return end
    if InCombatLockdown() then
        print("|cffff6060[EUICoreStandaloneMinimap]|r Cannot enter Unlock Mode during combat.")
        return
    end
    ...
    if not EUICoreStandaloneMinimap._unlockReturnModule then
        local panel = EUICoreStandaloneMinimap._mainFrame
        if panel and panel:IsShown() then
            EUICoreStandaloneMinimap._unlockReturnModule = EUICoreStandaloneMinimap.GetActiveModule
                and EUICoreStandaloneMinimap:GetActiveModule() or nil
            EUICoreStandaloneMinimap._unlockReturnPage = EUICoreStandaloneMinimap.GetActivePage
                and EUICoreStandaloneMinimap:GetActivePage() or nil
        end
    end
    ...
    do
        local activeGid = EUICoreStandaloneMinimap.SpecOverrides_UnlockActive
            and EUICoreStandaloneMinimap.SpecOverrides_UnlockActive() or nil
        local g
        if type(activeGid) == "string" then
            ...
            g = cid and EUICoreStandaloneMinimap.Conditions_GroupById
                and EUICoreStandaloneMinimap.Conditions_GroupById(cid) or nil
        elseif activeGid then
            g = EUICoreStandaloneMinimap.SpecOverrides_GroupById
                and EUICoreStandaloneMinimap.SpecOverrides_GroupById(activeGid) or nil
        end
        EUICoreStandaloneMinimap._specialUnlockGroup = g
    end
    ...
    if EUICoreStandaloneMinimap.SpecOverrides_CloseEditSessions then
        EUICoreStandaloneMinimap.SpecOverrides_CloseEditSessions()
    end
    ...
    if EUICoreStandaloneMinimap.SpecOverrides_UnlockValueSnapBegin then
        EUICoreStandaloneMinimap.SpecOverrides_UnlockValueSnapBegin()
    end
    ...
    isUnlocked = true
    EUICoreStandaloneMinimap._unlockActive = true
    EUICoreStandaloneMinimap._unlockModeActive = true
    ...
    -- Also hide EUICoreStandaloneMinimap QT background (separate UIParent child)
    local qtBg = _G.EUICoreStandaloneMinimapQTBackground
    if qtBg then
        qtBg:SetAlpha(0)
    ...
    if EUICoreStandaloneMinimap._ClearFallbackGhostTempHides then EUICoreStandaloneMinimap._ClearFallbackGhostTempHides() end
    if EUICoreStandaloneMinimap._ClearOverrideGhostTempHides then EUICoreStandaloneMinimap._ClearOverrideGhostTempHides() end
    ...
    EUICoreStandaloneMinimap.RunAnchorShiftEnters()
    ...
    if EUICoreStandaloneMinimap.AnchorShiftWantsApply()
       and EUICoreStandaloneMinimap.ReapplyAllUnlockAnchors then
        ...
        EUICoreStandaloneMinimap._reapplyForceEdgePreserve = true
        pcall(EUICoreStandaloneMinimap.ReapplyAllUnlockAnchors)
        EUICoreStandaloneMinimap._reapplyForceEdgePreserve = false
    end
    SnapshotPositions()
    ...
    EUICoreStandaloneMinimap:_NotifyUnlockModeListeners(true)
    ...
    do
        local ov = EUICoreStandaloneMinimap._specialUnlockGroup and "-override" or ""
        if hudFrame and hudFrame._bannerTex then
            hudFrame._bannerTex:SetTexture(
                "Interface\\AddOns\\EUIStandaloneMinimap\\media\\eui-unlocked-banner-2" .. ov .. ".png")
        end
        if openAnimFrame then
            if openAnimFrame._outer then
                openAnimFrame._outer:SetTexture(
                    "Interface\\AddOns\\EUIStandaloneMinimap\\media\\eui-unlocked-outer-2" .. ov .. ".png")
            end
            if openAnimFrame._inner then
                openAnimFrame._inner:SetTexture(
                    "Interface\\AddOns\\EUIStandaloneMinimap\\media\\eui-unlocked-inner-2" .. ov .. ".png")
            end
            if openAnimFrame._top then
                openAnimFrame._top:SetTexture(
                    "Interface\\AddOns\\EUIStandaloneMinimap\\media\\eui-unlocked-top-2" .. ov .. ".png")
            end
        end
    end
    ...
    unlockFrame:SetScript("OnUpdate", function(self, dt)
        ...
        do
            ...
            if dt > 0 then
                local dx = nx - EUICoreStandaloneMinimap._unlockCursorX
                local dy = ny - EUICoreStandaloneMinimap._unlockCursorY
                -- Store squared speed to avoid sqrt; TryExpand compares against squared threshold
                EUICoreStandaloneMinimap._unlockCursorSpeed = (dx * dx + dy * dy) / (dt * dt)
            ...
            EUICoreStandaloneMinimap._unlockCursorX = nx; EUICoreStandaloneMinimap._unlockCursorY = ny
        end
        ...
        if elapsed >= GRID_START then
            if not gridStarted then
                ...
                if EUICoreStandaloneMinimap._unlockRefreshSpecOvMarks then EUICoreStandaloneMinimap._unlockRefreshSpecOvMarks() end
                ...
                if EUICoreStandaloneMinimap._RefreshFallbackGhosts then
                    EUICoreStandaloneMinimap._RefreshFallbackGhosts()
                    if EUICoreStandaloneMinimap._SetFallbackGhostsAlpha then
                        EUICoreStandaloneMinimap._SetFallbackGhostsAlpha(0)
                    end
                end
                ...
                if EUICoreStandaloneMinimap._ReapplyOverrideAnchors then EUICoreStandaloneMinimap._ReapplyOverrideAnchors() end
                if EUICoreStandaloneMinimap._RefreshOverrideGhosts then
                    EUICoreStandaloneMinimap._RefreshOverrideGhosts()
                    if EUICoreStandaloneMinimap._SetOverrideGhostsAlpha then
                        EUICoreStandaloneMinimap._SetOverrideGhostsAlpha(0)
                    end
                end
                ...
                retryTicker = C_Timer.NewTicker(0.5, function()
                    ...
                    if EUICoreStandaloneMinimap._unlockRegistrationDirty or retryAttempts <= 3 then
            ...
            if EUICoreStandaloneMinimap._SetFallbackGhostsAlpha then
                if moverFadeT > 0 then
                    EUICoreStandaloneMinimap._SetFallbackGhostsAlpha(min(1, moverFadeT / GLITCH_DUR))
                else
                    EUICoreStandaloneMinimap._SetFallbackGhostsAlpha(0)
                end
            end
            ...
            if EUICoreStandaloneMinimap._SetOverrideGhostsAlpha then
                if moverFadeT > 0 then
                    EUICoreStandaloneMinimap._SetOverrideGhostsAlpha(min(1, moverFadeT / GLITCH_DUR))
                else
                    EUICoreStandaloneMinimap._SetOverrideGhostsAlpha(0)
                end
            end
        ...
        if elapsed < MORPH then
            ...
            if panel and not panelHidden then
                ...
                if t > 0.95 then
                    ...
                    panel:SetAlpha(1)
                    if EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.Hide then
        ...
        if not panelHidden then
            panelHidden = true
            if panel then panel:SetScale(panelRealScale); panel:SetAlpha(1) end
            if EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.Hide then EUICoreStandaloneMinimap:Hide() end
        end
        ...
        if EUICoreStandaloneMinimap._unlockAutoSelectKey then
            local autoKey = EUICoreStandaloneMinimap._unlockAutoSelectKey
            EUICoreStandaloneMinimap._unlockAutoSelectKey = nil
...
-- Expose on the global EUICoreStandaloneMinimap so SelectPage can intercept "Unlock Mode"
if EUICoreStandaloneMinimap then
    EUICoreStandaloneMinimap._openUnlockMode = ns.OpenUnlockMode
    function EUICoreStandaloneMinimap:OpenUnlockMode()
        ns.OpenUnlockMode()
    end
end

-- Toggle helper + active flag alias used by options pages
if EUICoreStandaloneMinimap and not EUICoreStandaloneMinimap.ToggleUnlockMode then
    function EUICoreStandaloneMinimap:ToggleUnlockMode()
        if isUnlocked then
            ns.CloseUnlockMode()
        else
            ns.OpenUnlockMode()
...
if EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.RegisterOnShow then
    ...
    C_Timer.After(0, function()
        for bk, m in pairs(movers) do
            if not m._dragging then
                EUICoreStandaloneMinimap.RecenterBarAnchor(bk)
            end
        end
    end)
end
