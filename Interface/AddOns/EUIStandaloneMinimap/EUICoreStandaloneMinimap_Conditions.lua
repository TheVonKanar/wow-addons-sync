...
-------------------------------------------------------------------------------
--  EUICoreStandaloneMinimap_Conditions.lua
...
--  integration lives in EUICoreStandaloneMinimap_SpecOverrides.lua (it shares that file's
...
local function DarkModeMasterOn()
    return (EUICoreStandaloneMinimap.IsDarkModeAllOn and EUICoreStandaloneMinimap.IsDarkModeAllOn(_dmMainFilter)) or false
end

EUICoreStandaloneMinimap.CONDITIONS = {
...
local function GetProfileRoot()
    if not (EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimap.GetProfilesDB) then return nil end
...
function EUICoreStandaloneMinimap.Conditions_GetGroups(create)
...
function EUICoreStandaloneMinimap.Conditions_GroupById(gid)
...
function EUICoreStandaloneMinimap.Conditions_NewGroupId()
...
function EUICoreStandaloneMinimap.Conditions_ActiveGroup()
    local groups = EUICoreStandaloneMinimap.Conditions_GetGroups()
...
function EUICoreStandaloneMinimap.Conditions_ActiveGid()
    local g = EUICoreStandaloneMinimap.Conditions_ActiveGroup()
...
function EUICoreStandaloneMinimap.Conditions_AppliedGid()
    ...
    if gid and EUICoreStandaloneMinimap.Conditions_GroupById and EUICoreStandaloneMinimap.Conditions_GroupById(gid) then
        return gid
    end
    return nil
end
...
function EUICoreStandaloneMinimap.Conditions_MarkStale()
    _appliedGid = nil
    _flipPending = false
    _establish = true
end
...
function EUICoreStandaloneMinimap.Conditions_EstablishPending()
    return _establish
end

function EUICoreStandaloneMinimap.Conditions_Recheck()
    ...
    local applied = EUICoreStandaloneMinimap.Conditions_AppliedGid()
    ...
    if EUICoreStandaloneMinimap._unlockModeActive and EUICoreStandaloneMinimap.ForceCloseUnlockDiscard then
        EUICoreStandaloneMinimap.ForceCloseUnlockDiscard()
    end
    local handler = EUICoreStandaloneMinimap.SpecOverrides_CondTransition
    ...
    if not est and EUICoreStandaloneMinimap.SpecOverrides_SuspendDefaultView then
        resumeView = EUICoreStandaloneMinimap.SpecOverrides_SuspendDefaultView()
    end
    if resumeView then
        ...
        g = EUICoreStandaloneMinimap.Conditions_ActiveGroup()
        gid = g and g.id or nil
        applied = EUICoreStandaloneMinimap.Conditions_AppliedGid()
        if gid == applied then
            _appliedGid = applied
            if EUICoreStandaloneMinimap.SpecOverrides_ResumeDefaultView then
                EUICoreStandaloneMinimap.SpecOverrides_ResumeDefaultView()
            end
            return
        end
    end
    ...
    if resumeView and EUICoreStandaloneMinimap.SpecOverrides_ResumeDefaultView then
        EUICoreStandaloneMinimap.SpecOverrides_ResumeDefaultView()
    end
end
...
function EUICoreStandaloneMinimap.Conditions_ToggleKey(gid)
    if InCombatLockdown() then return end
    local g = EUICoreStandaloneMinimap.Conditions_GroupById(gid)
...
function EUICoreStandaloneMinimap.Conditions_RebuildKeyBindings()
    ...
    for _, g in ipairs(groups) do
        if g.conds and g.conds.keybind and g.key and g.key ~= "" then
            ...
            local gid = g.id
            btn:SetScript("OnClick", function() EUICoreStandaloneMinimap.Conditions_ToggleKey(gid) end)
...
--  regen drain: this file loads after EUICoreStandaloneMinimap_Profiles.lua, so the spec
