...
--  shared by all EUICoreStandaloneMinimap addons).
--
...
-- Profiles/Patch Notes are their own sidebar pages (single-page modules), not tabs under Global Settings. Keys match the sidebar buttons in EUICoreStandaloneMinimap.lua.
...
-------------------------------------------------------------------------------
function EUICoreStandaloneMinimap.RunCDMSpellExportFlow(activeName, exportFn)
    local function pickThenExport()
        local specs = {}
        local sp = EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.spellAssignments
            and EUICoreStandaloneMinimapDB.spellAssignments.profiles
            and EUICoreStandaloneMinimapDB.spellAssignments.profiles[activeName]
            and EUICoreStandaloneMinimapDB.spellAssignments.profiles[activeName].specProfiles
        ...
        EUICoreStandaloneMinimap:ShowCDMSpecPickerPopup({
            title         = EUICoreStandaloneMinimap.L("Export CDM Spells"),
            subtitle      = EUICoreStandaloneMinimap.L("This can't change which spells the user tracks in Blizzard's CDM.\nIt's recommended to also share your Blizzard CDM layout for any spec you choose here."),
            subtitleColor = { 1, 0.82, 0.2 },
            subtitleAtBottom = true,
            confirmText   = EUICoreStandaloneMinimap.L("Export"),
            specs         = specs,
    ...
    EUICoreStandaloneMinimap:ShowConfirmPopup({
        title       = EUICoreStandaloneMinimap.L("Include CDM Spell Layout?"),
        message     = EUICoreStandaloneMinimap.L("Include your Cooldown Manager spell layout (which spells sit on which bars) plus all per-spell settings for any specs you choose."),
        confirmText = EUICoreStandaloneMinimap.L("Yes"),
        cancelText  = EUICoreStandaloneMinimap.L("No"),
...
--  listings, fix lines. Content: EUICoreStandaloneMinimap._WHATSNEW_PATCHES (newest
...
-------------------------------------------------------------------------------
function EUICoreStandaloneMinimap._BuildWhatsNewPage(pageName, parent, yOffset)
    local PP  = EUICoreStandaloneMinimap.PanelPP
    local EG  = EUICoreStandaloneMinimap.ELLESMERE_GREEN
    local PAD = EUICoreStandaloneMinimap.CONTENT_PAD
    local W   = EUICoreStandaloneMinimap.Widgets
    local MakeFont   = EUICoreStandaloneMinimap.MakeFont
    local MakeBorder = EUICoreStandaloneMinimap.MakeBorder
    ...
    local function TitleOf(e)
        return ((e.module and EUICoreStandaloneMinimap.L(e.module) .. ": ") or "") .. (EUICoreStandaloneMinimap.L(e.title) or "")
    end
    ...
    local function GoTo(nav)
        if nav and nav.module then
            EUICoreStandaloneMinimap:NavigateToElementSettings(nav.module, nav.page, nav.section, nav.preSelect, nav.highlight)
        end
    end
    ...
    local function MakeHeroCard(x, cy, w, hgt, entry)
        ...
        descFs:SetText(EUICoreStandaloneMinimap.L(entry.desc) or "")
    ...
    local function MakeBannerCard(x, cy, w, entry)
        ...
        eyebrow:SetText(EUICoreStandaloneMinimap.L(entry.eyebrow or "SPECIAL UPDATE"))
        ...
        titleFs:SetText(EUICoreStandaloneMinimap.L(entry.title) or "")
    ...
    -- shared EUICoreStandaloneMinimap.MIDNIGHT_VIDEO_URL (EUICoreStandaloneMinimap_VideoGuides.lua).
    local function MakeVideoBannerCard(x, cy, w, entry)
        ...
        eyebrow:SetJustifyH("CENTER"); eyebrow:SetWordWrap(false)
        eyebrow:SetText(EUICoreStandaloneMinimap.L(entry.eyebrow or "WATCH FIRST"))
        ...
        local url = entry.url or EUICoreStandaloneMinimap.MIDNIGHT_VIDEO_URL or ""
        local FONT = EUICoreStandaloneMinimap._font or ("Interface\\AddOns\\EUIStandaloneMinimap\\media\\fonts\\Expressway.ttf")
        ...
        hintFs:SetText(EUICoreStandaloneMinimap.L("Click the link, then Ctrl+C to copy it"))
        ...
        eb:SetScript("OnKeyDown", function(self, key)
            if key == "C" and IsControlKeyDown() then
                hintFs:SetText(EUICoreStandaloneMinimap.L("Link copied - paste it into your browser"))
    ...
    local function MakeListing(cy, w, entry)
        ...
        subFs:SetText(EUICoreStandaloneMinimap.L(entry.desc) or "")
    ...
    hint:SetText (EUICoreStandaloneMinimap.L("Click any new feature to go directly to the setting"))

    -- "New Patch Reminder Dot" opt-out for the pulsing sidebar dot shown when the account version increases (Patch Notes button in EUICoreStandaloneMinimap.lua).
    do
        local BOX = 14
        local row = CreateFrame("Button", nil, parent)
        ...
        lbl:SetText(EUICoreStandaloneMinimap.L("New Patch Reminder Dot"))
        row:SetSize(BOX + 10 + (lbl:GetStringWidth() or 130), 20)
        PP.Point(row, "TOPRIGHT", parent, "TOPRIGHT", -PAD, y + 2)
        local function Paint()
            local on = not (EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.patchDotDisabled)
            check:SetShown(on)
            if on then
        ...
        row:SetScript("OnClick", function()
            ...
            EUICoreStandaloneMinimapDB.patchDotDisabled = (not EUICoreStandaloneMinimapDB.patchDotDisabled) and true or nil
            Paint()
        ...
        row:SetScript("OnEnter", function(self)
            ...
            if EUICoreStandaloneMinimap.ShowWidgetTooltip then
                EUICoreStandaloneMinimap.ShowWidgetTooltip(self, EUICoreStandaloneMinimap.L("Show a pulsing dot on the Patch Notes button whenever EUICoreStandaloneMinimap updates to a new version."))
            end
        end)
        row:SetScript("OnLeave", function()
            lbl:SetAlpha(0.5)
    ...
    for pi = 1, shown do
        ...
        if isMini then
            ...
            ver:SetText("EUICoreStandaloneMinimap " .. (patch.version or ""))
        ...
        if #fixes > 0 then
            ...
            for _, fx in ipairs(fixes) do
                local fh = MakeFixLine(y, ((fx.module and EUICoreStandaloneMinimap.L(fx.module) .. ": ") or "") .. (EUICoreStandaloneMinimap.L(fx.text) or "")); y = y - fh
            end
        end
...
-------------------------------------------------------------------------------
EUICoreStandaloneMinimap._LEGENDS = {
...
function EUICoreStandaloneMinimap._BuildLegendsPage(pageName, parent, yOffset)
    local PP  = EUICoreStandaloneMinimap.PanelPP
    local EG  = EUICoreStandaloneMinimap.ELLESMERE_GREEN
    local PAD = EUICoreStandaloneMinimap.CONTENT_PAD
    local L   = EUICoreStandaloneMinimap.L
    local MakeFont   = EUICoreStandaloneMinimap.MakeFont
    local MakeBorder = EUICoreStandaloneMinimap.MakeBorder

    parent._showRowDivider = nil

    local data   = EUICoreStandaloneMinimap._LEGENDS or {}
    local y      = yOffset - 14
    local totalW = parent:GetWidth() - PAD * 2
    local CARD_GAP = 14
    ...
    desc:SetText(L("Thank you to all who support EUICoreStandaloneMinimap and its incredible support team!"))
    ...
    local function MakeWall(x, headerText)
        ...
        local function UpdateThumb()
            local maxScroll = EUICoreStandaloneMinimap.SafeScrollRange and EUICoreStandaloneMinimap.SafeScrollRange(sf) or 0
    ...
    footer:SetText(L("Thank you for making EUICoreStandaloneMinimap possible."))
    y = y - 22
...
-------------------------------------------------------------------------------
EUICoreStandaloneMinimap._WHATSNEW_PATCHES = {
    {
        version = "8.9.4",
        mini = true,
        fixes = {
        ...
        features = {
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapBags", page = "Bags", section = "EXTRAS", highlight = "Group Armory by Slot" },
            },
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapBlizzardSkin", page = "Blizzard Window Skins", section = "CORE OPTIONS", highlight = "Show Item Durability" },
            },
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapCooldownManager", page = "Tracking Bars", section = "Display", highlight = "Visibility" },
            },
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapUnitFrames", page = "Player Aura Bars" },
            },
            {
                module = "Quickdraw",
                ...
                nav    = { module = "EUICoreStandaloneMinimapQuickdraw", page = "Quickdraw" },
            },
            {
    ...
    },
    {
        ...
        features = {
            ...
            },
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapMythicTimer", page = "Target/Focus Bars" },
            },
            {
                module = "Nameplates",
                ...
                nav    = { module = "EUICoreStandaloneMinimapNameplates", page = "General", section = "EXTRA AURA OPTIONS", highlight = "Dispel Glow Style" },
            },
            {
                module = "Nameplates",
                ...
                nav    = { module = "EUICoreStandaloneMinimapNameplates", page = "General", section = "EXTRAS" },
            },
            {
                module = "Performance",
            ...
            },
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapQoL", page = "Raid Tools", section = "GROUP BUTTONS", highlight = "Show Role Check" },
            },
            {
                module = "Quickdraw",
            ...
            },
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapRaidFrames", page = "Frames", section = "FRAME SIZES" },
            },
            {
                module = "Raid Frames",
                ...
                nav    = { module = "EUICoreStandaloneMinimapRaidFrames", page = "Buff Manager" },
            },
            {
                module = "Raid Frames",
                ...
                nav    = { module = "EUICoreStandaloneMinimapRaidFrames", page = "HoverCast" },
            },
        },
        fixes = {
        ...
        heroes = {
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapUnitFrames", page = "Main Frames" },
            },
            {
                module = "Nameplates",
                title  = "Hover Effect",
                ...
                nav    = { module = "EUICoreStandaloneMinimapNameplates", page = "Display" },
            },
        },
        features = {
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapBags", page = "Bags" },
            },
            {
                module = "Cooldown Manager",
                title  = "Liquid Luster",
                desc   = "New potion preset, and the swap option now covers all three combat potions",
                nav    = { module = "EUICoreStandaloneMinimapCooldownManager", page = "CDM Bars" },
            },
            {
                module = "Nameplates",
                ...
                nav    = { module = "EUICoreStandaloneMinimapNameplates", page = "General" },
            },
            {
                module = "Raid Frames",
                ...
                nav    = { module = "EUICoreStandaloneMinimapRaidFrames", page = "Frames" },
            },
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapRaidFrames", page = "Debuff Manager" },
            },
        },
        fixes = {
    ...
    },
    {
        ...
        heroes = {
            ...
            },
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapBlizzardSkin", page = "Blizzard Window Skins" },
            },
        },
        features = {
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapActionBars", page = "Menu, Bags & XP Bars" },
            },
            {
                module = "Bags",
            ...
            },
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapCooldownManager", page = "Tracking Bars" },
            },
            {
                module = "General",
            ...
            },
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapNameplates", page = "Colors" },
            },
            {
            ...
            },
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapQoL", page = "QoL", section = "GENERAL", highlight = "Hide Loot Rolls Window" },
            },
            {
                module = "Quickdraw",
                title  = "Pings Preset",
            ...
            },
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapRaidFrames", page = "Frames", section = "FRIENDLY BOSS FRAMES", highlight = "Add Friendly Boss Group" },
            },
            {
                module = "Unit Frames",
                title  = "Small Frame Background Colors",
                desc   = "Own background swatches for target of target, focus target and pet",
                nav    = { module = "EUICoreStandaloneMinimapUnitFrames", page = "Mini Frames" },
            },
            {
                module = "Unlock Mode",
                title  = "Quick Hides",
        ...
        features = {
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapQoL", page = "QoL", section = "EXTRAS", highlight = "Secondary Stat Display" },
            },
            {
                module = "Raid Frames",
                title  = "Frame Strata Control",
                desc   = "Lift raid and party frames above spell effects that overlapped them",
                nav    = { module = "EUICoreStandaloneMinimapRaidFrames", page = "Frames", section = "EXTRAS", highlight = "Frame Strata" },
            },
            {
                module = "Cooldown Manager",
    ...
    },
    {
        ...
        heroes = {
            ...
            },
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapChat", page = "Chat" },
            },
        },
        features = {
            {
            ...
            },
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapUnitFrames", page = "Player Aura Bars", section = "CORE", highlight = "Growth Direction" },
            },
            {
                module = "Quality of Life",
                ...
                nav    = { module = "EUICoreStandaloneMinimapQoL", page = "QoL" },
            },
            {
                module = "Raid Frames",
        ...
        fixes = {
            ...
            { module = "Unit Frames", text = "Enable Boss Frames applies live with the EUICoreStandaloneMinimap source." },
        ...
        features = {
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapRaidFrames", page = "Frames", section = "LAYOUT", highlight = "Group Growth" },
            },
            {
                module = "Chat",
                title  = "Timestamp All Messages",
                desc   = "Stamps system messages, loot, achievements, and addon prints, not just player chat",
                nav    = { module = "EUICoreStandaloneMinimapChat", page = "Chat", section = "EXTRAS", highlight = "Timestamp All Messages" },
            },
            {
                module = "Quickdraw",
        ...
        heroes = {
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapMythicTimer", page = "Targeted Spell Bars", section = "TARGETED SPELL BARS", highlight = "Enable Targeted Spell Bars" },
            },
            {
                module = "Mythic+ Tools",
                ...
                nav    = { module = "EUICoreStandaloneMinimapMythicTimer", page = "Target/Focus Bars", section = "TARGET CAST BAR", highlight = "Enable Target Cast Bar" },
            },
        },
        features = {
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapDamageMeters", page = "Damage Meters", section = "STANDALONE COMBAT TIMER", highlight = "Lock Position & Disable Click" },
            },
            {
                module = "General",
            ...
            },
            {
                ...
                nav    = { module = "EUIStandaloneMinimap", page = "Minimap", section = "DISPLAY", highlight = "Reset Zoom" },
            },
            {
                module = "Nameplates",
                title  = "Out of Range Opacity",
                desc   = "Fade enemy plates beyond your attack range",
                nav    = { module = "EUICoreStandaloneMinimapNameplates", page = "Display", section = "EXTRAS", highlight = "Range Check" },
            },
            {
                module = "Nameplates",
                title  = "Hide Enemy Nameplates out of Combat",
                desc   = "Plates return the instant combat starts",
                nav    = { module = "EUICoreStandaloneMinimapNameplates", page = "Display", section = "EXTRAS", highlight = "Hide Enemy Nameplates out of Combat" },
            },
            {
                module = "QoL",
                title  = "FPS & Latency in Secondary Stats",
                desc   = "Attach the readout as rows in the stats block",
                nav    = { module = "EUICoreStandaloneMinimapQoL", page = "QoL", section = "EXTRAS", highlight = "Show FPS Counter" },
            },
            {
                module = "Unit Frames",
                title  = "Centered Aura Growth",
                desc   = "Player Aura Bars grow from the center, spacing can go negative",
                nav    = { module = "EUICoreStandaloneMinimapUnitFrames", page = "Player Aura Bars", section = "DISPLAY", highlight = "Spacing" },
            },
        },
        fixes = {
        ...
        features = {
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapCooldownManager", page = "CDM Bars", section = "ICON DISPLAY", highlight = "Charge/Stack Size" },
            },
            {
                module = "Quickdraw",
        ...
        features = {
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapCooldownManager", page = "CDM Bars", section = "EXTRAS", highlight = "Always Show Cooldown Edge" },
            },
            {
                module = "Quickdraw",
            ...
            },
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapResourceBars", page = "Class, Power and Health Bars", section = "CLASS RESOURCE BAR", highlight = "Fill Color" },
            },
            {
                module = "Unit Frames",
        ...
        heroes = {
            {
                ...
                -- EUICoreStandaloneMinimap.MIDNIGHT_VIDEO_URL (EUICoreStandaloneMinimap_VideoGuides.lua).
                videoBanner = true,
                eyebrow = "12.1 LAUNCH",
                title   = "Everything New in 12.1",
                desc    = "One video walks you through the most important changes in 12.1 and how to get the most out of EUICoreStandaloneMinimap under them. Copy the link and give it a watch before you dive in.",
            },
            {
                module = "Quickdraw",
            ...
            },
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapChat", page = "Chat", section = "EXTRAS", highlight = "Shortened Channel Names" },
            },
            {
                module = "Unit Frames",
            ...
            },
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapAuraBuffReminders", page = "Auras, Buffs & Consumables" },
            },
            {
                module = "Damage Meters",
                ...
                nav    = { module = "EUICoreStandaloneMinimapDamageMeters", page = "Damage Meters" },
            },
        },
        features = {
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapActionBars", page = "Menu, Bags & XP Bars", section = "VEHICLE BAR", highlight = "Hide Blizzard's Vehicle Bar" },
            },
            {
                module = "Action Bars",
                title  = "Show When Spellbook Is Open",
                desc   = "Reveal bars while the spellbook is open",
                nav    = { module = "EUICoreStandaloneMinimapActionBars", page = "Bar Display", section = "VISIBILITY" },
            },
            {
                module = "Cooldown Manager",
            ...
            },
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapDamageMeters", page = "Damage Meters", section = "STANDALONE COMBAT TIMER", highlight = "Standalone Combat Timer" },
            },
            {
                module = "General",
                title  = "EUI Legends",
            ...
            },
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapNameplates", page = "Colors", section = "ENEMY COLORS", highlight = "Darken Enemies Out of Combat" },
            },
            {
                module = "Nameplates",
                title  = "Near Aggro Glow",
                desc   = "Warning glow while close to pulling aggro",
                nav    = { module = "EUICoreStandaloneMinimapNameplates", page = "Colors", section = "THREAT COLORS (INSTANCES ONLY)", highlight = "Non-Tank Threat" },
            },
            {
                module = "Nameplates",
                title  = "Core Text Strata",
                desc   = "Layer each text slot independently",
                nav    = { module = "EUICoreStandaloneMinimapNameplates", page = "Display", section = "CORE TEXT POSITIONS" },
            },
            {
                module = "QoL",
            ...
            },
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapRaidFrames", page = "Frames", section = "EXTRAS", highlight = "Healer Mana Display" },
            },
            {
                module = "Raid Frames",
            ...
            },
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapRaidFrames", page = "Frames", section = "INDICATORS", highlight = "Role Icons" },
            },
            {
                module = "Raid Frames",
                title  = "Exclude Myself from Extra Frames",
                desc   = "Hide your own frame from Show Tanks",
                nav    = { module = "EUICoreStandaloneMinimapRaidFrames", page = "Frames", section = "EXTRA FRAMES" },
            },
            {
                module = "Resource Bars",
                title  = "Pip Borders",
                desc   = "Border each resource pip individually",
                nav    = { module = "EUICoreStandaloneMinimapResourceBars", page = "Class, Power and Health Bars", section = "CLASS RESOURCE BAR", highlight = "Border Style" },
            },
            {
                module = "Resource Bars",
                title  = "Spec Override Anchors",
                desc   = "Per-spec element anchoring in Unlock Mode",
                nav    = { module = "EUICoreStandaloneMinimapResourceBars", page = "Class, Power and Health Bars" },
            },
            {
                module = "Unit & Raid Frames",
                title  = "New Absorb Placements",
                desc   = "Overlay Reverse absorbs, From Left overshields",
                nav    = { module = "EUICoreStandaloneMinimapUnitFrames", page = "Main Frames", section = "ABSORBS", highlight = "Absorb Style" },
            },
            {
                module = "Unit Frames",
                title  = "Per-Frame Strata",
                desc   = "Frame Strata per frame, plus Boss Frames",
                nav    = { module = "EUICoreStandaloneMinimapUnitFrames", page = "Main Frames", section = "DISPLAY", highlight = "Frame Strata" },
            },
            {
                module = "Unit Frames",
                title  = "Boss Spell Target Offsets",
                desc   = "Move the boss cast target text",
                nav    = { module = "EUICoreStandaloneMinimapUnitFrames", page = "Boss Frames" },
            },
        },
        fixes = {
            ...
            { module = "Friends List", text = "The new 12.1 Social UI friend cards get EUICoreStandaloneMinimap styling: online-status gradients and class-color accents. The classic friends list keeps its full skin." },
        ...
        features = {
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapCooldownManager", page = "Tracking Bars", section = "BAR LAYOUT", highlight = "Fill Up" },
            },
        },
        fixes = {
        ...
        features = {
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapCooldownManager", page = "Tracking Bars", section = "BAR LAYOUT", highlight = "Name Text" },
            },
            {
            ...
            },
            {
                ...
                nav    = { module = "EUIStandaloneMinimap", page = "Minimap", section = "BLIZZARD ELEMENTS", highlight = "Show Addon Compartment" },
            },
            {
                module = "Unit Frames",
                title  = "Player Castbar Spell Target",
                desc   = "Show who you are casting on, via the Spell Target dropdown",
                nav    = { module = "EUICoreStandaloneMinimapUnitFrames", page = "Main Frames", section = "CAST BAR", highlight = "Spell Target",
                           preSelect = function() EUICoreStandaloneMinimap._setUnitFrameUnit("player"); EUICoreStandaloneMinimap._pendingUnitSelect = "player" end },
            },
        },
        fixes = {
        ...
        features = {
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapBags", page = "Bags", section = "EXTRAS", highlight = "Show Sort Icon" },
            },
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapDataBars", page = "DataBars" },
            },
            {
                module = "Nameplates",
                title  = "Standalone Level Text",
                desc   = "Show level and name together",
                nav    = { module = "EUICoreStandaloneMinimapNameplates", page = "Display", section = "CORE TEXT POSITIONS", highlight = "Top Text" },
            },
        },
        fixes = {
            ...
            { module = "Chat", text = "Fixed errors when whispering and when using items in the chat tab right-click menu. The Font Size list in that Blizzard menu is back to Blizzard's own sizes; EUICoreStandaloneMinimap's own chat font size setting still covers the full range." },
    ...
    },
    {
        ...
        heroes = {
            ...
            },
            {
                ...
                desc   = "The whole loot window collection in EUICoreStandaloneMinimap style: the loot window with quality-colored icon borders and a clean hover, the need / greed roll popups, the pending-rolls window, and loot toasts with quality strips, and a scale slider.",
                nav    = { module = "EUICoreStandaloneMinimapBlizzardSkin", page = "Blizzard Window Skins" },
            },
        },
        features = {
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapActionBars", page = "Bar Display", section = "PAGING", highlight = "Disable Form Paging" },
            },
            {
            ...
            },
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapPartyMode", page = "Party Mode", highlight = "Spinning Action Bars" },
            },
            {
                module = "Quality of Life",
                title  = "Raid Tools Grow Direction",
                desc   = "Pick which way the windows open from the icon",
                nav    = { module = "EUICoreStandaloneMinimapQoL", page = "Raid Tools", section = "GENERAL", highlight = "Menu Grow Direction" },
            },
            {
                module = "Unit Frames",
                title  = "Elite/Rare Indicator",
                desc   = "Dragon badge on elite and rare targets",
                nav    = { module = "EUICoreStandaloneMinimapUnitFrames", page = "Main Frames", section = "EXTRAS", highlight = "Elite/Rare Indicator",
                           preSelect = function()
                               if EUICoreStandaloneMinimap._setUnitFrameUnit then EUICoreStandaloneMinimap._setUnitFrameUnit("target") end
                           end },
            },
        },
        fixes = {
        ...
        features = {
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapActionBars", page = "Bar Display", section = "TEXT", highlight = "Cooldown Text Size" },
            },
            {
            ...
            },
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapResourceBars", page = "Class, Power and Health Bars", section = "BAR DISPLAY", highlight = "Background" },
            },
        },
        fixes = {
        ...
        heroes = {
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapRaidFrames", page = "Frames", section = "HEALTH BAR", highlight = "Health Bar Texture" },
            },
            {
                module = "Chat",
                ...
                nav    = { module = "EUICoreStandaloneMinimapChat", page = "Chat", section = "IDLE FADE", highlight = "Fade Strength" },
            },
        },
        features = {
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapChat", page = "Sidebar", section = "ICONS", highlight = "Scroll Button on Chat Panel" },
            },
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapChat", page = "Tabs", section = "LAYOUT", highlight = "Inner Padding X" },
            },
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapDamageMeters", page = "Damage Meters", section = "BARS", highlight = "Show Breakdown on Hover" },
            },
            {
            ...
            },
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapMythicTimer", page = "Mythic+ Timer", section = "FORCES", highlight = "Enemy Text Format" },
            },
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapNameplates", page = "Display", section = "CORE POSITIONS" },
            },
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapResourceBars", page = "Class, Power and Health Bars", section = "BAR DISPLAY", highlight = "Texture" },
            },
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapBlizzardSkin", page = "Tooltips, Menus & Popups", section = "BLIZZARD TOOLTIP", highlight = "Show Spell ID on Tooltip" },
            },
        },
        fixes = {
            ...
            { module = "Chat", text = "Panel border no longer draws above the EUICoreStandaloneMinimap settings window." },
        ...
        heroes = {
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapNameplates", page = "General", section = "OTHER NAMEPLATES", highlight = "Subtitle Text" },
            },
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapQoL", page = "QoL", section = "BATTLE RES", highlight = "Display Style" },
            },
        },
        features = {
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapActionBars", page = "Bar Display", section = "ICON EFFECTS", highlight = "Hide Charge Count at 0",
                           preSelect = function()
                               if EUICoreStandaloneMinimap._setActionBarKey then EUICoreStandaloneMinimap._setActionBarKey("MainBar") end
                           end },
            },
            {
                module = "Bags",
                ...
                nav    = { module = "EUICoreStandaloneMinimapBags", page = "Bags", section = "DISPLAY", highlight = "Merge Duplicate Items" },
            },
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapUnitFrames", page = "Mini Frames", section = "DISPLAY", highlight = "Enable Pet Frame",
                           preSelect = function()
                               if EUICoreStandaloneMinimap._setMiniUnit then EUICoreStandaloneMinimap._setMiniUnit("pet") end
                           end },
            },
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapUnitFrames", page = "Main Frames", section = "HEALTH BAR", highlight = "Left Text",
                           preSelect = function()
                               if EUICoreStandaloneMinimap._setUnitFrameUnit then EUICoreStandaloneMinimap._setUnitFrameUnit("player") end
                           end },
            },
        },
        fixes = {
    ...
    },
    {
        ...
        heroes = {
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapQoL", page = "Raid Tools", section = "GENERAL", highlight = "Show Raid Tools" },
            },
        },
        features = {
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapChat", page = "Chat", section = "INPUT FIELD", highlight = "Edit Box Height" },
            },
            {
                module = "Chat",
                title  = "Sidebar Width",
                desc   = "Size the sidebar from 30 to 100 pixels",
                nav    = { module = "EUICoreStandaloneMinimapChat", page = "Sidebar", section = "SIDEBAR", highlight = "Sidebar Width" },
            },
            {
                module = "General",
                title  = "Name Font",
            ...
            },
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapPartyMode", page = "Party Mode", section = "PARTY MODE", highlight = "Play Sound on Party Mode" },
            },
            {
                ...
                nav    = { module = "EUICoreStandaloneMinimapUnitFrames", page = "Main Frames", section = "BUFFS AND DEBUFFS", highlight = "Buff Display",
                           preSelect = function()
                               if EUICoreStandaloneMinimap._setUnitFrameUnit then EUICoreStandaloneMinimap._setUnitFrameUnit("player") end
                           end },
            },
        ...
        fixes = {
            ...
            { module = "General", text = "Tooltips inside the EUICoreStandaloneMinimap options panel and its popups now follow your EUI Options Panel Scale setting, instead of always showing at the default size." },
...
-------------------------------------------------------------------------------
--  FCT font -- handled by EUICoreStandaloneMinimap_Startup.lua which runs earlier.
-------------------------------------------------------------------------------

-- Wait for EUICoreStandaloneMinimap to exist
local initFrame = CreateFrame("Frame")
initFrame:RegisterEvent("PLAYER_LOGIN")
initFrame:SetScript("OnEvent", function(self)
    self:UnregisterEvent("PLAYER_LOGIN")

    if not EUICoreStandaloneMinimap or not EUICoreStandaloneMinimap.RegisterModule then return end
    local PP = EUICoreStandaloneMinimap.PanelPP

    local GLOBAL_KEY = EUICoreStandaloneMinimap.GLOBAL_KEY or "_EUIGlobal"
    ...
    if not EUICoreStandaloneMinimapDB or EUICoreStandaloneMinimapDB.suppressErrors ~= false then
    ...
    local function BuildGeneralPage(pageName, parent, yOffset)
        ...
        local function ApplyOptimizedGfx()
            ...
            local rl = EUICoreStandaloneMinimap._widgetRefreshList
            if rl then for i = 1, #rl do rl[i]() end end
        end

        local function RestoreGfxSettings()
            if not EUICoreStandaloneMinimapDB or not EUICoreStandaloneMinimapDB.gfxBackup then return end
            local backup = EUICoreStandaloneMinimapDB.gfxBackup
            for _, entry in ipairs(OPTIMIZED_CVARS) do
            ...
            EUICoreStandaloneMinimapDB.gfxBackup = nil
            local rl2 = EUICoreStandaloneMinimap._widgetRefreshList
        ...
        do
            ...
            local totalW = parent:GetWidth() - EUICoreStandaloneMinimap.CONTENT_PAD * 2
            PP.Size(gfxFrame, totalW, ROW_H)
            PP.Point(gfxFrame, "TOPLEFT", parent, "TOPLEFT", EUICoreStandaloneMinimap.CONTENT_PAD, y)
            ...
            EUICoreStandaloneMinimap.MakeStyledButton(optBtn, "Optimize My FPS and Graphics", 14,
                EUICoreStandaloneMinimap.WB_COLOURS, ApplyOptimizedGfx)
            optBtn:HookScript("OnEnter", function()
                EUICoreStandaloneMinimap.ShowWidgetTooltip(optBtn, "Optimizes your graphics settings for maximum FPS and visual clarity.")
            end)
            optBtn:HookScript("OnLeave", function() EUICoreStandaloneMinimap.HideWidgetTooltip() end)
            ...
            local _, _, restLbl = EUICoreStandaloneMinimap.MakeStyledButton(restBtn, "Restore My Settings", 10,
                EUICoreStandaloneMinimap.RB_COLOURS, RestoreGfxSettings)
            ...
            local function RefreshRestoreVisibility()
                if EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.gfxBackup then
                    restBtn:Show()
            ...
            EUICoreStandaloneMinimap.RegisterWidgetRefresh(RefreshRestoreVisibility)
            ...
            infoFS:SetFont(EUICoreStandaloneMinimap.EXPRESSWAY, 12, EUICoreStandaloneMinimap.GetFontOutlineFlag())
            infoFS:SetTextColor(EG.r, EG.g, EG.b, 0.70)
            infoFS:SetText(EUICoreStandaloneMinimap.L("More Information"))
            ...
            infoBtn:SetScript("OnClick", function()
                EUICoreStandaloneMinimap:ShowInfoPopup({
        ...
        local themeValues = {}
        for _, name in ipairs(EUICoreStandaloneMinimap.THEME_ORDER) do
        ...
        themeRow, h = W:DualRow(parent, y,
            { type="multiSwatch", text="UI Accent Color",
              tooltip="Sets the accent color used across all EUICoreStandaloneMinimap elements (tabs, glows, highlights, borders). Defaults to your theme color.",
              swatches = {
                { tooltip = "Class Color",
                  getValue = function()
                      local cr, cg, cb = EUICoreStandaloneMinimap.GetPlayerClassColor()
                      return cr, cg, cb, 1
                  end,
                  setValue = function() end,
                  onClick = function()
                      -- Per-profile: set use-class, then re-resolve + apply live.
                      EUICoreStandaloneMinimap.SetActiveProfileAccent(nil, true)
                      EUICoreStandaloneMinimap.RefreshAccent()
                      EUICoreStandaloneMinimap:RefreshPage()
                  end,
                  refreshAlpha = function()
                      return (select(1, EUICoreStandaloneMinimap.GetActiveAccentState())) and 1 or 0.3
                  end },
                { tooltip = "Custom Color",
                  hasAlpha = false,
                  getValue = function()
                      local _, ca = EUICoreStandaloneMinimap.GetActiveAccentState()
                      if ca then return ca.r, ca.g, ca.b, 1 end
                      return EUICoreStandaloneMinimap.DEFAULT_ACCENT_R, EUICoreStandaloneMinimap.DEFAULT_ACCENT_G, EUICoreStandaloneMinimap.DEFAULT_ACCENT_B, 1
                  end,
                  setValue = function(r, g, b)
                      -- Persists per-profile (custom + useClass=false), applies live.
                      EUICoreStandaloneMinimap.SetAccentColor(r, g, b)
                  end,
                  onClick = function(self)
                      if select(1, EUICoreStandaloneMinimap.GetActiveAccentState()) then
                          -- Switch class -> custom: clear the per-profile class flag and re-resolve (profile custom -> global -> theme).
                          EUICoreStandaloneMinimap.SetActiveProfileAccent(nil, false)
                          EUICoreStandaloneMinimap.RefreshAccent()
                          EUICoreStandaloneMinimap:RefreshPage()
                          return
                      end
                  ...
                  refreshAlpha = function()
                      return (select(1, EUICoreStandaloneMinimap.GetActiveAccentState())) and 0.3 or 1
                  end },
              } },
            { type="dropdown", text="EUI Options Theme",
              values=themeValues,
              order=EUICoreStandaloneMinimap.THEME_ORDER,
              getValue=function()
                return EUICoreStandaloneMinimap.GetActiveTheme()
              end,
              setValue=function(v)
                EUICoreStandaloneMinimap.SetActiveTheme(v)
                -- Re-resolve the accent so the sidebar highlight tracks the theme.
                if EUICoreStandaloneMinimap.RefreshAccent then
                    EUICoreStandaloneMinimap.RefreshAccent()  -- ApplyAccentLive already refreshes the page
                else
                    EUICoreStandaloneMinimap:RefreshPage()
                end
              end }
        ...
        if not EUICoreStandaloneMinimap._prebuilding then
            local rightRgn = themeRow._rightRegion
            local function isCustomColorOff()
                return EUICoreStandaloneMinimap.GetActiveTheme() ~= "Custom Color"
            end
            ...
            local tcGet = function()
                ...
                return EUICoreStandaloneMinimap.GetAccentColor()
            end
            local tcSet = function(r, g, b)
                if not EUICoreStandaloneMinimapDB then EUICoreStandaloneMinimapDB = {} end
                EUICoreStandaloneMinimapDB.accentColor = { r = r, g = g, b = b }
                -- Only update the window background, not the accent color
                if EUICoreStandaloneMinimap._applyBgTint then
                    EUICoreStandaloneMinimap._applyBgTint(r, g, b)
                end
            end
            local tcSwatch, tcUpdateSwatch = EUICoreStandaloneMinimap.BuildColorSwatch(rightRgn, rightRgn:GetFrameLevel() + 5, tcGet, tcSet, nil, 20)
            PP.Point(tcSwatch, "RIGHT", rightRgn._control, "LEFT", -12, 0)
            rightRgn._lastInline = tcSwatch
            EUICoreStandaloneMinimap.RegisterWidgetRefresh(function()
            ...
            tcSwatch:SetScript("OnEnter", function(self)
                if isCustomColorOff() then
                    EUICoreStandaloneMinimap.ShowWidgetTooltip(self, "This option is only available for the Custom Color Theme")
                end
            end)
            tcSwatch:SetScript("OnLeave", function()
                EUICoreStandaloneMinimap.HideWidgetTooltip()
            end)
        end
        ...
        uiScaleRow, h = W:DualRow(parent, y,
            { type="slider", text="UI Scale",
              ...
              disabled=function() return EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.ppFixedScale end,
              disabledTooltip="Set UI Scale to 0.5333", requireState="disabled",
              getValue=function()
                if EUICoreStandaloneMinimap._uiScaleDragVal then
                    return EUICoreStandaloneMinimap._uiScaleDragVal
                end
                return EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.ppUIScale or EUICoreStandaloneMinimap.PP.PixelBestSize()
              end,
              setValue=function(v)
                if not EUICoreStandaloneMinimapDB then EUICoreStandaloneMinimapDB = {} end
                ...
                EUICoreStandaloneMinimap._uiScaleDragVal = v
                EUICoreStandaloneMinimapDB.ppUIScaleAuto = false
                local mf = EUICoreStandaloneMinimap._mainFrame
                local panelScaleBefore
                if mf then panelScaleBefore = mf:GetEffectiveScale() end
                EUICoreStandaloneMinimap.PP.SetUIScale(v)
                ...
                if not EUICoreStandaloneMinimap._uiScaleCleanup then
                    EUICoreStandaloneMinimap._uiScaleCleanup = true
                    C_Timer.After(0, function()
                        if not EUICoreStandaloneMinimap._sliderDragging then
                            EUICoreStandaloneMinimap._uiScaleDragVal = nil
                            EUICoreStandaloneMinimap:ShowConfirmPopup({
                        ...
                        EUICoreStandaloneMinimap._uiScaleCleanup = false
                    end)
                end
              end },
            { type="dropdown", text="EUI Options Panel Scale",
              ...
              getValue=function()
                local raw = (EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.panelScale) or 1.0
              ...
              setValue=function(v)
                ...
                if EUICoreStandaloneMinimap.SetPanelScale then
                    EUICoreStandaloneMinimap:SetPanelScale(scale)
                end
              end }
        );  y = y - h
        ...
        if not EUICoreStandaloneMinimap._prebuilding then
            ...
            local _, cogShow = EUICoreStandaloneMinimap.BuildCogPopup({
                title = "UI Scale Options",
                rows = {
                    { type="toggle", label="Set UI Scale to 0.5333",
                      tooltip="Sets the UI scale to the exact pixel-perfect value used by other addons. EUICoreStandaloneMinimap does not require this to be pixel perfect.",
                      get=function()
                          return EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.ppFixedScale or false
                      end,
                      set=function(v)
                          if not EUICoreStandaloneMinimapDB then EUICoreStandaloneMinimapDB = {} end
                          EUICoreStandaloneMinimapDB.ppFixedScale = v
                          if v then
                              EUICoreStandaloneMinimapDB.ppUIScaleAuto = false
                              EUICoreStandaloneMinimapDB.ppUIScale = 0.5333333333
                              local mf = EUICoreStandaloneMinimap._mainFrame
                              local panelScaleBefore
                              if mf then panelScaleBefore = mf:GetEffectiveScale() end
                              EUICoreStandaloneMinimap.PP.SetUIScale(0.5333333333)
                              if mf and panelScaleBefore then
                                  local newEff = UIParent:GetEffectiveScale()
                                  if newEff > 0 then mf:SetScale(panelScaleBefore / newEff) end
                              end
            ...
            cogTex:SetAllPoints(); cogTex:SetTexture(EUICoreStandaloneMinimap.COGS_ICON)
        ...
        local euiBtnItems = {
            { key = "pause",   label = "Hide Pause Menu Button",
              tooltip = "Hides the EUICoreStandaloneMinimap button from the game's Escape/pause menu." },
        ...
        euiBtnRow, h = W:DualRow(parent, y,
            { type="dropdown", text="EUI Buttons",
              tooltip="Toggle EUICoreStandaloneMinimap's optional buttons: the Escape menu buttons and the minimap button.",
            ...
            { type="toggle", text="Disable Sync Icons",
              ...
              getValue=function()
                  return EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.hideSyncIcons or false
              end,
              setValue=function(v)
                  if not EUICoreStandaloneMinimapDB then EUICoreStandaloneMinimapDB = {} end
                  EUICoreStandaloneMinimapDB.hideSyncIcons = v
                  if EUICoreStandaloneMinimap._refreshAllSyncIcons then EUICoreStandaloneMinimap._refreshAllSyncIcons() end
              end }
        );  y = y - h
        ...
        if not EUICoreStandaloneMinimap._prebuilding then
            ...
            local cbDD, cbDDRefresh = EUICoreStandaloneMinimap.BuildVisOptsCBDropdown(
                rgn, 210, rgn:GetFrameLevel() + 2,
                euiBtnItems,
                function(k)
                    if k == "pause" then
                        return EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.hideGameMenuButton or false
                    elseif k == "unlock" then
                        return not EUICoreStandaloneMinimapDB or EUICoreStandaloneMinimapDB.hideUnlockMenuButton ~= false
                    elseif k == "minimap" then
                        return not (EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.showMinimapButton == false)
                    end
                    return false
                end,
                function(k, v)
                    if not EUICoreStandaloneMinimapDB then EUICoreStandaloneMinimapDB = {} end
                    if k == "pause" then
                        EUICoreStandaloneMinimapDB.hideGameMenuButton = v
                    elseif k == "unlock" then
                        EUICoreStandaloneMinimapDB.hideUnlockMenuButton = v
                    elseif k == "minimap" then
                        EUICoreStandaloneMinimapDB.showMinimapButton = v
                        if v then EUICoreStandaloneMinimap.ShowMinimapButton() else EUICoreStandaloneMinimap.HideMinimapButton() end
                    end
                end)
            PP.Point(cbDD, "RIGHT", rgn, "RIGHT", -20, 0)
            rgn._control = cbDD
            rgn._lastInline = nil
            EUICoreStandaloneMinimap.RegisterWidgetRefresh(cbDDRefresh)
        end
            ...
            local _, cogShow = EUICoreStandaloneMinimap.BuildCogPopup({
                ...
                rows = {
                    { type="toggle", label="Only Hide Fully Synced",
                      get=function()
                          return EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.hideSyncIconsOnlyFull or false
                      end,
                      set=function(v)
                          if not EUICoreStandaloneMinimapDB then EUICoreStandaloneMinimapDB = {} end
                          EUICoreStandaloneMinimapDB.hideSyncIconsOnlyFull = v
                          if EUICoreStandaloneMinimap._refreshAllSyncIcons then EUICoreStandaloneMinimap._refreshAllSyncIcons() end
                      end },
                },
            })
        ...
        do
            ...
            local langValues = {
                _noLoc = true,
                ["auto"] = { text = EUICoreStandaloneMinimap.L("Automatic (Client)") },
                ["enUS"] = { text = "English" },
                ["deDE"] = { text = "Deutsch" },
                ["frFR"] = { text = "Français" },
            ...
            _, h = W:DualRow(parent, y,
                { type="dropdown", text="EUI Options Language",
                  tooltip="The display language for the EUICoreStandaloneMinimap options panel. Auto follows your game client. Untranslated text falls back to English.",
                  values=langValues, order=langOrder,
                  getValue=function() return (EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.displayLocale) or "auto" end,
                  setValue=function(v)
                      if v == "auto" then v = nil end
                      if EUICoreStandaloneMinimapDB then EUICoreStandaloneMinimapDB.displayLocale = v end
                      LanguageReload()
                  end },
                { type="toggle", text="Enable Tutorial Tips",
                  ...
                  getValue=function()
                      return not (EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.tutorialTipsDisabled)
                  end,
                  setValue=function(v)
                      if not EUICoreStandaloneMinimapDB then EUICoreStandaloneMinimapDB = {} end
                      EUICoreStandaloneMinimapDB.tutorialTipsDisabled = (not v) and true or nil
                      if EUICoreStandaloneMinimap.VideoGuides and EUICoreStandaloneMinimap.VideoGuides.RefreshTips then
                          EUICoreStandaloneMinimap.VideoGuides.RefreshTips()
                      end
                  end });  y = y - h
        end
        ...
        _, h = W:DualRow(parent, y,
            { type="toggle", text="Auto Expand Less Common Settings",
              ...
              getValue=function() return (EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.autoExpandLessCommon) == true end,
              setValue=function(v)
                  if not EUICoreStandaloneMinimapDB then EUICoreStandaloneMinimapDB = {} end
                  EUICoreStandaloneMinimapDB.autoExpandLessCommon = v and true or nil
                  -- Cached pages hold the old expand state; drop them all so they rebuild on next visit, then rebuild this page in place.
                  EUICoreStandaloneMinimap:InvalidatePageCache()
        ...
        local FCT_FONT_DIR = "Interface\\AddOns\\EUIStandaloneMinimap\\media\\fonts\\"
        local fctFontValues = {
        ...
        if EUICoreStandaloneMinimap.AppendSharedMediaFonts then
            EUICoreStandaloneMinimap.AppendSharedMediaFonts(fctFontValues, fctFontOrder)
        end
        _, h = W:DualRow(parent, y,
            ...
            { type="dropdown", text="Combat Text Font",
              ...
              getValue=function()
                return (EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.fctFont) or "default"
              end,
              setValue=function(v)
                if not EUICoreStandaloneMinimapDB then EUICoreStandaloneMinimapDB = {} end
                if v == "default" then
                    EUICoreStandaloneMinimapDB.fctFont = nil
                    EUICoreStandaloneMinimapDB.fctFontPath = nil
                    EUICoreStandaloneMinimapDB.fctFontPathFor = nil
                else
                    EUICoreStandaloneMinimapDB.fctFont = v
                    ...
                    EUICoreStandaloneMinimapDB.fctFontPath = e and e.font
                    EUICoreStandaloneMinimapDB.fctFontPathFor = e and v
                end
                EUICoreStandaloneMinimap:ShowConfirmPopup({
        ...
        if not EUICoreStandaloneMinimap._prebuilding then
            ...
            local _, dmgCogShow = EUICoreStandaloneMinimap.BuildCogPopup({
            ...
            dmgCogTex:SetTexture(EUICoreStandaloneMinimap.COGS_ICON)
            ...
            dmgCogBlock:SetScript("OnEnter", function()
                EUICoreStandaloneMinimap.ShowWidgetTooltip(dmgCogBtn, EUICoreStandaloneMinimap.DisabledTooltip("Show Combat Damage Text"))
            end)
            dmgCogBlock:SetScript("OnLeave", function() EUICoreStandaloneMinimap.HideWidgetTooltip() end)
        ...
        if playerClass == "DRUID" then
            _, h = W:DualRow(parent, y,
                { type="toggle", text="Prevent Swiftmend Icon Dim",
                  ...
                  setValue=function(v)
                      if not EUICoreStandaloneMinimapDB then EUICoreStandaloneMinimapDB = {} end
                      EUICoreStandaloneMinimapDB.brightenSwiftmend = v
                      if v then
        ...
        local _devDupesAvailable = C_AddOns and C_AddOns.IsAddOnLoaded
            and C_AddOns.IsAddOnLoaded("EUICoreStandaloneMinimapQoL")
            and C_AddOns.IsAddOnLoaded("EUICoreStandaloneMinimapBlizzardSkin")
        if not _devDupesAvailable then
            ...
            _, h = W:DualRow(parent, y,
                { type="toggle", text="Suppress Lua Errors",
                  getValue=function()
                    return not (EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.suppressErrors == false)
                  end,
                  setValue=function(v)
                    if not EUICoreStandaloneMinimapDB then EUICoreStandaloneMinimapDB = {} end
                    EUICoreStandaloneMinimapDB.suppressErrors = v
                    SetCVarSafe("scriptErrors", v and "0" or "1")
                  end },
                { type="toggle", text="Show Spell ID on Tooltip",
                  getValue=function()
                    return EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.showSpellID or false
                  end,
                  setValue=function(v)
                    if not EUICoreStandaloneMinimapDB then EUICoreStandaloneMinimapDB = {} end
                    EUICoreStandaloneMinimapDB.showSpellID = v
                    -- Engine-side combat aura-ID CVar rides this setting.
                    if EUICoreStandaloneMinimap.SyncAuraSpellIDCVar then EUICoreStandaloneMinimap.SyncAuraSpellIDCVar() end
                  end });  y = y - h
        ...
        do
            ...
            local lerp = EUICoreStandaloneMinimap.lerp
            local DARK_BG = EUICoreStandaloneMinimap.DARK_BG or { r = 0.05, g = 0.07, b = 0.09 }
            local btn = CreateFrame("Button", nil, parent)
            btn:SetSize(BTN_W, BTN_H)
            btn:SetPoint("TOP", parent, "TOP", 0, y)
            btn:SetFrameLevel(parent:GetFrameLevel() + 5)
            btn:SetAlpha(0.85)
            local brd = EUICoreStandaloneMinimap.MakeBorder(btn, 0.8, 0.2, 0.2, 0.5, EUICoreStandaloneMinimap.PanelPP)
            local bg = EUICoreStandaloneMinimap.SolidTex(btn, "BACKGROUND", DARK_BG.r, DARK_BG.g, DARK_BG.b, 0.92)
            bg:SetAllPoints()
            local lbl = EUICoreStandaloneMinimap.MakeFont(btn, 13, nil, 0.9, 0.3, 0.3)
            lbl:SetAlpha(0.7)
            lbl:SetPoint("CENTER")
            lbl:SetText(EUICoreStandaloneMinimap.L("Reset ALL EUI Addon Settings"))
            do
                local FADE_DUR = 0.1
            ...
            btn:SetScript("OnClick", function()
                EUICoreStandaloneMinimap:ShowConfirmPopup({
                    ...
                    onConfirm   = function()
                        ...
                        local svNames = {
                            "EUICoreStandaloneMinimapActionBarsDB",
                            "EUICoreStandaloneMinimapAuraBuffRemindersDB",
                            "EUICoreStandaloneMinimapCooldownManagerDB",
                            "EUICoreStandaloneMinimapNameplatesDB",
                            "EUICoreStandaloneMinimapResourceBarsDB",
                            "EUICoreStandaloneMinimapUnitFramesDB",
                        }
                        for _, name in ipairs(svNames) do
                            _G[name] = {}
                        end
                        local oldScale = EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.ppUIScale
                        local oldScaleAuto = EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.ppUIScaleAuto
                        -- Preserve friend group data across reset
                        local oldGlobal = EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.global
                        local savedFriends
                        if oldGlobal then
                            savedFriends = {
                        ...
                        -- Preserve QoL settings (stored on EUICoreStandaloneMinimapDB root)
                        local qolKeys = {
                        ...
                        for _, k in ipairs(qolKeys) do
                            if EUICoreStandaloneMinimapDB[k] ~= nil then
                                savedQoL[k] = EUICoreStandaloneMinimapDB[k]
                            end
                        end
                        _G["EUICoreStandaloneMinimapDB"] = {}
                        EUICoreStandaloneMinimapDB = _G["EUICoreStandaloneMinimapDB"]
                        if oldScale then EUICoreStandaloneMinimapDB.ppUIScale = oldScale end
                        if oldScaleAuto ~= nil then EUICoreStandaloneMinimapDB.ppUIScaleAuto = oldScaleAuto end
                        if savedFriends then
                            if not EUICoreStandaloneMinimapDB.global then EUICoreStandaloneMinimapDB.global = {} end
                            for k, v in pairs(savedFriends) do
                                EUICoreStandaloneMinimapDB.global[k] = v
                            end
                        end
                        for k, v in pairs(savedQoL) do
                            EUICoreStandaloneMinimapDB[k] = v
                        end
                        ReloadUI()
                    end,
                })
    ...
    local function BuildCoreOptionsPage(pageName, parent, yOffset)
        ...
        local EAB = EUICoreStandaloneMinimap.Lite and EUICoreStandaloneMinimap.Lite.GetAddon("EUICoreStandaloneMinimapActionBars", true)
    ...
    EUICoreStandaloneMinimap:RegisterOnShow(function()
        if EUICoreStandaloneMinimap:GetActiveModule() == GLOBAL_KEY then
            EUICoreStandaloneMinimap:RefreshPage()
        end
    end)
    ...
    --  Colors Page
    ---------------------------------------------------------------------------
    local CLASS_ORDER = EUICoreStandaloneMinimap.CLASS_TOKEN_ORDER
    local CLASS_LABELS = {
    ...
    local function BuildColorsPage(pageName, parent, yOffset)
        ...
        local GetCustomColorsDB = EUICoreStandaloneMinimap.GetCustomColorsDB
        local CLASS_COLOR_MAP = EUICoreStandaloneMinimap.CLASS_COLOR_MAP
        local DEFAULT_POWER_COLORS = EUICoreStandaloneMinimap.DEFAULT_POWER_COLORS
        local CONTENT_PAD = EUICoreStandaloneMinimap.CONTENT_PAD or 20
        ...
        local function SaveColorEntry(category, key, data)
            ...
            EUICoreStandaloneMinimap.ApplyColorsToOUF()
        end
        ...
        local function BuildColorGrid(par, yPos, items)            local totalRows = math.ceil(#items / GRID_COLS)
            ...
            for row = 0, totalRows - 1 do
                ...
                EUICoreStandaloneMinimap.RowBg(rowFrame, par)
                ...
                for col = 0, GRID_COLS - 1 do
                    ...
                    local swatch, updateSwatch = EUICoreStandaloneMinimap.BuildColorSwatch(cell, cell:GetFrameLevel() + 2,
                        function()
                    ...
                    EUICoreStandaloneMinimap.RegisterWidgetRefresh(updateSwatch)
                    ...
                    undoTex:SetTexture(EUICoreStandaloneMinimap.UNDO_ICON)
                    undoBtn:SetScript("OnEnter", function(self)
                        self:SetAlpha(0.6)
                        EUICoreStandaloneMinimap.ShowWidgetTooltip(self, "Reset to default")
                    end)
        ...
        local localeRestricted = EUICoreStandaloneMinimap.LOCALE_FONT_FALLBACK ~= nil
        ...
        local localeCyrillic = localeRestricted and EUICoreStandaloneMinimap.LOCALE_SCRIPT == "cyrillic"

        local fontDropValues = {}
        local fontDropOrder  = {}
        if localeRestricted then
            fontDropValues[EUICoreStandaloneMinimap.SYSTEM_FONT_KEY] = { text = "System Default", font = EUICoreStandaloneMinimap.LOCALE_FONT_FALLBACK }
            fontDropOrder[#fontDropOrder + 1] = EUICoreStandaloneMinimap.SYSTEM_FONT_KEY
            if localeCyrillic then
                ...
                local FONT_DIR_CYR = EUICoreStandaloneMinimap.MEDIA_PATH .. "fonts\\"
                local sepDone = false
                for _, name in ipairs(EUICoreStandaloneMinimap.FONT_ORDER) do
                    local cyrFile = name ~= "---" and EUICoreStandaloneMinimap.FONT_CYRILLIC[name]
                        and EUICoreStandaloneMinimap.FONT_FILES[name]
                    if cyrFile then
                        if not sepDone then
                        ...
                        fontDropValues[name] = {
                            text = (EUICoreStandaloneMinimap.FONT_DISPLAY_NAMES and EUICoreStandaloneMinimap.FONT_DISPLAY_NAMES[name]) or name,
                ...
                fontDropValues[EUICoreStandaloneMinimap.EXPRESSWAY_FORCED_KEY] = { text = "Expressway (Latin only)",
                    font = EUICoreStandaloneMinimap.MEDIA_PATH .. "fonts\\Expressway.TTF" }
                fontDropOrder[#fontDropOrder + 1] = EUICoreStandaloneMinimap.EXPRESSWAY_FORCED_KEY
            end
            if EUICoreStandaloneMinimap.AppendExternalSharedMediaFonts then
                EUICoreStandaloneMinimap.AppendExternalSharedMediaFonts(fontDropValues, fontDropOrder)
            end
        else
            local FONT_DIR_GLOBAL = EUICoreStandaloneMinimap.MEDIA_PATH .. "fonts\\"
            for _, name in ipairs(EUICoreStandaloneMinimap.FONT_ORDER) do
                if name == "---" then
                    fontDropOrder[#fontDropOrder + 1] = "---"
                else
                    local path = EUICoreStandaloneMinimap.FONT_BLIZZARD[name]
                        or (FONT_DIR_GLOBAL .. (EUICoreStandaloneMinimap.FONT_FILES[name] or "Expressway.TTF"))
            ...
            if EUICoreStandaloneMinimap.AppendSharedMediaFonts then
                EUICoreStandaloneMinimap.AppendSharedMediaFonts(fontDropValues, fontDropOrder, { keyByName = true })
            end
        end
        ...
        _, h = W:DualRow(parent, y,
            { type="dropdown", text="Global Font",
              ...
              getValue=function()
                  local g = EUICoreStandaloneMinimap.GetFontsDB().global or "Expressway"
                  ...
                  if localeCyrillic and g == EUICoreStandaloneMinimap.EXPRESSWAY_FORCED_KEY then return "Expressway" end
                  ...
                  if localeRestricted and not fontDropValues[g] then return EUICoreStandaloneMinimap.SYSTEM_FONT_KEY end
                  return g
              end,
              setValue=function(v)
                  EUICoreStandaloneMinimap.GetFontsDB().global = v
                  local rl = EUICoreStandaloneMinimap._widgetRefreshList
                  if rl then for i2 = 1, #rl do rl[i2]() end end
                  FontReload()
              end },
            { type="dropdown", text="Outline Mode",
              ...
              getValue=function()
                  local v = EUICoreStandaloneMinimap.GetFontsDB().outlineMode or "none"
                  if v == "shadow" then v = "none" end
                  return v
              end,
              setValue=function(v)
                  EUICoreStandaloneMinimap.GetFontsDB().outlineMode = v
                  local rl = EUICoreStandaloneMinimap._widgetRefreshList
                  if rl then for i2 = 1, #rl do rl[i2]() end end
                  FontReload()
              end });  y = y - h
        ...
        if not EUICoreStandaloneMinimap._prebuilding then
            ...
            oitRow, h = W:DualRow(parent, y,
                ...
                { type="toggle", text="Apply to All Game Text",
                  ...
                  getValue=function() return EUICoreStandaloneMinimap.GetFontsDB().applyToAllGameText == true end,
                  setValue=function(v)
                      EUICoreStandaloneMinimap.GetFontsDB().applyToAllGameText = v and true or false
                      FontReload()
                  end }
            );  y = y - h
            ...
            local cbDD, cbDDRefresh = EUICoreStandaloneMinimap.BuildVisOptsCBDropdown(
                ...
                function(k, v)
                    ...
                    if type(f.outlineIconText) ~= "table" then
                        local t = {}
                        local seed = EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.outlineIconText
        ...
        -- DAMAGE_TEXT_FONT), so both need a LOGOUT (not FontReload's reload) and treat "Blizzard Default" as leave-it-alone. Applied in EUICoreStandaloneMinimap_Startup.lua.
        ...
        do
            ...
            nssRow, h = W:DualRow(parent, y,
                { type="toggle", text="Disable Slug Outline",
                  ...
                  getValue=function() return EUICoreStandaloneMinimap.IsSlugDisabled() end,
                  setValue=function(v)
                      EUICoreStandaloneMinimap.GetFontsDB().neverShowSlug = v and true or false
                      FontReload()
                  end },
                { type="dropdown", text="Name Font",
                  ...
                  getValue=function()
                      local n = EUICoreStandaloneMinimap.GetFontsDB().unitNameFont
                      if not n or not nameFontValues[n] then return NAME_FONT_DEFAULT end
                      return n
                  end,
                  setValue=function(v)
                      local fdb = EUICoreStandaloneMinimap.GetFontsDB()
        ...
        -- login pass as Apply to All Game Text (EUICoreStandaloneMinimap.ApplyGlobalFontToGameText)
        ...
        -- (EUICoreStandaloneMinimap._sliderDragging, the same contract CommitInput reads),
        ...
        do
            ...
            _, h = W:DualRow(parent, y,
                { type="slider", text="Game Text Scale", min=75, max=125, step=5,
                  ...
                  getValue=function() return EUICoreStandaloneMinimap.GetFontsDB().gameTextScale or 100 end,
                  setValue=function(v)
                      local f = EUICoreStandaloneMinimap.GetFontsDB()
                      ...
                      if EUICoreStandaloneMinimap._sliderDragging then
        ...
        do
            local eg = EUICoreStandaloneMinimap.ELLESMERE_GREEN or {r=0.047, g=0.824, b=0.624}
            local fontPath = EUICoreStandaloneMinimap.EXPRESSWAY
            local outlineFlag = EUICoreStandaloneMinimap.GetFontOutlineFlag()
            local RebuildModuleFontList  -- forward declaration

            -- Build module list from ADDON_ROSTER (exclude comingSoon)
            local moduleEntries = {}
            for _, entry in ipairs(EUICoreStandaloneMinimap.ADDON_ROSTER) do
            ...
            ddBg:SetAllPoints()
            ddBg:SetColorTexture(EUICoreStandaloneMinimap.DD_BG_R, EUICoreStandaloneMinimap.DD_BG_G, EUICoreStandaloneMinimap.DD_BG_B, EUICoreStandaloneMinimap.DD_BG_A)
            EUICoreStandaloneMinimap.MakeBorder(ddBtn, 1, 1, 1, EUICoreStandaloneMinimap.DD_BRD_A, PP)
            ...
            ddLbl:SetText(EUICoreStandaloneMinimap.L("Select Module"))

            local ddArrow = EUICoreStandaloneMinimap.MakeDropdownArrow(ddBtn, 12, PP)
            ...
            local function GetSelectedLabel()
                ...
                for i, me in ipairs(moduleEntries) do
                    if selectedModuleMap[i] then
                        names[#names + 1] = EUICoreStandaloneMinimap.L(me.display)
                    end
                end
                if #names == 0 then return EUICoreStandaloneMinimap.L("Select Module") end
            ...
            EUICoreStandaloneMinimap.MakeBorder(popup, 1, 1, 1, 0.12, PP)
            ...
            sPlaceholder:SetText(EUICoreStandaloneMinimap.L("Search..."))
            ...
            for i, me in ipairs(moduleEntries) do
                ...
                EUICoreStandaloneMinimap.MakeBorder(cb, 1, 1, 1, 0.12, PP)
                ...
                lbl2:SetText(EUICoreStandaloneMinimap.L(me.display))
            ...
            -----------------------------------------------------------
            local _c = EUICoreStandaloneMinimap.WB_COLOURS
            local MF_BTN_COLOURS = {
                _c[1],  _c[2],  _c[3],  _c[4],   _c[5],  _c[6],  _c[7],  _c[8],
                1, 1, 1, EUICoreStandaloneMinimap.DD_BRD_A,   1, 1, 1, EUICoreStandaloneMinimap.DD_BRD_HA,
            ...
            EUICoreStandaloneMinimap.MakeStyledButton(addBtn, "Add Module Font", 11, MF_BTN_COLOURS, function()
                ...
                if #toAdd == 0 then
                    ...
                    if not ddBtn._redPulse then
                        ...
                        local border = EUICoreStandaloneMinimap.MakeBorder(rf, 1, 0.2, 0.2, 1, PP)
                        rf._border = border
                        ddBtn._redPulse = rf
                    end
                ...
                local fontsDB = EUICoreStandaloneMinimap.GetFontsDB()
            ...
            RebuildModuleFontList = function()
                ...
                local mfFontValues, mfFontOrder = EUICoreStandaloneMinimap.BuildFontDropdownData()
                ...
                for idx, entry in ipairs(mfList) do
                    ...
                    dualRow, dualH = W:DualRow(listContainer, -totalH,
                        { type = "dropdown", text = EUICoreStandaloneMinimap.Lf("%1$s Font", EUICoreStandaloneMinimap.L(entry.display)),
                        ...
                        { type = "dropdown", text = EUICoreStandaloneMinimap.Lf("%1$s Outline", EUICoreStandaloneMinimap.L(entry.display)),
                    ...
                    delIcon:SetTexture(EUICoreStandaloneMinimap.MEDIA_PATH .. "icons\\eui-close.png")
        ...
        do
            local DM_DEF = EUICoreStandaloneMinimap.DEFAULT_DARK_MODE
            ...
            dmMasterRow, h = W:DualRow(parent, y,
                { type = "toggle", text = "Dark Mode",
                  ...
                  getValue = function() return EUICoreStandaloneMinimap.IsDarkModeAllOn(_dmNotRB) end,
                  setValue = function(v)
                      EUICoreStandaloneMinimap.SetDarkModeAll(v, _dmNotRB)
                      EUICoreStandaloneMinimap:RefreshPage()
                  end },
                { type = "toggle", text = "Dark Mode (Class Resource Bar)",
                  tooltip = "Turns Dark Mode on or off for the class resource bar.",
                  getValue = function() return EUICoreStandaloneMinimap.IsDarkModeAllOn(_dmIsRB) end,
                  setValue = function(v)
                      EUICoreStandaloneMinimap.SetDarkModeAll(v, _dmIsRB)
                      EUICoreStandaloneMinimap:RefreshPage()
                  end });  y = y - h
            ...
            if EUICoreStandaloneMinimap.SpecOverrides_AttachEditLock and not EUICoreStandaloneMinimap._prebuilding then
                EUICoreStandaloneMinimap.SpecOverrides_AttachEditLock(dmMasterRow._leftRegion,
                    "Dark Mode drives a Dark Mode override condition and can't be changed while editing an override",
                    EUICoreStandaloneMinimap.SpecOverrides_DarkCondEditActive)
            end
            ...
            _, h = W:DualRow(parent, y,
                { type = "slider", text = "Class Color Darken",
                  ...
                  getValue = function() return EUICoreStandaloneMinimap.GetDarkModeDB().classDarken or 0 end,
                  setValue = function(v)
                      EUICoreStandaloneMinimap.GetDarkModeDB().classDarken = v
                      EUICoreStandaloneMinimap.RefreshDarkMode()
                  end },
                { type = "slider", text = "Power Color Darken",
                  min = 0, max = 100, step = 5,
                  tooltip = "Blackens every power colour by this amount, everywhere power colours are used.",
                  getValue = function() return EUICoreStandaloneMinimap.GetDarkModeDB().powerDarken or 0 end,
                  setValue = function(v)
                      EUICoreStandaloneMinimap.GetDarkModeDB().powerDarken = v
                      EUICoreStandaloneMinimap.RefreshDarkMode()
                  end });  y = y - h
            ...
            _, h = W:DualRow(parent, y,
                { type = "slider", text = "Resource Color Darken",
                  ...
                  getValue = function() return EUICoreStandaloneMinimap.GetDarkModeDB().resourceDarken or 0 end,
                  setValue = function(v)
                      EUICoreStandaloneMinimap.GetDarkModeDB().resourceDarken = v
                      EUICoreStandaloneMinimap.RefreshDarkMode()
                  end },
                { type = "slider", text = "BG Power Color Darken",
                  ...
                  getValue = function() return EUICoreStandaloneMinimap.GetDarkModeDB().powerBgDarken or 0 end,
                  setValue = function(v)
                      EUICoreStandaloneMinimap.GetDarkModeDB().powerBgDarken = v
                      EUICoreStandaloneMinimap.RefreshDarkMode()
                  end });  y = y - h
        end
        ...
        do
            local profileOrder = select(1, EUICoreStandaloneMinimap.GetProfileList()) or {}
            local pullValues = {}
            ...
            _, h = W:DualRow(parent, y,
                { type="toggle", text="Apply to All Profiles",
                  ...
                  getValue=function() return EUICoreStandaloneMinimapDB.colorsApplyToAllProfiles ~= false end,
                  setValue=function(v)
                      EUICoreStandaloneMinimapDB.colorsApplyToAllProfiles = v
                      EUICoreStandaloneMinimap.ApplyColorsToOUF()
                ...
                { type="dropdown", text="Pull Colors From",
                  ...
                  disabled=function() return EUICoreStandaloneMinimapDB.colorsApplyToAllProfiles == false end,
                  disabledTooltip="Apply to All Profiles",
                  getValue=function() return EUICoreStandaloneMinimapDB.colorsPullFrom or profileOrder[1] end,
                  setValue=function(v)
                      EUICoreStandaloneMinimapDB.colorsPullFrom = v
                      EUICoreStandaloneMinimap.ApplyColorsToOUF()
                      EUICoreStandaloneMinimap:RefreshPage()
                  end });  y = y - h
        end
        ...
        for _, token in ipairs(CLASS_ORDER) do
            ...
            classItems[#classItems + 1] = {
                label = EUICoreStandaloneMinimap.L(lbl),
                classToken = token,
                getColor = function()
                    local db = GetCustomColorsDB()
        ...
        for _, pk in ipairs(POWER_ORDER) do
            ...
            powerItems[#powerItems + 1] = {
                ...
                resetFn = function()
                    EUICoreStandaloneMinimap.ResetPowerColor(pk)
                end,
            }
        end
        ...
        do
            ...
            for _, it in ipairs(items) do
                ...
                resourceItems[#resourceItems + 1] = {
                    label = EUICoreStandaloneMinimap.L(it.label),
                    getColor = function()
                        return EUICoreStandaloneMinimap.GetClassResourceColor(key)
                            or { r = 1, g = 1, b = 1 }
                    end,
                    setColor = function(c)
        ...
        do
            local gates = {}
            local CPAD = EUICoreStandaloneMinimap.CONTENT_PAD or 20  -- side inset so the overlay matches the grid content width
            local function MakeColorGate(topY, botY)
                ...
                local msg = EUICoreStandaloneMinimap.MakeFont(ov, 13, nil, 1, 1, 1)
            ...
            local function UpdateColorGate()
                local locked = EUICoreStandaloneMinimap.IsColorEditingLocked and EUICoreStandaloneMinimap.IsColorEditingLocked()
                local text
                if locked then
                    local p = EUICoreStandaloneMinimap.GetProfilesDB()
                    local srcName = EUICoreStandaloneMinimapDB.colorsPullFrom or (p.profileOrder and p.profileOrder[1]) or ""
                    text = EUICoreStandaloneMinimap.Lf("Colors are shared globally from the \"%1$s\" profile.\nSwitch to it (or set Pull Colors From to this profile) to edit.", srcName)
                end
            ...
            UpdateColorGate()
            EUICoreStandaloneMinimap.RegisterWidgetRefresh(UpdateColorGate)
        end
    ...
    local function BuildScaleWarning(payload, skipScale)
        ...
        local warnings = {}
        local myScale  = EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.ppUIScale or (UIParent and UIParent:GetScale()) or 1
        ...
        if not skipScale and expScale and math.abs(myScale - expScale) > 0.02 then
            ...
            warnings[#warnings + 1] = EUICoreStandaloneMinimap.Lf("UI Scale Issue: Profile was made at %1$d%%, yours is %2$d%%", expPct, myPct)
        end
        ...
        if expSW > 0 and expSH > 0 and (mySW ~= expSW or mySH ~= expSH) then
            warnings[#warnings + 1] = EUICoreStandaloneMinimap.Lf("Resolution Issue: Profile was made at %1$dx%2$d, yours is %3$dx%4$d", expSW, expSH, mySW, mySH)
        end
        if #warnings == 0 then return nil end
        return EUICoreStandaloneMinimap.L("WARNING: Frame positions may be off.") .. "\n" .. table.concat(warnings, "\n")
    end
    ...
    local function MaybeConfirmUIScale(payload, cont)
        ...
        local myScale = EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.ppUIScale
        ...
        local myPct  = math.floor(myScale * 100 + 0.5)
        EUICoreStandaloneMinimap:ShowConfirmPopup({
            title = EUICoreStandaloneMinimap.L("UI Scale Mismatch"),
            message = EUICoreStandaloneMinimap.Lf("This profile was made at %1$d%% UI scale; yours is %2$d%%. Change your UI scale to match the imported profile? This will show all profiles at this scale as UI Scale is not a per-profile setting, but can be changed at any time back to your original value.", expPct, myPct),
            confirmText = EUICoreStandaloneMinimap.L("Match Scale"),
            cancelText = EUICoreStandaloneMinimap.L("Keep Mine"),
    ...
    local function BuildProfilesPage(pageName, parent, yOffset)
        ...
        local y = yOffset
        local _, h
        local FONT = EUICoreStandaloneMinimap.EXPRESSWAY
        local EG = EUICoreStandaloneMinimap.ELLESMERE_GREEN
        local MEDIA = "Interface\\AddOns\\EUIStandaloneMinimap\\media\\"
        ...
        do
            ...
            if sid then
                local assigned = EUICoreStandaloneMinimap.GetSpecProfile(sid)
                if assigned then
                    local current = EUICoreStandaloneMinimap.GetActiveProfileName()
                    if assigned ~= current then
                        local _, profiles = EUICoreStandaloneMinimap.GetProfileList()
                        if profiles and profiles[assigned] then
                            local fontWillChange = EUICoreStandaloneMinimap.ProfileChangesFont(profiles[assigned])
                            local skinsWillChange = EUICoreStandaloneMinimap.ProfileChangesWindowSkins
                                and EUICoreStandaloneMinimap.ProfileChangesWindowSkins(profiles[assigned])
                            EUICoreStandaloneMinimap.SwitchProfile(assigned)
                            -- true = budgeted: manual apply (no spec change
                            -- in flight), watchdog-sliced module refresh.
                            EUICoreStandaloneMinimap.RefreshAllAddons(true)
                            if fontWillChange or skinsWillChange then
                                EUICoreStandaloneMinimap:ShowConfirmPopup({
                                    title       = EUICoreStandaloneMinimap.L("Reload Required"),
                                    message     = fontWillChange
                                        and EUICoreStandaloneMinimap.L("Font changed. A UI reload is needed to apply the new font.")
                                        or EUICoreStandaloneMinimap.L("Window skins changed for this profile. A UI reload is needed to apply them."),
                                    confirmText = EUICoreStandaloneMinimap.L("Reload Now"),
                                    cancelText  = EUICoreStandaloneMinimap.L("Later"),
                                    onConfirm   = function() ReloadUI() end,
                                })
                            end
                        end
        ...
        local scrollFrame = EUICoreStandaloneMinimap._scrollFrame
        if not scrollFrame then return 0 end

        if EUICoreStandaloneMinimap._profilesRoot then
            EUICoreStandaloneMinimap._profilesRoot:Hide()
            EUICoreStandaloneMinimap._profilesRoot:SetParent(nil)
        end

        local root = CreateFrame("Frame", nil, scrollFrame)
        root:SetAllPoints(scrollFrame)
        root:SetFrameLevel(scrollFrame:GetFrameLevel() + 5)
        EUICoreStandaloneMinimap._profilesRoot = root
        ...
        local function MakeDropdown(parentFrame, w, ddH, getLabel)
            ...
            bg:SetAllPoints()
            bg:SetColorTexture(EUICoreStandaloneMinimap.DD_BG_R, EUICoreStandaloneMinimap.DD_BG_G, EUICoreStandaloneMinimap.DD_BG_B, EUICoreStandaloneMinimap.DD_BG_A)
            local brd = EUICoreStandaloneMinimap.MakeBorder(btn, 1, 1, 1, EUICoreStandaloneMinimap.DD_BRD_A, PP)
            local lbl = EUICoreStandaloneMinimap.MakeFont(btn, 13, nil, 1, 1, 1)
            lbl:SetAlpha(EUICoreStandaloneMinimap.DD_TXT_A)
            lbl:SetJustifyH("LEFT")
            lbl:SetWordWrap(false)
            lbl:SetMaxLines(1)
            lbl:SetPoint("LEFT", btn, "LEFT", 12, 0)
            local arrow = EUICoreStandaloneMinimap.MakeDropdownArrow(btn, 12, PP)
            lbl:SetPoint("RIGHT", arrow, "LEFT", -5, 0)
            lbl:SetText(getLabel())
            local s = EUICoreStandaloneMinimap.RD_DD_COLOURS
        ...
        local function MakeDropdownMenu(anchor, w)
            ...
            bg:SetAllPoints()
            bg:SetColorTexture(EUICoreStandaloneMinimap.DD_BG_R, EUICoreStandaloneMinimap.DD_BG_G, EUICoreStandaloneMinimap.DD_BG_B, 0.98)
            EUICoreStandaloneMinimap.MakeBorder(menuFrame, 1, 1, 1, EUICoreStandaloneMinimap.DD_BRD_A, PP)
        ...
        local function MakeMenuItems(menuFrame, items, onSelect)
            ...
            for i, item in ipairs(items) do
                ...
                lbl:SetFont(FONT, 13, EUICoreStandaloneMinimap.GetFontOutlineFlag())
                lbl:SetPoint("LEFT", itm, "LEFT", 10, 0)
                lbl:SetJustifyH("LEFT")
                lbl:SetTextColor(1, 1, 1, EUICoreStandaloneMinimap.TEXT_DIM_A)
                itm._lbl = lbl
                local hl = itm:CreateTexture(nil, "ARTWORK")
                hl:SetAllPoints(); hl:SetColorTexture(1, 1, 1, 1); hl:SetAlpha(0)
                itm._hl = hl
                itm:SetScript("OnEnter", function() lbl:SetTextColor(1,1,1,1); hl:SetAlpha(EUICoreStandaloneMinimap.DD_ITEM_HL_A) end)
                itm:SetScript("OnLeave", function()
                    lbl:SetTextColor(1, 1, 1, EUICoreStandaloneMinimap.TEXT_DIM_A)
                    hl:SetAlpha(itm._isSel and EUICoreStandaloneMinimap.DD_ITEM_SEL_A or 0)
                end)
        ...
        local function DoPresetImportFlow(exportString, defaultName, editModeString, editModeLayoutName)
            ...
            EUICoreStandaloneMinimap.DecodeImportStringAsync(exportString, function(payload, err)
                if not payload then
                    EUICoreStandaloneMinimap:ShowInfoPopup({ title = EUICoreStandaloneMinimap.L("Import Failed"), content = err or EUICoreStandaloneMinimap.L("Invalid preset data.") })
                    return
                end
        ...
        local function FormatKey(key)
            if not key then return EUICoreStandaloneMinimap.L("Not Bound") end
            local parts = {}
        ...
        local function ShowProfileKeybindPopup(profileName)
            ...
            EUICoreStandaloneMinimap.MakeBorder(popup, 1, 1, 1, 0.20, PP)

            local title = EUICoreStandaloneMinimap.MakeFont(popup, 14, nil, 1, 1, 1)
            title:SetPoint("TOP", popup, "TOP", 0, -14)
            title:SetText(EUICoreStandaloneMinimap.Lf("Keybind: %1$s", profileName))
            ...
            local kbBg = EUICoreStandaloneMinimap.SolidTex(kbBtn, "BACKGROUND", EUICoreStandaloneMinimap.DD_BG_R, EUICoreStandaloneMinimap.DD_BG_G, EUICoreStandaloneMinimap.DD_BG_B, EUICoreStandaloneMinimap.DD_BG_A)
            kbBg:SetAllPoints()
            kbBtn._border = EUICoreStandaloneMinimap.MakeBorder(kbBtn, 1, 1, 1, EUICoreStandaloneMinimap.DD_BRD_A, PP)
            local kbLbl = EUICoreStandaloneMinimap.MakeFont(kbBtn, 13, nil, 1, 1, 1)
            kbLbl:SetAlpha(EUICoreStandaloneMinimap.DD_TXT_A or 0.85)
            kbLbl:SetPoint("CENTER")

            local function RefreshLabel()
                local kkey = EUICoreStandaloneMinimap.GetProfileKeybind(profileName)
                kbLbl:SetText(FormatKey(kkey))
            end
            RefreshLabel()

            local hint = EUICoreStandaloneMinimap.MakeFont(popup, 10, nil, 1, 1, 1, 0.35)
            hint:SetPoint("BOTTOM", popup, "BOTTOM", 0, 12)
            hint:SetText(EUICoreStandaloneMinimap.L("Left-click to set  |  Right-click to unbind  |  Esc to close"))
            ...
            kbBtn:SetScript("OnClick", function(self, button)
                if button == "RightButton" then
                    ...
                    EUICoreStandaloneMinimap.SetProfileKeybind(profileName, nil)
                    RefreshLabel()
                    return
                end
                if listening then return end
                listening = true
                kbLbl:SetText(EUICoreStandaloneMinimap.L("Press a key..."))
            ...
            kbBtn:SetScript("OnKeyDown", function(self, kkey)
                ...
                EUICoreStandaloneMinimap.SetProfileKeybind(profileName, fullKey)
                listening = false
                self:EnableKeyboard(false)
                RefreshLabel()
            end)
            ...
            kbBtn:SetScript("OnEnter", function()
                kbBg:SetColorTexture(EUICoreStandaloneMinimap.DD_BG_R, EUICoreStandaloneMinimap.DD_BG_G, EUICoreStandaloneMinimap.DD_BG_B, EUICoreStandaloneMinimap.DD_BG_HA or 0.98)
                if kbBtn._border and kbBtn._border.SetColor then
                    kbBtn._border:SetColor(1, 1, 1, 0.3)
                end
                EUICoreStandaloneMinimap.ShowWidgetTooltip(kbBtn, EUICoreStandaloneMinimap.L("Left-click to set a keybind.\nRight-click to unbind."))
            end)
            kbBtn:SetScript("OnLeave", function()
                if listening then return end
                kbBg:SetColorTexture(EUICoreStandaloneMinimap.DD_BG_R, EUICoreStandaloneMinimap.DD_BG_G, EUICoreStandaloneMinimap.DD_BG_B, EUICoreStandaloneMinimap.DD_BG_A)
                if kbBtn._border and kbBtn._border.SetColor then
                    kbBtn._border:SetColor(1, 1, 1, EUICoreStandaloneMinimap.DD_BRD_A)
                end
                EUICoreStandaloneMinimap.HideWidgetTooltip()
            end)
        ...
        local function BuildErrorFlash(btn, brd)
            ...
            flashFrame:SetScript("OnUpdate", function(self, dt)
                ...
                if elapsed >= FLASH_DUR then
                    self:Hide()
                    brd:SetColor(1, 1, 1, EUICoreStandaloneMinimap.DD_BRD_A)
                    return
                end
                local t = elapsed / FLASH_DUR
                brd:SetColor(lerp(0.9, 1, t), lerp(0.15, 1, t), lerp(0.15, 1, t), lerp(0.7, EUICoreStandaloneMinimap.DD_BRD_A, t))
            end)
            return function()
                elapsed = 0
        ...
        ShowImportPage = function(exportString, payload, defaultName, editModeString, editModeLayoutName, applyImportedScale)
            ...
            local ADDON_DB_MAP_LOCAL = EUICoreStandaloneMinimap._ADDON_DB_MAP
            local PAD        = EUICoreStandaloneMinimap.CONTENT_PAD
            ...
            local ADDON_DESCS = {
                EUICoreStandaloneMinimapActionBars        = "Modern action bars built for performance and clarity.",
                EUICoreStandaloneMinimapNameplates        = "Clean, lightweight nameplates with endless customization.",
                EUICoreStandaloneMinimapUnitFrames        = "Simple unit frames with a modern visual style.",
                EUICoreStandaloneMinimapCooldownManager   = "A CDM replacement focused on performance, customizations and alerts.",
                EUICoreStandaloneMinimapResourceBars      = "Custom Resource Bars with thresholds, hash lines and more.",
                EUICoreStandaloneMinimapRaidFrames        = "Incredibly light performance, modern raid frames with endless flexibility.",
                EUICoreStandaloneMinimapAuraBuffReminders = "Simple raid buff, auras, consumables and talent reminders.",
                EUICoreStandaloneMinimapQoL               = "Lightweight quality of life tools and enhancements.",
                EUICoreStandaloneMinimapDragonRiding      = "Skyriding HUD with speed, vigor and second wind tracking.",
                EUICoreStandaloneMinimapBlizzardSkin       = "Clean and beautiful visual refreshes for Blizzard UI elements.",
                EUICoreStandaloneMinimapFriends           = "A modern friends list with built-in organization tools.",
                EUICoreStandaloneMinimapMythicTimer       = "Mythic+ timer, targeted spell bars, and standalone cast bars.",
                EUICoreStandaloneMinimapQuestTracker      = "A clean, updated reskin of Blizzard's Quest Tracker.",
                EUIStandaloneMinimap           = "A new age minimap with clean styling and square layout options.",
                EUICoreStandaloneMinimapDamageMeters      = "Lightweight damage meters with simple but powerful customization.",
                EUICoreStandaloneMinimapChat              = "Modern chat enhancements with useful utilities.",
                EUICoreStandaloneMinimapBags              = "A beautiful visual refresh of Blizzard Bags with intuitive organization.",
                EUICoreStandaloneMinimapQuickdraw         = "Hold a key to open a menu of actions; point or scroll to choose, release to fire.",
            }
            ...
            local backBrd = EUICoreStandaloneMinimap.MakeBorder(backBtn, 1, 1, 1, 0.12, PP)
            ...
            local backLbl = EUICoreStandaloneMinimap.MakeFont(backBtn, 12, nil, 1, 1, 1, 0.55)
            PP.Point(backLbl, "LEFT", backIcon, "RIGHT", 6, 0)
            backLbl:SetText(EUICoreStandaloneMinimap.L("Back"))
            ...
            local titleFs = EUICoreStandaloneMinimap.MakeFont(importPage, 16, nil, 1, 1, 1, 0.95)
            PP.Point(titleFs, "TOP", importPage, "TOP", 0, iy - BACK_H / 2 + 8)
            titleFs:SetText(EUICoreStandaloneMinimap.Lf("Importing %1$s", (defaultName or EUICoreStandaloneMinimap.L("Profile"))))
            titleFs:SetJustifyH("CENTER")

            iy = iy - BACK_H - 8

            if scaleWarnText then
                local warnFs = EUICoreStandaloneMinimap.MakeFont(importPage, 13, nil, 0.9, 0.2, 0.2, 0.85)
            ...
            do
                local INPUT_H = 30
                local INPUT_W = 300
                local nameLabel = EUICoreStandaloneMinimap.MakeFont(importPage, 12, nil, 1, 1, 1, 0.45)
                PP.Point(nameLabel, "TOPLEFT", importPage, "TOPLEFT", PAD, iy)
                nameLabel:SetText(EUICoreStandaloneMinimap.L("Profile Name"))
                ...
                iBg:SetAllPoints()
                iBg:SetColorTexture(EUICoreStandaloneMinimap.DD_BG_R, EUICoreStandaloneMinimap.DD_BG_G, EUICoreStandaloneMinimap.DD_BG_B, EUICoreStandaloneMinimap.DD_BG_A)
                local inputBrd = EUICoreStandaloneMinimap.MakeBorder(inputFrame, 1, 1, 1, EUICoreStandaloneMinimap.DD_BRD_A, PP)
                ...
                editBox:SetFont(FONT, 12, EUICoreStandaloneMinimap.GetFontOutlineFlag())
                ...
                placeholder:SetFont(FONT, 12, EUICoreStandaloneMinimap.GetFontOutlineFlag())
                placeholder:SetTextColor(1, 1, 1, 0.25)
                placeholder:SetPoint("LEFT", editBox, "LEFT", 0, 0)
                placeholder:SetText(EUICoreStandaloneMinimap.L("Profile name..."))
            ...
            for _, entry in ipairs(ADDON_DB_MAP_LOCAL) do
                ...
                local loaded = EUICoreStandaloneMinimap.IsModuleAddonLoaded(folder)
            ...
            do
                ...
                importComponents = EUICoreStandaloneMinimap.BuildModuleComponents(
                    ul, EUICoreStandaloneMinimap.BuildImportKeyToFolder(ul, meta and meta.keyToFolder))
            end
            ...
            local function RefreshImportCount()
                ...
                importCountFs:SetText(EUICoreStandaloneMinimap.Lf("Import will include %1$s of %2$s addons.", count, #addonItems))
            end
            ...
            EUICoreStandaloneMinimap.MakeBorder(sectionBg, 1, 1, 1, 0.10, PP)
            ...
            local hdrTitle = EUICoreStandaloneMinimap.MakeFont(hdrFrame, 14, nil, 1, 1, 1, 0.9)
            PP.Point(hdrTitle, "TOPLEFT", hdrFrame, "TOPLEFT", SIDE_PAD, -20)
            hdrTitle:SetText(EUICoreStandaloneMinimap.L("Import Addons"))
            hdrTitle:SetJustifyH("LEFT")

            local hdrDesc = EUICoreStandaloneMinimap.MakeFont(hdrFrame, 11, nil, 1, 1, 1, 0.35)
            PP.Point(hdrDesc, "TOPLEFT", hdrTitle, "BOTTOMLEFT", 0, -9)
            PP.Point(hdrDesc, "RIGHT", hdrFrame, "RIGHT", -(160 + SIDE_PAD), 0)
            hdrDesc:SetText(EUICoreStandaloneMinimap.L("Choose which addons to import. Any addons not included will use your active profile's settings in the new profile."))
            ...
            do
                ...
                selAllLbl:SetFont(FONT, 12, EUICoreStandaloneMinimap.GetFontOutlineFlag())
                selAllLbl:SetText(EUICoreStandaloneMinimap.L("Select All"))
                ...
                deselLbl:SetFont(FONT, 12, EUICoreStandaloneMinimap.GetFontOutlineFlag())
                deselLbl:SetText(EUICoreStandaloneMinimap.L("Deselect All"))
            ...
            local colAddon = EUICoreStandaloneMinimap.MakeFont(colHdrFrame, 11, nil, 1, 1, 1, 0.40)
            PP.Point(colAddon, "LEFT", colHdrFrame, "LEFT", SIDE_PAD, 0)
            colAddon:SetText(EUICoreStandaloneMinimap.L("Addon"))
            colAddon:SetJustifyH("LEFT")

            local colStatus = EUICoreStandaloneMinimap.MakeFont(colHdrFrame, 11, nil, 1, 1, 1, 0.40)
            PP.Point(colStatus, "RIGHT", colHdrFrame, "RIGHT", -SIDE_PAD, 0)
            colStatus:SetText(EUICoreStandaloneMinimap.L("Status"))
            colStatus:SetJustifyH("RIGHT")

            local colInclude = EUICoreStandaloneMinimap.MakeFont(colHdrFrame, 11, nil, 1, 1, 1, 0.40)
            PP.Point(colInclude, "CENTER", colHdrFrame, "RIGHT", INCLUDE_CENTER_X, 0)
            colInclude:SetText(EUICoreStandaloneMinimap.L("Include"))
            ...
            for i, item in ipairs(addonItems) do
                ...
                local nameFs = EUICoreStandaloneMinimap.MakeFont(rowFrame, 13, nil, 1, 1, 1, 0.9)
                nameFs:SetPoint("TOPLEFT", rowFrame, "TOPLEFT", SIDE_PAD, -10)
                nameFs:SetPoint("RIGHT", rowFrame, "RIGHT", -(CHK_SZ + STATUS_W + SIDE_PAD * 2 + 20), 0)
                nameFs:SetJustifyH("LEFT")
                nameFs:SetWordWrap(false)
                nameFs:SetText(EUICoreStandaloneMinimap.L(item.display))

                local descFs = EUICoreStandaloneMinimap.MakeFont(rowFrame, 11, nil, 1, 1, 1, 0.30)
                descFs:SetPoint("TOPLEFT", nameFs, "BOTTOMLEFT", 0, -5)
                descFs:SetPoint("RIGHT", nameFs, "RIGHT", 0, 0)
                descFs:SetJustifyH("LEFT")
                descFs:SetWordWrap(false)
                descFs:SetText(EUICoreStandaloneMinimap.L(item.desc))

                local statusFs = EUICoreStandaloneMinimap.MakeFont(rowFrame, 11, nil, 1, 1, 1, 0.40)
                ...
                local chkBrd = EUICoreStandaloneMinimap.MakeBorder(chkFrame, 0.25, 0.25, 0.28, 0.6, PP)
                ...
                local function ApplyRowVisual()
                    ...
                    if not item.canImport then
                        ...
                        if not item.inPayload then
                            statusFs:SetText(EUICoreStandaloneMinimap.L("Not Included"))
                            statusFs:SetTextColor(0.9, 0.2, 0.2, 0.7)
                        else
                            statusFs:SetText(EUICoreStandaloneMinimap.L("Not Loaded"))
                    ...
                    elseif on then
                        ...
                        statusFs:SetText(EUICoreStandaloneMinimap.L("Ready"))
                        ...
                        statusFs:SetText(EUICoreStandaloneMinimap.L("Skipped"))
                        ...
                        if members then
                            ...
                            for f in pairs(members) do
                                if f ~= item.canon then
                                    names[#names + 1] = EUICoreStandaloneMinimap.L(CANON_DISPLAY[f] or f)
                                end
                            end
                            if #names > 0 then
                                table.sort(names)
                                EUICoreStandaloneMinimap.ShowWidgetTooltip(rowFrame,
                                    EUICoreStandaloneMinimap.Lf("Linked by Anchor/Width/Height Matching to: %1$s. These import together.", table.concat(names, ", ")))
                            end
                        end
                    end)
            ...
            importCountFs = EUICoreStandaloneMinimap.MakeFont(footerFrame, 12, nil, 1, 1, 1, 0.40)
            ...
            if hasSpecAssign then
                ...
                EUICoreStandaloneMinimap.MakeBorder(box, 0.25, 0.25, 0.28, 0.6, PP)
                ...
                local lbl = EUICoreStandaloneMinimap.MakeFont(aaBtn, 12, nil, 1, 1, 1, 0.6)
                lbl:SetPoint("LEFT", box, "RIGHT", 6, 0)
                lbl:SetText(EUICoreStandaloneMinimap.L("Auto Assign to Specs"))
                ...
                aaBtn:SetScript("OnEnter", function()
                    EUICoreStandaloneMinimap.ShowWidgetTooltip(aaBtn, EUICoreStandaloneMinimap.L("Assign this profile to the same specializations it was assigned to on export. Off = your current spec assignments stay as they are."))
                end)
                aaBtn:SetScript("OnLeave", function() EUICoreStandaloneMinimap.HideWidgetTooltip() end)
            end
            ...
            local DB = EUICoreStandaloneMinimap.DARK_BG
            local impBrd = EUICoreStandaloneMinimap.MakeBorder(importBtn, EG.r, EG.g, EG.b, 0.7, PP)
            local impBg = EUICoreStandaloneMinimap.SolidTex(importBtn, "BACKGROUND", DB.r, DB.g, DB.b, 0.92)
            impBg:SetAllPoints()
            local impLbl = EUICoreStandaloneMinimap.MakeFont(importBtn, 12, nil, EG.r, EG.g, EG.b)
            impLbl:SetAlpha(0.7)
            impLbl:SetPoint("CENTER")
            impLbl:SetText(EUICoreStandaloneMinimap.L("Import Selected Addons"))
            ...
            do
                ...
                local incLbl = EUICoreStandaloneMinimap.MakeFont(footerFrame, 12, nil, 1, 1, 1, 0.6)
                PP.Point(incLbl, "RIGHT", ddBtn, "LEFT", -8, 0)
                incLbl:SetText(EUICoreStandaloneMinimap.L("Include:"))
                ...
                local rowDefs = {
                    ...
                    { label = "Window & Tooltip Skins", sum = "Window Skins",
                      ...
                      set   = function(refresh)
                          ...
                          EUICoreStandaloneMinimap:ShowConfirmPopup({
                              title       = EUICoreStandaloneMinimap.L("Overwrite Window & Tooltip Settings?"),
                              message     = EUICoreStandaloneMinimap.L("This will replace YOUR Blizz UI Enhanced settings (the Window Skins and Tooltips, Menus & Popups tabs) with the sharer's, across ALL of your profiles. Your current settings on those two tabs cannot be recovered afterward."),
                              confirmText = EUICoreStandaloneMinimap.L("OK"),
                              cancelText  = EUICoreStandaloneMinimap.L("Cancel"),
                              onConfirm   = function()
                ...
                local function Summary()
                    ...
                    for _, def in ipairs(rowDefs) do
                        if def.enabled then
                            total = total + 1
                            if def.get() then parts[#parts + 1] = EUICoreStandaloneMinimap.L(def.sum) end
                        end
                    end
                    if #parts == 0 then return EUICoreStandaloneMinimap.L("Nothing Extra") end
                    if #parts == total then return EUICoreStandaloneMinimap.L("Everything") end
                    return table.concat(parts, ", ")
                end
                ...
                for i, def in ipairs(rowDefs) do
                    ...
                    mark:SetColorTexture(EG.r, EG.g, EG.b, 1)
                    marks[i] = mark
                    local lbl = EUICoreStandaloneMinimap.MakeFont(row, 12, nil, 1, 1, 1, 0.7)
                    lbl:SetPoint("LEFT", box, "RIGHT", 8, 0)
                    lbl:SetText(EUICoreStandaloneMinimap.L(def.label))
                    if def.enabled then
                        row:SetScript("OnEnter", function()
                            hl:SetAlpha(0.05)
                            EUICoreStandaloneMinimap.ShowWidgetTooltip(row, EUICoreStandaloneMinimap.L(def.tip))
                        end)
                        row:SetScript("OnLeave", function()
                            hl:SetAlpha(0)
                            EUICoreStandaloneMinimap.HideWidgetTooltip()
                        ...
                        row:SetAlpha(0.35)
                        row:SetScript("OnEnter", function()
                            EUICoreStandaloneMinimap.ShowWidgetTooltip(row, EUICoreStandaloneMinimap.L(def.offTip))
                        end)
                        row:SetScript("OnLeave", function() EUICoreStandaloneMinimap.HideWidgetTooltip() end)
                    end
                end
            ...
            local IMP_FADE = 0.1
            local impLerp = EUICoreStandaloneMinimap.lerp
            ...
            importBtn:SetScript("OnClick", function()
                ...
                local apiS = EUICoreStandaloneMinimap._apiImportSession
                local apiOverwrite = apiS and apiS.state ~= "done" and apiS.name == name
                local _, existingProfiles = EUICoreStandaloneMinimap.GetProfileList()
                if existingProfiles and existingProfiles[name] and not apiOverwrite then
                    EUICoreStandaloneMinimap:ShowConfirmPopup({
                        title = EUICoreStandaloneMinimap.L("Name Taken"),
                        message = EUICoreStandaloneMinimap.Lf("A profile named \"%1$s\" already exists. Please choose a different name.", name),
                        confirmText = EUICoreStandaloneMinimap.L("OK"),
                        hideCancel = true,
                ...
                local filteredPayload = EUICoreStandaloneMinimap._DeepCopy(payload)
                ...
                if filteredPayload and filteredPayload.data then
                    if not selectedImports["EUICoreStandaloneMinimapCooldownManager"] then
                ...
                if filteredPayload and filteredPayload.data then
                    ...
                    if ul and includeLayoutImport then
                        ...
                        local k2f = EUICoreStandaloneMinimap.BuildImportKeyToFolder(ul, meta and meta.keyToFolder)
                        filteredPayload.data.unlockLayout =
                            EUICoreStandaloneMinimap.FilterLayoutToFolders(ul, selectedImports, k2f)
                    else
                ...
                local function commit()
                    ...
                    local apiSession = EUICoreStandaloneMinimap._apiImportSession
                    if apiSession and apiSession.state == "done" then apiSession = nil end
                    if apiSession then apiSession.committing = true end
                    local ok, err, status = EUICoreStandaloneMinimap.ImportProfile(filteredPayload, name)
                    ...
                    if ok and importPage._editModeString then
                        pcall(EUICoreStandaloneMinimap.ApplyPresetEditMode, importPage._editModeString, importPage._editModeLayoutName)
                    end
                    if ok and apiSession then
                        ...
                        if status == "spec_locked" then
                            EUICoreStandaloneMinimap.Print(EUICoreStandaloneMinimap.Lf("\"%1$s\" was saved but cannot be loaded because this spec has an assigned profile.", name))
                        end
                        EUICoreStandaloneMinimap._FinishApiImportSession(true)
                        if EUICoreStandaloneMinimap._ProfilesResetToMain then pcall(EUICoreStandaloneMinimap._ProfilesResetToMain) end
                        EUICoreStandaloneMinimap:Hide()
                    elseif ok and status == "spec_locked" then
                        EUICoreStandaloneMinimap:ShowInfoPopup({
                            title   = EUICoreStandaloneMinimap.L("Profile Imported"),
                            content = EUICoreStandaloneMinimap.Lf("\"%1$s\" was saved but cannot be loaded because this spec has an assigned profile. Switch specs or remove the spec assignment to use it.", name),
                        })
                        ReloadUI()
                    elseif ok then
                        ReloadUI()
                    else
                        EUICoreStandaloneMinimap:ShowInfoPopup({ title = EUICoreStandaloneMinimap.L("Import Failed"), content = err or EUICoreStandaloneMinimap.L("Unknown error") })
                    end
                end
            ...
            local nameErrorFs = EUICoreStandaloneMinimap.MakeFont(footerFrame, 11, nil, 0.9, 0.2, 0.2)
            nameErrorFs:SetJustifyH("RIGHT")
            PP.Point(nameErrorFs, "TOPRIGHT", importBtn, "BOTTOMRIGHT", 0, -14)
            nameErrorFs:SetText(EUICoreStandaloneMinimap.L("*Please enter a profile name"))
            nameErrorFs:Hide()
        ...
        local function ShowPastePage()
            ...
            local titleFs = EUICoreStandaloneMinimap.MakeFont(pastePage, 16, nil, 1, 1, 1, 0.95)
            PP.Point(titleFs, "TOP", pastePage, "TOP", 0, py - BACK_H / 2 + 8)
            titleFs:SetText(EUICoreStandaloneMinimap.L("Import Profile"))
            titleFs:SetJustifyH("CENTER")
            ...
            EUICoreStandaloneMinimap.MakeBorder(panelFrame, 1, 1, 1, 0.10, PP)
            ...
            pasteBox:SetFont(FONT, 11, EUICoreStandaloneMinimap.GetFontOutlineFlag())
            ...
            pastePlaceholder:SetFont(FONT, 12, EUICoreStandaloneMinimap.GetFontOutlineFlag())
            pastePlaceholder:SetTextColor(1, 1, 1, 0.20)
            pastePlaceholder:SetPoint("TOPLEFT", pasteSF, "TOPLEFT", 0, 0)
            pastePlaceholder:SetText(EUICoreStandaloneMinimap.L("Paste your profile string here..."))
            ...
            local pasteAbsorber = EUICoreStandaloneMinimap.AttachImportPasteAbsorber(pasteBox, function()
                EUICoreStandaloneMinimap:ShowInfoPopup({
                    title   = EUICoreStandaloneMinimap.L("Paste Interrupted"),
                    content = EUICoreStandaloneMinimap.L("The pasted string could not be read completely. Please paste it again."),
                })
            end)
            ...
            local cDB = EUICoreStandaloneMinimap.DARK_BG
            local contBrd = EUICoreStandaloneMinimap.MakeBorder(contBtn, EG.r, EG.g, EG.b, 0.7, PP)
            local contBg = EUICoreStandaloneMinimap.SolidTex(contBtn, "BACKGROUND", cDB.r, cDB.g, cDB.b, 0.92)
            contBg:SetAllPoints()
            local contLbl = EUICoreStandaloneMinimap.MakeFont(contBtn, 13, nil, EG.r, EG.g, EG.b)
            contLbl:SetAlpha(0.7)
            contLbl:SetPoint("CENTER")
            contLbl:SetText(EUICoreStandaloneMinimap.L("Continue"))

            local contProgress, contTarget = 0, 0
            local CONT_FADE = 0.1
            local contLerp = EUICoreStandaloneMinimap.lerp
            ...
            contBtn:SetScript("OnClick", function()
                ...
                ContApply(0)
                contLbl:SetText(EUICoreStandaloneMinimap.L("Processing") .. "...")
                ...
                decodeRun = EUICoreStandaloneMinimap.DecodeImportStringAsync(importStr,
                    function(payload, err)
                        ...
                        if not payload then
                            EUICoreStandaloneMinimap:ShowInfoPopup({ title = EUICoreStandaloneMinimap.L("Import Failed"), content = err or EUICoreStandaloneMinimap.L("Invalid import string.") })
                            return
                        end
                        ...
                        if EUICoreStandaloneMinimap.IsFullAccountPayload
                           and EUICoreStandaloneMinimap.IsFullAccountPayload(payload) then
                            pastePage:Hide()
                            EUICoreStandaloneMinimap:ShowConfirmPopup({
                                title = EUICoreStandaloneMinimap.L("Import Full Account Data"),
                                message = EUICoreStandaloneMinimap.L("This string is a FULL ACCOUNT export. It replaces your account-wide settings with the sender's, including Quality of Life, HoverCast bindings, Cooldown Manager spell setups, unlock anchors, UI scale, and profile keybinds -- not just a profile. Your other profiles are kept, but a profile with the same name is replaced."),
                                disclaimer = EUICoreStandaloneMinimap.L("This cannot be undone. Export your own profile as a backup first."),
                                typeToConfirm = "Confirm",
                                confirmText = EUICoreStandaloneMinimap.L("Import & Reload"),
                                cancelText = EUICoreStandaloneMinimap.L("Cancel"),
                                onConfirm = function()
                                    if EUICoreStandaloneMinimap.ImportFullAccountData then
                                        EUICoreStandaloneMinimap.ImportFullAccountData(payload)
                                    end
                                end,
                            })
                            return
                    ...
                    function(frac)
                        contLbl:SetFormattedText("%s %d%%", EUICoreStandaloneMinimap.L("Processing"), frac * 100)
                    end)
            end)
        ...
        local function ShowPresetsPage()
            ...
            backBtn:SetScript("OnClick", function()
                ...
                if EUICoreStandaloneMinimap:GetActivePage() == PAGE_PRESETS then
                    EUICoreStandaloneMinimap:SelectPage(PAGE_PROFILES)
                    return
                end
                presetsPage:Hide()
                mainPage:Show()
            ...
            local titleFs = EUICoreStandaloneMinimap.MakeFont(presetsPage, 16, nil, 1, 1, 1, 0.95)
            PP.Point(titleFs, "TOP", presetsPage, "TOP", 0, py - BACK_H / 2 + 23)
            titleFs:SetText(EUICoreStandaloneMinimap.L("Popular Presets"))
            titleFs:SetJustifyH("CENTER")

            local subFs = EUICoreStandaloneMinimap.MakeFont(presetsPage, 12, nil, 1, 1, 1, 0.40)
            PP.Point(subFs, "TOP", titleFs, "BOTTOM", 0, -6)
            subFs:SetText(EUICoreStandaloneMinimap.L("Handcrafted UI setups ready to import."))
            ...
            local presets = {}
            do
                local src = EUICoreStandaloneMinimap.POPULAR_PRESETS or {}
            ...
            local function MakeGreenButton(parent, w, btnH, text)
                ...
                local brd = EUICoreStandaloneMinimap.MakeBorder(btn, EG.r, EG.g, EG.b, 0.7, PP)
                local bg = EUICoreStandaloneMinimap.SolidTex(btn, "BACKGROUND", DB.r, DB.g, DB.b, 0.92)
                bg:SetAllPoints()
                local lbl = EUICoreStandaloneMinimap.MakeFont(btn, 12, nil, EG.r, EG.g, EG.b)
                lbl:SetAlpha(0.7)
                lbl:SetPoint("CENTER")
                lbl:SetText(EUICoreStandaloneMinimap.L(text))
            ...
            EUICoreStandaloneMinimap.MakeBorder(hero, 1, 1, 1, 0.12, PP)
            ...
            EUICoreStandaloneMinimap.MakeBorder(heroImgHolder, 1, 1, 1, 0.12, PP)
            ...
            local heroName = EUICoreStandaloneMinimap.MakeFont(hero, 20, nil, 1, 1, 1, 0.95)
            PP.Point(heroName, "TOPLEFT", hero, "TOPLEFT", detailX, heroNameY)
            heroName:SetJustifyH("LEFT")
            heroName:SetWordWrap(false)

            local heroByLbl = EUICoreStandaloneMinimap.MakeFont(hero, 13, nil, 1, 1, 1, 0.40)
            PP.Point(heroByLbl, "TOPLEFT", heroName, "BOTTOMLEFT", 0, byLblGap)
            heroByLbl:SetText(EUICoreStandaloneMinimap.L("by"))
            local heroAuthor = EUICoreStandaloneMinimap.MakeFont(hero, 13, nil, EG.r, EG.g, EG.b)
            PP.Point(heroAuthor, "LEFT", heroByLbl, "RIGHT", 5, 0)

            local heroDesc = EUICoreStandaloneMinimap.MakeFont(hero, 12, nil, 1, 1, 1, 0.55)
            ...
            local function BuildTagPills(tags)
                ...
                for _, tag in ipairs(tags or {}) do
                    ...
                    local pf = EUICoreStandaloneMinimap.MakeFont(pill, 11, nil, 1, 1, 1, 0.6)
                    pf:SetPoint("CENTER")
                    pf:SetText(EUICoreStandaloneMinimap.L(tag))
                    ...
                    EUICoreStandaloneMinimap.MakeBorder(pill, 1, 1, 1, 0.15, PP)
            ...
            local heroVerFs = EUICoreStandaloneMinimap.MakeFont(heroVerPill, 12, nil, EG.r, EG.g, EG.b)
            ...
            EUICoreStandaloneMinimap.MakeBorder(heroVerPill, 1, 1, 1, 0.15, PP)
            ...
            do
                ...
                local function UserUIScale()
                    return (EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.ppUIScale) or (UIParent and UIParent:GetScale()) or 1
                end
                ...
                local function DoImport(displayKey)
                    ...
                    if not str then
                        EUICoreStandaloneMinimap:ShowInfoPopup({ title = EUICoreStandaloneMinimap.L("Not Available Yet"),
                            content = EUICoreStandaloneMinimap.Lf("%1$s is not available to import for that resolution yet.", (current and current.name) or EUICoreStandaloneMinimap.L("This preset")) })
                        return
                    end
                ...
                local function MakeImportBtn(accent, text, displayKey)
                    ...
                    if accent then
                        ...
                        EUICoreStandaloneMinimap.MakeStyledButton(b, text, 11, PROF_BTN_COLOURS, function() DoImport(displayKey) end)
                    end
                    return b
                end

                -- Regular (left) + Ultrawide (right), bottom flush with the hero image.
                local btnRegular = MakeImportBtn(not isUltrawide, EUICoreStandaloneMinimap.L("Regular Import"), "regular")
                PP.Point(btnRegular, "BOTTOMLEFT", heroImgHolder, "BOTTOMLEFT", detailX - IMG_PAD, 0)
                local btnUltrawide = MakeImportBtn(isUltrawide, EUICoreStandaloneMinimap.L("Ultrawide Import"), "ultrawide")
            ...
            local function MakeGridCard(preset, col, row)
                ...
                bg:SetAllPoints()
                bg:SetColorTexture(0.06, 0.08, 0.10, 0.50)
                local brd = EUICoreStandaloneMinimap.MakeBorder(card, 1, 1, 1, 0.12, PP)
                ...
                local authFs = EUICoreStandaloneMinimap.MakeFont(card, 11, nil, EG.r, EG.g, EG.b)
                ...
                local nameFs = EUICoreStandaloneMinimap.MakeFont(card, 14, nil, 1, 1, 1, 0.9)
                PP.Point(nameFs, "TOPLEFT", imgHolder, "BOTTOMLEFT", 12, -9)
                PP.Point(nameFs, "RIGHT", authFs, "LEFT", -8, 0)
                nameFs:SetJustifyH("LEFT")
                nameFs:SetWordWrap(false)
                nameFs:SetText(EUICoreStandaloneMinimap.L(preset.name))

                -- The preset's full description, clipped to one line.
                local descFs = EUICoreStandaloneMinimap.MakeFont(card, 11, nil, 1, 1, 1, 0.40)
                PP.Point(descFs, "TOPLEFT", nameFs, "BOTTOMLEFT", 0, -5)
                PP.Point(descFs, "RIGHT", imgHolder, "BOTTOMRIGHT", -12, 0)
                descFs:SetJustifyH("LEFT")
                descFs:SetWordWrap(false)
                descFs:SetMaxLines(1)
                descFs:SetText(EUICoreStandaloneMinimap.L(preset.description or ""))
                ...
                if preset.version then
                    ...
                    local vf = EUICoreStandaloneMinimap.MakeFont(verPill, 11, nil, EG.r, EG.g, EG.b)
                    vf:SetPoint("CENTER")
                    ...
                    EUICoreStandaloneMinimap.MakeBorder(verPill, 1, 1, 1, 0.15, PP)
                end
            ...
            scrollTrack:Hide()
            local trackBg = EUICoreStandaloneMinimap.SolidTex(scrollTrack, "BACKGROUND", 1, 1, 1, 0.02)
            ...
            local thumbTex = EUICoreStandaloneMinimap.SolidTex(scrollThumb, "ARTWORK", 1, 1, 1, 0.27)
            ...
            local function UpdateThumb()
                local maxScroll = EUICoreStandaloneMinimap.SafeScrollRange(gridSF)
            ...
            UpdateHero = function(preset)
                ...
                heroName:SetText(EUICoreStandaloneMinimap.L(preset.name or ""))
                heroAuthor:SetText(preset.author or "")
                heroDesc:SetText(EUICoreStandaloneMinimap.L(preset.description or ""))
        ...
        do
            ...
            PP.Point(rowFrame, "TOPLEFT", parent, "TOPLEFT", EUICoreStandaloneMinimap.CONTENT_PAD, y)
            ...
            local function MakeActionCard(parentRow, xOff, iconPath, cardTitle, cardDesc, onClick)
                ...
                local titleFs = EUICoreStandaloneMinimap.MakeFont(card, 13, nil, 1, 1, 1, 0.9)
                PP.Point(titleFs, "TOPLEFT", icon, "TOPRIGHT", 20, 2)
                PP.Point(titleFs, "RIGHT", card, "RIGHT", -14, 0)
                titleFs:SetJustifyH("LEFT")
                titleFs:SetWordWrap(false)
                titleFs:SetText(EUICoreStandaloneMinimap.L(cardTitle))

                local descFs = EUICoreStandaloneMinimap.MakeFont(card, 11, nil, 1, 1, 1, 0.35)
                PP.Point(descFs, "TOPLEFT", titleFs, "BOTTOMLEFT", 0, -4)
                PP.Point(descFs, "RIGHT", card, "RIGHT", -14, 0)
                descFs:SetJustifyH("LEFT")
                descFs:SetWordWrap(false)
                descFs:SetText(EUICoreStandaloneMinimap.L(cardDesc))
            ...
            MakeActionCard(rowFrame, cardX, MEDIA .. "icons\\import.png",
                EUICoreStandaloneMinimap.L("Import Profile"), EUICoreStandaloneMinimap.L("Import a profile from string."), function()
                    ShowPastePage()
                end)
            ...
            MakeActionCard(rowFrame, cardX, MEDIA .. "icons\\dark-overlay.png",
                EUICoreStandaloneMinimap.L("Popular Presets"), EUICoreStandaloneMinimap.L("Browse community presets."), function()
                    EUICoreStandaloneMinimap:SelectPage(PAGE_PRESETS)
                end)
        ...
        do
            ...
            local rowBrd = EUICoreStandaloneMinimap.MakeBorder(rowFrame, 1, 1, 1, 0.10, PP)

            -- "Active Profile" label
            local profLabel = EUICoreStandaloneMinimap.MakeFont(rowFrame, 12, nil, EG.r, EG.g, EG.b, 0.7)
            PP.Point(profLabel, "TOPLEFT", rowFrame, "TOPLEFT", PAD_X, -PAD_Y)
            profLabel:SetText(EUICoreStandaloneMinimap.L("Active Profile"))
            profLabel:SetJustifyH("LEFT")

            -- Active Profile dropdown
            local ddBtn, ddLabelFS, ddBg, ddBrd = MakeDropdown(rowFrame, DD_W, CTRL_H, function()
                return EUICoreStandaloneMinimap.GetActiveProfileName()
            end)
            EUICoreStandaloneMinimap._profileDDBtn = ddBtn
            ddLabel = ddLabelFS
            PP.Point(ddBtn, "TOPLEFT", profLabel, "BOTTOMLEFT", 0, -6)

            -- Profile dropdown menu with inline rename/delete/keybind
            local aS = EUICoreStandaloneMinimap.RD_DD_COLOURS
            ...
            local function RebuildProfileMenu()
                ...
                local order, profiles = EUICoreStandaloneMinimap.GetProfileList()
                local mH = 4
                local idx = 0
                local activeName = EUICoreStandaloneMinimap.GetActiveProfileName()
                local specAssigned
                do
                    local si = GetSpecialization and GetSpecialization() or 0
                    local sid = si and si > 0 and GetSpecializationInfo(si) or nil
                    if sid then specAssigned = EUICoreStandaloneMinimap.GetSpecProfile(sid) end
                end
                    ...
                    if profiles[name] then
                        ...
                        if not itm then
                            ...
                            itm:SetScript("OnEnter", function()
                                ...
                                hl:SetAlpha(EUICoreStandaloneMinimap.DD_ITEM_HL_A)
                            ...
                            xBtn:SetScript("OnEnter", function(self)
                                InlineBtnEnter(self)
                                EUICoreStandaloneMinimap.ShowWidgetTooltip(self, EUICoreStandaloneMinimap.L("Delete"))
                            end)
                            ...
                            editBtn:SetScript("OnEnter", function(self)
                                InlineBtnEnter(self)
                                EUICoreStandaloneMinimap.ShowWidgetTooltip(self, EUICoreStandaloneMinimap.L("Rename"))
                            end)
                            ...
                            kbBtnI:SetScript("OnEnter", function(self)
                                InlineBtnEnter(self)
                                EUICoreStandaloneMinimap.ShowWidgetTooltip(self, EUICoreStandaloneMinimap.L("Keybind"))
                            end)
                        ...
                        if specLocked then
                            ...
                            itm:SetScript("OnEnter", function()
                                EUICoreStandaloneMinimap.ShowWidgetTooltip(itm, EUICoreStandaloneMinimap.L("Your current spec has an assigned profile so you cannot switch to another. Please unassign to switch."))
                            end)
                            itm:SetScript("OnLeave", function()
                                EUICoreStandaloneMinimap.HideWidgetTooltip()
                            end)
                            ...
                            iLbl:SetTextColor(1, 1, 1, EUICoreStandaloneMinimap.TEXT_DIM_A)
                            iEditBtn:Show()
                            iKbBtnL:Show()
                            ...
                            itm:SetScript("OnEnter", function()
                                ...
                                iHl:SetAlpha(EUICoreStandaloneMinimap.DD_ITEM_HL_A)
                                SetAllAlpha(0.8)
                            end)
                            itm:SetScript("OnLeave", function()
                                if IsOverInline() then return end
                                iLbl:SetTextColor(1, 1, 1, EUICoreStandaloneMinimap.TEXT_DIM_A)
                                iHl:SetAlpha(itm._isSel and EUICoreStandaloneMinimap.DD_ITEM_SEL_A or 0)
                                SetAllAlpha(0.4)
                            end)
                            itm:SetScript("OnClick", function()
                                if capName == activeName then return end
                                menu:Hide()
                                local _, profs = EUICoreStandaloneMinimap.GetProfileList()
                                local fontWillChange = EUICoreStandaloneMinimap.ProfileChangesFont(profs and profs[capName])
                                local skinsWillChange = EUICoreStandaloneMinimap.ProfileChangesWindowSkins
                                    and EUICoreStandaloneMinimap.ProfileChangesWindowSkins(profs and profs[capName])
                                EUICoreStandaloneMinimap.SwitchProfile(capName)
                                ddLabel:SetText(EUICoreStandaloneMinimap.GetActiveProfileName())
                            ...
                            iXBtn:SetScript("OnClick", function()
                                ...
                                menu:Hide()
                                EUICoreStandaloneMinimap:ShowConfirmPopup({
                                    title       = EUICoreStandaloneMinimap.L("Delete Profile"),
                                    message     = EUICoreStandaloneMinimap.Lf("Delete \"%1$s\"?", capName),
                                    confirmText = EUICoreStandaloneMinimap.L("Delete"),
                                    cancelText  = EUICoreStandaloneMinimap.L("Cancel"),
                                    onConfirm   = function()
                                        EUICoreStandaloneMinimap.DeleteProfile(capName)
                                        ddLabel:SetText(EUICoreStandaloneMinimap.GetActiveProfileName())
                                        EUICoreStandaloneMinimap:InvalidatePageCache()
                                        EUICoreStandaloneMinimap:RefreshPage(true)
                                    end,
                            ...
                            iEditBtn:SetScript("OnClick", function()
                                menu:Hide()
                                EUICoreStandaloneMinimap:ShowInputPopup({
                                    title       = EUICoreStandaloneMinimap.L("Rename Profile"),
                                    message     = EUICoreStandaloneMinimap.Lf("Enter a new name for \"%1$s\":", capName),
                                    placeholder = capName,
                                    confirmText = EUICoreStandaloneMinimap.L("Rename"),
                                    cancelText  = EUICoreStandaloneMinimap.L("Cancel"),
                                    onConfirm   = function(newName)
                                        newName = newName and strtrim(newName) or ""
                                        if newName == "" or newName == capName then return end
                                        if newName == "Default" then
                                            print(EUICoreStandaloneMinimap.L("|cffff6060[EUICoreStandaloneMinimap]|r Cannot rename to \"Default\"."))
                                            return
                                        end
                                        local _, profs = EUICoreStandaloneMinimap.GetProfileList()
                                        if profs and profs[newName] then
                                            print(EUICoreStandaloneMinimap.Lf("|cffff6060[EUICoreStandaloneMinimap]|r A profile named \"%1$s\" already exists.", newName))
                                            return
                                        end
                                        EUICoreStandaloneMinimap.RenameProfile(capName, newName)
                                        ddLabel:SetText(EUICoreStandaloneMinimap.GetActiveProfileName())
                                        EUICoreStandaloneMinimap:InvalidatePageCache()
                                        EUICoreStandaloneMinimap:RefreshPage(true)
                                    end,
            ...
            local specLabel = EUICoreStandaloneMinimap.MakeFont(rowFrame, 12, nil, 1, 1, 1, 0.45)
            PP.Point(specLabel, "LEFT", profLabel, "LEFT", DD_W + GAP, 0)
            specLabel:SetText(EUICoreStandaloneMinimap.L("Assign to Spec"))
            ...
            EUICoreStandaloneMinimap.MakeStyledButton(assignBtn, "Assign to Spec", 11, PROF_BTN_COLOURS, function()
                ...
                local curActiveName = EUICoreStandaloneMinimap.GetActiveProfileName()
                EUICoreStandaloneMinimap:ShowSpecAssignPopup({
                    db = tempDB,
                    dbKey = "_profileSpecs",
            ...
            -- "New Profile" label
            local newLabel = EUICoreStandaloneMinimap.MakeFont(rowFrame, 12, nil, 1, 1, 1, 0.45)
            PP.Point(newLabel, "LEFT", specLabel, "LEFT", BTN_W + GAP, 0)
            newLabel:SetText(EUICoreStandaloneMinimap.L("New Profile"))
            ...
            EUICoreStandaloneMinimap.MakeStyledButton(copyBtn, "Create New (Copy)", 11, PROF_BTN_COLOURS, function()
                EUICoreStandaloneMinimap:ShowInputPopup({
                    title       = EUICoreStandaloneMinimap.L("Copy Profile"),
                    message     = EUICoreStandaloneMinimap.L("Enter a name for the new profile:"),
                    placeholder = EUICoreStandaloneMinimap.L("My Profile"),
                    confirmText = EUICoreStandaloneMinimap.L("Save"),
                    cancelText  = EUICoreStandaloneMinimap.L("Cancel"),
                    onConfirm   = function(name)
                        ...
                        EUICoreStandaloneMinimap.SaveCurrentAsProfile(name)
                        ReloadUI()
                    end,
                })
            end)
        ...
        do
            ...
            local hdrTitle = EUICoreStandaloneMinimap.MakeFont(hdrFrame, 14, nil, 1, 1, 1, 0.9)
            PP.Point(hdrTitle, "TOPLEFT", hdrFrame, "TOPLEFT", SIDE_PAD, -20)
            hdrTitle:SetText(EUICoreStandaloneMinimap.L("Export Profile"))
            hdrTitle:SetJustifyH("LEFT")

            local hdrDesc = EUICoreStandaloneMinimap.MakeFont(hdrFrame, 11, nil, 1, 1, 1, 0.35)
            PP.Point(hdrDesc, "TOPLEFT", hdrTitle, "BOTTOMLEFT", 0, -9)
            PP.Point(hdrDesc, "RIGHT", hdrFrame, "RIGHT", -(160 + SIDE_PAD), 0)
            hdrDesc:SetText(EUICoreStandaloneMinimap.L("Choose which addons to include in your exported profile. Check everything for a full profile export."))
            hdrDesc:SetJustifyH("LEFT")
            hdrDesc:SetWordWrap(true)
            ...
            local exportComponents = EUICoreStandaloneMinimap.BuildModuleComponents({
                anchors     = EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.unlockAnchors,
                widthMatch  = EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.unlockWidthMatch,
                heightMatch = EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.unlockHeightMatch,
            })
            local FOLDER_DISPLAY = {}
            for _, e in ipairs(ADDON_DB_MAP_LOCAL) do FOLDER_DISPLAY[e.folder] = e.display end

            for _, entry in ipairs(ADDON_DB_MAP_LOCAL) do
                local loaded = EUICoreStandaloneMinimap.IsModuleAddonLoaded(entry.folder)
                local folder = entry.folder
                addonItems[#addonItems + 1] = {
                    folder  = folder,
                    display = entry.display,
                    ...
                    setVal  = function(v)
                        ...
                        if members then
                            for f in pairs(members) do
                                if EUICoreStandaloneMinimap.IsModuleAddonLoaded(f) then selectedAddons[f] = v or nil end
                            end
                        else
            ...
            local function RefreshFooterCount()
                ...
                footerCountFs:SetText(EUICoreStandaloneMinimap.Lf("Export will include %1$s of %2$s addons.", count, #addonItems))
            end
            ...
            for i, item in ipairs(addonItems) do
                ...
                if item.loaded then
                        ...
                        if members then
                            ...
                            for f in pairs(members) do
                                if f ~= item.folder then
                                    names[#names + 1] = (EUICoreStandaloneMinimap.L(FOLDER_DISPLAY[f] or f))
                                end
                            end
                            if #names > 0 then
                                table.sort(names)
                                EUICoreStandaloneMinimap.ShowWidgetTooltip(rowFrame,
                                    EUICoreStandaloneMinimap.Lf("Linked by Anchor/Width/Height Matching to: %1$s. These export together.", table.concat(names, ", ")))
                            end
                        end
                    end)
                    clickBtn:SetScript("OnLeave", function()
                    ...
                    blockFrame:SetScript("OnEnter", function()
                        hoverTex:Show()
                        EUICoreStandaloneMinimap.ShowWidgetTooltip(rowFrame, EUICoreStandaloneMinimap.L("Addon not loaded"))
                    end)
            ...
            footerCountFs = EUICoreStandaloneMinimap.MakeFont(footerFrame, 12, nil, 1, 1, 1, 0.40)
            ...
            local DB = EUICoreStandaloneMinimap.DARK_BG
            local eaBrd = EUICoreStandaloneMinimap.MakeBorder(exportSelBtn, EG.r, EG.g, EG.b, 0.7, PP)
            local eaBg = EUICoreStandaloneMinimap.SolidTex(exportSelBtn, "BACKGROUND", DB.r, DB.g, DB.b, 0.92)
            eaBg:SetAllPoints()
            local eaLbl = EUICoreStandaloneMinimap.MakeFont(exportSelBtn, 12, nil, EG.r, EG.g, EG.b)
            eaLbl:SetAlpha(0.7)
            eaLbl:SetPoint("CENTER")
            eaLbl:SetText(EUICoreStandaloneMinimap.L("Export Profile"))
            ...
            do
                ...
                local hasBlizzSkinRow = EUICoreStandaloneMinimap.IsModuleAddonLoaded("EUICoreStandaloneMinimapBlizzardSkin")

                local function Summary()
                    local parts = {}
                    local total = hasBlizzSkinRow and 4 or 3
                    if EffectiveIncludeOverrides() then parts[#parts + 1] = EUICoreStandaloneMinimap.L("Overrides") end
                    if includeLayoutExport then parts[#parts + 1] = EUICoreStandaloneMinimap.L("Layout") end
                    if includeGlobalsExport then parts[#parts + 1] = EUICoreStandaloneMinimap.L("Globals") end
                    if hasBlizzSkinRow and includeWindowSkinsExport then parts[#parts + 1] = EUICoreStandaloneMinimap.L("Window Skins") end
                    if #parts == 0 then return EUICoreStandaloneMinimap.L("Nothing Extra") end
                    if #parts == total then return EUICoreStandaloneMinimap.L("Everything") end
                    return table.concat(parts, ", ")
                end
            ...
            local EA_FADE = 0.1
            local eaLerp = EUICoreStandaloneMinimap.lerp
            ...
            exportSelBtn:SetScript("OnClick", function()
                ...
                local function finishExport(includeCDM, cdmSpecs)
                    local str = EUICoreStandaloneMinimap.ExportProfile(activeName, exportFolders, includeLayoutExport, includeCDM, cdmSpecs, includeGlobalsExport, includeOverrides, includeWindowSkinsExport)
                    if str then EUICoreStandaloneMinimap:ShowExportPopup(str) end
                end
                ...
                if folders["EUICoreStandaloneMinimapCooldownManager"] then
                    EUICoreStandaloneMinimap.RunCDMSpellExportFlow(activeName, finishExport)
                else
        ...
        -------------------------------------------------------------------
        --  Interactive Import API hookup (EUICoreStandaloneMinimap.ImportProfileInteractive)
        -------------------------------------------------------------------
        ...
        EUICoreStandaloneMinimap._ProfilesResetToMain = function()
            importPage:Hide()
            pastePage:Hide()
            presetsPage:Hide()
            mainPage:Show()
        ...
        EUICoreStandaloneMinimap._ProfilesShowPresetsPage = function()
            importPage:Hide()
            pastePage:Hide()
            ShowPresetsPage()
        end
        ...
        EUICoreStandaloneMinimap._ProfilesApiProceed = function(payload)
            local s = EUICoreStandaloneMinimap._apiImportSession
            ...
            if s.scaleAsked then
                ...
                MaybeConfirmUIScale(payload, function(applyScale)
                    ...
                    local go = EUICoreStandaloneMinimap._ProfilesApiProceed
        ...
        EUICoreStandaloneMinimap._ProfilesConsumeApiImport = function()
            local s = EUICoreStandaloneMinimap._apiImportSession
            if not s or s.state == "done" then return end
            if EUICoreStandaloneMinimap._EnsureApiImportCloseHook then EUICoreStandaloneMinimap._EnsureApiImportCloseHook() end
            s.state = "active"
            if s.payload then
                EUICoreStandaloneMinimap._ProfilesApiProceed(s.payload)
            elseif not s.decoding then
                s.decoding = true
                EUICoreStandaloneMinimap.DecodeImportStringAsync(s.str, function(payload, err)
                    s.decoding = nil
                    -- The session may have been declined or replaced while the decode was in flight; drop a stale result.
                    if EUICoreStandaloneMinimap._apiImportSession ~= s or s.state == "done" then return end
                    if not payload then
                        EUICoreStandaloneMinimap:ShowInfoPopup({
                            title   = EUICoreStandaloneMinimap.L("Import Failed"),
                            content = err or EUICoreStandaloneMinimap.L("Invalid import string."),
                        })
                        EUICoreStandaloneMinimap._FinishApiImportSession(false)
                        return
                    end
                    s.payload = payload
                    local go = EUICoreStandaloneMinimap._ProfilesApiProceed
        ...
        EUICoreStandaloneMinimap._ProfilesConsumeApiImport()
        ...
        if EUICoreStandaloneMinimap._pendingShowPresetsPage then
            EUICoreStandaloneMinimap._pendingShowPresetsPage = nil
            local s = EUICoreStandaloneMinimap._apiImportSession
    ...
    local function CleanupProfilesRoot()
        if EUICoreStandaloneMinimap._profilesRoot then
            EUICoreStandaloneMinimap._profilesRoot:Hide()
            EUICoreStandaloneMinimap._profilesRoot:SetParent(nil)
            EUICoreStandaloneMinimap._profilesRoot = nil
        end
    end
    ...
    EUICoreStandaloneMinimap:RegisterModule(GLOBAL_KEY, {
        title       = "Global Settings",
        description = "General options for all EUICoreStandaloneMinimap addons.",
        pages       = globalPages,
        buildPage   = function(pageName, parent, yOffset)
            ...
            if EUICoreStandaloneMinimap._prebuilding then
                ...
                elseif pageName == PAGE_WHATSNEW then
                    return EUICoreStandaloneMinimap._BuildWhatsNewPage(pageName, parent, yOffset)
                end
                return
            end
        ...
        onPageCacheRestore = function(pageName)
            if pageName ~= PAGE_PROFILES then
                CleanupProfilesRoot()
            elseif pageName == PAGE_PROFILES and not EUICoreStandaloneMinimap._profilesRoot then
                C_Timer.After(0, function()
                    if EUICoreStandaloneMinimap:GetActiveModule() == GLOBAL_KEY then
        ...
        onReset     = function()
            ...
            EUICoreStandaloneMinimap.ResetTheme()
            -- Reset all custom class, power, and resource colors to defaults
            if EUICoreStandaloneMinimapDB then
                EUICoreStandaloneMinimapDB.customColors = nil
            end
            -- Reset fonts to defaults
            if EUICoreStandaloneMinimapDB then
                EUICoreStandaloneMinimapDB.fonts = nil
            end
            EUICoreStandaloneMinimap.InvalidateFontCache()
            EUICoreStandaloneMinimap.ApplyColorsToOUF()
            -- Reset panel scale to 100%
            if EUICoreStandaloneMinimap.SetPanelScale then
                EUICoreStandaloneMinimap:SetPanelScale(1.0)
            end
            -- Reset right-click targeting to default (disabled = off)
            if EUICoreStandaloneMinimapDB then
                EUICoreStandaloneMinimapDB.disableRightClickTarget = false
                EUICoreStandaloneMinimapDB.disableRightClickTargetAllyCombat = false
                ...
                if EUICoreStandaloneMinimap.QoLExtrasSet then
                    EUICoreStandaloneMinimap.QoLExtrasSet("showFPS", false)
                    EUICoreStandaloneMinimap.QoLExtrasSet("showSecondaryStats", false)
                end
                EUICoreStandaloneMinimapDB.guildChatPrivacy = false
                EUICoreStandaloneMinimapDB.repairWarning = nil
                ...
                EUICoreStandaloneMinimapDB.showSpellID = false
                if EUICoreStandaloneMinimap.SyncAuraSpellIDCVar then EUICoreStandaloneMinimap.SyncAuraSpellIDCVar() end
                EUICoreStandaloneMinimapDB.suppressErrors = true
                ...
                EUICoreStandaloneMinimapDB.crosshairSize = "None"
                if EUICoreStandaloneMinimap._applyCrosshair then EUICoreStandaloneMinimap._applyCrosshair() end
                -- Reset unlock mode layout data
                EUICoreStandaloneMinimapDB.unlockAnchors = nil
                EUICoreStandaloneMinimapDB.unlockWidthMatch = nil
                EUICoreStandaloneMinimapDB.unlockHeightMatch = nil
                -- QoL Features are NOT reset here; they have their own module reset
            end
            if EUICoreStandaloneMinimap._applyRightClickTarget then
                EUICoreStandaloneMinimap._applyRightClickTarget()
            end
            if EUICoreStandaloneMinimap._applyHideBlizzardPartyFrame then
                EUICoreStandaloneMinimap._applyHideBlizzardPartyFrame()
            end
            ...
            if EUICoreStandaloneMinimap._applyFPSDisplay then
                EUICoreStandaloneMinimap._applyFPSDisplay()
            elseif EUICoreStandaloneMinimap._applySecondaryStats then
                EUICoreStandaloneMinimap._applySecondaryStats()
            end
            if EUICoreStandaloneMinimap._applyCrosshair then
                EUICoreStandaloneMinimap._applyCrosshair()
            end
            if EUICoreStandaloneMinimap._applyGuildChatPrivacy then
                EUICoreStandaloneMinimap._applyGuildChatPrivacy()
            end
            -- Apply suppress errors default (on)
            SetCVarSafe("scriptErrors", "0")
            EUICoreStandaloneMinimap:SelectPage(PAGE_GENERAL)
        end,
    })

    -- Profiles & Presets: its own sidebar module, reusing the profiles page builder; the profiles-root lifecycle rides the shared
    -- CleanupProfilesRoot hooks below (keyed to PROFILES_KEY). Second tab: the Overrides management list (built by EUICoreStandaloneMinimap_SpecOverrides.lua).
    --
    ...
    local function BuildOverridesModeToggle(parent, y)
        local EG = EUICoreStandaloneMinimap.ELLESMERE_GREEN or { r = 0.05, g = 0.82, b = 0.62 }
        local fontPath = (EUICoreStandaloneMinimap.GetFontPath and EUICoreStandaloneMinimap.GetFontPath())
        ...
        if EUICoreStandaloneMinimap.PP then
            EUICoreStandaloneMinimap.PP.CreateBorder(wrap, 1, 1, 1, 0.10, 1)
        end
        local MODES = {
            { key = "spec", label = "Spec Overrides" },
            { key = "cond", label = "Conditional Overrides" },
        }
        local cur = EUICoreStandaloneMinimap._overridesTabMode or "spec"
        for i, m in ipairs(MODES) do
            ...
            lbl:SetPoint("CENTER")
            lbl:SetText(EUICoreStandaloneMinimap.L(m.label))
            if cur == m.key then
                ...
                btn:SetScript("OnClick", function()
                    EUICoreStandaloneMinimap._overridesTabMode = m.key
    ...
    -- FULL EXPORT tab: a warning and one button, with its own exporter (EUICoreStandaloneMinimap.ExportFullAccountData) sharing NO code path with the
    ...
    local function BuildFullExportPage(parent, yOffset)
        local PAD = EUICoreStandaloneMinimap.CONTENT_PAD or 40
        local y = yOffset - 10
        ...
        local wbg = EUICoreStandaloneMinimap.SolidTex(warn, "BACKGROUND", 0.10, 0.04, 0.04, 0.55)
        wbg:SetAllPoints()
        EUICoreStandaloneMinimap.MakeBorder(warn, 0.8, 0.2, 0.2, 0.55)
        local wtext = EUICoreStandaloneMinimap.MakeFont(warn, 13, nil, 1, 0.55, 0.55, 1)
        wtext:SetPoint("TOPLEFT", warn, "TOPLEFT", 16, -14)
        wtext:SetWidth(width - 32)
        wtext:SetJustifyH("LEFT")
        wtext:SetSpacing(3)
        wtext:SetText(EUICoreStandaloneMinimap.L("This export includes cross profile settings that will overwrite the importing user's settings including Quality of Life, Hovercast and more that should typically not be shared with standard profiles. THIS IS NOT RECOMMENDED for public sharing of profiles."))
        ...
        btn:SetFrameLevel(parent:GetFrameLevel() + 5)
        local DARK_BG = EUICoreStandaloneMinimap.DARK_BG or { r = 0.05, g = 0.07, b = 0.09 }
        local bbg = EUICoreStandaloneMinimap.SolidTex(btn, "BACKGROUND", DARK_BG.r, DARK_BG.g, DARK_BG.b, 0.92)
        bbg:SetAllPoints()
        local EG = EUICoreStandaloneMinimap.ELLESMERE_GREEN or { r = 0.05, g = 0.82, b = 0.62 }
        local bbrd = EUICoreStandaloneMinimap.MakeBorder(btn, EG.r, EG.g, EG.b, 0.5)
        local blbl = EUICoreStandaloneMinimap.MakeFont(btn, 13, nil, EG.r, EG.g, EG.b, 1)
        blbl:SetAlpha(0.8)
        blbl:SetPoint("CENTER")
        blbl:SetText(EUICoreStandaloneMinimap.L("Export All Data with Profile"))
        btn:SetScript("OnEnter", function()
            blbl:SetAlpha(1)
        ...
        btn:SetScript("OnClick", function()
            local str = EUICoreStandaloneMinimap.ExportFullAccountData and EUICoreStandaloneMinimap.ExportFullAccountData()
            if str then
                EUICoreStandaloneMinimap:ShowExportPopup(str)
            else
                EUICoreStandaloneMinimap:ShowInfoPopup({
                    title = EUICoreStandaloneMinimap.L("Export Failed"),
                    content = EUICoreStandaloneMinimap.L("Could not build the export string."),
                })
            end
        end)
        y = y - BTN_H - 20
    ...
    local function BuildOverridesPage(parent, yOffset)
        ...
        if (EUICoreStandaloneMinimap._overridesTabMode or "spec") == "cond" then
            if EUICoreStandaloneMinimap.Conditions_BuildListPage then
                return EUICoreStandaloneMinimap.Conditions_BuildListPage(parent, y)
            end
        elseif EUICoreStandaloneMinimap.SpecOverrides_BuildListPage then
            return EUICoreStandaloneMinimap.SpecOverrides_BuildListPage(parent, y)
        end
        return 200
    end
    ...
    EUICoreStandaloneMinimap:RegisterModule(PROFILES_KEY, {
        title       = "Profiles & Presets",
        description = "Import, export, and switch EUICoreStandaloneMinimap profiles and presets.",
        ...
        buildPage   = function(pageName, parent, yOffset)
            ...
            if EUICoreStandaloneMinimap._prebuilding then
                if pageName == PAGE_OVERRIDES then
                    ...
                    if EUICoreStandaloneMinimap.SpecOverrides_BuildListPage then
                        return EUICoreStandaloneMinimap.SpecOverrides_BuildListPage(parent, yOffset)
                    end
                    return 200
                end
                return
            ...
            if pageName == PAGE_PRESETS then
                EUICoreStandaloneMinimap._pendingShowPresetsPage = true
        ...
        onPageCacheRestore = function(pageName)
            ...
            elseif pageName == PAGE_OVERRIDES then
                ...
                C_Timer.After(0, function()
                    if EUICoreStandaloneMinimap:GetActiveModule() == PROFILES_KEY
                       and EUICoreStandaloneMinimap:GetActivePage() == pageName then
                        EUICoreStandaloneMinimap:RefreshPage(true)
                    end
                end)
            elseif pageName == PAGE_PRESETS then
                if EUICoreStandaloneMinimap._profilesRoot then
                    ...
                    if s and s.state ~= "done" then
                        if EUICoreStandaloneMinimap._ProfilesConsumeApiImport then
                            EUICoreStandaloneMinimap._ProfilesConsumeApiImport()
                        end
                    elseif EUICoreStandaloneMinimap._ProfilesShowPresetsPage then
                        EUICoreStandaloneMinimap._ProfilesShowPresetsPage()
                    end
                else
                    EUICoreStandaloneMinimap._pendingShowPresetsPage = true
                    C_Timer.After(0, function()
                        if EUICoreStandaloneMinimap:GetActiveModule() == PROFILES_KEY
                           and EUICoreStandaloneMinimap:GetActivePage() == PAGE_PRESETS then
                            BuildProfilesPage(PAGE_PROFILES, nil, -6)
                        else
                            EUICoreStandaloneMinimap._pendingShowPresetsPage = nil
                        end
            ...
            elseif not EUICoreStandaloneMinimap._profilesRoot then
                C_Timer.After(0, function()
                    if EUICoreStandaloneMinimap:GetActiveModule() == PROFILES_KEY then
                        BuildProfilesPage(PAGE_PROFILES, nil, -6)
                    end
                end)
            else
                ...
                elseif EUICoreStandaloneMinimap._ProfilesResetToMain then
                    EUICoreStandaloneMinimap._ProfilesResetToMain()
                end
            end
        end,
    })
    ...
    if not IS_STANDALONE then
        EUICoreStandaloneMinimap:RegisterModule(PATCHNOTES_KEY, {
            title       = "Patch Notes",
            description = EUICoreStandaloneMinimap.L("What's new in EUICoreStandaloneMinimap."),
            pages       = { PAGE_WHATSNEW, PAGE_LEGENDS },
            buildPage   = function(pageName, parent, yOffset)
                if pageName == PAGE_LEGENDS then
                    return EUICoreStandaloneMinimap._BuildLegendsPage(pageName, parent, yOffset)
                end
                return EUICoreStandaloneMinimap._BuildWhatsNewPage(pageName, parent, yOffset)
            end,
        })
    ...
    EUICoreStandaloneMinimap:RegisterOnHide(function()
        CleanupProfilesRoot()
    end)

    -- Clean up profiles root when switching to any module other than Profiles
    if EUICoreStandaloneMinimap.SelectModule then
        hooksecurefunc(EUICoreStandaloneMinimap, "SelectModule", function(_, folderName)
    ...
    local origHideRoots = EUICoreStandaloneMinimap._hideScrollFrameRoots
    EUICoreStandaloneMinimap._hideScrollFrameRoots = function()
