...
-------------------------------------------------------------------------------
--  EUICoreStandaloneMinimap_Presets.lua
--
--  Spec assignment popup for the global profile system.
--  Split from EUICoreStandaloneMinimap.lua -- see EUICoreStandaloneMinimap.lua for the base addon code.
--
--  Load order (via TOC):
--    1. EUICoreStandaloneMinimap.lua        -- constants, utils, popups, main frame
--    2. EUICoreStandaloneMinimap_Widgets.lua -- shared widget helpers, widget factory
--    3. EUICoreStandaloneMinimap_Presets.lua -- THIS FILE
-------------------------------------------------------------------------------
...
local CB_BOX_R     = EUICoreStandaloneMinimap.CB_BOX_R
local CB_BOX_G     = EUICoreStandaloneMinimap.CB_BOX_G
local CB_BOX_B     = EUICoreStandaloneMinimap.CB_BOX_B
local CB_BRD_A     = EUICoreStandaloneMinimap.CB_BRD_A
local CB_ACT_BRD_A = EUICoreStandaloneMinimap.CB_ACT_BRD_A
...
do
    ...
    EUICoreStandaloneMinimap._SPEC_DATA = SPEC_DATA
    ...
    function EUICoreStandaloneMinimap:ShowSpecAssignPopup(opts)
        local db        = opts.db
        ...
        if not specPopup then
            ...
            local POPUP_H = 740
            local ppScale = EUICoreStandaloneMinimap.GetPopupScale()
            ...
            local pf = EUICoreStandaloneMinimap._popupFrames
            if pf then pf[#pf + 1] = { popup = popup, dimmer = dimmer } end
            PP.Size(popup, POPUP_W, POPUP_H)
            PP.Point(popup, "CENTER", EUICoreStandaloneMinimap._mainFrame, "CENTER", 0, 0)
            ...
            title:SetText(EUICoreStandaloneMinimap.L("Assign Preset to Specs"))
            popup._title = title
            ...
            defDDLabel:SetText(EUICoreStandaloneMinimap.L("Default Profile (for non-assigned specs)"))
            ...
            closeLbl:SetText(EUICoreStandaloneMinimap.L("Done"))
            ...
            cancelLbl:SetText(EUICoreStandaloneMinimap.L("Cancel"))
        ...
        if opts.title then
            specPopup._title:SetText(EUICoreStandaloneMinimap.L(opts.title))
        else
            specPopup._title:SetText(EUICoreStandaloneMinimap.L("Assign Preset to Specs"))
        end
        if opts.subtitle then
            specPopup._subtitle:SetText(EUICoreStandaloneMinimap.L(opts.subtitle))
        else
            local presetName
            if presetKey == "custom" then presetName = EUICoreStandaloneMinimap.L("Custom")
            elseif presetKey == "ellesmereui" then presetName = "EUICoreStandaloneMinimap"
            ...
            specPopup._subtitle:SetText(EUICoreStandaloneMinimap.Lf("Select which specs you want %1$s to be assigned to", presetName))
        end
        ...
        if specPopup._closeLbl then
            specPopup._closeLbl:SetText(EUICoreStandaloneMinimap.L(opts.buttonText or "Done"))
        end
                ...
                for pKey, specList in pairs(fullMap) do
                    if pKey ~= presetKey and type(specList) == "table" then
                        for sID in pairs(specList) do
                            local dName
                            if pKey == "custom" then dName = EUICoreStandaloneMinimap.L("Custom")
                            elseif pKey == "ellesmereui" then dName = "EUICoreStandaloneMinimap"
                ...
                for _, spec in ipairs(cls.specs) do
                    ...
                    row:SetScript("OnEnter", function(self)
                        if self._disabled and disabledTip then
                            EUICoreStandaloneMinimap.ShowWidgetTooltip(self._box,
                                EUICoreStandaloneMinimap.DisabledTooltip(disabledTip))
                        elseif self._lockedOn and type(lockedOnTip) == "string" then
                            EUICoreStandaloneMinimap.ShowWidgetTooltip(self._box,
                                EUICoreStandaloneMinimap.DisabledTooltip(lockedOnTip))
                        end
                        if self._lockedOn or self._locked or self._disabled then return end
        ...
        if defaultKey and allPresetKeysFn then
            ...
            local function DefPresetDisplayName(key)
                ...
                if key == "custom" then return EUICoreStandaloneMinimap.L("Custom") end
                if key == "ellesmereui" then return "EUICoreStandaloneMinimap" end
            ...
            specPopup._rebuildDefMenu = function()
                ...
                local mH = 4
                local ITEM_FONT = EUICoreStandaloneMinimap._font or ("Interface\\AddOns\\EUIStandaloneMinimap\\media\\fonts\\Expressway.ttf")
