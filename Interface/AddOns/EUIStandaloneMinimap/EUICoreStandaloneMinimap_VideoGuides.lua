...
-------------------------------------------------------------------------------
--  EUICoreStandaloneMinimap_VideoGuides.lua
--
...
--
--  API (EUICoreStandaloneMinimap.VideoGuides):
...
--
--  Seen map: EUICoreStandaloneMinimapDB.videoGuidesSeen[id] = true (per account, one flat
...
if not EUICoreStandaloneMinimap then return end

-- Suite-only: guides describe suite surfaces. Trigger sites nil-guard on
-- EUICoreStandaloneMinimap.VideoGuides so standalone builds degrade to normal clicks.
...
-- (EUICoreStandaloneMinimapOptions/EUI__General_Options.lua, MakeVideoBannerCard) both read this.
EUICoreStandaloneMinimap.MIDNIGHT_VIDEO_URL = "https://youtu.be/JadIGEr4Ixc"
...
local function MarkSeen(id)
    ...
    if not EUICoreStandaloneMinimapDB.videoGuidesSeen then EUICoreStandaloneMinimapDB.videoGuidesSeen = {} end
    EUICoreStandaloneMinimapDB.videoGuidesSeen[id] = true
end

local function HasSeen(id)
    return (EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.videoGuidesSeen
        and EUICoreStandaloneMinimapDB.videoGuidesSeen[id]) or false
end
...
local function TipSeen(id)
    return (EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.tutorialTipsSeen
        and EUICoreStandaloneMinimapDB.tutorialTipsSeen[id]) or false
end

local function MarkTipSeen(id)
    if not EUICoreStandaloneMinimapDB then EUICoreStandaloneMinimapDB = {} end
    if not EUICoreStandaloneMinimapDB.tutorialTipsSeen then EUICoreStandaloneMinimapDB.tutorialTipsSeen = {} end
    EUICoreStandaloneMinimapDB.tutorialTipsSeen[id] = true
end
...
local function BuildShell()
    ...
    FONT = EUICoreStandaloneMinimap._font or ("Interface\\AddOns\\EUIStandaloneMinimap\\media\\fonts\\Expressway.ttf")
    ui = {}
...
local function SetGuide(id, def)
    local accent = def.accent or EUICoreStandaloneMinimap.ELLESMERE_GREEN or { r = 0.05, g = 0.83, b = 0.62 }
...
local function Show(id)
    ...
    ui.popup:SetScale(EUICoreStandaloneMinimap.PopupBump(1.15))
    SetGuide(id, def)
    ui.dimmer:Show()
...
local function AttachTip(region, tipId, opts)
    ...
    icon:SetAllPoints()
    icon:SetTexture("Interface\\AddOns\\EUIStandaloneMinimap\\media\\icons\\play.png")
    local EG = EUICoreStandaloneMinimap.ELLESMERE_GREEN
    if EG then
        icon:SetVertexColor(EG.r, EG.g, EG.b, 1)
        if EUICoreStandaloneMinimap.RegAccent then
            EUICoreStandaloneMinimap.RegAccent({ type = "vertex", obj = icon })
        end
    else
    ...
    tip:SetScript("OnEnter", function(self)
        ...
        if EUICoreStandaloneMinimap.ShowWidgetTooltip then
            local t = (EUICoreStandaloneMinimap.L and EUICoreStandaloneMinimap.L(tipText)) or tipText
            local hint = (EUICoreStandaloneMinimap.L and EUICoreStandaloneMinimap.L("Shift + right click to hide video guide icons"))
                or "Shift + right click to hide video guide icons"
            EUICoreStandaloneMinimap.ShowWidgetTooltip(self, t .. "\n|cff909090" .. hint .. "|r")
        end
    end)
    ...
    tip:SetScript("OnClick", function(self, mouseButton)
        if mouseButton == "RightButton" then
            ...
            if not EUICoreStandaloneMinimapDB then EUICoreStandaloneMinimapDB = {} end
            EUICoreStandaloneMinimapDB.tutorialTipsDisabled = true
            if EUICoreStandaloneMinimap.HideWidgetTooltip then EUICoreStandaloneMinimap.HideWidgetTooltip() end
            RefreshTips()
...
EUICoreStandaloneMinimap.VideoGuides = {
...
SlashCmdList["EUIVIDEOS"] = function()
    if EUICoreStandaloneMinimapDB then
        EUICoreStandaloneMinimapDB.videoGuidesSeen = nil
        EUICoreStandaloneMinimapDB.tutorialTipsSeen = nil
    end
    RefreshTips()
    print("|cff00ff98EUICoreStandaloneMinimap:|r Video guides and tutorial tips reset. Badges are back; one-time popups will fire again.")
end
...
do
    local function ReleaseLaunchChain()
        EUICoreStandaloneMinimap._launchVideoIntroPending = nil
        if EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.firstInstallPopupShown and EUICoreStandaloneMinimap._RunConflictCheck then
            C_Timer.After(0.3, EUICoreStandaloneMinimap._RunConflictCheck)
        end
    end

    EUICoreStandaloneMinimap.VideoGuides.Register("midnight_121", {
        eyebrow  = "IMPORTANT EUI VIDEO GUIDE",
        title    = "Everything New in 12.1",
        blurb    = "Patch 12.1 changes a lot, and EUICoreStandaloneMinimap changed with it. This video walks you through the most important changes so you can jump straight back in. Copy the link below and watch it in your browser.",
        url      = EUICoreStandaloneMinimap.MIDNIGHT_VIDEO_URL,
    ...
    --  ADDON_LOADED, EUICoreStandaloneMinimapDB still reflects ONLY the previous session's
    ...
    --  holds the auto addon-conflict check (EUICoreStandaloneMinimap.lua) until dismiss.
    ---------------------------------------------------------------------------
    local _decision

    local function ComputeDecision()
        if not EUICoreStandaloneMinimapDB then return "new" end
        if HasSeen("midnight_121") then return "done" end
        local profiles = EUICoreStandaloneMinimapDB.profiles
    ...
    loader:SetScript("OnEvent", function(self, event, addonName)
        if event == "ADDON_LOADED" then
            ...
            if _decision == "show" then
                EUICoreStandaloneMinimap._launchVideoIntroPending = true
            end
        elseif event == "PLAYER_LOGIN" then
            ...
            C_Timer.After(0.8, function()
                ...
                if not EUICoreStandaloneMinimap.VideoGuides.FireOnce("midnight_121") then
                    ReleaseLaunchChain()
                end
            end)
        end
...
--  OnClick in EUICoreStandaloneMinimap_SpecOverrides.lua / SpecOverrides_SetupButton).
...
do
    ...
    EUICoreStandaloneMinimap.VideoGuides.Register("settings_overrides", {
        title = "Settings Overrides",
        blurb = "Settings Overrides are the most powerful system in EUICoreStandaloneMinimap, and the deepest. A few minutes with this video will save you hours of working it out on your own.",
    ...
    --  Fired from EUICoreStandaloneMinimap._NotifySettingWrite (EUICoreStandaloneMinimap_SpecOverrides.lua)
    ...
    ---------------------------------------------------------------------------
    EUICoreStandaloneMinimap.VideoGuides.Register("override_default_edit", {
...
--  in EUICoreStandaloneMinimap.lua where the sidebar builds).
-------------------------------------------------------------------------------
do
    EUICoreStandaloneMinimap.VideoGuides.Register("unlock_mode", {
        title = "Unlock Mode",
        blurb = "Unlock Mode is how you move and resize everything in EUICoreStandaloneMinimap, and it is packed with tools: anchors, width matching, fallback anchors, per-group layouts. This video gets you fluent fast.",
...
--  left of the "Cooldown Manager" label (attached in EUICoreStandaloneMinimap.lua's
--  CreateAddonChildRow).
-------------------------------------------------------------------------------
do
    EUICoreStandaloneMinimap.VideoGuides.Register("cooldown_manager", {
