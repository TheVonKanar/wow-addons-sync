...
-------------------------------------------------------------------------------
--  EUICoreStandaloneMinimap_GlobalSearch.lua
--  Unified fuzzy search across every registered EUICoreStandaloneMinimap sub-addon's
--  options.
--
--  Optional / modular by design: TagOptionRow and SectionHeader in
--  EUICoreStandaloneMinimap_Widgets.lua call EUICoreStandaloneMinimap._RegisterSearchEntry only if it
...
function EUICoreStandaloneMinimap._RegisterSearchEntry(label, labelLoc, tooltip, moduleFolder, page, sectionName, selectorSetter, selectorKey, isSection)
    ...
    if EUICoreStandaloneMinimap._searchIndexSuppress then return end
...
local function BuildCoarseCandidates()
    _coarseCandidates = {}
    for folder, config in pairs(EUICoreStandaloneMinimap._modules or {}) do
        if config.pages then
            local moduleLabel = EUICoreStandaloneMinimap.L(config.title or folder)
            for _, page in ipairs(config.pages) do
                local pageLabel = EUICoreStandaloneMinimap.L(page)
...
local function BuildModuleAliases()
    ...
    for folder, config in pairs(EUICoreStandaloneMinimap._modules or {}) do
        Add(config.title and EUICoreStandaloneMinimap.L(config.title), folder)
    end
    if EUICoreStandaloneMinimap.ADDON_ROSTER then
        for _, entry in ipairs(EUICoreStandaloneMinimap.ADDON_ROSTER) do
            if EUICoreStandaloneMinimap._modules and EUICoreStandaloneMinimap._modules[entry.folder] then
                Add(entry.display and EUICoreStandaloneMinimap.L(entry.display), entry.folder)
            end
        end
    end
    ...
    local EXTRA_ALIASES = {
        EUICoreStandaloneMinimapMythicTimer     = { "m+ timer", "m+", "m+ tools", "mythic+ timer" },
        EUICoreStandaloneMinimapCooldownManager = { "cdm" },
        EUICoreStandaloneMinimapQuickdraw       = { "radial", "wheel", "ring menu", "palette", "grid", "arc", "fan", "action wheel", "action palette", "action menu" },
    }
    for folder, list in pairs(EXTRA_ALIASES) do
        if EUICoreStandaloneMinimap._modules and EUICoreStandaloneMinimap._modules[folder] then
...
--  run against a stub widget factory: EUICoreStandaloneMinimap.Widgets is swapped to
...
--  IndexSlotForSearch lines read (verified 1:1 against EUICoreStandaloneMinimap_Widgets
...
-- Mirrors IndexSlotForSearch in EUICoreStandaloneMinimap_Widgets.lua exactly: empty
...
local function AbsorberRegister(labelText, tooltipText, isSection)
    ...
    EUICoreStandaloneMinimap._RegisterSearchEntry(labelText, loc ~= labelText and loc or nil,
        ...
        EUICoreStandaloneMinimap._buildingModule, EUICoreStandaloneMinimap._buildingPage,
...
local function PrebuildOnce(config, folder, page, selectorSetter, selectorKey)
    ...
    EUICoreStandaloneMinimap._buildingModule = folder
    EUICoreStandaloneMinimap._buildingPage = page
    EUICoreStandaloneMinimap._prebuilding = true
    ...
    local refreshSnap = EUICoreStandaloneMinimap._SnapshotAndClearWidgetRefreshList and EUICoreStandaloneMinimap._SnapshotAndClearWidgetRefreshList()
    ...
    -- this synchronous call (builders fetch `local W = EUICoreStandaloneMinimap.Widgets`
    ...
    _absSection = nil
    local realWidgets = EUICoreStandaloneMinimap.Widgets
    EUICoreStandaloneMinimap.Widgets = AbsorberW
    local ok, err = pcall(config.buildPage, page, wrapper, -6)
    EUICoreStandaloneMinimap.Widgets = realWidgets
    if not ok and EUICoreStandaloneMinimap.IsDevModeActive and EUICoreStandaloneMinimap.IsDevModeActive() then
        print("|cffff6060EUI GlobalSearch:|r prebuild failed for "
            .. tostring(folder) .. "::" .. tostring(page) .. ": " .. tostring(err))
    end
    if refreshSnap then EUICoreStandaloneMinimap._RestoreWidgetRefreshList(refreshSnap) end
    ...
    -- etc.) must guard their creation with EUICoreStandaloneMinimap._prebuilding itself rather than
    ...
    EUICoreStandaloneMinimap._prebuilding = nil
end
...
local function PrebuildJob(job)
    ...
    if not (EUICoreStandaloneMinimap._pageCache and EUICoreStandaloneMinimap._pageCache[cacheKey]) then
        local savedMethods = {}
        for _, name in ipairs(_CONTENT_HEADER_METHODS) do
            savedMethods[name] = EUICoreStandaloneMinimap[name]
            EUICoreStandaloneMinimap[name] = function() end
        end
        PrebuildOnce(config, job.folder, job.page, job.selectorSetter, job.selectorKey)
        for _, name in ipairs(_CONTENT_HEADER_METHODS) do
            EUICoreStandaloneMinimap[name] = savedMethods[name]
        end
    end
...
--  "Search Features...", EUICoreStandaloneMinimap._sidebarSearchBox) as the entry point
...
local function GetModuleDisplayName(folder)
    local config = EUICoreStandaloneMinimap._modules and EUICoreStandaloneMinimap._modules[folder]
    if config and config.title then return EUICoreStandaloneMinimap.L(config.title) end
    if EUICoreStandaloneMinimap.ADDON_ROSTER then
        for _, entry in ipairs(EUICoreStandaloneMinimap.ADDON_ROSTER) do
            if entry.folder == folder then return EUICoreStandaloneMinimap.L(entry.display) end
        end
    end
    return folder
end
...
local BREADCRUMB_ARROW = " |TInterface\\AddOns\\EUIStandaloneMinimap\\media\\icons\\eui-arrow-right.png:12:12|t "
...
local function JumpToResult(entry, sidebarSearchBox)
    ...
    -- matching row via EUICoreStandaloneMinimap:NavigateToElementSettings (the same deep-link
    ...
    if entry.kind == "page" or not entry.section then
        EUICoreStandaloneMinimap:SelectModule(entry.module)
        ...
        EUICoreStandaloneMinimap:NavigateToElementSettings(entry.module, entry.page, entry.section, preSelectFn, entry.label)
    end
...
local function EnsureSearchUI()
    ...
    local clickArea = EUICoreStandaloneMinimap._clickArea
    local sidebarSearchBox = EUICoreStandaloneMinimap._sidebarSearchBox
    ...
    if EUICoreStandaloneMinimap.MakeBorder then EUICoreStandaloneMinimap.MakeBorder(popup, 1, 1, 1, 0.12, PP) end
    ...
    for i = 1, MAX_VISIBLE_RESULTS do
        ...
        if EUICoreStandaloneMinimap.PrimeFontShadow then EUICoreStandaloneMinimap.PrimeFontShadow(lbl, true) end
        ...
        if EUICoreStandaloneMinimap.PrimeFontShadow then EUICoreStandaloneMinimap.PrimeFontShadow(sub, true) end
    ...
    RunSearch = function()
        ...
        -- SetLessCommonSearchActive in EUICoreStandaloneMinimap_Widgets.lua).
        if EUICoreStandaloneMinimap.SetLessCommonSearchActive then
            EUICoreStandaloneMinimap.SetLessCommonSearchActive(query ~= "")
        end
        ...
        for i, row in ipairs(resultRows) do
            ...
            if entry then
                if entry.kind == "page" then
                    ...
                    row._label:SetText(accentHex .. EUICoreStandaloneMinimap.L("Page") .. ":|r " .. entry.displayLabel)
                ...
                elseif entry.isSection then
                    ...
                    row._label:SetText(accentHex .. EUICoreStandaloneMinimap.L("Section") .. ":|r " .. name)
                    row._sub:SetText(JoinBreadcrumb(GetModuleDisplayName(entry.module), EUICoreStandaloneMinimap.L(entry.page)))
                else
...
--  this file hooks into). No edits to EUICoreStandaloneMinimap.lua needed for this --
--  standard same-addon method wrapping from a later-loaded file.
-------------------------------------------------------------------------------
for _, methodName in ipairs({ "Show", "Toggle", "ShowModule" }) do
    local original = EUICoreStandaloneMinimap[methodName]
    if original then
        EUICoreStandaloneMinimap[methodName] = function(self, ...)
            original(self, ...)
            EnsureSearchUI()
        end
    end
