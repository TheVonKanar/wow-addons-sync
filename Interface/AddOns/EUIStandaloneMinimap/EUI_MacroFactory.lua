...
--  Called by BuildQoLPage via EUICoreStandaloneMinimap.BuildMacroFactory(parent, y, PP)
-------------------------------------------------------------------------------
...
local function GetHealthMacroDB()
    ...
    if not EUICoreStandaloneMinimapDB.macroFactory then EUICoreStandaloneMinimapDB.macroFactory = {} end
    if not EUICoreStandaloneMinimapDB.macroFactory[EUI_HEALTH_MACRO_NAME] then
        EUICoreStandaloneMinimapDB.macroFactory[EUI_HEALTH_MACRO_NAME] = {}
    end
    return EUICoreStandaloneMinimapDB.macroFactory[EUI_HEALTH_MACRO_NAME]
end
...
local function ApplyHealthRecoveryMacro(items)
    ...
    EditMacro(idx, nil, nil, EUICoreStandaloneMinimap.BuildHealthRecoveryMacroBody(GetHealthMacroDB(), items))
    lastHealthRecoveryKey = key
    healthMacroPendingUpdate = false
end

function EUICoreStandaloneMinimap.BuildHealthRecoveryMacroBody(db, items)
    db = db or {}
    items = items or CollectHealthRecoveryItems()
    local lines = {}
...
local function OptionDisplayName(opt, noRequest)
    ...
    return EUICoreStandaloneMinimap.L(opt.label)
end


function EUICoreStandaloneMinimap.BuildMacroFactory(parent, startY, PP)
    local ICON_SIZE = 40
    local ICON_GAP = 40
    ...
    local ROW_STRIDE = 66
    local fontPath = EUICoreStandaloneMinimap.GetFontPath and EUICoreStandaloneMinimap.GetFontPath() or STANDARD_TEXT_FONT
        ...
        },
        {
            ...
            extraBody = function(db)
                ...
                if db.announce then
                    ...
                    -- macros).
                    local focusWord = (EUICoreStandaloneMinimap.L and EUICoreStandaloneMinimap.L("Focus")) or "Focus"
    ...
    local function GetMacroDB(macroName)
        if not EUICoreStandaloneMinimapDB then EUICoreStandaloneMinimapDB = {} end
        if not EUICoreStandaloneMinimapDB.macroFactory then EUICoreStandaloneMinimapDB.macroFactory = {} end
        if not EUICoreStandaloneMinimapDB.macroFactory[macroName] then EUICoreStandaloneMinimapDB.macroFactory[macroName] = {} end
        return EUICoreStandaloneMinimapDB.macroFactory[macroName]
    end
    ...
    local function BuildMacroBody(def, db)
        ...
        elseif def.healthRecovery then
            return EUICoreStandaloneMinimap.BuildHealthRecoveryMacroBody(db, nil)
    ...
    local function BuildMacroGroup(defs, anchorSide, titleText, perRow, gap, maxVisibleRows)
        ...
        titleFS:SetText(EUICoreStandaloneMinimap.L(titleText))
        ...
        for gi, def in ipairs(defs) do
            ...
            labelFS:SetText(EUICoreStandaloneMinimap.L(def.label):gsub("\n", " "))
            btn._label = labelFS
            ...
            local function PlayFlash()
                flashElapsed = 0; flashFS:SetText(EUICoreStandaloneMinimap.L("Macro Created")); flashFS:SetTextColor(1, 1, 1, 0)
            ...
            local function BuildMenu()
                ...
                local function RefAct()
                    if MacroExists() then aL:SetText(EUICoreStandaloneMinimap.L("|cffff4444Delete Macro|r")) else aL:SetText(EUICoreStandaloneMinimap.L("Create Macro")) end
                end
                ...
                local tBrd = EUICoreStandaloneMinimap.MakeBorder(tB, 0.4, 0.4, 0.4, 0.6, PP)
                ...
                tL:SetTextColor(0.75, 0.75, 0.75, 1); tL:SetPoint("LEFT", tR, "LEFT", 12, 0); tL:SetText(EUICoreStandaloneMinimap.L("Show Tooltip"))
                ...
                if xRows > 0 then
                    ...
                    for _, xt in ipairs(xToggles or {}) do
                        ...
                        local xBrd = EUICoreStandaloneMinimap.MakeBorder(xB, 0.4, 0.4, 0.4, 0.6, PP)
                        ...
                        xL:SetText(EUICoreStandaloneMinimap.L(xt.label))
                    ...
                    for _, xc in ipairs(xChoices or {}) do
                        ...
                        cL:SetText(EUICoreStandaloneMinimap.L(xc.label))
                        ...
                        local function BuildList()
                            ...
                            lBg:SetColorTexture(EUICoreStandaloneMinimap.DD_BG_R, EUICoreStandaloneMinimap.DD_BG_G, EUICoreStandaloneMinimap.DD_BG_B, 0.98)
                            EUICoreStandaloneMinimap.MakeBorder(list, 1, 1, 1, EUICoreStandaloneMinimap.DD_BRD_A, PP)
                            for oi = 1, 8 do
                                local oY = -2 - (oi - 1) * LRH
                ...
                if hasCheckboxes then
                    ...
                    hfs:SetTextColor(1, 1, 1, 0.25); hfs:SetPoint("CENTER"); hfs:SetText(EUICoreStandaloneMinimap.L("Drag to Reorder"))
                    mY = mY - HH
                    ...
                    for ci, cb in ipairs(cbItems) do
                        ...
                        local rBrd = EUICoreStandaloneMinimap.MakeBorder(rb, 0.4, 0.4, 0.4, 0.6, PP)
            ...
            btn:SetScript("OnEnter", function(self)
                self._hoverBdr:Show()
                local fullName = EUICoreStandaloneMinimap.L(def.label):gsub("\n", " ")
                if def.tooltip then
                    local status = self._isGray and EUICoreStandaloneMinimap.L("|cff888888Created|r") or EUICoreStandaloneMinimap.L("|cff888888Click to create|r")
                    EUICoreStandaloneMinimap.ShowWidgetTooltip(self, fullName .. "\n" .. EUICoreStandaloneMinimap.L(def.tooltip) .. "\n" .. status)
                elseif self._isGray then
                    EUICoreStandaloneMinimap.ShowWidgetTooltip(self, fullName .. "\n" .. EUICoreStandaloneMinimap.L("|cff888888Created. Right-click to configure.|r"))
                else
                    EUICoreStandaloneMinimap.ShowWidgetTooltip(self, fullName .. "\n" .. EUICoreStandaloneMinimap.L("|cff888888Click to create. Right-click to configure.|r"))
                end
            end)
            btn:SetScript("OnLeave", function(self) self._hoverBdr:Hide(); EUICoreStandaloneMinimap.HideWidgetTooltip() end)
    ...
    if #activeSpecDefs > 0 then
        BuildMacroGroup(activeSpecDefs, "RIGHT", (activeSpecName or EUICoreStandaloneMinimap.L("Spec")) .. " " .. activeClassName .. " " .. EUICoreStandaloneMinimap.L("Macro Factory"), SPEC_ICONS_PER_ROW, SPEC_ICON_GAP, MAX_SPEC_VISIBLE_ROWS)
    else
        ...
        emptyFS:SetText(EUICoreStandaloneMinimap.L("No spec macros for ") .. (activeSpecName or EUICoreStandaloneMinimap.L("this spec")))
    ...
    local mf = EUICoreStandaloneMinimap._macroFactory
    if not mf then mf = {}; EUICoreStandaloneMinimap._macroFactory = mf end
...
function EUICoreStandaloneMinimap.RefreshMacroFactory()
    local mf = EUICoreStandaloneMinimap._macroFactory
    ...
    EUICoreStandaloneMinimap._searchIndexSuppress = true
    pcall(EUICoreStandaloneMinimap.BuildMacroFactory, mf.parent, mf.startY, mf.PP)
    EUICoreStandaloneMinimap._searchIndexSuppress = nil
...
EUICoreStandaloneMinimap._macroSpecWatcher = EUICoreStandaloneMinimap._macroSpecWatcher or CreateFrame("Frame")
EUICoreStandaloneMinimap._macroSpecWatcher:UnregisterAllEvents()
EUICoreStandaloneMinimap._macroSpecWatcher:RegisterEvent("PLAYER_SPECIALIZATION_CHANGED")
EUICoreStandaloneMinimap._macroSpecWatcher:SetScript("OnEvent", function()
    if EUICoreStandaloneMinimap.RefreshMacroFactory then EUICoreStandaloneMinimap.RefreshMacroFactory() end
