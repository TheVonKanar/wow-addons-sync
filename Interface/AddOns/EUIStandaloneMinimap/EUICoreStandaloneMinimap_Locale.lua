...
--------------------------------------------------------------------------------
--  EUICoreStandaloneMinimap_Locale.lua
--  Central multi-language engine for the EUICoreStandaloneMinimap options panel, unlock mode,
...
--
--  TIMING: SavedVariables (EUICoreStandaloneMinimapDB, which holds the manual language override)
...
--  receives byte-identical text -- and the EUICoreStandaloneMinimapLocales data addon is
...
local EUICoreStandaloneMinimap = EUICoreStandaloneMinimap
...
-- EUICoreStandaloneMinimapLocales files via RegisterLocale() when that LoadOnDemand child
...
EUICoreStandaloneMinimap.Lf = function(s, ...)
    local t = L(s)
...
EUICoreStandaloneMinimap.EnKey = function(s)
    if type(s) ~= "string" then return s end
    return reverse[s] or s
end

-- Entry point every EUICoreStandaloneMinimapLocales/<code>.lua file calls. Returns the
...
EUICoreStandaloneMinimap.RegisterLocale = function(code)
...
-- (see EUICoreStandaloneMinimap.FONT_CYRILLIC), while no bundled face has CJK coverage at all.
...
local function Activate()
    ...
    local override = EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.displayLocale
    if override == "auto" or not SUPPORTED[override or ""] then override = nil end

    local locale = override or client
    EUICoreStandaloneMinimap.LOCALE     = locale
    EUICoreStandaloneMinimap.IS_ENGLISH = (locale == "enUS")
    EUICoreStandaloneMinimap._localeFont = GlyphFont(locale)
    EUICoreStandaloneMinimap._localeScript = GlyphScript(locale)
...
-- resolves the CLIENT locale only and sets the glyph font for EUICoreStandaloneMinimap.lua
-- (which reads EUICoreStandaloneMinimap._localeFont at its own file-load). The active catalog
...
-- EUICoreStandaloneMinimapLocales child (2.8MB of source English clients never parse):
...
f:SetScript("OnEvent", function(self, _, loaded)
    if loaded == ADDON_NAME then
        ...
        if locale ~= "enUS" and SUPPORTED[locale] and not localeData[locale]
            ...
            C_AddOns.LoadAddOn("EUICoreStandaloneMinimapLocales")
        end
        Activate()
    end
end)
