...
--------------------------------------------------------------------------------
--  EUICoreStandaloneMinimap_Migration.lua -- loaded after EUICoreStandaloneMinimap_Lite.lua, before
--  EUICoreStandaloneMinimap_Profiles.lua; runs at ADDON_LOADED for "EUICoreStandaloneMinimap" (before
...
function EUICoreStandaloneMinimap.RoundSizeFields(keys, tables)
...
--  host: "global" -> ctx.db=EUICoreStandaloneMinimapDB (flag on EUICoreStandaloneMinimapDB); "profile" ->
...
EUICoreStandaloneMinimap._migrationErrors = _migrationErrors

local VALID_SCOPES = { global = true, profile = true, specProfile = true }

function EUICoreStandaloneMinimap.RegisterMigration(spec)
...
local function RunMigration(spec)
    if spec.scope == "global" then
        RunOne(spec, { db = EUICoreStandaloneMinimapDB }, EUICoreStandaloneMinimapDB)

    elseif spec.scope == "profile" then
        if EUICoreStandaloneMinimapDB.profiles then
            for profName, profData in pairs(EUICoreStandaloneMinimapDB.profiles) do
    ...
    elseif spec.scope == "specProfile" then
        ...
        local sa = EUICoreStandaloneMinimapDB.spellAssignments
...
function EUICoreStandaloneMinimap.RunRegisteredMigrations()
    if not EUICoreStandaloneMinimapDB then
        ...
        EUICoreStandaloneMinimapDB = {}
        local flags = GetFlagTable(EUICoreStandaloneMinimapDB)
...
EUICoreStandaloneMinimap.RegisterMigration({
    ...
    body        = function(ctx)
        local rf = ctx.profile.addons and ctx.profile.addons.EUICoreStandaloneMinimapRaidFrames
...
--  Used by position_snap_v3 and exposed as EUICoreStandaloneMinimap.SnapProfilePositions for
--  profile import. MakeSnappers reads EUICoreStandaloneMinimapDB.ppUIScale and
...
local function MakeSnappers()
    ...
    local uiScale = EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.ppUIScale or perfect
...
local function SnapProfilePositions(profData)
    ...
    local uf = addons.EUICoreStandaloneMinimapUnitFrames
    if uf then snapPosMap(uf.positions) end

    local eab = addons.EUICoreStandaloneMinimapActionBars
    if eab then snapPosMap(eab.barPositions) end

    local cdm = addons.EUICoreStandaloneMinimapCooldownManager
    if cdm then snapPosMap(cdm.cdmBarPositions) end

    local erb = addons.EUICoreStandaloneMinimapResourceBars
    ...
    local abr = addons.EUICoreStandaloneMinimapAuraBuffReminders
    if type(abr) == "table" and abr.unlockPos then
        snapPos(abr.unlockPos)
    end

    local cursor = addons.EUICoreStandaloneMinimapCursor
...
EUICoreStandaloneMinimap.SnapProfilePositions = SnapProfilePositions
...
-- EUICoreStandaloneMinimapDB.specProfiles (the spec-to-profile auto-switch map).
--
...
do
    local function RearmRBAdvancedMigration(prof)
        local rb = prof and prof.addons and prof.addons.EUICoreStandaloneMinimapResourceBars
        ...
        body        = function(ctx)
            ...
            if type(prof.specOverrides) == "table" then
                for _, e in ipairs(prof.specOverrides) do
                    if type(e) == "table" and e.module == "EUICoreStandaloneMinimapResourceBars" then
                        return
                    end
                end
            end
...
EUICoreStandaloneMinimap.RegisterMigration({
    ...
    body        = function(ctx)
        local prof = ctx.profile
        if not prof then return end
        local PREFIX = "EUICoreStandaloneMinimapCooldownManager\31cdmBars\30bars\30"
...
-- The Skyriding HUD sub-DB registers its own folder (EUICoreStandaloneMinimapDragonRiding),
...
EUICoreStandaloneMinimap.RegisterMigration({
    ...
    body        = function(ctx)
        local prof = ctx.profile
        if not prof then return end
        local PREFIX = "EUICoreStandaloneMinimapDragonRiding\31"
...
EUICoreStandaloneMinimap.RegisterMigration({
    ...
    body        = function(ctx)
        ...
        local DeepCopy = EUICoreStandaloneMinimap._DeepCopy or (EUICoreStandaloneMinimap.Lite and EUICoreStandaloneMinimap.Lite.DeepCopy)
...
EUICoreStandaloneMinimap.RegisterMigration({
    ...
    body = function()
        local sv = _G.EUICoreStandaloneMinimapQuestTrackerDB
...
EUICoreStandaloneMinimap.RegisterMigration({
    ...
    body = function(ctx)
        ...
        if EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.global
           and EUICoreStandaloneMinimapDB.global._friendNotesMigrated then return end
...
EUICoreStandaloneMinimap.RegisterMigration({
    ...
    body = function(ctx)
        ...
        local dm = addons.EUICoreStandaloneMinimapDamageMeters
        if dm then
            snapPos(dm.unlockPos)
            roundFields(dm, { "dmWidth", "dmHeight" })
        end

        local chat = addons.EUICoreStandaloneMinimapChat
        if chat then
...
EUICoreStandaloneMinimap.RegisterMigration({
    ...
    body = function(ctx)
        ...
        local np = ctx.profile.addons and ctx.profile.addons.EUICoreStandaloneMinimapNameplates
...
EUICoreStandaloneMinimap.RegisterMigration({
    ...
    body = function(ctx)
        ...
        -- Scoped STRICTLY to EUICoreStandaloneMinimapUnitFrames -- RaidFrames' own unrelated
        -- powerBorderSize (border already worked) must NOT be zeroed here.
        local uf = ctx.profile.addons and ctx.profile.addons.EUICoreStandaloneMinimapUnitFrames
...
EUICoreStandaloneMinimap.RegisterMigration({
    ...
    body = function(ctx)
        ...
        local cdm = ctx.profile.addons and ctx.profile.addons.EUICoreStandaloneMinimapCooldownManager
    ...
    body = function(ctx)
        ...
        if ctx.db.profiles then
            for _, profData in pairs(ctx.db.profiles) do
                local cdm = profData.addons and profData.addons.EUICoreStandaloneMinimapCooldownManager
                local cdmBars = cdm and cdm.cdmBars
                local bars = cdmBars and cdmBars.bars
...
EUICoreStandaloneMinimap.RegisterMigration({
    ...
    body = function(ctx)
        ...
        local prof = EUICoreStandaloneMinimapDB.profiles and EUICoreStandaloneMinimapDB.profiles[ctx.profileName]
        local cdm = prof and prof.addons and prof.addons.EUICoreStandaloneMinimapCooldownManager
...
EUICoreStandaloneMinimap.RegisterMigration({
    ...
    body = function(ctx)
        ...
        local specProfs = CollectSpecProfiles(EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.spellAssignments)
...
EUICoreStandaloneMinimap.RegisterMigration({
    ...
    body = function(ctx)
        local addons = ctx.profile.addons
        if type(addons) ~= "table" then return end
        local qt = addons.EUICoreStandaloneMinimapQuestTracker
            and addons.EUICoreStandaloneMinimapQuestTracker.questTracker
        if type(qt) == "table" then
            if not qt.enabled then qt.enabled = true end
            if not qt.visibility then qt.visibility = "always" end
        end
        local mm = addons.EUIStandaloneMinimap
            and addons.EUIStandaloneMinimap.minimap
...
EUICoreStandaloneMinimap.RegisterMigration({
    ...
    scope       = "global",
    description = "Wipe obsolete Mythic+ bestRuns data (feature removed). Clears EUICoreStandaloneMinimapDB.global.bestRuns and every profile's addons.EUICoreStandaloneMinimapMythicTimer.bestRuns.",
    body = function(ctx)
        ...
        if type(profiles) == "table" then
            for _, profData in pairs(profiles) do
                local mt = profData and profData.addons and profData.addons.EUICoreStandaloneMinimapMythicTimer
...
EUICoreStandaloneMinimap.RegisterMigration({
    ...
    body = function(ctx)
        local chatDB = _G.EUICoreStandaloneMinimapChatDB
...
EUICoreStandaloneMinimap.RegisterMigration({
    ...
    body = function(ctx)
        ...
        local anchors = EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.unlockAnchors
...
EUICoreStandaloneMinimap.RegisterMigration({
    ...
    body = function(ctx)
        local emt = ctx.profile.addons and ctx.profile.addons.EUICoreStandaloneMinimapMythicTimer
...
EUICoreStandaloneMinimap.RegisterMigration({
    ...
    body = function(ctx)
        local np = ctx.profile.addons and ctx.profile.addons.EUICoreStandaloneMinimapNameplates
        if not np then np = {}; ctx.profile.addons = ctx.profile.addons or {}; ctx.profile.addons.EUICoreStandaloneMinimapNameplates = np end
...
EUICoreStandaloneMinimap.RegisterMigration({
    ...
    body = function(ctx)
        local ab = ctx.profile.addons and ctx.profile.addons.EUICoreStandaloneMinimapActionBars
...
EUICoreStandaloneMinimap.RegisterMigration({
    ...
    body = function(ctx)
        local erb = ctx.profile.addons and ctx.profile.addons.EUICoreStandaloneMinimapResourceBars
...
-------------------------------------------------------------------------------
--  Bags profile migration: copy flat EUICoreStandaloneMinimapDB root keys into each
--  profile's addons.EUICoreStandaloneMinimapBags so the Bags module can use NewDB().
-------------------------------------------------------------------------------
EUICoreStandaloneMinimap.RegisterMigration({
    id          = "bags_to_profile_v1",
    scope       = "global",
    description = "Migrate Bags settings from EUICoreStandaloneMinimapDB root to per-profile storage.",
    body = function(ctx)
        local db = ctx.db
        ...
        for profName, profData in pairs(db.profiles) do
            if type(profData) == "table" then
                ...
                if not profData.addons.EUICoreStandaloneMinimapBags then
                    local bags = {}
                    ...
                    if next(bags) then
                        profData.addons.EUICoreStandaloneMinimapBags = bags
                    end
                end
            end
        end
...
-- this, so ApplyProfileData (EUICoreStandaloneMinimap_Profiles.lua) forward-copies the legacy
...
EUICoreStandaloneMinimap.RegisterMigration({
    ...
    body = function(ctx)
        local bags = ctx.profile.addons and ctx.profile.addons.EUICoreStandaloneMinimapBags
...
-- populates EUICoreStandaloneMinimapDB.profiles, so only SavedVariables profiles (existing
        ...
        for _, profData in pairs(db.profiles) do
            ...
            if type(profData) == "table" and type(profData.addons) == "table"
               and next(profData.addons) then
                local rb = profData.addons.EUICoreStandaloneMinimapResourceBars
                if type(rb) ~= "table" then
                    rb = {}
                    profData.addons.EUICoreStandaloneMinimapResourceBars = rb
                end
...
EUICoreStandaloneMinimap.RegisterMigration({
    ...
    body = function(ctx)
        ...
        for _, profData in pairs(db.profiles) do
            ...
            if type(profData) == "table" and type(profData.addons) == "table"
               and next(profData.addons) then
                local uf = profData.addons.EUICoreStandaloneMinimapUnitFrames
                if type(uf) ~= "table" then
                    uf = {}
                    profData.addons.EUICoreStandaloneMinimapUnitFrames = uf
                end
...
EUICoreStandaloneMinimap.RegisterMigration({
    ...
    body = function(ctx)
        ...
        local oldBags = db.syncedModules and db.syncedModules.EUICoreStandaloneMinimapBags
        ...
        db.syncedModules = {}
        if count >= 2 then
            db.syncedModules.EUICoreStandaloneMinimapBags = carried
        end
    end,
})
...
-- EUICoreStandaloneMinimapDB.customColors stays the source of truth while the mode is off.
...
-- Secondary Stats + FPS counter moved from the account-wide EUICoreStandaloneMinimapDB root
-- into the per-profile QoL blob (EUICoreStandaloneMinimapQoL) so they travel with profiles,
-- export/import and module sync (see EUICoreStandaloneMinimap.QoLExtrasGet/Set in
-- EUICoreStandaloneMinimapQoL.lua). Seeds EVERY existing profile from the old
    ...
    body        = function(ctx)
        ...
        for _, pd in pairs(db.profiles) do
            if type(pd) == "table" then
                ...
                local q = pd.addons.EUICoreStandaloneMinimapQoL
                if type(q) ~= "table" then q = {}; pd.addons.EUICoreStandaloneMinimapQoL = q end
...
EUICoreStandaloneMinimap.RegisterMigration({
    ...
    body        = function(ctx)
        ...
        local qol = profile.addons.EUICoreStandaloneMinimapQoL
        if type(qol) ~= "table" then
            qol = {}
            profile.addons.EUICoreStandaloneMinimapQoL = qol
        end

        local enabled = qol.showTertiaryStats
        if enabled == nil then
            enabled = EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.showTertiaryStats
        end
...
-- EUICoreStandaloneMinimap.lua) + dormant backup; nil guard = re-run safe.
EUICoreStandaloneMinimap.RegisterMigration({
...
EUICoreStandaloneMinimap.RegisterMigration({
    ...
    body = function(ctx)
        ...
        local MODULES = {
            "EUICoreStandaloneMinimapUnitFrames", "EUICoreStandaloneMinimapRaidFrames", "EUICoreStandaloneMinimapNameplates",
            "EUICoreStandaloneMinimapResourceBars", "EUICoreStandaloneMinimapDamageMeters",
        }
...
EUICoreStandaloneMinimap.RegisterMigration({
    ...
    body = function()
        if EUICoreStandaloneMinimapDB then
            EUICoreStandaloneMinimapDB._anchorDebugLog = nil
        end
    end,
})
...
EUICoreStandaloneMinimap.RegisterMigration({
    ...
    body = function(ctx)
        ...
        local emm = ctx.profile.addons and ctx.profile.addons.EUIStandaloneMinimap
...
function EUICoreStandaloneMinimap.MigrateCdmSpellSettingsShape(specProf, barsCfg)
...
EUICoreStandaloneMinimap.RegisterMigration({
    ...
    body = function(ctx)
        ...
        EUICoreStandaloneMinimap.MigrateCdmSpellSettingsShape(ctx.specProfile, barsCfg)
    end,
})
...
function EUICoreStandaloneMinimap.MigrateCdmHostedBuffSettings(specProf)
...
EUICoreStandaloneMinimap.RegisterMigration({
    ...
    body = function(ctx)
        EUICoreStandaloneMinimap.MigrateCdmHostedBuffSettings(ctx.specProfile)
    end,
})
...
function EUICoreStandaloneMinimap.MigrateCdmThresholdText(cdm, specProfiles)
    local barsCfg = cdm and cdm.cdmBars and cdm.cdmBars.bars
    ...
    body = function(ctx)
        ...
        for profName, profData in pairs(db.profiles) do
            local cdm = type(profData) == "table" and type(profData.addons) == "table"
                and profData.addons.EUICoreStandaloneMinimapCooldownManager
            if type(cdm) == "table" then
                ...
                EUICoreStandaloneMinimap.MigrateCdmThresholdText(cdm, sp)
            end
        end
    end,
})
...
function EUICoreStandaloneMinimap.MigrateCdmBuffOrderUserFlag(specProf)
...
EUICoreStandaloneMinimap.RegisterMigration({
    ...
    body = function(ctx)
        EUICoreStandaloneMinimap.MigrateCdmBuffOrderUserFlag(ctx.specProfile)
    end,
})
    ...
    body = function(ctx)
        ...
        for _, profData in pairs(db.profiles) do
            if type(profData) == "table" and type(profData.addons) == "table" then
                local mt = profData.addons.EUICoreStandaloneMinimapMythicTimer
...
function EUICoreStandaloneMinimap.MigrateCdmBuffCdClaims(specProf)
...
EUICoreStandaloneMinimap.RegisterMigration({
    ...
    body = function(ctx)
        EUICoreStandaloneMinimap.MigrateCdmBuffCdClaims(ctx.specProfile)
    end,
})
...
EUICoreStandaloneMinimap.RegisterMigration({
    ...
    body = function(ctx)
        local qol = ctx.profile.addons and ctx.profile.addons.EUICoreStandaloneMinimapQoL
...
migrationFrame:SetScript("OnEvent", function(self, event, addonName)
    if addonName ~= "EUICoreStandaloneMinimap" then return end
    ...
    ---------------------------------------------------------------------------
    EUICoreStandaloneMinimap.RunRegisteredMigrations()
    ...
    if EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.profiles then
        for _, profData in pairs(EUICoreStandaloneMinimapDB.profiles) do
            local dm = profData.addons
                and profData.addons.EUICoreStandaloneMinimapDamageMeters
                and profData.addons.EUICoreStandaloneMinimapDamageMeters.dm
    ...
    if EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.profiles then
        for _, profData in pairs(EUICoreStandaloneMinimapDB.profiles) do
            local bars = profData.addons
                and profData.addons.EUICoreStandaloneMinimapCooldownManager
                and profData.addons.EUICoreStandaloneMinimapCooldownManager.cdmBars
                and profData.addons.EUICoreStandaloneMinimapCooldownManager.cdmBars.bars
            if bars then
                for i = #bars, 1, -1 do
                    if bars[i].key == "__ghost_buffs" then
                        table.remove(bars, i)
        ...
        for _, specData in ipairs(CollectSpecProfiles(EUICoreStandaloneMinimapDB.spellAssignments)) do
            if specData.barSpells then
                specData.barSpells["__ghost_buffs"] = nil
            end
        end
...
--  from Resource Bars' OnInitialize (exports EUICoreStandaloneMinimap._RBSectionDefaults,
...
do
    ...
    local RB_FOLDER = "EUICoreStandaloneMinimapResourceBars"
    ...
    local DeepCopyT = EUICoreStandaloneMinimap.Lite.DeepCopy
    ...
    --- Migrates one profile root table (EUICoreStandaloneMinimapDB.profiles[name] shape:
    --- .addons.EUICoreStandaloneMinimapResourceBars + spec-override tables at root).
    function EUICoreStandaloneMinimap.MigrateRBAdvancedProfile(prof)
        if type(prof) ~= "table" then return end
        local rb = prof.addons and prof.addons.EUICoreStandaloneMinimapResourceBars
        ...
        local DEFS = EUICoreStandaloneMinimap._RBSectionDefaults
...
-- OnInitialize exports EUICoreStandaloneMinimap._RBSectionDefaults then invokes
...
-- ledger (addons.EUICoreStandaloneMinimapDataBars.characters) and the QoL upgrade calculator's
-- per-character scan state (addons.EUICoreStandaloneMinimapQoL.chars) sat at profile scope, so
-- exported profiles carried the sharer's character names, realms and gold. Both stores
-- are account-wide now (EUICoreStandaloneMinimapDB.dataBarsGold / .qolUpgradeCalcChars); module init
...
-- on login/next scan. Folder literals contain "EUICoreStandaloneMinimap", which the standalone
...
EUICoreStandaloneMinimap.RegisterMigration({
    ...
    body        = function(ctx)
        ...
        for _, pd in pairs(db.profiles) do
            ...
            if addons then
                local dbars = addons["EUICoreStandaloneMinimapDataBars"]
                if type(dbars) == "table" then dbars.characters = nil end
                local qol = addons["EUICoreStandaloneMinimapQoL"]
...
-- New installs seed panelScale from display height in EUICoreStandaloneMinimap_Startup.lua;
...
EUICoreStandaloneMinimap.RegisterMigration({
    ...
    body        = function(ctx)
        ...
        if EUICoreStandaloneMinimap.SnapPanelScale then seeded = EUICoreStandaloneMinimap.SnapPanelScale(seeded) end
...
--  full Cyrillic block (EUICoreStandaloneMinimap.FONT_CYRILLIC) and ResolveFontName now
...
EUICoreStandaloneMinimap.RegisterMigration({
    ...
    body        = function(ctx)
        if EUICoreStandaloneMinimap.LOCALE_SCRIPT ~= "cyrillic" then return end
        ...
        if fonts.global == "Expressway" then
            fonts.global = EUICoreStandaloneMinimap.SYSTEM_FONT_KEY
        end
    end,
})
