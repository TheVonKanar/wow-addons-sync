...
-------------------------------------------------------------------------------
--  EUICoreStandaloneMinimap_UICore.lua
...
--  EUICoreStandaloneMinimap_Widgets.lua, so runtime callers keep working when the options
...
EUICoreStandaloneMinimap.MakeStyledButton = MakeStyledButton
EUICoreStandaloneMinimap.WB_COLOURS       = WB_COLOURS
EUICoreStandaloneMinimap.RB_COLOURS       = RB_COLOURS
...
local function DisabledTooltip(requirement, state)
    ...
    if type(requirement) == "string" and requirement:find("^This option") then
        return EUICoreStandaloneMinimap.L(requirement)
    end
    ...
    return EUICoreStandaloneMinimap.Lf("This option requires %1$s to be %2$s", EUICoreStandaloneMinimap.L(requirement), EUICoreStandaloneMinimap.L(verb))
end
EUICoreStandaloneMinimap.DisabledTooltip = DisabledTooltip
...
local function GetTooltipFrame()
    ...
    tooltipFrame.bg:SetColorTexture(EUICoreStandaloneMinimap.GetTooltipBg())
    return tooltipFrame
end
...
local function IsPanelFamilyAnchor(region)
    ...
    local pops = EUICoreStandaloneMinimap._popupFrames
    local node = region
    while node do
...
local function ShowWidgetTooltip(label, text, opts)
    ...
    tt.text:SetText(EUICoreStandaloneMinimap.L(text))
    ...
    if not ttScaleMult then
        ttScaleMult = 1
        local us = EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.panelScale
...
EUICoreStandaloneMinimap.ShowWidgetTooltip = ShowWidgetTooltip
EUICoreStandaloneMinimap.HideWidgetTooltip = HideWidgetTooltip
...
EUICoreStandaloneMinimap.DEFAULT_ACCENT = { r = EUICoreStandaloneMinimap.DEFAULT_ACCENT_R, g = EUICoreStandaloneMinimap.DEFAULT_ACCENT_G, b = EUICoreStandaloneMinimap.DEFAULT_ACCENT_B }

EUICoreStandaloneMinimap.GetAccentColor = function()
    return ELLESMERE_GREEN.r, ELLESMERE_GREEN.g, ELLESMERE_GREEN.b
end

--- Active theme name (default "EUICoreStandaloneMinimap")
...
local function ResolveThemeColor(theme)
    theme = EUICoreStandaloneMinimap._ResolveFactionTheme(theme)
    if theme == "Class Colored" then
        local clr = EUICoreStandaloneMinimap.CLASS_COLOR_MAP[EUICoreStandaloneMinimap._playerClass]
        if clr then return clr.r, clr.g, clr.b end
        return EUICoreStandaloneMinimap.DEFAULT_ACCENT_R, EUICoreStandaloneMinimap.DEFAULT_ACCENT_G, EUICoreStandaloneMinimap.DEFAULT_ACCENT_B
    elseif theme == "Custom Color" then
        local sa = EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.accentColor
        return sa and sa.r or EUICoreStandaloneMinimap.DEFAULT_ACCENT_R, sa and sa.g or EUICoreStandaloneMinimap.DEFAULT_ACCENT_G, sa and sa.b or EUICoreStandaloneMinimap.DEFAULT_ACCENT_B
    else
        local preset = EUICoreStandaloneMinimap.THEME_PRESETS[theme]
        if preset then return preset.r, preset.g, preset.b end
        return EUICoreStandaloneMinimap.DEFAULT_ACCENT_R, EUICoreStandaloneMinimap.DEFAULT_ACCENT_G, EUICoreStandaloneMinimap.DEFAULT_ACCENT_B
    end
end
EUICoreStandaloneMinimap.ResolveThemeColor = ResolveThemeColor
...
local function UpdateAccentElements(r, g, b)
    for _, entry in ipairs(EUICoreStandaloneMinimap._accentElements) do
...
accentFadeTicker:SetScript("OnUpdate", function(self, elapsed)
    ...
    if accentFadeProgress >= 1 then
        ...
        for i = 1, #EUICoreStandaloneMinimap._widgetRefreshList do EUICoreStandaloneMinimap._widgetRefreshList[i]() end
...
local function ApplyAccentAnimated(r, g, b)
    ...
    EUICoreStandaloneMinimap._InvalidateConfirmPopup()
...
EUICoreStandaloneMinimap.SetActiveTheme = function(theme)
    if not EUICoreStandaloneMinimapDB then EUICoreStandaloneMinimapDB = {} end
    EUICoreStandaloneMinimapDB.activeTheme = theme
    ELLESMERE_GREEN._themeEnabled = true
    local r, g, b = ResolveThemeColor(theme)

    if EUICoreStandaloneMinimap._applyThemeBG then
        EUICoreStandaloneMinimap._applyThemeBG(theme, r, g, b)
    end
end

--- SetAccentColor: persists accent color (per-profile) and applies live.
EUICoreStandaloneMinimap.SetAccentColor = function(r, g, b)
    if not EUICoreStandaloneMinimapDB then EUICoreStandaloneMinimapDB = {} end
    -- Persist on the active profile; the global root stays frozen as fallback.
    EUICoreStandaloneMinimap.SetActiveProfileAccent({ r = r, g = g, b = b }, false)
    ApplyAccentLive(r, g, b)
end

--- Applies accent live without persisting (custom/class accent mode switches).
EUICoreStandaloneMinimap.ApplyAccentColorLive = function(r, g, b)
    ApplyAccentLive(r, g, b)
end
...
--  The global keys EUICoreStandaloneMinimapDB.customAccentColor / useClassAccentColor stay
...
-------------------------------------------------------------------------------
function EUICoreStandaloneMinimap.GetActiveProfileData()
...
function EUICoreStandaloneMinimap.SetActiveProfileAccent(custom, useClass)
    local db = EUICoreStandaloneMinimapDB
...
function EUICoreStandaloneMinimap.GetActiveAccentState()
    local p   = EUICoreStandaloneMinimap.GetActiveProfileData()
    local acc = p and p.euiAccent
    local useClass
    if acc and acc.useClass ~= nil then
        useClass = acc.useClass
    else
        useClass = (EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.useClassAccentColor) or false
    end
    local custom = (acc and acc.custom) or (EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.customAccentColor)
...
function EUICoreStandaloneMinimap.ResolveProfileAccent(profileData)
    local themeR, themeG, themeB = ResolveThemeColor(EUICoreStandaloneMinimap.GetActiveTheme())
    local acc = profileData and profileData.euiAccent
    -- 1) per-profile
    if acc and acc.useClass then
        local c = EUICoreStandaloneMinimap.CLASS_COLOR_MAP[EUICoreStandaloneMinimap._playerClass]
    ...
    if (not acc) and EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.useClassAccentColor then
        local c = EUICoreStandaloneMinimap.CLASS_COLOR_MAP[EUICoreStandaloneMinimap._playerClass]
        if c then return true, c.r, c.g, c.b end
    end
    local gca = EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.customAccentColor
...
function EUICoreStandaloneMinimap.ResolveActiveAccent()
    local _, r, g, b = EUICoreStandaloneMinimap.ResolveProfileAccent(EUICoreStandaloneMinimap.GetActiveProfileData())
    return r, g, b
end
...
function EUICoreStandaloneMinimap.RefreshAccent()
    local r, g, b = EUICoreStandaloneMinimap.ResolveActiveAccent()
    ApplyAccentLive(r, g, b)
end

--- Class color for the current player
EUICoreStandaloneMinimap.GetPlayerClassColor = function()
    local clr = EUICoreStandaloneMinimap.CLASS_COLOR_MAP[EUICoreStandaloneMinimap._playerClass]
    if clr then return clr.r, clr.g, clr.b end
    return EUICoreStandaloneMinimap.DEFAULT_ACCENT_R, EUICoreStandaloneMinimap.DEFAULT_ACCENT_G, EUICoreStandaloneMinimap.DEFAULT_ACCENT_B
end
...
EUICoreStandaloneMinimap.ResetAccentColor = function()
    if EUICoreStandaloneMinimapDB then EUICoreStandaloneMinimapDB.accentColor = nil end
    local theme = EUICoreStandaloneMinimap.GetActiveTheme()
    local r, g, b = ResolveThemeColor(theme)
    ApplyAccentLive(r, g, b)
end

--- Wipes all style/theme settings back to defaults; called by the global "Reset to Defaults" button before ReloadUI().
EUICoreStandaloneMinimap.ResetTheme = function()
    if not EUICoreStandaloneMinimapDB then return end
    EUICoreStandaloneMinimapDB.accentColor   = nil
    EUICoreStandaloneMinimapDB.activeTheme   = nil
end
...
local function ShowContextMenu(anchor, items)
    local PP_L = EUICoreStandaloneMinimap.PP
    if not _ctxMenu then
        ...
        local RS = EUICoreStandaloneMinimap.RESKIN or {}
    ...
    local ITEM_H = 26
    local MENU_PAD = 4
    local fontPath = (EUICoreStandaloneMinimap.GetFontPath and EUICoreStandaloneMinimap.GetFontPath()) or STANDARD_TEXT_FONT or "Fonts\\FRIZQT__.TTF"
    local outline  = (EUICoreStandaloneMinimap.GetFontOutlineFlag and EUICoreStandaloneMinimap.GetFontOutlineFlag()) or ""
    ...
    for _, item in ipairs(items) do
        mfs:SetText(EUICoreStandaloneMinimap.L(item.text or ""))
    ...
    local hlAlpha = EUICoreStandaloneMinimap.DD_ITEM_HL_A or 0.08
    ...
    for i, item in ipairs(items) do
        ...
        btn._lbl:SetText(EUICoreStandaloneMinimap.L(item.text or ""))
...
EUICoreStandaloneMinimap.ShowContextMenu = ShowContextMenu
