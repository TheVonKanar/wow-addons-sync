...
-------------------------------------------------------------------------------
--  EUICoreStandaloneMinimap_Range.lua
...
if EUICoreStandaloneMinimap.Range_SetActive then return end -- already loaded
...
function EUICoreStandaloneMinimap.Range_GetAttackCutoff(customCutoff, holyPaladinMelee)
...
local function ItemChecksAllowed(unit)
    if not (InCombatLockdown()
        or (EUICoreStandaloneMinimap.InProtectedInstance and EUICoreStandaloneMinimap.InProtectedInstance())) then
        return true
    end
...
function EUICoreStandaloneMinimap.Range_LowerBound(unit)
...
function EUICoreStandaloneMinimap.Range_ItemBracket(unit, stopRange)
    if not unit or not UnitExists(unit) then return nil end
...
function EUICoreStandaloneMinimap.Range_BeyondCutoff(unit, cutoff)
    if not unit or not UnitExists(unit) then return nil end
    if not (C_Spell and C_Spell.IsSpellInRange) then return nil end
    EnsureLadder()
...
function EUICoreStandaloneMinimap.Range_IsBeyondAttackRange(unit, cutoff)
    cutoff = cutoff or EUICoreStandaloneMinimap.Range_GetAttackCutoff()
    if cutoff > 5 then
        local beyond = EUICoreStandaloneMinimap.Range_BeyondCutoff(unit, cutoff)
        if beyond ~= nil then return beyond end
    end
    local minY, maxY = EUICoreStandaloneMinimap.Range_ItemBracket(unit, cutoff)
...
local SWEEP_TTL = 0.4
function EUICoreStandaloneMinimap.Range_SweepBeyond(unit, cutoff)
    if not unit or not UnitExists(unit) then return nil end
    cutoff = cutoff or EUICoreStandaloneMinimap.Range_GetAttackCutoff()
    local now = GetTime()
    ...
    local beyond
    if cutoff > 5 then
        beyond = EUICoreStandaloneMinimap.Range_BeyondCutoff(unit, cutoff)
    end
...
function EUICoreStandaloneMinimap.Range_SetActive(key, on)
