...
-------------------------------------------------------------------------------
--  EUICoreStandaloneMinimap_Startup.lua
...
--
--  SavedVariables (EUICoreStandaloneMinimapDB) aren't available at file scope — they load
...
do
    ...
    local function SyncMultOnly()
        if EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.PP then
            if EUICoreStandaloneMinimap.PP.UpdateMult then EUICoreStandaloneMinimap.PP.UpdateMult() end
            if EUICoreStandaloneMinimap.PP.ResnapAllBorders then EUICoreStandaloneMinimap.PP.ResnapAllBorders() end
        end
    end
    ...
    scaleFrame:SetScript("OnEvent", function(self, event, addonName)
        if event == "ADDON_LOADED" then
            ...
            if EUICoreStandaloneMinimapDB.ppUIScale then
                -- Migrate 0.53 to exact pixel-perfect 0.5333... (768/1440)
                if EUICoreStandaloneMinimapDB.ppUIScale == 0.53 then
                    EUICoreStandaloneMinimapDB.ppUIScale = 0.5333333333
                -- Migrate 0.71 to exact pixel-perfect 0.7111... (768/1080)
                elseif EUICoreStandaloneMinimapDB.ppUIScale == 0.71 then
                    EUICoreStandaloneMinimapDB.ppUIScale = 0.7111111111
                end
                scaleKnown = true
                ...
                ApplyScaleSafe(EUICoreStandaloneMinimapDB.ppUIScale)
            end

        elseif event == "PLAYER_LOGIN" then
            self:UnregisterEvent("PLAYER_LOGIN")

            if scaleKnown and EUICoreStandaloneMinimapDB.ppUIScale then
        ...
        elseif event == "UI_SCALE_CHANGED" then
            local saved = EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.ppUIScale
            if saved then
        ...
        elseif event == "PLAYER_ENTERING_WORLD" then
            ...
            if EUICoreStandaloneMinimapDB.ppUIScale == nil then
                local blizzScale = UIParent:GetScale()
                local clamped = max(0.4, min(blizzScale, 1.15))
                EUICoreStandaloneMinimapDB.ppUIScale = clamped
                ...
                -- EUICoreStandaloneMinimap.SnapPanelScale) -- an off-menu value leaves the control
                ...
                if EUICoreStandaloneMinimapDB.panelScale == nil then
                    local _, physH = GetPhysicalScreenSize()
                    if type(physH) == "number" and physH > 0 then
                        local seeded = max(1, min(physH / 1440, 2))
                        EUICoreStandaloneMinimapDB.panelScale =
                            (EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.SnapPanelScale
                                and EUICoreStandaloneMinimap.SnapPanelScale(seeded)) or seeded
                    end
                end
            end

            local scale = EUICoreStandaloneMinimapDB.ppUIScale
            ...
            C_Timer.After(2, function()
                ...
                if EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.ppUIScale then
                    ApplyScaleSafe(EUICoreStandaloneMinimapDB.ppUIScale)
                end
                SyncMultOnly()
            end)
...
--
-- Reads EUICoreStandaloneMinimapDB.fonts directly rather than going through GetFontsDB():
...
local function ApplyUnitNameFont()
    local fonts = EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.fonts
    local name = fonts and fonts.unitNameFont
    if not name or name == "" then return end
    local path = fonts.unitNameFontPath
    if not ProbeFont(path) and EUICoreStandaloneMinimap and EUICoreStandaloneMinimap.ResolveFontName then
        ...
        -- default.
        local resolved = EUICoreStandaloneMinimap.ResolveFontName(name)
...
do
    ...
    fixFrame:SetScript("OnEvent", function(self, _, initialLogin, reloadingUi)
        ...
        if EUICoreStandaloneMinimapDB then EUICoreStandaloneMinimapDB.chatPosRebased = nil end
...
do
    ...
    f:SetScript("OnEvent", function(self, event, addonName)
        ...
        elseif addonName == "Blizzard_CombatText"
            or not (EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.fctFont)
...
-- and read a dead legacy key (EUICoreStandaloneMinimapDB.fontSettings.global), so it always
...
-- locale-aware EUICoreStandaloneMinimap.ApplyGlobalFontToGameText() ("Apply to All Game Text"),
-- which runs once at PLAYER_LOGIN. Reskinned Blizzard elements still pick up the
-- EUICoreStandaloneMinimap font on their own via per-element, locale-aware SetFont calls
-- (EUICoreStandaloneMinimap.GetFontPath("blizzardSkin")), so reskinning no longer touches the
-- global font and never affects other addons.

-------------------------------------------------------------------------------
--  Auto-disable EUICoreStandaloneMinimapBags when a dedicated bag addon is present.
--  Once the user manually toggles the Bags module (sidebar power button or
--  first-install popup), we set EUICoreStandaloneMinimapDB.bagsUserChosen and never
...
do
    ...
    f:SetScript("OnEvent", function(self, event, addonName)
        ...
        if not EUICoreStandaloneMinimapDB then EUICoreStandaloneMinimapDB = {} end
        if EUICoreStandaloneMinimapDB.bagsUserChosen then return end
        ...
        local bagsEnabled = C_AddOns.GetAddOnEnableState("EUICoreStandaloneMinimapBags") > 0
        if EUICoreStandaloneMinimapDB.bagsAutoDisabled and bagsEnabled then
            EUICoreStandaloneMinimapDB.bagsUserChosen = true
            EUICoreStandaloneMinimapDB.bagsAutoDisabled = nil
            return
        end
        for _, name in ipairs(BAG_ADDONS) do
            if C_AddOns.GetAddOnEnableState(name) > 0 then
                C_AddOns.DisableAddOn("EUICoreStandaloneMinimapBags")
                EUICoreStandaloneMinimapDB.bagsAutoDisabled = true
                return
            end
        end
        EUICoreStandaloneMinimapDB.bagsAutoDisabled = nil
...
-- Retire stale EUICoreStandaloneMinimapBasics copies: the v6.6 split shim was removed from the
...
if C_AddOns and C_AddOns.DoesAddOnExist and C_AddOns.DoesAddOnExist("EUICoreStandaloneMinimapBasics")
   and C_AddOns.GetAddOnEnableState and C_AddOns.GetAddOnEnableState("EUICoreStandaloneMinimapBasics") > 0 then
    C_AddOns.DisableAddOn("EUICoreStandaloneMinimapBasics")
end
