...
-------------------------------------------------------------------------------
--  EUICoreStandaloneMinimap_Widgets.lua -- Shared Widget Helpers + Widget Factory
--  Constants & utilities live in EUICoreStandaloneMinimap.lua.
--  DEFERRED: body runs on first EUICoreStandaloneMinimap:EnsureLoaded() call, not at load.
-------------------------------------------------------------------------------
local EUICoreStandaloneMinimap = _G.EUICoreStandaloneMinimap
EUICoreStandaloneMinimap._deferredInits[#EUICoreStandaloneMinimap._deferredInits + 1] = function()
local PP = EUICoreStandaloneMinimap.PanelPP
local isRussian = GetLocale() == "ruRU"

-- Utility functions (used heavily)
local SolidTex         = EUICoreStandaloneMinimap.SolidTex
local MakeFont         = EUICoreStandaloneMinimap.MakeFont
local MakeBorder       = EUICoreStandaloneMinimap.MakeBorder
local DisablePixelSnap = EUICoreStandaloneMinimap.DisablePixelSnap
local RowBg            = EUICoreStandaloneMinimap.RowBg
local lerp             = EUICoreStandaloneMinimap.lerp
local MakeDropdownArrow = EUICoreStandaloneMinimap.MakeDropdownArrow
local RegisterWidgetRefresh = EUICoreStandaloneMinimap.RegisterWidgetRefresh
local RegAccent        = EUICoreStandaloneMinimap.RegAccent

-- Visual constants (used in hot paths)
local EXPRESSWAY       = EUICoreStandaloneMinimap.EXPRESSWAY
local ELLESMERE_GREEN  = EUICoreStandaloneMinimap.ELLESMERE_GREEN
local CONTENT_PAD      = EUICoreStandaloneMinimap.CONTENT_PAD
local DARK_BG          = EUICoreStandaloneMinimap.DARK_BG
local BORDER_COLOR     = EUICoreStandaloneMinimap.BORDER_COLOR
local TEXT_WHITE       = EUICoreStandaloneMinimap.TEXT_WHITE
local TEXT_DIM         = EUICoreStandaloneMinimap.TEXT_DIM
local TEXT_SECTION     = EUICoreStandaloneMinimap.TEXT_SECTION
local MEDIA_PATH       = EUICoreStandaloneMinimap.MEDIA_PATH
local CS               = EUICoreStandaloneMinimap.CS

-- Numeric constants (used frequently in widget builders)
local TEXT_WHITE_R = EUICoreStandaloneMinimap.TEXT_WHITE_R
local TEXT_WHITE_G = EUICoreStandaloneMinimap.TEXT_WHITE_G
local TEXT_WHITE_B = EUICoreStandaloneMinimap.TEXT_WHITE_B
local TEXT_DIM_R   = EUICoreStandaloneMinimap.TEXT_DIM_R
local TEXT_DIM_G   = EUICoreStandaloneMinimap.TEXT_DIM_G
local TEXT_DIM_B   = EUICoreStandaloneMinimap.TEXT_DIM_B
local TEXT_DIM_A   = EUICoreStandaloneMinimap.TEXT_DIM_A
local BORDER_R     = EUICoreStandaloneMinimap.BORDER_R
local BORDER_G     = EUICoreStandaloneMinimap.BORDER_G
local BORDER_B     = EUICoreStandaloneMinimap.BORDER_B
local ROW_BG_ODD   = EUICoreStandaloneMinimap.ROW_BG_ODD
local ROW_BG_EVEN  = EUICoreStandaloneMinimap.ROW_BG_EVEN

-- Slider constants (packed into table to reduce upvalue count)
local SL = {
    TRACK_R = EUICoreStandaloneMinimap.SL_TRACK_R, TRACK_G = EUICoreStandaloneMinimap.SL_TRACK_G,
    TRACK_B = EUICoreStandaloneMinimap.SL_TRACK_B, TRACK_A = EUICoreStandaloneMinimap.SL_TRACK_A,
    FILL_A  = EUICoreStandaloneMinimap.SL_FILL_A,
    INPUT_R = EUICoreStandaloneMinimap.SL_INPUT_R, INPUT_G = EUICoreStandaloneMinimap.SL_INPUT_G,
    INPUT_B = EUICoreStandaloneMinimap.SL_INPUT_B, INPUT_A = EUICoreStandaloneMinimap.SL_INPUT_A,
    INPUT_BRD_A = EUICoreStandaloneMinimap.SL_INPUT_BRD_A,
    MW_INPUT_BOOST = EUICoreStandaloneMinimap.MW_INPUT_ALPHA_BOOST,
    MW_TRACK_BOOST = EUICoreStandaloneMinimap.MW_TRACK_ALPHA_BOOST,
}

-- Toggle constants (packed into table to reduce upvalue count)
local TG = {
    OFF_R = EUICoreStandaloneMinimap.TG_OFF_R, OFF_G = EUICoreStandaloneMinimap.TG_OFF_G,
    OFF_B = EUICoreStandaloneMinimap.TG_OFF_B, OFF_A = EUICoreStandaloneMinimap.TG_OFF_A,
    ON_A  = EUICoreStandaloneMinimap.TG_ON_A,
    KNOB_OFF_R = EUICoreStandaloneMinimap.TG_KNOB_OFF_R, KNOB_OFF_G = EUICoreStandaloneMinimap.TG_KNOB_OFF_G,
    KNOB_OFF_B = EUICoreStandaloneMinimap.TG_KNOB_OFF_B, KNOB_OFF_A = EUICoreStandaloneMinimap.TG_KNOB_OFF_A,
    KNOB_ON_R  = EUICoreStandaloneMinimap.TG_KNOB_ON_R,  KNOB_ON_G  = EUICoreStandaloneMinimap.TG_KNOB_ON_G,
    KNOB_ON_B  = EUICoreStandaloneMinimap.TG_KNOB_ON_B,  KNOB_ON_A  = EUICoreStandaloneMinimap.TG_KNOB_ON_A,
}

-- Checkbox constants (packed into table to reduce upvalue count)
local CB = {
    BOX_R = EUICoreStandaloneMinimap.CB_BOX_R, BOX_G = EUICoreStandaloneMinimap.CB_BOX_G, BOX_B = EUICoreStandaloneMinimap.CB_BOX_B,
    BRD_A = EUICoreStandaloneMinimap.CB_BRD_A, ACT_BRD_A = EUICoreStandaloneMinimap.CB_ACT_BRD_A,
}
...
local function PixelsToPanel(px, es)
    ...
    local RealPP = EUICoreStandaloneMinimap.PP
...
local function BuildToggleControl(parent, frameLevel, getValue, setValue, opts)
    ...
    do
        ...
        setValue = function(...)
            _s(...)
            if EUICoreStandaloneMinimap._NotifySettingWrite then EUICoreStandaloneMinimap._NotifySettingWrite(parent) end
        end
    end
    do local _r = setValue; setValue = function(...) _r(...); EUICoreStandaloneMinimap._settingsChanged = true end end
    opts = opts or {}
    local RealPP = EUICoreStandaloneMinimap.PP
    ...
    local PanelPP = EUICoreStandaloneMinimap.PanelPP or RealPP
...
-- Button constants
local BTN_BG_R  = EUICoreStandaloneMinimap.BTN_BG_R
local BTN_BG_G  = EUICoreStandaloneMinimap.BTN_BG_G
local BTN_BG_B  = EUICoreStandaloneMinimap.BTN_BG_B
local BTN_BG_A  = EUICoreStandaloneMinimap.BTN_BG_A
local BTN_BG_HA = EUICoreStandaloneMinimap.BTN_BG_HA
local BTN_BRD_A  = EUICoreStandaloneMinimap.BTN_BRD_A
local BTN_BRD_HA = EUICoreStandaloneMinimap.BTN_BRD_HA
local BTN_TXT_A  = EUICoreStandaloneMinimap.BTN_TXT_A
local BTN_TXT_HA = EUICoreStandaloneMinimap.BTN_TXT_HA

-- Dropdown constants
local DD_BG_R  = EUICoreStandaloneMinimap.DD_BG_R
local DD_BG_G  = EUICoreStandaloneMinimap.DD_BG_G
local DD_BG_B  = EUICoreStandaloneMinimap.DD_BG_B
local DD_BG_A  = EUICoreStandaloneMinimap.DD_BG_A
local DD_BG_HA = EUICoreStandaloneMinimap.DD_BG_HA
local DD_BRD_A  = EUICoreStandaloneMinimap.DD_BRD_A
local DD_BRD_HA = EUICoreStandaloneMinimap.DD_BRD_HA
local DD_TXT_A  = EUICoreStandaloneMinimap.DD_TXT_A
local DD_TXT_HA = EUICoreStandaloneMinimap.DD_TXT_HA
local DD_ITEM_HL_A  = EUICoreStandaloneMinimap.DD_ITEM_HL_A
local DD_ITEM_SEL_A = EUICoreStandaloneMinimap.DD_ITEM_SEL_A

-- Layout constants
local DUAL_ITEM_W  = EUICoreStandaloneMinimap.DUAL_ITEM_W
local DUAL_GAP     = EUICoreStandaloneMinimap.DUAL_GAP
local TRIPLE_ITEM_W = EUICoreStandaloneMinimap.TRIPLE_ITEM_W
local TRIPLE_GAP    = EUICoreStandaloneMinimap.TRIPLE_GAP

-------------------------------------------------------------------------------
--  Shared Widget Helpers  (reduce duplication across widget factories)
-------------------------------------------------------------------------------

-- MakeStyledButton + WB_COLOURS/RB_COLOURS live in EUICoreStandaloneMinimap_UICore.lua (runtime login popups use them too).
local MakeStyledButton = EUICoreStandaloneMinimap.MakeStyledButton
local WB_COLOURS = EUICoreStandaloneMinimap.WB_COLOURS
local RB_COLOURS = EUICoreStandaloneMinimap.RB_COLOURS

-- Widget tooltip system lives in EUICoreStandaloneMinimap_UICore.lua.
local ShowWidgetTooltip, HideWidgetTooltip = EUICoreStandaloneMinimap.ShowWidgetTooltip, EUICoreStandaloneMinimap.HideWidgetTooltip

-- Global search index registration for ONE setting. Single-setting rows go via TagOptionRow; multi-slot rows (DualRow, TripleRow, offset rows, wide button rows) call this once per slot so every result is exactly one setting, never a concatenated row label. No-op until the optional EUICoreStandaloneMinimap_GlobalSearch.lua defines it.
local function IndexSlotForSearch(parent, labelText, tooltipText)
    if not EUICoreStandaloneMinimap._RegisterSearchEntry then return end
    ...
    local loc = EUICoreStandaloneMinimap.L(labelText)
    -- Some pages show different content per internal selector (CDM bar / action bar / unit); see _buildingSelector.
    local sel = EUICoreStandaloneMinimap._buildingSelector
    EUICoreStandaloneMinimap._RegisterSearchEntry(labelText, loc ~= labelText and loc or nil,
        type(tooltipText) == "string" and tooltipText or nil,
        EUICoreStandaloneMinimap._buildingModule, EUICoreStandaloneMinimap._buildingPage, sectionName,
...
local function TagOptionRow(frame, parent, labelText, tooltipText, multiSlot)
    ...
    local _loc = EUICoreStandaloneMinimap.L(labelText)
...
-- Disabled-widget tooltip wrapper lives in EUICoreStandaloneMinimap_UICore.lua.
local DisabledTooltip = EUICoreStandaloneMinimap.DisabledTooltip
...
local function ResolveDisabledTip(cfg)
    ...
    if raw then return EUICoreStandaloneMinimap.L(tt) end
...
local function DDResolveLabel(values, order, curKey)
    ...
    local function TR(s)
        ...
        return EUICoreStandaloneMinimap.L(s)
    end
...
local function BuildDropdownMenu(ddBtn, menuW, order, values, getValue, setValue, ddLbl, style, disabledValuesFn)
    do
        ...
        setValue = function(...)
            _r(...)
            EUICoreStandaloneMinimap._settingsChanged = true
            -- Spec Overrides auto-capture (see BuildToggleControl): dropdown button sits inside the host slot's region.
            if EUICoreStandaloneMinimap._NotifySettingWrite then EUICoreStandaloneMinimap._NotifySettingWrite(ddBtn) end
        end
    end
    ...
    if _moSearchable then
        -- Options panel is Expressway-locked by design; EUICoreStandaloneMinimap.EXPRESSWAY is locale-aware (CJK/Cyrillic get the system glyph font). The user's global font intentionally never restyles the settings UI.
        local fontPath = EUICoreStandaloneMinimap.EXPRESSWAY or "Fonts\\FRIZQT__.TTF"
        ...
        searchPlaceholder:SetText(EUICoreStandaloneMinimap.L("Search..."))
...
local function WireDropdownScripts(ddBtn, ddLbl, bg, brd, menu, refresh, s, keepClickHandler)
    ...
    menu:SetScript("OnShow", function(self)
        ...
        EUICoreStandaloneMinimap._openDropdownMenu = self
        ApplyHover()
        refresh()
    ...
    menu:SetScript("OnHide", function(self)
        self:SetScript("OnUpdate", nil)
        if EUICoreStandaloneMinimap._openDropdownMenu == self then EUICoreStandaloneMinimap._openDropdownMenu = nil end
...
local function BuildSliderCore(parent, trackW, trackH, thumbSz, inputW, inputH, inputFontSz, inputAlpha, minVal, maxVal, step, getValue, setValue, isMultiWidget, snapPoints)
    ...
    local function SliderOnUpdate(self)
        ...
        if not IsMouseButtonDown("LeftButton") then
            ...
            EUICoreStandaloneMinimap._sliderDragging = math.max(0, (EUICoreStandaloneMinimap._sliderDragging or 1) - 1)
            if EUICoreStandaloneMinimap._sliderDragging == 0 then EUICoreStandaloneMinimap._sliderDragging = nil end
            CommitSnap()
            return
        end
    ...
    local function BeginDrag()
        isDragging = true
        EUICoreStandaloneMinimap._sliderDragging = (EUICoreStandaloneMinimap._sliderDragging or 0) + 1
    ...
    local function EndDrag()
        ...
        dragScale = nil; dragTrackLeft = nil; lastCommitX = nil
        EUICoreStandaloneMinimap._sliderDragging = math.max(0, (EUICoreStandaloneMinimap._sliderDragging or 1) - 1)
        if EUICoreStandaloneMinimap._sliderDragging == 0 then
            EUICoreStandaloneMinimap._sliderDragging = nil
        end
        ...
        if not EUICoreStandaloneMinimap._sliderDragging then
            ...
            if EUICoreStandaloneMinimap._deferredDriftChecks then
                local checks = EUICoreStandaloneMinimap._deferredDriftChecks
                EUICoreStandaloneMinimap._deferredDriftChecks = nil
            ...
            if EUICoreStandaloneMinimap.RefreshPage then EUICoreStandaloneMinimap:RefreshPage() end
        end
    end
    ...
    local function CommitInput()
        ...
        if changed and not EUICoreStandaloneMinimap._sliderDragging and EUICoreStandaloneMinimap.RefreshPage then
...
local function PixelizeSliderCfg(cfg)
    -- Convert against the GAME screen grid (EUICoreStandaloneMinimap.PP), not this file's panel-scale PanelPP: saved values live in UIParent coordinate units.
    local gamePP = EUICoreStandaloneMinimap.PP
...
-------------------------------------------------------------------------------
local WidgetFactory = {}
EUICoreStandaloneMinimap.Widgets = WidgetFactory
EUICoreStandaloneMinimap._font  = EXPRESSWAY
EUICoreStandaloneMinimap.CONTENT_PAD = CONTENT_PAD

EUICoreStandaloneMinimap.DD_STYLE = {
...
function WidgetFactory:SectionHeader(parent, text, yOffset)
    ...
    EUICoreStandaloneMinimap._rowCounters[parent] = 0
    ...
    local _snLoc = EUICoreStandaloneMinimap.L(text)
    if _snLoc ~= text then frame._sectionNameLoc = _snLoc end
    parent._currentSection = frame

    -- Global search index hook (see TagOptionRow); no-op without the optional EUICoreStandaloneMinimap_GlobalSearch.lua. Trailing true marks this a SECTION so results render as "Section: Title Case" instead of an option row.
    if EUICoreStandaloneMinimap._RegisterSearchEntry then
        local sel = EUICoreStandaloneMinimap._buildingSelector
        EUICoreStandaloneMinimap._RegisterSearchEntry(text, frame._sectionNameLoc, nil, EUICoreStandaloneMinimap._buildingModule, EUICoreStandaloneMinimap._buildingPage, text, sel and sel.setter, sel and sel.key, true)
    end
...
local function ClampRowLabel(label, rightFrame, rightPoint, gap, text, tooltip)
    ...
    if truncated and tooltip then
        ...
        return EUICoreStandaloneMinimap.L(text) .. "\n" .. EUICoreStandaloneMinimap.L(tooltip), true
...
local function LabelTooltipText(region, label, tip)
    if region._labelTruncated then
        return (label:GetText() or "") .. "\n" .. EUICoreStandaloneMinimap.L(tip)
    end
    return tip
end
...
local function GetPickerDB()
    ...
    if not EUICoreStandaloneMinimapDB.colorPicker then EUICoreStandaloneMinimapDB.colorPicker = {} end
    return EUICoreStandaloneMinimapDB.colorPicker
end
...
local function BuildColorPickerPopup()
    ...
    local popup = CreateFrame("Frame", "EUICoreStandaloneMinimapColorPicker", UIParent)
    ...
    titleLbl:SetAlpha(0.5); titleLbl:SetPoint("CENTER", 0, -10); titleLbl:SetText(EUICoreStandaloneMinimap.L("Color Picker"))
    ...
    nl:SetPoint("TOPLEFT", rightCol, "TOPLEFT", 0, 0); nl:SetText(EUICoreStandaloneMinimap.L("New"))
    ...
    pl:SetPoint("TOPLEFT", prevPrev, "BOTTOMLEFT", 0, -6); pl:SetText(EUICoreStandaloneMinimap.L("Prev"))

    local hexLbl = MakeFont(rightCol, 10, nil, 1,1,1); hexLbl:SetAlpha(TEXT_DIM_A)
    hexLbl:SetPoint("TOPLEFT", pl, "BOTTOMLEFT", 0, -21); hexLbl:SetText(EUICoreStandaloneMinimap.L("Hex#"))
    ...
    opacityLbl:SetPoint("TOPLEFT", hexBox, "BOTTOMLEFT", 0, -10); opacityLbl:SetText(EUICoreStandaloneMinimap.L("Opacity"))
    ...
    cancelText:SetText(EUICoreStandaloneMinimap.L("cancel"))
    ...
    local function MakeSwatchBtn(parent, isFavorites)
        ...
        btn:SetScript("OnEnter", function(self)
            ...
            local hint = isFavorites and EUICoreStandaloneMinimap.L("Right-click: remove favorite")
                                      or  EUICoreStandaloneMinimap.L("Right-click: favorite")
    ...
    favLbl:SetText(EUICoreStandaloneMinimap.L("Favorites"))
    ...
    rcLbl:SetText(EUICoreStandaloneMinimap.L("Recent Colors"))
    ...
    popup:SetScript("OnHide", function()
        EUICoreStandaloneMinimap._colorPickerOpen = false
    ...
    EUICoreStandaloneMinimap.RegisterEscapeClose(popup)
    ...
    function popup:Open(info, anchorFrame)
        ...
        local mainFrame = EUICoreStandaloneMinimap._mainFrame
        ...
        EUICoreStandaloneMinimap._colorPickerOpen = true
        RefreshSwatchRows()
    ...
    EUICoreStandaloneMinimap._colorPickerPopup = popup
    return popup
end

function EUICoreStandaloneMinimap:ShowColorPicker(info, anchorFrame)
...
local function BuildColorSwatch(parentFrame, baseLevel, getValue, setValue, hasAlpha, overrideSize)
    ...
    do
        ...
        setValue = function(...)
            _s(...)
            if EUICoreStandaloneMinimap._NotifySettingWrite then EUICoreStandaloneMinimap._NotifySettingWrite(parentFrame) end
        end
    end
    do local _r = setValue; setValue = function(...) _r(...); EUICoreStandaloneMinimap._settingsChanged = true end end
    ...
    swatch:SetScript("OnClick", function()
        ...
        local function OnColorChanged()
            local popup = EUICoreStandaloneMinimap._colorPickerPopup
        ...
        EUICoreStandaloneMinimap:ShowColorPicker(info, swatch)
    end)
    ...
    if EUICoreStandaloneMinimap.AddCaptureAccessor and not parentFrame._noCapture then
        EUICoreStandaloneMinimap.AddCaptureAccessor(parentFrame, {
...
local function BuildTrioColorSwatch(parentFrame, baseLevel, opts)
    ...
    if opts.hasClassColor then
        classSwatch = BuildColorSwatch(parentFrame, baseLevel,
            function()
                local cc = EUICoreStandaloneMinimap.GetClassColor(EUICoreStandaloneMinimap._playerClass)
...
-- Color Picker row (swatch that opens the EUICoreStandaloneMinimap picker popup)
...
function EUICoreStandaloneMinimap.AddCaptureAccessor(region, acc)
...
function WidgetFactory:DualRow(parent, yOffset, leftCfg, rightCfg)
    ...
    local function BuildHalf(region, cfg)
        ...
        label:SetWordWrap(false)
        label:SetMaxLines(1)
        label:SetText(EUICoreStandaloneMinimap.L(cfg.text or ""))
        region._label = label
        ...
        elseif t == "input" then
            ...
            if isPopupStyle then
                ...
                if cfg.placeholder then
                    ...
                    ph:SetText(EUICoreStandaloneMinimap.L(cfg.placeholder))
...
function WidgetFactory:TripleRow(parent, yOffset, leftCfg, midCfg, rightCfg, splits)
    ...
    local function BuildThird(region, cfg)
        ...
        elseif t == "checkbox" then
            ...
            cbLabel:SetText(EUICoreStandaloneMinimap.L(cfg.text or ""))
...
function WidgetFactory:DropdownWithOffsets(parent, yOffset, dropdownCfg, xSliderCfg, ySliderCfg)
    ...
    ddLabel:SetText(EUICoreStandaloneMinimap.L(dropdownCfg.text or ""))
    ...
    local function BuildMiniSlider(slCfg, align)
        ...
        axisLabel:SetText(EUICoreStandaloneMinimap.L(slCfg.text or ""))
...
function WidgetFactory:WideDropdown(parent, title, yOffset, values, getValue, setValue, order, btnWidth, disabledValuesFn)
    ...
    local titleLabel = MakeFont(frame, 13, nil, EUICoreStandaloneMinimap.TEXT_SECTION_R, EUICoreStandaloneMinimap.TEXT_SECTION_G, EUICoreStandaloneMinimap.TEXT_SECTION_B, EUICoreStandaloneMinimap.TEXT_SECTION_A)
    PP.Point(titleLabel, "TOP", frame, "TOP", 0, 0)
    titleLabel:SetText(EUICoreStandaloneMinimap.L(title))
...
function WidgetFactory:TripleDropdown(parent, configs, yOffset)
    ...
    for idx, cfg in ipairs(configs) do
        ...
        local titleLbl = MakeFont(frame, 11, nil, EUICoreStandaloneMinimap.TEXT_SECTION_R, EUICoreStandaloneMinimap.TEXT_SECTION_G, EUICoreStandaloneMinimap.TEXT_SECTION_B, EUICoreStandaloneMinimap.TEXT_SECTION_A)
        PP.Point(titleLbl, "TOP", frame, "TOPLEFT", col + DD_W / 2, 0)
        titleLbl:SetText(EUICoreStandaloneMinimap.L(cfg.title))
...
local function BuildCogPopup(opts)
    ...
    if opts.captureRegion and EUICoreStandaloneMinimap.AddCaptureAccessor and opts.rows then
        for _, row in ipairs(opts.rows) do
            if row.get and row.set and not row.noCapture
               ...
               and row.type ~= "reordercheck" then
                EUICoreStandaloneMinimap.AddCaptureAccessor(opts.captureRegion, {
    ...
    if opts.rows then
        for _, row in ipairs(opts.rows) do
            if row.set then
                ...
                row.set = function(...)
                    _s(...)
                    if EUICoreStandaloneMinimap._NotifySettingWrite then
                        EUICoreStandaloneMinimap._NotifySettingWrite(popupOwner or opts.captureRegion)
                    end
                end
            end
        end
    ...
    local function CreatePopup()
        ...
        for _, row in ipairs(opts.rows) do
            if row.type == "slider" or row.type == "input" then
                tmpFS:SetText(EUICoreStandaloneMinimap.L(row.label))
        ...
        local ppScale = EUICoreStandaloneMinimap.GetPopupScale and EUICoreStandaloneMinimap.GetPopupScale() or 1
        pf:SetScale(ppScale)
        if EUICoreStandaloneMinimap._popupFrames then
            EUICoreStandaloneMinimap._popupFrames[#EUICoreStandaloneMinimap._popupFrames + 1] = { popup = pf }
        end
        ...
        titleFS:SetText(EUICoreStandaloneMinimap.L(opts.title or ""))
        ...
        for i, row in ipairs(opts.rows) do
            ...
            if row.type == "slider" then
                ...
                lbl:SetText(EUICoreStandaloneMinimap.L(row.label))
                ...
                if row.tooltip then
                    ...
                    hitFrame:SetScript("OnEnter", function()
                        if EUICoreStandaloneMinimap.ShowWidgetTooltip then
                            EUICoreStandaloneMinimap.ShowWidgetTooltip(lbl, row.tooltip)
                        end
                    end)
                ...
                if row.disabled then
                    ...
                    sliderDis:SetScript("OnLeave", function() if EUICoreStandaloneMinimap.HideWidgetTooltip then EUICoreStandaloneMinimap.HideWidgetTooltip() end end)
                end
            ...
            elseif row.type == "toggle" then
                ...
                if row.disabled then
                    ...
                    toggleDis:SetScript("OnLeave", function() if EUICoreStandaloneMinimap.HideWidgetTooltip then EUICoreStandaloneMinimap.HideWidgetTooltip() end end)
                end
            ...
            elseif row.type == 'dropdown' then
                ...
                if row.disabled then
                    ...
                    ddDis:SetScript("OnLeave", function() if EUICoreStandaloneMinimap.HideWidgetTooltip then EUICoreStandaloneMinimap.HideWidgetTooltip() end end)
                end
            ...
            elseif row.type == 'reordercheck' then
                ...
                local ddBtn, refresh = EUICoreStandaloneMinimap.BuildReorderCBDropdown(
            ...
            elseif row.type == 'segmented' then
                ...
                local seg, _seg2, segRefresh = EUICoreStandaloneMinimap.BuildSegmentedControl({
                    parent     = pf,
                    keys       = row.keys,
                    labels     = row.labels,
                    autoWidth  = true,
                ...
                if row.disabled then
                    ...
                    segDis:SetScript("OnLeave", function() if EUICoreStandaloneMinimap.HideWidgetTooltip then EUICoreStandaloneMinimap.HideWidgetTooltip() end end)
                end
            ...
            elseif row.type == 'colorpicker' then
                ...
                if row.disabled then
                    ...
                    cpSwBlock:SetScript("OnEnter", function()
                        local tip = ResolveDisabledTip(row)
                        if tip and EUICoreStandaloneMinimap.ShowWidgetTooltip then
                            EUICoreStandaloneMinimap.ShowWidgetTooltip(cpSwatch, tip)
                        end
                    end)
                    cpSwBlock:SetScript("OnLeave", function() if EUICoreStandaloneMinimap.HideWidgetTooltip then EUICoreStandaloneMinimap.HideWidgetTooltip() end end)
                    ...
                    cpLblBlock:SetScript("OnEnter", function()
                        local tip = ResolveDisabledTip(row)
                        if tip and EUICoreStandaloneMinimap.ShowWidgetTooltip then
                            EUICoreStandaloneMinimap.ShowWidgetTooltip(lbl, tip)
                        end
                    end)
                    cpLblBlock:SetScript("OnLeave", function() if EUICoreStandaloneMinimap.HideWidgetTooltip then EUICoreStandaloneMinimap.HideWidgetTooltip() end end)
                end
            ...
            elseif row.type == 'multiswatch' then
                ...
                for i = #mswSwatches, 1, -1 do
                    ...
                    if sc.tooltip then
                        swatch:HookScript('OnEnter', function()
                            if EUICoreStandaloneMinimap.ShowWidgetTooltip then EUICoreStandaloneMinimap.ShowWidgetTooltip(swatch, sc.tooltip) end
                        end)
                        swatch:HookScript('OnLeave', function()
                            if EUICoreStandaloneMinimap.HideWidgetTooltip then EUICoreStandaloneMinimap.HideWidgetTooltip() end
                        end)
                ...
                if row.disabled then
                    ...
                    mswDis:SetScript("OnLeave", function() if EUICoreStandaloneMinimap.HideWidgetTooltip then EUICoreStandaloneMinimap.HideWidgetTooltip() end end)
                    local initDis = type(row.disabled) == "function" and row.disabled() or row.disabled
            ...
            elseif row.type == 'input' then
                ...
                if not commitOnBlur then
                    ...
                    saveLbl:SetText(EUICoreStandaloneMinimap.L("Save"))
                ...
                local function ApplyInput()
                    ...
                    if saveBg then
                        ...
                        saveLbl:SetText(EUICoreStandaloneMinimap.L("Saved"))
                ...
                if row.disabled then
                    ...
                    inputDis:SetScript("OnLeave", function() if EUICoreStandaloneMinimap.HideWidgetTooltip then EUICoreStandaloneMinimap.HideWidgetTooltip() end end)
                end
            ...
            elseif row.type == 'button' then
                ...
                btnLbl:SetText(EUICoreStandaloneMinimap.L(row.label))
            ...
            elseif row.type == 'reorder' then
                ...
                local rBg = SolidTex(ddBtn, "BACKGROUND", EUICoreStandaloneMinimap.DD_BG_R, EUICoreStandaloneMinimap.DD_BG_G, EUICoreStandaloneMinimap.DD_BG_B, EUICoreStandaloneMinimap.DD_BG_A)
                rBg:SetAllPoints()
                local rBrd = MakeBorder(ddBtn, 1, 1, 1, EUICoreStandaloneMinimap.DD_BRD_A, PP)
                local rLbl = MakeFont(ddBtn, 12, nil, 1, 1, 1)
                rLbl:SetAlpha(EUICoreStandaloneMinimap.DD_TXT_A)
                rLbl:SetJustifyH("LEFT"); rLbl:SetWordWrap(false); rLbl:SetMaxLines(1)
                rLbl:SetPoint("LEFT", ddBtn, "LEFT", 8, 0)
                rLbl:SetText(EUICoreStandaloneMinimap.L(row.label))
                local rArrow = MakeDropdownArrow(ddBtn, 12, PP)
                rLbl:SetPoint("RIGHT", rArrow, "LEFT", -5, 0)
                ddBtn:SetScript("OnEnter", function()
                    rBg:SetColorTexture(EUICoreStandaloneMinimap.DD_BG_R, EUICoreStandaloneMinimap.DD_BG_G, EUICoreStandaloneMinimap.DD_BG_B, EUICoreStandaloneMinimap.DD_BG_HA)
                    rBrd:SetColor(1, 1, 1, EUICoreStandaloneMinimap.DD_BRD_HA)
                    rLbl:SetAlpha(EUICoreStandaloneMinimap.DD_TXT_HA)
                end)
                ddBtn:SetScript("OnLeave", function()
                    rBg:SetColorTexture(EUICoreStandaloneMinimap.DD_BG_R, EUICoreStandaloneMinimap.DD_BG_G, EUICoreStandaloneMinimap.DD_BG_B, EUICoreStandaloneMinimap.DD_BG_A)
                    rBrd:SetColor(1, 1, 1, EUICoreStandaloneMinimap.DD_BRD_A)
                    rLbl:SetAlpha(EUICoreStandaloneMinimap.DD_TXT_A)
                end)
                ...
                menu:SetWidth(RR_W)
                menu:Hide()
                local mBg2 = SolidTex(menu, "BACKGROUND", EUICoreStandaloneMinimap.DD_BG_R, EUICoreStandaloneMinimap.DD_BG_G, EUICoreStandaloneMinimap.DD_BG_B, 0.98)
                mBg2:SetAllPoints()
                MakeBorder(menu, 1, 1, 1, EUICoreStandaloneMinimap.DD_BRD_A, PP)
                menu:SetPoint("TOPLEFT", ddBtn, "BOTTOMLEFT", 0, -2)
                ...
                ht:SetText(EUICoreStandaloneMinimap.L(row.hint or "Drag to Reorder"))
                mY = mY - 18
                ...
                insLine:SetHeight(2)
                local EG2 = EUICoreStandaloneMinimap.ELLESMERE_GREEN or { r = 0.05, g = 0.82, b = 0.62 }
                ...
                if row.disabled then
                    ...
                    reorderDis:SetScript("OnEnter", function(self)
                        local tip = ResolveDisabledTip(row)
                        if tip and EUICoreStandaloneMinimap.ShowWidgetTooltip then EUICoreStandaloneMinimap.ShowWidgetTooltip(self, tip) end
                    end)
                    reorderDis:SetScript("OnLeave", function() if EUICoreStandaloneMinimap.HideWidgetTooltip then EUICoreStandaloneMinimap.HideWidgetTooltip() end end)
                    local initDis = type(row.disabled) == "function" and row.disabled() or row.disabled
        ...
        if opts.footer and opts.footer.unlockKey then
            ...
            line1:SetText(EUICoreStandaloneMinimap.L("Reposition freely with"))
            ...
            unlockFS:SetText(EUICoreStandaloneMinimap.L("Unlock Mode"))
            unlockFS:SetPoint("CENTER")
            unlockBtn:SetScript("OnClick", function()
                pf:Hide()
                if EUICoreStandaloneMinimap._openUnlockMode then
                    EUICoreStandaloneMinimap._unlockAutoSelectKey = opts.footer.unlockKey
            ...
            for _, rw in ipairs(rowWidgets) do
                ...
                elseif rw.type == 'dropdown' then
                    ...
                    if rw.lbl and rw.get and rw.values then
                        rw.lbl:SetText(EUICoreStandaloneMinimap.L(DDText(rw.values[rw.get()]) or tostring(rw.get())))
        ...
        -- Close when the main EUICoreStandaloneMinimap frame hides
        if EUICoreStandaloneMinimap._mainFrame then
            EUICoreStandaloneMinimap._mainFrame:HookScript("OnHide", function()
...
local function BuildSegmentedControl(cfg)
    ...
    local tmpFS = UIParent:CreateFontString(nil, "OVERLAY")
    tmpFS:SetFont(EUICoreStandaloneMinimap.EXPRESSWAY or "Fonts\\FRIZQT__.TTF", FONT_SZ, "")
    local segWidths = {}
    local pillW = 0
    for _, key in ipairs(cfg.keys) do
        tmpFS:SetText(EUICoreStandaloneMinimap.L(cfg.labels[key] or key))
    ...
    for i, key in ipairs(cfg.keys) do
        ...
        lbl:SetFont(EUICoreStandaloneMinimap.EXPRESSWAY, FONT_SZ, "")
        local lblOfsX = 0
        if i == 1 then lblOfsX = -capW / 2 end
        if i == numKeys then lblOfsX = capW / 2 end
        lbl:SetPoint("CENTER", lblOfsX, 0)
        lbl:SetText(EUICoreStandaloneMinimap.L(cfg.labels[key] or key))
...
-------------------------------------------------------------------------------
--  Exports  (widget helpers EUICoreStandaloneMinimap table for EUICoreStandaloneMinimap_Presets.lua)
-------------------------------------------------------------------------------
EUICoreStandaloneMinimap.DDText              = DDText
EUICoreStandaloneMinimap.BuildDropdownMenu   = BuildDropdownMenu
EUICoreStandaloneMinimap.WireDropdownScripts = WireDropdownScripts
EUICoreStandaloneMinimap.WD_DD_COLOURS       = WD_DD_COLOURS
EUICoreStandaloneMinimap.RD_DD_COLOURS       = RD_DD_COLOURS
--------------------------------------------------------------------------------
...
local function PlaySyncFlash(targetFrame)
    ...
    if not glow then
        ...
        local ar, ag, ab = EUICoreStandaloneMinimap.GetAccentColor()
...
EUICoreStandaloneMinimap.PlaySyncFlash = PlaySyncFlash
...
EUICoreStandaloneMinimap.PlayWhiteFlash = PlayWhiteFlash
...
local function BuildMultiApplyDropdown(anchorFrame, opts, flashTargets)
    ...
    mBg:SetAllPoints()
    mBg:SetColorTexture(EUICoreStandaloneMinimap.DD_BG_R, EUICoreStandaloneMinimap.DD_BG_G, EUICoreStandaloneMinimap.DD_BG_B, 0.96)
    EUICoreStandaloneMinimap.MakeBorder(menu, 1, 1, 1, EUICoreStandaloneMinimap.DD_BRD_A, PP)
    ...
    local DB_BG = EUICoreStandaloneMinimap.DARK_BG or { r = 0.05, g = 0.07, b = 0.09 }
    ...
    local applyBrd = EUICoreStandaloneMinimap.MakeBorder(applyRow, 1, 1, 1, 0.4, PP)

    local applyLbl = applyRow:CreateFontString(nil, "OVERLAY")
    applyLbl:SetFont(fontPath, 12, "")
    applyLbl:SetTextColor(1, 1, 1, 0.5)
    applyLbl:SetText(EUICoreStandaloneMinimap.L("Apply"))
    ...
    for _, key in ipairs(keys) do
        ...
        local boxBrd = EUICoreStandaloneMinimap.MakeBorder(box, 0.4, 0.4, 0.4, 0.6, PP)

        local chk = box:CreateTexture(nil, "ARTWORK")
        PP.SetInside(chk, box, 2, 2)
        local gr = EUICoreStandaloneMinimap.ELLESMERE_GREEN
        chk:SetColorTexture(gr.r, gr.g, gr.b, 1)

        local lbl = row:CreateFontString(nil, "OVERLAY")
        lbl:SetFont(fontPath, 13, "")
        lbl:SetPoint("LEFT", box, "RIGHT", 8, 0)
        lbl:SetText(EUICoreStandaloneMinimap.L(labels[key] or key))
...
local function BuildSyncIcon(opts)
    ...
    prefixText:SetText(EUICoreStandaloneMinimap.L("Apply to:"))
    ...
    allText:SetText(EUICoreStandaloneMinimap.L("All"))
    ...
    allBtn:SetScript("OnEnter", function()
        local r, g, b = EUICoreStandaloneMinimap.GetAccentColor()
    ...
    if opts.multiApply then
        ...
        multiText:SetText(EUICoreStandaloneMinimap.L("Multiple"))
    ...
    EUICoreStandaloneMinimap.RegisterWidgetRefresh(function()
        ...
        if EUICoreStandaloneMinimap._sliderDragging then
            if target == 1 then
                ...
                if not EUICoreStandaloneMinimap._deferredDriftChecks then
                    EUICoreStandaloneMinimap._deferredDriftChecks = {}
                end
                EUICoreStandaloneMinimap._deferredDriftChecks[function()
                    local r2, g2, b2 = EUICoreStandaloneMinimap.GetAccentColor()
                    prefixText:SetTextColor(1, 1, 1, 0.65)
        ...
        if EUICoreStandaloneMinimap._colorPickerOpen then
            if not EUICoreStandaloneMinimap._deferredDriftChecks then
                EUICoreStandaloneMinimap._deferredDriftChecks = {}
            end
            EUICoreStandaloneMinimap._deferredDriftChecks[function()
...
--      local expanded
--      expanded, y = EUICoreStandaloneMinimap.BuildLessCommonExpander(parent, y,
--          "rfIndicators", "Show Less Common Indicator Options")
--      if expanded then
--          ... build the less-common rows ...
--      end
--      y = EUICoreStandaloneMinimap.FinishLessCommonExpander(parent, y,
--          "rfIndicators", "Show Less Common Indicator Options")
--  Expansion is session-only per sectionKey (never saved). The global "Auto Expand Less Common Settings" toggle
--  (EUICoreStandaloneMinimapDB.autoExpandLessCommon, Global Settings -> General -> Display) renders everything expanded and
...
-------------------------------------------------------------------------------
local LESS_COMMON_ARROW_DOWN = "Interface\\AddOns\\EUIStandaloneMinimap\\media\\icons\\eui-arrow-down3.png"
local LESS_COMMON_ARROW_UP   = "Interface\\AddOns\\EUIStandaloneMinimap\\media\\icons\\eui-arrow-up3.png"
...
local function BuildLessCommonLink(parent, y, sectionKey, label, expanded)
    ...
    local fs = EUICoreStandaloneMinimap.MakeFont(btn, 13, nil, 1, 1, 1)
    fs:SetPoint("LEFT", btn, "LEFT", ARROW_SZ + ARROW_GAP, 0)
    fs:SetText(EUICoreStandaloneMinimap.L(text))
    fs:SetAlpha(0.7)
    ...
    btn:SetScript("OnClick", function(_, button)
        if button == "MiddleButton" then
            ...
            EUICoreStandaloneMinimapDB.autoExpandLessCommon = true
            HideWidgetTooltip()
            EUICoreStandaloneMinimap:InvalidatePageCache()
            EUICoreStandaloneMinimap:RefreshPage(true)
            return
        end
        local sess = EUICoreStandaloneMinimap._lessCommonExpanded
        if not sess then sess = {}; EUICoreStandaloneMinimap._lessCommonExpanded = sess end
...
local function BuildLessCommonExpander(parent, y, sectionKey, label)
    ...
    if EUICoreStandaloneMinimap._prebuilding then return true, y end
    if EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.autoExpandLessCommon then return true, y end
    ...
    if EUICoreStandaloneMinimap._lessCommonSearchActive then return true, y end
    local sess = EUICoreStandaloneMinimap._lessCommonExpanded
    if not sess then sess = {}; EUICoreStandaloneMinimap._lessCommonExpanded = sess end
...
local function FinishLessCommonExpander(parent, y, sectionKey, label)
    if EUICoreStandaloneMinimap._prebuilding then return y end
    if EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.autoExpandLessCommon then return y end
    if EUICoreStandaloneMinimap._lessCommonSearchActive then return y end
    local sess = EUICoreStandaloneMinimap._lessCommonExpanded
...
local function SetLessCommonSearchActive(active)
    ...
    if (EUICoreStandaloneMinimap._lessCommonSearchActive or false) == active then return end
    EUICoreStandaloneMinimap._lessCommonSearchActive = active
    ...
    if EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.autoExpandLessCommon then return end
    EUICoreStandaloneMinimap:InvalidatePageCache()
    EUICoreStandaloneMinimap:RefreshPage(true)
end
...
EUICoreStandaloneMinimap.BuildInlineSwatches = BuildInlineSwatches
...
--        getValue=...,
--        setValue=EUICoreStandaloneMinimap.SectionToggleSetValue(function(v)
...
--      -- trigger dropdown:
--      setValue = EUICoreStandaloneMinimap.DependentSetValue(
--          function() return SVal("healAbsorbTextMode", "none") ~= "none" end,
--          function(v) SSet("healAbsorbTextMode", v); EUICoreStandaloneMinimap:RefreshPage() end),
--
...
EUICoreStandaloneMinimap.BuildSliderCore     = BuildSliderCore
EUICoreStandaloneMinimap.BuildDropdownControl = BuildDropdownControl
EUICoreStandaloneMinimap.BuildColorSwatch    = BuildColorSwatch
EUICoreStandaloneMinimap.BuildTrioColorSwatch = BuildTrioColorSwatch
EUICoreStandaloneMinimap.BuildToggleControl   = BuildToggleControl
EUICoreStandaloneMinimap.BuildInlineToggle    = BuildInlineToggle
EUICoreStandaloneMinimap.BuildCheckboxControl = BuildCheckboxControl
EUICoreStandaloneMinimap.BuildCogPopup       = BuildCogPopup
EUICoreStandaloneMinimap.BuildSyncIcon       = BuildSyncIcon
EUICoreStandaloneMinimap.BuildMultiApplyDropdown = BuildMultiApplyDropdown
EUICoreStandaloneMinimap.BuildSegmentedControl = BuildSegmentedControl
EUICoreStandaloneMinimap.BuildLessCommonExpander   = BuildLessCommonExpander
EUICoreStandaloneMinimap.FinishLessCommonExpander  = FinishLessCommonExpander
EUICoreStandaloneMinimap.SetLessCommonSearchActive = SetLessCommonSearchActive
EUICoreStandaloneMinimap.SectionToggleSetValue     = SectionToggleSetValue
EUICoreStandaloneMinimap.DependentSetValue         = DependentSetValue
...
-------------------------------------------------------------------------------
function EUICoreStandaloneMinimap.ShowPickMenu(anchor, opts)
    opts = opts or {}
    ...
    local menu = EUICoreStandaloneMinimap._pickMenu
    if not menu then
        menu = CreateFrame("Frame", nil, UIParent)
        EUICoreStandaloneMinimap._pickMenu = menu
        menu:SetFrameStrata("FULLSCREEN_DIALOG")
        menu:SetFrameLevel(220)
        menu:EnableMouse(true)
        menu:SetClampedToScreen(true)
        ...
        if EUICoreStandaloneMinimap.MakeBorder then EUICoreStandaloneMinimap.MakeBorder(menu, 1, 1, 1, 0.2) end
...
local function BuildCursorAnchorRow(opts)
    ...
    do
        ...
        suffix:SetFont(EUICoreStandaloneMinimap.EXPRESSWAY, 11, "")
        suffix:SetTextColor(1, 1, 1, 0.35)
        suffix:SetText(EUICoreStandaloneMinimap.L("(Applies on Window Close)"))
        local anchorLabel
        for i = 1, row._leftRegion:GetNumRegions() do
            local reg = select(i, row._leftRegion:GetRegions())
            if reg and reg.GetText and EUICoreStandaloneMinimap.EnKey(reg:GetText()) == "Anchor to Cursor" then
    ...
    do
        ...
        makeCogBtn(rightRgn, cogShow, nil, EUICoreStandaloneMinimap.DIRECTIONS_ICON)
    end

    return row, h
end
EUICoreStandaloneMinimap.BuildCursorAnchorRow = BuildCursorAnchorRow
...
do
    ...
    if LSM then
        ...
        if smFonts then
            ...
            EUICoreStandaloneMinimap._smFontPaths = lut
        end
    end
end
...
--  items = EUICoreStandaloneMinimap.VIS_OPT_ITEMS (or a subset)
...
function EUICoreStandaloneMinimap.ShowTrackedAurasPopup(opts)
    local PP = EUICoreStandaloneMinimap.PP or EUICoreStandaloneMinimap.PanelPP
    local fontPath = opts.fontPath or EUICoreStandaloneMinimap.EXPRESSWAY or "Fonts\\FRIZQT__.TTF"
    local ppScale = (EUICoreStandaloneMinimap.GetPopupScale and EUICoreStandaloneMinimap.GetPopupScale()) or 1
    ...
    eyebrow:SetText(EUICoreStandaloneMinimap.L(opts.eyebrow or "TRACKED AURAS"))
    local title = panel:CreateFontString(nil, "OVERLAY")
    title:SetFont(fontPath, 20, "")
    title:SetPoint("TOP", panel, "TOP", 0, -32)
    title:SetTextColor(1, 1, 1, 0.95)
    title:SetText(EUICoreStandaloneMinimap.L(opts.title or "Tracked Auras"))
    ...
    closeTex:SetTexture("Interface\\AddOns\\EUIStandaloneMinimap\\media\\icons\\eui-close.png")
    ...
    if opts.showAll then
        ...
        local bbrd = EUICoreStandaloneMinimap.MakeBorder(box, 0.4, 0.4, 0.4, 0.6, PP)
        local chk = box:CreateTexture(nil, "ARTWORK")
        PP.SetInside(chk, box, 2, 2)
        ...
        tl:SetText(EUICoreStandaloneMinimap.L(sa.label or "Show All"))
    ...
    elseif opts.copyFrom then
        ...
        lbl:SetText(EUICoreStandaloneMinimap.L(cf.label or "Copy From:"))
        ...
        local apBrd = EUICoreStandaloneMinimap.MakeBorder and EUICoreStandaloneMinimap.MakeBorder(applyBtn, EG.r, EG.g, EG.b, 0.35, PP)
        ...
        apLbl:SetText(EUICoreStandaloneMinimap.L("Apply"))
        ...
        EUICoreStandaloneMinimap.MakeBorder(ddBtn, 0.4, 0.4, 0.4, 0.6, PP)
        ...
        ddLbl:SetText(EUICoreStandaloneMinimap.L("Select..."))
        if EUICoreStandaloneMinimap.MakeDropdownArrow then EUICoreStandaloneMinimap.MakeDropdownArrow(ddBtn, 10, PP) end
        ...
        EUICoreStandaloneMinimap.MakeBorder(menu, 1, 1, 1, 0.15, PP)
        menu:Hide()
        for i = 1, #cf.choices do
            ...
            rl:SetText(EUICoreStandaloneMinimap.L(choice.label))
            ...
            mrow:SetScript("OnClick", function()
                chosen = choice.key
                ddLbl:SetText(EUICoreStandaloneMinimap.L(choice.label))
    ...
    local function MakeSpellColumn(x, titleText, promptText, listFn, otherFn)
        ...
        colTitle:SetText(EUICoreStandaloneMinimap.L(titleText))
        ...
        local abrd = EUICoreStandaloneMinimap.MakeBorder and EUICoreStandaloneMinimap.MakeBorder(addBtn, EG.r, EG.g, EG.b, 0.35, PP)
        local al = addBtn:CreateFontString(nil, "OVERLAY")
        al:SetFont(fontPath, 12, "")
        al:SetPoint("CENTER")
        al:SetTextColor(EG.r, EG.g, EG.b, 0.7)
        al:SetText(EUICoreStandaloneMinimap.L("Add Spell ID"))
        ...
        local function RefreshList()
            ...
            for i = 1, #sorted do
                ...
                if not row then
                    ...
                    row.boxBrd = EUICoreStandaloneMinimap.MakeBorder(row.box, 0.4, 0.4, 0.4, 0.6, PP)
                    ...
                    row.x.tex:SetTexture("Interface\\AddOns\\EUIStandaloneMinimap\\media\\icons\\eui-close.png")
                    row.x:SetAlpha(0.5)
                    row.x:SetScript("OnEnter", function(self)
                        self:SetAlpha(0.9)
                        EUICoreStandaloneMinimap.ShowWidgetTooltip(self, EUICoreStandaloneMinimap.L("Remove"))
                    end)
        ...
        addBtn:SetScript("OnClick", function()
            EUICoreStandaloneMinimap:ShowInputPopup({
                title = EUICoreStandaloneMinimap.L("Add Spell ID"),
                message = promptText,
                confirmText = EUICoreStandaloneMinimap.L("Add"),
    ...
    local refreshInc = MakeSpellColumn(24, "INCLUDED DEBUFFS",
        EUICoreStandaloneMinimap.L(opts.includePrompt or "Enter the spell ID to always show."),
        opts.includeGet,
        opts.excludeGet)
    local refreshEx = MakeSpellColumn(268, "EXCLUDED DEBUFFS",
        EUICoreStandaloneMinimap.L(opts.excludePrompt or "Enter the spell ID to exclude."),
        opts.excludeGet,
        opts.includeGet)
    ...
    local dbrd = EUICoreStandaloneMinimap.MakeBorder and EUICoreStandaloneMinimap.MakeBorder(doneBtn, EG.r, EG.g, EG.b, 0.9, PP)
    local dl2 = doneBtn:CreateFontString(nil, "OVERLAY")
    dl2:SetFont(fontPath, 14, "")
    dl2:SetPoint("CENTER")
    dl2:SetText(EUICoreStandaloneMinimap.L("Done"))
...
function EUICoreStandaloneMinimap.ShowSpellBlacklistPopup(opts)
    if EUICoreStandaloneMinimap._blacklistPopup then
        EUICoreStandaloneMinimap._blacklistPopup:Hide()
        EUICoreStandaloneMinimap._blacklistPopup = nil
    end
    opts = opts or {}
    local fontPath = EUICoreStandaloneMinimap.EXPRESSWAY or "Fonts\\FRIZQT__.TTF"
    local eg = EUICoreStandaloneMinimap.ELLESMERE_GREEN or { r = 0.05, g = 0.82, b = 0.62 }
    ...
    EUICoreStandaloneMinimap.SolidTex(dimmer, "BACKGROUND", 0, 0, 0, 0.25):SetAllPoints(dimmer)
    EUICoreStandaloneMinimap._blacklistPopup = dimmer
    ...
    if EUICoreStandaloneMinimap.GetPopupScale then popup:SetScale(EUICoreStandaloneMinimap.GetPopupScale()) end
    EUICoreStandaloneMinimap.SolidTex(popup, "BACKGROUND", 13 / 255, 17 / 255, 25 / 255, 0.98):SetAllPoints(popup)
    EUICoreStandaloneMinimap.MakeBorder(popup, 1, 1, 1, 0.22)

    local title = popup:CreateFontString(nil, "OVERLAY")
    title:SetFont(fontPath, 14, "")
    title:SetPoint("TOP", popup, "TOP", 0, -12)
    title:SetTextColor(1, 1, 1, 0.95)
    title:SetText(EUICoreStandaloneMinimap.L(opts.title or "Blacklist"))
    ...
    hint:SetText(EUICoreStandaloneMinimap.L("Blacklisted spells never display."))
    ...
    EUICoreStandaloneMinimap.SolidTex(addBox, "BACKGROUND", 0, 0, 0, 0.4):SetAllPoints(addBox)
    EUICoreStandaloneMinimap.MakeBorder(addBox, 1, 1, 1, 0.15)
    ...
    ph:SetText(EUICoreStandaloneMinimap.L("Spell ID..."))
    ...
    addLbl:SetText(EUICoreStandaloneMinimap.L("Add"))
    ...
    emptyLbl:SetText(EUICoreStandaloneMinimap.L("No blacklisted spells."))
...
-- re-evaluates live.
function EUICoreStandaloneMinimap.AttachEmptyFilterWarn(rgn, cbDD, warnText, hasContentFn)
    local PP = EUICoreStandaloneMinimap.PanelPP

    local bubble = CreateFrame("Frame", nil, rgn)
    bubble:SetFrameLevel(cbDD:GetFrameLevel() + 10)
    local fs = EUICoreStandaloneMinimap.MakeFont(bubble, 12, nil, 1, 0.4, 0.4)
    fs:SetPoint("CENTER")
    fs:SetText(warnText)
    ...
    Update()
    EUICoreStandaloneMinimap.RegisterWidgetRefresh(Update)
    return function()
        Update()
...
function EUICoreStandaloneMinimap.BuildVisOptsCBDropdown(parentFrame, ddW, fLevel, items, getFn, setFn, onChanged, maxVisibleItems, searchable, closeButton, onMenuClosed)
    local PP = EUICoreStandaloneMinimap.PP or EUICoreStandaloneMinimap.PanelPP
    ...
    local ddBrd = EUICoreStandaloneMinimap.MakeBorder(ddBtn, 1, 1, 1, EUICoreStandaloneMinimap.DD_BRD_A, PP)
    local ddLbl = ddBtn:CreateFontString(nil, "OVERLAY")
    -- Options panel is Expressway-locked by design (locale-aware: CJK/Cyrillic get the system glyph font). The user's global font intentionally does not restyle the settings UI.
    local fontPath = EUICoreStandaloneMinimap.EXPRESSWAY or "Fonts\\FRIZQT__.TTF"
    ddLbl:SetFont(fontPath, 13, "")
    ddLbl:SetTextColor(1, 1, 1, EUICoreStandaloneMinimap.DD_TXT_A)
    ddLbl:SetJustifyH("LEFT")
    ddLbl:SetWordWrap(false)
    ddLbl:SetMaxLines(1)
    ddLbl:SetPoint("LEFT", ddBtn, "LEFT", 12, 0)
    local arrow = EUICoreStandaloneMinimap.MakeDropdownArrow(ddBtn, 12, PP)
    ddLbl:SetPoint("RIGHT", arrow, "LEFT", -5, 0)
    ...
    local function SummaryLabel()
        ...
        for _, item in ipairs(items) do
            if not item.isHeader and not item.isTopAction then
                total = total + 1
                if getFn(item.key) then names[#names + 1] = EUICoreStandaloneMinimap.L(item.label) end
        ...
        local base
        if #names == 0 then base = EUICoreStandaloneMinimap.L("None")
        elseif #names == total then base = EUICoreStandaloneMinimap.L("All")
    ...
    local function EnsureMenu()
        ...
        local mBg = menu:CreateTexture(nil, "BACKGROUND")
        mBg:SetAllPoints()
        mBg:SetColorTexture(EUICoreStandaloneMinimap.DD_BG_R, EUICoreStandaloneMinimap.DD_BG_G, EUICoreStandaloneMinimap.DD_BG_B, EUICoreStandaloneMinimap.DD_BG_HA or 0.92)
        EUICoreStandaloneMinimap.MakeBorder(menu, 1, 1, 1, EUICoreStandaloneMinimap.DD_BRD_A, PP)
        local ppScale = EUICoreStandaloneMinimap.GetPopupScale and EUICoreStandaloneMinimap.GetPopupScale() or 1
        menu:SetScale(ppScale)
        ...
        if #topActions > 0 then
            ...
            for i = 1, #topActions do
                ...
                local lbl = row:CreateFontString(nil, "OVERLAY")
                lbl:SetFont(fontPath, 13, "")
                lbl:SetTextColor(EUICoreStandaloneMinimap.ELLESMERE_GREEN.r, EUICoreStandaloneMinimap.ELLESMERE_GREEN.g, EUICoreStandaloneMinimap.ELLESMERE_GREEN.b, 0.8)
                lbl:SetPoint("LEFT", row, "LEFT", 10, 0)
                lbl:SetPoint("RIGHT", row, "RIGHT", -10, 0)
                lbl:SetJustifyH("LEFT")
                lbl:SetWordWrap(false)
                lbl:SetMaxLines(1)
                lbl:SetText(EUICoreStandaloneMinimap.L(item.label))
                local hl = row:CreateTexture(nil, "ARTWORK")
                hl:SetAllPoints()
                hl:SetColorTexture(1, 1, 1, 0)
                ...
                row:SetScript("OnEnter", function()
                    ...
                    if TALocked() then
                        ...
                        if tip and EUICoreStandaloneMinimap.ShowWidgetTooltip then
                            EUICoreStandaloneMinimap.ShowWidgetTooltip(row, type(tip) == "function" and tip() or tip)
                        end
                        return
                    end
        ...
        for _, item in ipairs(items) do
            ...
            elseif item.isHeader then
                ...
                hdrLbl:SetText(EUICoreStandaloneMinimap.L(item.label))
                ...
                if item.rightLabel then
                    ...
                    hdrR:SetText(EUICoreStandaloneMinimap.L(item.rightLabel))
                ...
                if item.tooltip then
                    ...
                    hdr:SetScript("OnEnter", function()
                        EUICoreStandaloneMinimap.ShowWidgetTooltip(hdr, item.tooltip)
                    end)
            ...
            elseif item.isAction then
                ...
                lbl:SetWordWrap(false)
                lbl:SetMaxLines(1)
                lbl:SetText(EUICoreStandaloneMinimap.L(item.labelFn and item.labelFn() or item.label))
                local function UpdateActionLabel()
                    if item.labelFn then lbl:SetText(EUICoreStandaloneMinimap.L(item.labelFn())) end
                end
                row._updateActionLabel = UpdateActionLabel
                local hl = row:CreateTexture(nil, "ARTWORK")
                hl:SetAllPoints()
                hl:SetColorTexture(1, 1, 1, 0)
                local EG_r, EG_g, EG_b = EUICoreStandaloneMinimap.ELLESMERE_GREEN.r, EUICoreStandaloneMinimap.ELLESMERE_GREEN.g, EUICoreStandaloneMinimap.ELLESMERE_GREEN.b
            ...
            if not item.noCheck then
                ...
                boxBg:SetAllPoints()
                boxBg:SetColorTexture(0.12, 0.12, 0.14, 1)
                boxBrd = EUICoreStandaloneMinimap.MakeBorder(box, 0.4, 0.4, 0.4, 0.6, PP)
                chk = box:CreateTexture(nil, "ARTWORK")
                PP.SetInside(chk, box, 2, 2)
                chk:SetColorTexture(EUICoreStandaloneMinimap.ELLESMERE_GREEN.r, EUICoreStandaloneMinimap.ELLESMERE_GREEN.g, EUICoreStandaloneMinimap.ELLESMERE_GREEN.b, 1)
            ...
            if item.dual and not item.noCheck then
                ...
                negBrd = EUICoreStandaloneMinimap.MakeBorder(negBox, 0.4, 0.4, 0.4, 0.6, PP)
            ...
            local function UpdateCheck()
                if chk then
                    if getFn(item.key) then
                        chk:Show()
                        boxBrd:SetColor(EUICoreStandaloneMinimap.ELLESMERE_GREEN.r, EUICoreStandaloneMinimap.ELLESMERE_GREEN.g, EUICoreStandaloneMinimap.ELLESMERE_GREEN.b, 0.8)
                    else
                        chk:Hide()
                        boxBrd:SetColor(0.4, 0.4, 0.4, 0.6)
                    end
            ...
            row:SetScript("OnEnter", function()
                if row._isLocked then
                    ...
                    if lt then
                        EUICoreStandaloneMinimap.ShowWidgetTooltip(row, lt)
                    end
                    return
                end
                lbl:SetTextColor(1, 1, 1, 1)
                hl:SetColorTexture(1, 1, 1, 0.04)
                if item.tooltip then
                    EUICoreStandaloneMinimap.ShowWidgetTooltip(row, item.tooltip)
                end
            end)
            row:SetScript("OnLeave", function()
                if row._isLocked then
            ...
            if negBox then
                ...
                negBox:SetScript("OnEnter", function()
                    ...
                    lbl:SetTextColor(1, 1, 1, 1)
                    hl:SetColorTexture(1, 1, 1, 0.04)
                    EUICoreStandaloneMinimap.ShowWidgetTooltip(negBox, EUICoreStandaloneMinimap.L("Hide these instead of showing them"))
                end)
                ...
                if box then
                    box:SetScript("OnEnter", function()
                        ...
                        if type(tt) == "function" then tt = tt() end
                        if tt then EUICoreStandaloneMinimap.ShowWidgetTooltip(box, tt) end
                    end)
        ...
        if closeButton then
            ...
            closeLbl:SetText(EUICoreStandaloneMinimap.L(type(closeButton) == "string" and closeButton or "Okay"))
    ...
    local function ApplyNormal()
        ddLbl:SetTextColor(1, 1, 1, EUICoreStandaloneMinimap.DD_TXT_A)
        ddBrd:SetColor(1, 1, 1, EUICoreStandaloneMinimap.DD_BRD_A)
        ddBg:SetColorTexture(EUICoreStandaloneMinimap.DD_BG_R, EUICoreStandaloneMinimap.DD_BG_G, EUICoreStandaloneMinimap.DD_BG_B, EUICoreStandaloneMinimap.DD_BG_A)
    end
    local function ApplyHover()
        ddLbl:SetTextColor(1, 1, 1, EUICoreStandaloneMinimap.DD_TXT_HA)
        ddBrd:SetColor(1, 1, 1, EUICoreStandaloneMinimap.DD_BRD_HA)
        ddBg:SetColorTexture(EUICoreStandaloneMinimap.DD_BG_R, EUICoreStandaloneMinimap.DD_BG_G, EUICoreStandaloneMinimap.DD_BG_B, EUICoreStandaloneMinimap.DD_BG_HA)
    end
    ddBtn:SetScript("OnEnter", function()
        ApplyHover()
    end)
    ...
    local function ShowMenu()
        ...
        EUICoreStandaloneMinimap._openDropdownMenu = menu
...
--  EUICoreStandaloneMinimap_Visibility.lua. One checked item is stored and evaluated exactly like the legacy single mode;
...
EUICoreStandaloneMinimap.VIS_MODE_ITEMS = {
...
function EUICoreStandaloneMinimap.BuildVisibilityModeRow(W, parent, y, opts, rightCfg)
    ...
    local items = {}
    local listed = {}
    for _, def in ipairs(EUICoreStandaloneMinimap.VIS_MODE_ITEMS) do
        ...
        elseif not (def.key == "mouseover" and caps.noMouseover) then
            ...
            if caps.luaDragonriding and (def.key == "show_dragonriding" or def.key == "show_not_dragonriding") then
                item.lockedFn = function() return not EUICoreStandaloneMinimap._hasGlidingEvent end
    ...
    do
        ...
        if store then
            local sel, isMulti = EUICoreStandaloneMinimap.GetVisibilitySelection(store, legacyKey)
            if not isMulti then
                local cur = next(sel)
    ...
    local function GetChecked(k)
        ...
        local sel = EUICoreStandaloneMinimap.GetVisibilitySelection(store, legacyKey)
        return sel[k] == true
    end
    ...
    local function SetChecked(k, checked)
        ...
        local sel = EUICoreStandaloneMinimap.GetVisibilitySelection(store, legacyKey)
        if checked then
            if EUICoreStandaloneMinimap.VIS_COMBINABLE_KEYS[k] then
                -- Conditions and Mouseover combine with each other; only Never/Always/orphans get cleared.
                for key in pairs(sel) do
                    if not EUICoreStandaloneMinimap.VIS_COMBINABLE_KEYS[key] then sel[key] = nil end
                end
                sel[k] = true
            elseif k == "never" or k == "always" then
                ...
                sel[k] = true
                EUICoreStandaloneMinimap.SetVisibilitySelection(store, legacyKey, sel, opts.applyScalarFn)
                AfterChange(true)
                return
            else
    ...
    local function OnMenuClosed()
        if pendingRefresh then
            ...
            EUICoreStandaloneMinimap:RefreshPage(opts.refreshPageArg)
        end
    end
    ...
    if EUICoreStandaloneMinimap._prebuilding then return row, h end
    local leftRgn = row._leftRegion
    if leftRgn._control then leftRgn._control:Hide() end
    cbDD, cbDDRefresh = EUICoreStandaloneMinimap.BuildVisOptsCBDropdown(
    ...
    leftRgn._captureCfg = {
        ...
        setValue = function(v)
            ...
            if EUICoreStandaloneMinimap.VIS_CONDITION_KEYS[v] or v == "never" or v == "always" or v == "mouseover" then
                local one = {}
                one[v] = true
                EUICoreStandaloneMinimap.SetVisibilitySelection(s, legacyKey, one, opts.applyScalarFn)
            else
    ...
    if opts.disabledFn then
        ...
        EUICoreStandaloneMinimap.RegisterWidgetRefresh(ApplyChecklistDisabled)
...
-------------------------------------------------------------------------------
function EUICoreStandaloneMinimap.BuildReorderCBDropdown(parentFrame, ddW, fLevel, items, getFn, setFn, opts)
    opts = opts or {}
    local PP = EUICoreStandaloneMinimap.PP or EUICoreStandaloneMinimap.PanelPP
    local EG = EUICoreStandaloneMinimap.ELLESMERE_GREEN or { r = 0.05, g = 0.82, b = 0.62 }
    -- Options panel is Expressway-locked by design (locale-aware: CJK/Cyrillic get the system glyph font). The user's global font intentionally does not restyle the settings UI.
    ...
    local function SummaryLabel()
        ...
        if #names == 0 then return EUICoreStandaloneMinimap.L("None") end
        if #names == total then return EUICoreStandaloneMinimap.L("All") end
    ...
    local function EnsureMenu()
        ...
        if opts.hint2 then
            ...
            hint2:SetText(EUICoreStandaloneMinimap.L(opts.hint2))
        else
            hint:SetPoint("TOP", menu, "TOP", 0, -4 - (HINT_H - 10) / 2)
        end
        hint:SetText(EUICoreStandaloneMinimap.L(opts.hint or "Drag to Reorder"))
        ...
        local function RefreshMovableRows()
            for i = 1, #movableRows do
                ...
                rf._item = movable[i]
                rf._lbl:SetText(EUICoreStandaloneMinimap.L(movable[i].label))
                rf._updateCheck()
            end
        end
...
-------------------------------------------------------------------------------
function EUICoreStandaloneMinimap.BuildUnlockPlaceholder(opts)
    local parent = opts.parent
    local eg = EUICoreStandaloneMinimap.ELLESMERE_GREEN
    ...
    f._brd = EUICoreStandaloneMinimap.MakeBorder(f, ar, ag, ab, 0.6)

    -- White centered label matching unlock mode mover style
    local fontPath = (EUICoreStandaloneMinimap.GetFontPath and EUICoreStandaloneMinimap.GetFontPath("extras"))
        or "Interface\\AddOns\\EUIStandaloneMinimap\\media\\fonts\\Expressway.TTF"
    local label = f:CreateFontString(nil, "OVERLAY")
    label:SetFont(fontPath, 10, "OUTLINE, SLUG")
    label:SetText(EUICoreStandaloneMinimap.L(opts.text or "Move in Unlock Mode"))
