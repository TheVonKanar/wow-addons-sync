...
-------------------------------------------------------------------------------
--  EUICoreStandaloneMinimap Shared Ticker
--
...
--
--    * EUICoreStandaloneMinimap.Tick.Add/Remove -- the shared driver. Its frame is
--      created in this (parent) file, so ALL subscriber work is billed to
--      the parent addon. Fine for parent-owned work only.
--
--    * EUICoreStandaloneMinimap.Tick.NewDriver(frame) -- per-addon driver. The caller
...
--  Usage:
--      -- child file scope:
--      ns.Drv = EUICoreStandaloneMinimap.Tick.NewDriver(CreateFrame("Frame"))
--      -- anywhere:
...
--
--  Note: EUICoreStandaloneMinimap_Glows.lua deliberately keeps its own private driver. It
...
local Tick = {}
EUICoreStandaloneMinimap.Tick = Tick
