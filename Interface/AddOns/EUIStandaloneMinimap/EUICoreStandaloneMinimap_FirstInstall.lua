...
-------------------------------------------------------------------------------
--  EUICoreStandaloneMinimap_FirstInstall.lua
--
...
--
--  Fires at PLAYER_LOGIN, BEFORE the conflict-check popup (see EUICoreStandaloneMinimap.lua
--  which now gates its conflict check on EUICoreStandaloneMinimapDB.firstInstallPopupShown).
-------------------------------------------------------------------------------
...
-- "EUICoreStandaloneMinimap"), true in any "EUIStandalone*" host. See the loader guard below.
...
local BORDER_R     = EUICoreStandaloneMinimap.BORDER_R     or 1
local BORDER_G     = EUICoreStandaloneMinimap.BORDER_G     or 1
local BORDER_B     = EUICoreStandaloneMinimap.BORDER_B     or 1
local CB_BOX_R     = EUICoreStandaloneMinimap.CB_BOX_R     or 0.075
local CB_BOX_G     = EUICoreStandaloneMinimap.CB_BOX_G     or 0.113
local CB_BOX_B     = EUICoreStandaloneMinimap.CB_BOX_B     or 0.141
local CB_BRD_A     = EUICoreStandaloneMinimap.CB_BRD_A     or 0.25
local CB_ACT_BRD_A = EUICoreStandaloneMinimap.CB_ACT_BRD_A or 0.85
...
local GROUPS = {
    {
        header = "Core Addons",
        entries = {
            { label = "Action Bars",      addon = "EUICoreStandaloneMinimapActionBars" },
            { label = "Nameplates",       addon = "EUICoreStandaloneMinimapNameplates" },
            { label = "Unit Frames",      addon = "EUICoreStandaloneMinimapUnitFrames" },
            { label = "Cooldown Manager", addon = "EUICoreStandaloneMinimapCooldownManager" },
            { label = "Resource Bars",    addon = "EUICoreStandaloneMinimapResourceBars" },
            { label = "Raid Frames",      addon = "EUICoreStandaloneMinimapRaidFrames" },
        },
    },
    {
        header = "QoL Addons",
        entries = {
            { label = "Quality of Life",     addon = "EUICoreStandaloneMinimapQoL" },
            { label = "AuraBuff Reminders",  addon = "EUICoreStandaloneMinimapAuraBuffReminders" },
            { label = "DataBars",            addon = "EUICoreStandaloneMinimapDataBars" },
            { label = "Quickdraw",           addon = "EUICoreStandaloneMinimapQuickdraw" },
    ...
    },
    {
        ...
        entries = {
            { label = "Blizz UI Enhanced", addon = "EUICoreStandaloneMinimapBlizzardSkin" },
            { label = "Friends List",      addon = "EUICoreStandaloneMinimapFriends" },
            { label = "Damage Meters",     addon = "EUICoreStandaloneMinimapDamageMeters" },
            { label = "Chat",              addon = "EUICoreStandaloneMinimapChat" },
            { label = "Bags",              addon = "EUICoreStandaloneMinimapBags" },
            { label = "Mythic+ Tools",     addon = "EUICoreStandaloneMinimapMythicTimer" },
            { label = "Quest Tracker",     addon = "EUICoreStandaloneMinimapQuestTracker" },
        },
    },
}
...
local function ShowFirstInstallPopup()
    ...
    popup:SetScale(EUICoreStandaloneMinimap.PopupBump(1))
    ...
    if EUICoreStandaloneMinimap.ClampPopupToScreen then
        EUICoreStandaloneMinimap.ClampPopupToScreen(popup, POPUP_W, POPUP_H)
    end
    ...
    eyebrow:SetText(EUICoreStandaloneMinimap.L("FIRST TIME SETUP"))
    ...
    title:SetText(EUICoreStandaloneMinimap.L("Welcome to EUICoreStandaloneMinimap"))
    ...
    sub:SetText(EUICoreStandaloneMinimap.L("Choose which addons you want enabled. You can change any of these later in the EUICoreStandaloneMinimap settings panel."))
    ...
    checkAllLbl:SetText(EUICoreStandaloneMinimap.L("Check All"))
    ...
    uncheckAllLbl:SetText(EUICoreStandaloneMinimap.L("Uncheck All"))
    ...
    for colIdx, group in ipairs(GROUPS) do
        ...
        hdr:SetText(EUICoreStandaloneMinimap.L(group.header))
    ...
    local function RefreshButtonLabel()
        ...
        doneLbl:SetText(EUICoreStandaloneMinimap.L("Reload UI"))
    end
    RefreshButtonLabel()
    ...
    local function Close(triggerReload)
        ...
        EUICoreStandaloneMinimapDB.firstInstallPopupShown = true
        EUICoreStandaloneMinimapDB.bagsUserChosen = true
        EUICoreStandaloneMinimap._firstInstallPending = nil
        ...
        local function ApplyCursorCircle(enabled)
            if not EUICoreStandaloneMinimapDB then EUICoreStandaloneMinimapDB = {} end
            if not EUICoreStandaloneMinimapDB.profiles then EUICoreStandaloneMinimapDB.profiles = {} end
            local profileName = EUICoreStandaloneMinimapDB.activeProfile or "Default"
            if type(EUICoreStandaloneMinimapDB.profiles[profileName]) ~= "table" then
                EUICoreStandaloneMinimapDB.profiles[profileName] = {}
            end
            local prof = EUICoreStandaloneMinimapDB.profiles[profileName]
            if not prof.addons then prof.addons = {} end
            if not prof.addons.EUICoreStandaloneMinimapQoL then prof.addons.EUICoreStandaloneMinimapQoL = {} end
            if not prof.addons.EUICoreStandaloneMinimapQoL.cursor then prof.addons.EUICoreStandaloneMinimapQoL.cursor = {} end
            prof.addons.EUICoreStandaloneMinimapQoL.cursor.enabled = enabled and true or false
        end
        ...
        if EUICoreStandaloneMinimap._RunConflictCheck then
            C_Timer.After(0.5, EUICoreStandaloneMinimap._RunConflictCheck)
        end
    end
...
EUICoreStandaloneMinimap.ShowFirstInstallPopup = ShowFirstInstallPopup

-------------------------------------------------------------------------------
--  Trigger on first install only
-------------------------------------------------------------------------------
-- First-install detection captured at parent ADDON_LOADED. At that moment EUICoreStandaloneMinimapDB
...
-- addon-name mismatch (the renamed "EUICoreStandaloneMinimap" ADDON_LOADED gate would point at the
...
local function ComputeShowOnLogin()
    ...
    if EUICoreStandaloneMinimapDB.firstInstallPopupShown then
        return false
    end
...
loader:SetScript("OnEvent", function(self, event, addonName)
    if event == "ADDON_LOADED" then
        ...
        if _showPopupOnLogin then
            ...
            EUICoreStandaloneMinimap._firstInstallPending = true
        end
    elseif event == "PLAYER_LOGIN" then
        self:UnregisterEvent("PLAYER_LOGIN")
        if not _showPopupOnLogin then return end
        C_Timer.After(0.5, function()
            if EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.firstInstallPopupShown then return end
            ...
            if EUICoreStandaloneMinimap._externalInstaller then return end
