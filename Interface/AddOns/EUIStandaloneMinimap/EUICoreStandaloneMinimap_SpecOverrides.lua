...
-------------------------------------------------------------------------------
--  EUICoreStandaloneMinimap_SpecOverrides.lua
--
...
local FOLDER_BLACKLIST = {
    ...
    -- Skyriding HUD's sub-DB (EUICoreStandaloneMinimapDragonRidingDB) registers from inside
    ...
    EUICoreStandaloneMinimapDragonRiding      = true,
    ...
    -- at EUICoreStandaloneMinimapDB.spellAssignments and is not capturable).
}

-- folder -> global apply-function names (mirrors EUICoreStandaloneMinimap.RefreshAllAddons).
...
local REFRESH_FNS = {
    EUICoreStandaloneMinimapCooldownManager   = { "_ECME_Apply" },
    EUIStandaloneMinimap           = { "_EMM_FullRebuildMinimap" },
    EUICoreStandaloneMinimapResourceBars      = { "_ERB_Apply" },
    EUICoreStandaloneMinimapActionBars        = { "_EAB_Apply" },
    EUICoreStandaloneMinimapUnitFrames        = { "_EUF_ReloadFrames", "_EUF_RefreshUnitNames" },
    EUICoreStandaloneMinimapRaidFrames        = { "_ERF_RefreshAll" },
    EUICoreStandaloneMinimapNameplates        = { "_ENP_RefreshAllSettings" },
    EUICoreStandaloneMinimapQuestTracker      = { "_EQT_RefreshAll" },
    EUICoreStandaloneMinimapChat              = { "_ECHAT_RefreshAll" },
    EUICoreStandaloneMinimapFriends           = { "_EFR_ApplyFriends" },
    EUICoreStandaloneMinimapMythicTimer       = { "_EMT_Apply" },
    EUICoreStandaloneMinimapDamageMeters      = { "_EDM_Apply" },
    EUICoreStandaloneMinimapDataBars          = { "_EDB_Apply" },
    EUICoreStandaloneMinimapQuickdraw         = { "_EQD_Apply" },
    EUICoreStandaloneMinimapAuraBuffReminders = { "_EABR_RequestRefresh", "_EABR_ApplyUnlockPos" },
    ...
    EUICoreStandaloneMinimapDragonRiding      = { "_EDR_Rebuild" },
}

-- Class glyph sprite (toolbar button) + modern class sprite (group icons)
local GLYPH_SPRITE  = "Interface\\AddOns\\EUIStandaloneMinimap\\media\\icons\\class-full\\glyph.tga"
local MODERN_SPRITE = "Interface\\AddOns\\EUIStandaloneMinimap\\media\\icons\\class-full\\modern.tga"
-- Generic multi-spec group icon (standalone image, not a sprite)
local MULTISPEC_ICON = "Interface\\AddOns\\EUIStandaloneMinimap\\media\\icons\\class-full\\multispec.png"
local CLASS_COORDS = EUICoreStandaloneMinimap.CLASS_ICON_SPRITE_COORDS
...
local ROLE_MEDIA = "Interface\\AddOns\\EUICoreStandaloneMinimapRaidFrames\\Media\\"
local ROLE_ICONS = {
...
local L = function(s) return EUICoreStandaloneMinimap.L and EUICoreStandaloneMinimap.L(s) or s end
...
local function GetProfileRoot()
    return EUICoreStandaloneMinimap.GetActiveProfileData and EUICoreStandaloneMinimap.GetActiveProfileData()
end
...
local function DBFor(folder)
    local reg = EUICoreStandaloneMinimap.Lite and EUICoreStandaloneMinimap.Lite._dbRegistry
...
local MATCH_OWNED_FKEYS = {
    ["EUICoreStandaloneMinimapResourceBars\31health\30width"]        = { elem = "ERB_Health",        dim = "w" },
    ["EUICoreStandaloneMinimapResourceBars\31health\30height"]       = { elem = "ERB_Health",        dim = "h" },
    ["EUICoreStandaloneMinimapResourceBars\31primary\30width"]       = { elem = "ERB_Power",         dim = "w" },
    ["EUICoreStandaloneMinimapResourceBars\31primary\30height"]      = { elem = "ERB_Power",         dim = "h" },
    ["EUICoreStandaloneMinimapResourceBars\31secondary\30pipWidth"]  = { elem = "ERB_ClassResource", dim = "w" },
    ["EUICoreStandaloneMinimapResourceBars\31secondary\30pipHeight"] = { elem = "ERB_ClassResource", dim = "h" },
    ["EUICoreStandaloneMinimapResourceBars\31castBar\30width"]       = { elem = "ERB_CastBar",       dim = "w" },
    ["EUICoreStandaloneMinimapResourceBars\31castBar\30height"]      = { elem = "ERB_CastBar",       dim = "h" },
    ["EUICoreStandaloneMinimapResourceBars\31gcdBar\30width"]        = { elem = "ERB_GCDBar",        dim = "both" },
    ["EUICoreStandaloneMinimapResourceBars\31gcdBar\30height"]       = { elem = "ERB_GCDBar",        dim = "both" },
}
...
local function MatchOwnedFKey(fkey)
    ...
    if EUICoreStandaloneMinimap.SpecOverrides_UnlockBaselineLinks then
        local _, bw, bh = EUICoreStandaloneMinimap.SpecOverrides_UnlockBaselineLinks()
...
local CDM_BARS_PREFIX = "EUICoreStandaloneMinimapCooldownManager\31cdmBars\30bars\30"
local RAID_SIZE_OV_PREFIX = "EUICoreStandaloneMinimapRaidFrames\31raidSizeOverrides\30"
...
function EUICoreStandaloneMinimap.SpecOverrides_OnCDMBarsRestructured()
    ...
    if EUICoreStandaloneMinimap._CondOv then
        sweep(EUICoreStandaloneMinimap._CondOv.GetStore(), EUICoreStandaloneMinimap._CondOv.RebuildIndex)
    end
    RequestGoldWalk()
end
...
local function RunRefreshers(folders)
    ...
    if fallback and EUICoreStandaloneMinimap.RefreshAllAddons then
...
local function ApplyValuesFor(specID)
    ...
    if EUICoreStandaloneMinimap._CondOv and EUICoreStandaloneMinimap._CondOv._edit then
        ...
        local t2 = EUICoreStandaloneMinimap._CondOv.WriteValues(EUICoreStandaloneMinimap._CondOv._edit.id, true)
        if t2 then
            touched = touched or {}
...
--- Values-only apply, called at the TOP of EUICoreStandaloneMinimap.RefreshAllAddons so
...
function EUICoreStandaloneMinimap.SpecOverrides_ApplyValues(specID)
    ...
    if EUICoreStandaloneMinimap.SpecOverrides_ApplyUnlock then
        EUICoreStandaloneMinimap.SpecOverrides_ApplyUnlock(specID or _activeSpec or CurrentSpecID())
    end
    if EUICoreStandaloneMinimap.SpecOverrides_ApplyBm then
        EUICoreStandaloneMinimap.SpecOverrides_ApplyBm(specID or _activeSpec or CurrentSpecID())
    end
    if EUICoreStandaloneMinimap.SpecOverrides_ApplyDm then
        EUICoreStandaloneMinimap.SpecOverrides_ApplyDm(specID or _activeSpec or CurrentSpecID())
    end
    ...
    if EUICoreStandaloneMinimap._CondOv then EUICoreStandaloneMinimap._CondOv.ApplyValues() end
    ...
    if not EUICoreStandaloneMinimap._importGuardArmedNow then
        local prof = GetProfileRoot()
        if prof and prof._importEstablishPending then
            ...
            if sid then
                ...
                EUICoreStandaloneMinimap._unlockResetConvergePending = nil
                if EUICoreStandaloneMinimap.SpecOverrides_ApplyUnlock then
                    pcall(EUICoreStandaloneMinimap.SpecOverrides_ApplyUnlock, sid, true)
                end
                if EUICoreStandaloneMinimap.SpecOverrides_ApplyBm then
                    pcall(EUICoreStandaloneMinimap.SpecOverrides_ApplyBm, sid, true)
                end
                if EUICoreStandaloneMinimap.SpecOverrides_ApplyDm then
                    pcall(EUICoreStandaloneMinimap.SpecOverrides_ApplyDm, sid, true)
                end
            end
        end
    end
    ...
    if EUICoreStandaloneMinimap._unlockResetConvergePending then
        local sid = specID or _activeSpec or CurrentSpecID()
        if sid then
            EUICoreStandaloneMinimap._unlockResetConvergePending = nil
            if EUICoreStandaloneMinimap.SpecOverrides_ApplyUnlock then
...
function EUICoreStandaloneMinimap.SpecOverrides_Apply(specID, deferLogin)
    if deferLogin then
        C_Timer.After(0, function()
            C_Timer.After(0, function() EUICoreStandaloneMinimap.SpecOverrides_Apply(specID) end)
        end)
        return
    end
    ...
    if EUICoreStandaloneMinimap.SpecOverrides_ApplyUnlock then
        EUICoreStandaloneMinimap.SpecOverrides_ApplyUnlock(specID)
    end
    if EUICoreStandaloneMinimap.SpecOverrides_ApplyBm then
        EUICoreStandaloneMinimap.SpecOverrides_ApplyBm(specID)
    end
    if EUICoreStandaloneMinimap.SpecOverrides_ApplyDm then
        EUICoreStandaloneMinimap.SpecOverrides_ApplyDm(specID)
    end
...
function EUICoreStandaloneMinimap.SpecOverrides_OnSpecChanged(oldSpecID, newSpecID)
    ...
    if EUICoreStandaloneMinimap.SpecOverrides_HarvestUnlockLayout then
        EUICoreStandaloneMinimap.SpecOverrides_HarvestUnlockLayout()
    end
    if EUICoreStandaloneMinimap.SpecOverrides_HarvestBmLayout then
        EUICoreStandaloneMinimap.SpecOverrides_HarvestBmLayout()
    end
    if EUICoreStandaloneMinimap.SpecOverrides_HarvestDmLayout then
        EUICoreStandaloneMinimap.SpecOverrides_HarvestDmLayout()
    end
    -- An editing-as-conditional session banks and ends here; the transition
    -- re-establishes canonical live itself (noRestore).
    if EUICoreStandaloneMinimap._CondOv and EUICoreStandaloneMinimap._CondOv.ExitEdit then
        EUICoreStandaloneMinimap._CondOv.ExitEdit(true)
    end
    if _editGroup then
...
function EUICoreStandaloneMinimap.SpecOverrides_HarvestCurrent()
    ...
    if EUICoreStandaloneMinimap._CondOv and EUICoreStandaloneMinimap._CondOv._edit then
        EUICoreStandaloneMinimap._CondOv.ExitEdit(nil, true)
    ...
    local function BankCond()
        if EUICoreStandaloneMinimap._CondOv then
            EUICoreStandaloneMinimap._CondOv.Harvest(EUICoreStandaloneMinimap.Conditions_AppliedGid
                and EUICoreStandaloneMinimap.Conditions_AppliedGid() or nil)
        end
    end
    if _editGroup then
        local g = _editGroup
...
--  EUICoreStandaloneMinimapDB tables the Lite registry can't address, and unlock values bank
...
EUICoreStandaloneMinimap.SPECOV_NIL = NIL_SENT
EUICoreStandaloneMinimap._SPECOV_GOLD = { GOLD_R, GOLD_G, GOLD_B }
...
function EUICoreStandaloneMinimap.SpecOverrides_CurrentSpec()
    return CurrentSpecID()
end

function EUICoreStandaloneMinimap.SpecOverrides_GroupById(gid)
...
function EUICoreStandaloneMinimap.SpecOverrides_ViewActive()
    if _editGroup then return false end
    local C = EUICoreStandaloneMinimap._CondOv
...
function EUICoreStandaloneMinimap.SpecOverrides_EditSessionUnlockLayer()
...
function EUICoreStandaloneMinimap.SpecOverrides_EditSessionName()
    if _editGroup then return _editGroup.name or _editGroup.label end
    local C = EUICoreStandaloneMinimap._CondOv
    local e = C and C._edit
    if e and e.id and EUICoreStandaloneMinimap.Conditions_GroupById then
        local cg = EUICoreStandaloneMinimap.Conditions_GroupById(e.id)
...
function EUICoreStandaloneMinimap.SpecOverrides_OwningGroupFor(specID)
...
--- returned map (decode against EUICoreStandaloneMinimap.SPECOV_NIL); like the writers it is
...
function EUICoreStandaloneMinimap.SpecOverrides_PeekEffectiveValues(folder)
                   ...
                   and SplitFKey(fkey) == folder then
                    ...
                    if v == NIL_SENT or type(v) ~= "table" then
                        ...
                        if not specSrc and m ~= entry.values.default
                           and m[fkey] ~= nil then
                            local g = EUICoreStandaloneMinimap.SpecOverrides_OwningGroupFor(specID)
    ...
    if C and C.GetStore then
        ...
        if cstore and #cstore > 0 then
            local gid = EUICoreStandaloneMinimap.Conditions_AppliedGid
                and EUICoreStandaloneMinimap.Conditions_AppliedGid() or nil
                       ...
                       and not EntryOwning(fkey) and SplitFKey(fkey) == folder then
                        ...
                        if v == NIL_SENT or type(v) ~= "table" then
                            ...
                            if fromCond and not condSrc then
                                local cg = EUICoreStandaloneMinimap.Conditions_GroupById
                                    and EUICoreStandaloneMinimap.Conditions_GroupById(gid)
...
function EUICoreStandaloneMinimap.SpecOverrides_UnlockActive()
    local s = GetUnlockStore()
...
function EUICoreStandaloneMinimap.SpecOverrides_EffectiveUnlockLayer()
    local s = GetUnlockStore()
...
local function LiteProfile(folder)
    local a = EUICoreStandaloneMinimap.Lite and EUICoreStandaloneMinimap.Lite.GetAddon(folder, true)
...
local function LayerSkipsKey(key)
    ...
    local abk = EUICoreStandaloneMinimap._abBarKeys
...
local function HarvestLayer()
    local layer = {
        anchors     = DeepCopy(EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.unlockAnchors or {}),
        widthMatch  = DeepCopy(EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.unlockWidthMatch or {}),
        heightMatch = DeepCopy(EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.unlockHeightMatch or {}),
    ...
    local cdm = LiteProfile("EUICoreStandaloneMinimapCooldownManager")
    if cdm then
    ...
    local ab = LiteProfile("EUICoreStandaloneMinimapActionBars")
    if ab then
...
local function UnlockElemAnchorOwned(key)
    local a = EUICoreStandaloneMinimapDB and EUICoreStandaloneMinimapDB.unlockAnchors
...
local function ApplyLayer(layer, baseline)
    ...
    if EUICoreStandaloneMinimapDB then
        ...
        if not anchors then anchors = {}; EUICoreStandaloneMinimapDB.unlockAnchors = anchors end
        ...
        local wm = EUICoreStandaloneMinimapDB.unlockWidthMatch
        if not wm then wm = {}; EUICoreStandaloneMinimapDB.unlockWidthMatch = wm end
        local wmKept
        ...
        local hm = EUICoreStandaloneMinimapDB.unlockHeightMatch
        if not hm then hm = {}; EUICoreStandaloneMinimapDB.unlockHeightMatch = hm end
        local hmKept
...
function EUICoreStandaloneMinimap.SpecOverrides_ApplyUnlock(specID, force)
    local s = GetUnlockStore()
    ...
    if want then
        ...
        local cond = EUICoreStandaloneMinimap._CondOv and EUICoreStandaloneMinimap._CondOv.ResolveGid
            and EUICoreStandaloneMinimap._CondOv.ResolveGid() or nil
        if cond then
            local cs = EUICoreStandaloneMinimap._CondOv.GetUnlockStore()
...
function EUICoreStandaloneMinimap.SpecOverrides_HarvestUnlockLayout(userCommit)
    local s = GetUnlockStore()
    if not s then return end
    -- Zero-cost for non-users: nothing to bank into until a layer exists.
    local condStore = EUICoreStandaloneMinimap._CondOv and EUICoreStandaloneMinimap._CondOv.GetUnlockStore()
    ...
    if EUICoreStandaloneMinimap.Conditions_EstablishPending and EUICoreStandaloneMinimap.Conditions_EstablishPending() then
        return
    end
    ...
    if _defaultView or _editGroup
       or (EUICoreStandaloneMinimap._CondOv and EUICoreStandaloneMinimap._CondOv._edit) then return end
...
function EUICoreStandaloneMinimap.SpecOverrides_UnlockBaselineLinks()
    local s = GetUnlockStore()
...
function EUICoreStandaloneMinimap.SpecOverrides_UnlockResetActive(profRoot)
    local s = profRoot and profRoot.specUnlockOverrides
    if not s then return end
    if s.active ~= nil then
        EUICoreStandaloneMinimap._unlockResetConvergePending = true
    end
    s.active = nil
end
...
function EUICoreStandaloneMinimap.SpecOverrides_FlushUnlock()
    ...
    if InCombatLockdown() then
        ...
        if not _unlockFlushCombatWatch then
            ...
            _unlockFlushCombatWatch:SetScript("OnEvent", function(self)
                ...
                if _unlockFlushScheduled then EUICoreStandaloneMinimap.SpecOverrides_FlushUnlock() end
            end)
        end
    ...
    if pend then
        ...
        EUICoreStandaloneMinimap._unlockLayerApplying = true
        local elems = EUICoreStandaloneMinimap._unlockRegisteredElements
        ...
        EUICoreStandaloneMinimap._unlockLayerApplying = nil
    end
    ...
    if wroteLayerSizes and not _inTransition then
        ...
        if EUICoreStandaloneMinimap._CondOv then
            local t2 = EUICoreStandaloneMinimap._CondOv.ApplyValues()
            if t2 then
                touched = touched or {}
                for k in pairs(t2) do touched[k] = true end
            end
    ...
    if EUICoreStandaloneMinimap._unlockModeActive then return end
    _unlockSettleWanted = false
    if EUICoreStandaloneMinimap.ApplyAllWidthHeightMatches then pcall(EUICoreStandaloneMinimap.ApplyAllWidthHeightMatches) end
    if EUICoreStandaloneMinimap._applySavedPositions then pcall(EUICoreStandaloneMinimap._applySavedPositions) end
    if EUICoreStandaloneMinimap.ResyncAnchorOffsets then pcall(EUICoreStandaloneMinimap.ResyncAnchorOffsets) end
...
function EUICoreStandaloneMinimap.SpecOverrides_UnlockPokeFlush()
...
function EUICoreStandaloneMinimap.SpecOverrides_UnlockHasLayout(groupId)
    local s = GetUnlockStore()
...
function EUICoreStandaloneMinimap.SpecOverrides_RemoveUnlockLayout(groupId)
    local s = GetUnlockStore()
    ...
    if s.active == groupId then
        ...
        if s.baselineLayout then
            ...
            _unlockSettleWanted = true
            EUICoreStandaloneMinimap.SpecOverrides_FlushUnlock()
        end
    end
    return true
end

-- ---- conditional unlock layouts (engine in EUICoreStandaloneMinimap_Conditions.lua) -------
...
function EUICoreStandaloneMinimap.Conditions_RemoveUnlockLayout(condGid)
    local cs = EUICoreStandaloneMinimap._CondOv and EUICoreStandaloneMinimap._CondOv.GetUnlockStore()
...
local function BmHarvestLayer()
    local rf = LiteProfile("EUICoreStandaloneMinimapRaidFrames")
...
function EUICoreStandaloneMinimap.SpecOverrides_HarvestBmLayout()
    local s = GetBmStore()
    if not s then return end
    ...
    -- the runtime pointer.
    local sessGid = EUICoreStandaloneMinimap._bmSessionGid
    if sessGid then
...
--- NEVER harvests here.
function EUICoreStandaloneMinimap.SpecOverrides_ApplyBm(specID, force, noPageRefresh)
    local s = GetBmStore()
    if not s then return end
...
local function BmSessionEngage(gid)
    if EUICoreStandaloneMinimap._bmSessionGid == gid then return end
    local cs = GetCondBmStore()
    ...
    if BmApplyLayer(layer, true) then   -- callers run during a page build
        EUICoreStandaloneMinimap._bmSessionGid = gid
    end
end
...
function EUICoreStandaloneMinimap.SpecOverrides_BmSessionRelease(noApply)
    local gid = EUICoreStandaloneMinimap._bmSessionGid
    ...
    EUICoreStandaloneMinimap._bmSessionGid = nil
    if not noApply then
        EUICoreStandaloneMinimap.SpecOverrides_ApplyBm(_activeSpec or CurrentSpecID(), true, true)
    end
end
...
function EUICoreStandaloneMinimap.SpecOverrides_BmActiveInfo()
    local gid = EUICoreStandaloneMinimap._bmSessionGid
    ...
    local g
    if isCond then
        g = EUICoreStandaloneMinimap.Conditions_GroupById and EUICoreStandaloneMinimap.Conditions_GroupById(gid)
    else
        g = GroupById(gid)
    end
...
function EUICoreStandaloneMinimap.SpecOverrides_BmPageLockInfo()
    local mod = EUICoreStandaloneMinimap.GetActiveModule and EUICoreStandaloneMinimap:GetActiveModule()
    if mod ~= "EUICoreStandaloneMinimapRaidFrames" then return nil end
    local page = EUICoreStandaloneMinimap.GetActivePage and EUICoreStandaloneMinimap:GetActivePage()
    if page ~= "Buff Manager" then return nil end
    if EUICoreStandaloneMinimap._bmSessionGid then return "cond", EUICoreStandaloneMinimap._bmSessionGid end
    local s = GetBmStore()
    local a = s and s.active
...
function EUICoreStandaloneMinimap.SpecOverrides_BmPageLocked()
    return EUICoreStandaloneMinimap.SpecOverrides_BmPageLockInfo() ~= nil
end

--- True when the group has a custom Buff Manager.
function EUICoreStandaloneMinimap.SpecOverrides_BmHasLayout(groupId)
    local s = GetBmStore()
    return (s and s.layouts[groupId] ~= nil) and true or false
end
...
function EUICoreStandaloneMinimap.SpecOverrides_RemoveBmLayout(groupId)
    local s = GetBmStore()
    if not s or s.layouts[groupId] == nil then return false end
    s.layouts[groupId] = nil
    if s.active == groupId then
...
function EUICoreStandaloneMinimap.Conditions_RemoveBmLayout(condGid)
    local cs = GetCondBmStore()
    if not cs or cs.layouts[condGid] == nil then return false end
    cs.layouts[condGid] = nil
    local s = GetBmStore()
    ...
    if EUICoreStandaloneMinimap._bmSessionGid == condGid then
        EUICoreStandaloneMinimap._bmSessionGid = nil
        EUICoreStandaloneMinimap.SpecOverrides_ApplyBm(_activeSpec or CurrentSpecID(), true)
    end
    return true
end
...
function EUICoreStandaloneMinimap.SpecOverrides_BmResetActive(profRoot)
...
function EUICoreStandaloneMinimap.SpecOverrides_HarvestDmLayout()
    local s = GetDmStore()
    if not s then return end
    ...
    if not snap then return end
    local sessGid = EUICoreStandaloneMinimap._dmSessionGid
    if sessGid then
        if cs and cs.layouts[sessGid] then
            cs.layouts[sessGid] = snap
        end
...
function EUICoreStandaloneMinimap.SpecOverrides_ApplyDm(specID, force, noPageRefresh)
    local s = GetDmStore()
    if not s then return end
...
local function DmSessionEngage(gid)
    if EUICoreStandaloneMinimap._dmSessionGid == gid then return end
    local cs = GetCondDmStore()
    local layer = cs and cs.layouts[gid]
    if not layer then return end
    if DmApplyLayer(layer, true) then   -- flag only on a landed paint
        EUICoreStandaloneMinimap._dmSessionGid = gid
    end
end

--- Banks the session's live edits into the fork and restores the runtime
--- layer. noApply: a transition is about to apply the runtime layer itself.
function EUICoreStandaloneMinimap.SpecOverrides_DmSessionRelease(noApply)
    local gid = EUICoreStandaloneMinimap._dmSessionGid
    if not gid then return end
    local cs = GetCondDmStore()
    if cs and cs.layouts[gid] then
        local snap = DmHarvestLayer()
    ...
    EUICoreStandaloneMinimap._dmSessionGid = nil
    if not noApply then
        EUICoreStandaloneMinimap.SpecOverrides_ApplyDm(_activeSpec or CurrentSpecID(), true, true)
    end
end

--- Name of the override group whose Debuff Manager fork is LIVE, or nil when
--- live is the baseline.
function EUICoreStandaloneMinimap.SpecOverrides_DmActiveInfo()
    local gid = EUICoreStandaloneMinimap._dmSessionGid
    local isCond = gid ~= nil
    if not gid then
        local s = GetDmStore()
...
function EUICoreStandaloneMinimap.SpecOverrides_DmPageLockInfo()
    local mod = EUICoreStandaloneMinimap.GetActiveModule and EUICoreStandaloneMinimap:GetActiveModule()
    if mod ~= "EUICoreStandaloneMinimapRaidFrames" then return nil end
    local page = EUICoreStandaloneMinimap.GetActivePage and EUICoreStandaloneMinimap:GetActivePage()
    if page ~= "Debuff Manager" then return nil end
    if EUICoreStandaloneMinimap._dmSessionGid then return "cond", EUICoreStandaloneMinimap._dmSessionGid end
    local s = GetDmStore()
    local a = s and s.active
    if type(a) == "number" then return "spec", a end
    if type(a) == "string" then
...
function EUICoreStandaloneMinimap.SpecOverrides_DmPageLocked()
    return EUICoreStandaloneMinimap.SpecOverrides_DmPageLockInfo() ~= nil
end

--- True when the group has a custom Debuff Manager.
function EUICoreStandaloneMinimap.SpecOverrides_DmHasLayout(groupId)
    local s = GetDmStore()
    return (s and s.layouts[groupId] ~= nil) and true or false
end
...
--- baseline is applied back to live.
function EUICoreStandaloneMinimap.SpecOverrides_RemoveDmLayout(groupId)
    local s = GetDmStore()
    if not s or s.layouts[groupId] == nil then return false end
    s.layouts[groupId] = nil
    if s.active == groupId then
...
function EUICoreStandaloneMinimap.Conditions_RemoveDmLayout(condGid)
    local cs = GetCondDmStore()
    if not cs or cs.layouts[condGid] == nil then return false end
    cs.layouts[condGid] = nil
    local s = GetDmStore()
    ...
    if EUICoreStandaloneMinimap._dmSessionGid == condGid then
        EUICoreStandaloneMinimap._dmSessionGid = nil
        EUICoreStandaloneMinimap.SpecOverrides_ApplyDm(_activeSpec or CurrentSpecID(), true)
    end
    return true
end
...
function EUICoreStandaloneMinimap.SpecOverrides_DmResetActive(profRoot)
...
function EUICoreStandaloneMinimap.Conditions_EnterUnlockForGroup(g)
    if type(g) == "number" then g = EUICoreStandaloneMinimap.Conditions_GroupById(g) end
    ...
    local activeG = EUICoreStandaloneMinimap.Conditions_ActiveGroup and EUICoreStandaloneMinimap.Conditions_ActiveGroup()
    ...
    local cs = EUICoreStandaloneMinimap._CondOv.GetUnlockStore(true)
    ...
    if cs.layouts[g.id] == nil then
        EUICoreStandaloneMinimap:ShowConfirmPopup({
            ...
            onConfirm = function()
                ...
                s.active = "cond:" .. g.id
                EUICoreStandaloneMinimap.Conditions_EnterUnlockForGroup(g)
            end,
        })
        return
    end
    ...
    C_Timer.After(0, function()
        if EUICoreStandaloneMinimap._openUnlockMode then EUICoreStandaloneMinimap._openUnlockMode() end
    end)
end
...
function EUICoreStandaloneMinimap.SpecOverrides_EnterUnlockForGroup(g)
    ...
    if s.layouts[g.id] == nil then
        EUICoreStandaloneMinimap:ShowConfirmPopup({
            ...
            onConfirm = function()
                ...
                if RefreshCardsPopup then RefreshCardsPopup() end
                EUICoreStandaloneMinimap.SpecOverrides_EnterUnlockForGroup(g)
            end,
        })
        return
    end
...
-------------------------------------------------------------------------------
--  CONDITIONAL OVERRIDES integration (engine in EUICoreStandaloneMinimap_Conditions.lua). Same
...
local EXCLUDED_CONTEXTS = {
    ...
    ["EUICoreStandaloneMinimapFriends"]           = true,
    ["EUICoreStandaloneMinimapBags"]              = true,
    ["EUICoreStandaloneMinimapQoL"]               = true,   -- whole module
    ["EUICoreStandaloneMinimapAuraBuffReminders"] = true,
...
local Cond = {}
EUICoreStandaloneMinimap._CondOv = Cond
...
Cond.ICON_DIR = "Interface\\AddOns\\EUIStandaloneMinimap\\media\\icons\\overrides\\"
Cond.ICONS = {
...
function EUICoreStandaloneMinimap.SpecOverrides_EditSessionActive()
...
function EUICoreStandaloneMinimap.SpecOverrides_BmOverlayState()
    local g, kind
    if _editGroup then
    ...
    if forked and eligible then
        ...
        local liveNow = EUICoreStandaloneMinimap._bmSessionGid
            and ("cond:" .. EUICoreStandaloneMinimap._bmSessionGid) or (s and s.active)
...
function EUICoreStandaloneMinimap.SpecOverrides_BmSeedSources(kind, gid)
    local values, order = {}, {}
    ...
    if s and s.layouts then
        for _, g in ipairs(GetGroups() or {}) do
            if s.layouts[g.id] ~= nil and not (kind == "spec" and g.id == gid) then
                Add("spec:" .. g.id, EUICoreStandaloneMinimap.Lf("Copy from %1$s Buff Manager", g.name or "?"))
            end
        end
    end
    local cs = GetCondBmStore()
    if cs and cs.layouts and EUICoreStandaloneMinimap.Conditions_GetGroups then
        for _, g in ipairs(EUICoreStandaloneMinimap.Conditions_GetGroups() or {}) do
            if cs.layouts[g.id] ~= nil and not (kind == "cond" and g.id == gid) then
                Add("cond:" .. g.id, EUICoreStandaloneMinimap.Lf("Copy from %1$s Buff Manager", g.name or "?"))
            end
        end
    end
...
function EUICoreStandaloneMinimap.SpecOverrides_BmPagePrelude()
    ...
    if not _editGroup and not Cond._edit then
        EUICoreStandaloneMinimap.SpecOverrides_ApplyBm(CurrentSpecID(), false, true)
    end
    ...
    if Cond._edit then
        ...
        if cs and cs.layouts[Cond._edit.id]
           and EUICoreStandaloneMinimap._bmSessionGid ~= Cond._edit.id
    ...
    local state, needHeal = EUICoreStandaloneMinimap.SpecOverrides_BmOverlayState()
    if needHeal then
        EUICoreStandaloneMinimap.SpecOverrides_ApplyBm(CurrentSpecID(), true, true)
    end
    ...
    if EUICoreStandaloneMinimap.SpecOverrides_UpdateBmPassiveChrome then
        EUICoreStandaloneMinimap.SpecOverrides_UpdateBmPassiveChrome()
    end
    return state
end
...
function EUICoreStandaloneMinimap.SpecOverrides_ActivateBm(kind, gid, source)
    local state = EUICoreStandaloneMinimap.SpecOverrides_BmOverlayState()
    ...
    if kind == "spec" then
        ...
        cs.layouts[gid] = snap
        local ag = EUICoreStandaloneMinimap.Conditions_ActiveGroup and EUICoreStandaloneMinimap.Conditions_ActiveGroup()
    ...
    if EUICoreStandaloneMinimap.RefreshPage then EUICoreStandaloneMinimap:RefreshPage(true) end
end

--- Overlay policy for the Debuff Manager page (mirror of BmOverlayState;
--- same eligibility rules and returns).
function EUICoreStandaloneMinimap.SpecOverrides_DmOverlayState()
    local g, kind
    if _editGroup then
        g, kind = _editGroup, "spec"
    elseif Cond._edit then
    ...
    if forked and eligible then
        local liveNow = EUICoreStandaloneMinimap._dmSessionGid
            and ("cond:" .. EUICoreStandaloneMinimap._dmSessionGid) or (s and s.active)
        return nil, liveNow ~= liveKey
    end
    if not eligible then
        return { mode = "info", kind = kind, gid = g.id, text = text, sub = sub }
...
function EUICoreStandaloneMinimap.SpecOverrides_DmPagePrelude()
    if not _editGroup and not Cond._edit then
        EUICoreStandaloneMinimap.SpecOverrides_ApplyDm(CurrentSpecID(), false, true)
    end
    if Cond._edit then
        local cs = GetCondDmStore()
        if cs and cs.layouts[Cond._edit.id]
           and EUICoreStandaloneMinimap._dmSessionGid ~= Cond._edit.id
    ...
    local state, needHeal = EUICoreStandaloneMinimap.SpecOverrides_DmOverlayState()
    if needHeal then
        EUICoreStandaloneMinimap.SpecOverrides_ApplyDm(CurrentSpecID(), true, true)
    end
    if EUICoreStandaloneMinimap.SpecOverrides_UpdateBmPassiveChrome then
        EUICoreStandaloneMinimap.SpecOverrides_UpdateBmPassiveChrome()
    end
...
function EUICoreStandaloneMinimap.SpecOverrides_ActivateDm(kind, gid)
    local state = EUICoreStandaloneMinimap.SpecOverrides_DmOverlayState()
    if not state or state.mode ~= "activate" or state.kind ~= kind
       or state.gid ~= gid then return end
    local cur = CurrentSpecID()
    EUICoreStandaloneMinimap.SpecOverrides_HarvestDmLayout()
...
function EUICoreStandaloneMinimap.SpecOverrides_ModuleExcluded(folder)
...
function EUICoreStandaloneMinimap.SpecOverrides_PageExcluded(module, page)
...
function Cond.ApplyValues()
    ...
    if Cond._edit then
        ...
        gid = EUICoreStandaloneMinimap.Conditions_AppliedGid and EUICoreStandaloneMinimap.Conditions_AppliedGid()
    end
...
function EUICoreStandaloneMinimap.SpecOverrides_MergeImportedStores(merged, incoming)
    ...
    local rfImported = importedFolders["EUICoreStandaloneMinimapRaidFrames"] and true or false
    ...
    if importedFolders["EUICoreStandaloneMinimapRaidFrames"] then
...
function EUICoreStandaloneMinimap.SpecOverrides_RebaselineDefaults(folderSet)
...
function Cond.PruneEntries()
    ...
    for i = #store, 1, -1 do
        ...
        if not drop and e.group ~= nil
           and not (EUICoreStandaloneMinimap.Conditions_GroupById and EUICoreStandaloneMinimap.Conditions_GroupById(e.group)) then
            drop = true
        end
...
function EUICoreStandaloneMinimap.SpecOverrides_CondTransition(oldGid, newGid, establish)
    ...
    if not establish and EUICoreStandaloneMinimap.SpecOverrides_HarvestUnlockLayout then
        EUICoreStandaloneMinimap.SpecOverrides_HarvestUnlockLayout()
    end
    if not establish and EUICoreStandaloneMinimap.SpecOverrides_HarvestBmLayout then
        EUICoreStandaloneMinimap.SpecOverrides_HarvestBmLayout()
    end
    if not establish and EUICoreStandaloneMinimap.SpecOverrides_HarvestDmLayout then
        EUICoreStandaloneMinimap.SpecOverrides_HarvestDmLayout()
    end
    ...
    if EUICoreStandaloneMinimap.SpecOverrides_ApplyUnlock then
        ...
        EUICoreStandaloneMinimap.SpecOverrides_ApplyUnlock(CurrentSpecID(), establish)
        if EUICoreStandaloneMinimap.SpecOverrides_ApplyBm then
            EUICoreStandaloneMinimap.SpecOverrides_ApplyBm(CurrentSpecID(), establish)
        end
        if EUICoreStandaloneMinimap.SpecOverrides_ApplyDm then
            EUICoreStandaloneMinimap.SpecOverrides_ApplyDm(CurrentSpecID(), establish)
        end
...
function Cond.ResolveGid()
    ...
    return EUICoreStandaloneMinimap.Conditions_AppliedGid and EUICoreStandaloneMinimap.Conditions_AppliedGid() or nil
end
...
function EUICoreStandaloneMinimap.RegisterCaptureContext(folder, fn)
...
local function CurrentContext()
    local modFolder = EUICoreStandaloneMinimap.GetActiveModule and EUICoreStandaloneMinimap:GetActiveModule()
...
function EUICoreStandaloneMinimap.SpecOverrides_UnlockValueSnapBegin()
    _unlockValueSnap = nil
    local store = GetStore()
    if not store or #store == 0 then return end
    if EUICoreStandaloneMinimap._specialUnlockGroup then return end
    _unlockValueSnap = SnapshotProfiles()
end

function EUICoreStandaloneMinimap.SpecOverrides_UnlockValueSnapDiscard()
    _unlockValueSnap = nil
end

function EUICoreStandaloneMinimap.SpecOverrides_UnlockValueSnapCommit()
...
local function IsExcludedContext(section)
    local module = EUICoreStandaloneMinimap.GetActiveModule and EUICoreStandaloneMinimap:GetActiveModule()
    local ex = module and EXCLUDED_CONTEXTS[module]
    if ex == true then return true end
...
local function MakeBorderHost(region, r, g, b)
    ...
    if EUICoreStandaloneMinimap.PP and EUICoreStandaloneMinimap.PP.CreateBorder then
        EUICoreStandaloneMinimap.PP.CreateBorder(host, r, g, b, 0.9, 1, "OVERLAY", 7)
    end
    return host
end
...
local function SetSlotMark(region, mode, conflictSpecID, condName, tip)
    ...
    if mode == "gold" and tip and not region._specOvHover then
        ...
        ico:SetTexture("Interface\\AddOns\\EUIStandaloneMinimap\\media\\icons\\eui-info.png")
        ...
        badge:SetScript("OnEnter", function()
            ...
            if t and EUICoreStandaloneMinimap.ShowWidgetTooltip then
                EUICoreStandaloneMinimap.ShowWidgetTooltip(badge, t)
            end
        end)
    ...
    if mode == "red" and not region._specOvRed then
        ...
        blocker:SetScript("OnEnter", function(self)
            if self._tipText and EUICoreStandaloneMinimap.ShowWidgetTooltip then
                EUICoreStandaloneMinimap.ShowWidgetTooltip(self, self._tipText)
            end
        end)
    ...
    if mode == "condActive" and not region._specOvCondActive then
        ...
        if EUICoreStandaloneMinimap.PP and EUICoreStandaloneMinimap.PP.CreateBorder then
            EUICoreStandaloneMinimap.PP.CreateBorder(blocker, GOLD_R, GOLD_G, GOLD_B, 0.9, 1, "OVERLAY", 7)
        end
        local font = (EUICoreStandaloneMinimap.GetFontPath and EUICoreStandaloneMinimap.GetFontPath()) or "Fonts\\FRIZQT__.TTF"
        local flag = (EUICoreStandaloneMinimap.GetFontOutlineFlag and EUICoreStandaloneMinimap.GetFontOutlineFlag()) or ""
        local label = blocker:CreateFontString(nil, "OVERLAY")
        if EUICoreStandaloneMinimap.PrimeFontShadow and EUICoreStandaloneMinimap.GetFontUseShadow then
            EUICoreStandaloneMinimap.PrimeFontShadow(label, EUICoreStandaloneMinimap.GetFontUseShadow())
        end
...
function EUICoreStandaloneMinimap.SpecOverrides_DarkCondEditActive()
    local g = Cond and Cond._edit
    if not g then return false end
    if g.conds and g.conds.darkmode then return true end
    local groups = EUICoreStandaloneMinimap.Conditions_GetGroups and EUICoreStandaloneMinimap.Conditions_GetGroups()
    if groups then
...
function EUICoreStandaloneMinimap.SpecOverrides_AttachEditLock(region, tip, predicate)
    ...
    for fkey in pairs(sink) do
        ...
        if ce and ce.values then
            ...
            for k, m in pairs(ce.values) do
                if type(k) == "number" and type(m) == "table" and m[fkey] ~= nil then
                    local cg = EUICoreStandaloneMinimap.Conditions_GroupById
                        and EUICoreStandaloneMinimap.Conditions_GroupById(k)
                    if cg then
...
RequestGoldWalk = function()
    ...
    C_Timer.After(0, function()
        ...
        local root = _G.EUICoreStandaloneMinimapFrame
        ...
        if BeginTrace() then
            ...
            if not _editGroup and not Cond._edit then
                condGid = EUICoreStandaloneMinimap.Conditions_AppliedGid
                    and EUICoreStandaloneMinimap.Conditions_AppliedGid() or nil
                local g = condGid and EUICoreStandaloneMinimap.Conditions_GroupById
                    and EUICoreStandaloneMinimap.Conditions_GroupById(condGid) or nil
...
local function SampleAttribution()
    ...
    while n do
        ...
        if n._euiOptionsPopup or n == EUICoreStandaloneMinimap._colorPickerPopup then popup = true end
        if n == _G.EUICoreStandaloneMinimapFrame then inPanel = true end
        n = n:GetParent()
    end
    if region then
...
local function AutoCapture(changes)
    ...
    if EUICoreStandaloneMinimap._prebuilding then return end
    ...
    if #paths == 0 then
        if skippedBlack then
            if skippedBlack == "EUICoreStandaloneMinimapCooldownManager" then
    ...
    if not entry then
        isNew = true
        local crumbParts = {}
        local modTitle = module and EUICoreStandaloneMinimap.GetModuleTitle and EUICoreStandaloneMinimap:GetModuleTitle(module)
...
local function OnPageRebuilt()
    ...
    if EUICoreStandaloneMinimap._specOvBtnPageState then EUICoreStandaloneMinimap._specOvBtnPageState() end
    ...
    if not _editGroup and not _defaultView then
        C_Timer.After(0, function()
            ...
            if not _editGroup and not _defaultView and not _unlockRoundtrip
               and _G.EUICoreStandaloneMinimapFrame and _G.EUICoreStandaloneMinimapFrame:IsShown()
               and EnterDefaultView then
                EnterDefaultView()
            end
        end)
...
local function MaybeWarnDefaultEdit(frame)
    ...
    local VG = EUICoreStandaloneMinimap.VideoGuides
...
function EUICoreStandaloneMinimap._NotifySettingWrite(frame)
        ...
        if not EntryOwning(c.fkey) and (not c.num or NumAllowedFKey(c.fkey)) and not BlacklistedFKey(c.fkey)
           ...
           and not _sessionIgnored[c.fkey] then
            ...
            local entry = {
                ...
                crumb = (EUICoreStandaloneMinimap.GetModuleTitle and EUICoreStandaloneMinimap:GetModuleTitle(c.folder)) or c.folder,
                module = c.folder,
                group = group.id,
...
EnsurePanelHideHook = function()
    if panelHideHooked or not _G.EUICoreStandaloneMinimapFrame then return end
    panelHideHooked = true
    _G.EUICoreStandaloneMinimapFrame:HookScript("OnHide", function()
        ...
        if EUICoreStandaloneMinimap._unlockReturnModule ~= nil then
            if _editGroup then
        ...
        if EUICoreStandaloneMinimap._specOvCardsPopup then EUICoreStandaloneMinimap._specOvCardsPopup:Hide() end
        ...
        if EUICoreStandaloneMinimap._specOvNamePopup then EUICoreStandaloneMinimap._specOvNamePopup:Hide() end
    ...
    _G.EUICoreStandaloneMinimapFrame:HookScript("OnShow", function()
        C_Timer.After(0, function()
            if not (_G.EUICoreStandaloneMinimapFrame and _G.EUICoreStandaloneMinimapFrame:IsShown()) then return end
            ...
            if rt then
                ...
                elseif rt.kind == "cond" and Cond.EnterEdit then
                    local cgroups = EUICoreStandaloneMinimap.Conditions_GetGroups and EUICoreStandaloneMinimap.Conditions_GetGroups()
...
local function EnsureEditBanner()
    ...
    local root = _G.EUICoreStandaloneMinimapFrame or UIParent
    ...
    editBanner:SetPoint("BOTTOM", _G.EUICoreStandaloneMinimapClickArea or root, "TOP", 0, 0)
    ...
    editBannerText:SetFont(EUICoreStandaloneMinimap.EXPRESSWAY or "Fonts\\FRIZQT__.TTF", 13, "")
    ...
    editBannerStatus:SetFont(EUICoreStandaloneMinimap.EXPRESSWAY or "Fonts\\FRIZQT__.TTF", 11, "")
    ...
    EUICoreStandaloneMinimap.SolidTex(done, "BACKGROUND", 0.10, 0.10, 0.11, 0.9)
    local dbrd = EUICoreStandaloneMinimap.MakeBorder(done, 1, 1, 1, 0.22)
    local dlbl = EUICoreStandaloneMinimap.MakeFont(done, 11, nil, 1, 1, 1, 0.9)
...
ShowEditBanner = function(group)
    ...
    if EUICoreStandaloneMinimap.RefreshSidebarOverrideLocks then EUICoreStandaloneMinimap.RefreshSidebarOverrideLocks() end
end
...
-- panel background layers (both fill EUICoreStandaloneMinimapFrame edge to edge), one frame
-- level above them and below all content frames.
local EDIT_GLOW_TEXTURE = "Interface\\AddOns\\EUIStandaloneMinimap\\media\\backgrounds\\eui-glow-override.png"
local editOverlay
local editOverlayTexture = EDIT_GLOW_TEXTURE

function EUICoreStandaloneMinimap.SpecOverrides_SetEditBackground(texturePath)
...
function EUICoreStandaloneMinimap.SpecOverrides_UpdateBmPassiveChrome()
    if _editGroup or Cond._edit then return end   -- real session owns chrome
    local kind = EUICoreStandaloneMinimap.SpecOverrides_BmPageLockInfo
        and EUICoreStandaloneMinimap.SpecOverrides_BmPageLockInfo() or nil
    local status = L("Buff Manager changes on this page apply only to this override.")
    local nameFn = EUICoreStandaloneMinimap.SpecOverrides_BmActiveInfo
    if not kind then
        -- Same chrome serves the Debuff Manager page (one page is active at a
        -- time, so the two locks can never both report).
        kind = EUICoreStandaloneMinimap.SpecOverrides_DmPageLockInfo
            and EUICoreStandaloneMinimap.SpecOverrides_DmPageLockInfo() or nil
        if kind then
            status = L("Debuff Manager changes on this page apply only to this override.")
            nameFn = EUICoreStandaloneMinimap.SpecOverrides_DmActiveInfo
        end
    end
    if kind then
...
EnterDefaultView = function()
    ...
    EUICoreStandaloneMinimap.SpecOverrides_HarvestCurrent()
    _defaultView = true
    local touched = WriteDefaultValues()
    if touched then RunRefreshers(touched) end
...
function EUICoreStandaloneMinimap.SpecOverrides_SuspendDefaultView()
    if not _defaultView then return false end
    ExitDefaultView()
    return true
end

function EUICoreStandaloneMinimap.SpecOverrides_ResumeDefaultView()
...
ExitGroupEdit = function(noRecheck)
    ...
    if not noRecheck and EUICoreStandaloneMinimap.Conditions_Recheck then
        EUICoreStandaloneMinimap.Conditions_Recheck()
    end
end
...
function EUICoreStandaloneMinimap.SpecOverrides_CloseEditSessions()
...
EnterGroupEdit = function(group)
    ...
    do
        ...
        if EUICoreStandaloneMinimap.SpecOverrides_BmPageLockInfo then
            lockKind, lockGid = EUICoreStandaloneMinimap.SpecOverrides_BmPageLockInfo()
        end
        if not lockKind and EUICoreStandaloneMinimap.SpecOverrides_DmPageLockInfo then
            lockKind, lockGid = EUICoreStandaloneMinimap.SpecOverrides_DmPageLockInfo()
...
Cond.ExitEdit = function(noRestore, noRecheck)
    ...
    if EUICoreStandaloneMinimap.SpecOverrides_BmSessionRelease then
        EUICoreStandaloneMinimap.SpecOverrides_BmSessionRelease(noRestore and true or false)
    end
    if EUICoreStandaloneMinimap.SpecOverrides_DmSessionRelease then
        EUICoreStandaloneMinimap.SpecOverrides_DmSessionRelease(noRestore and true or false)
    end
    local g = Cond._edit
    Cond._edit = nil
    ...
    if not noRestore then
        ...
        local t2 = Cond.WriteValues(
            EUICoreStandaloneMinimap.Conditions_AppliedGid and EUICoreStandaloneMinimap.Conditions_AppliedGid() or nil)
        if t2 then
            touched = touched or {}
            for k in pairs(t2) do touched[k] = true end
        end
...
EUICoreStandaloneMinimap._specOvBtnPageState = function()
    ...
    local bmLock = false
    if not off and EUICoreStandaloneMinimap.SpecOverrides_BmPageLocked then
        bmLock = EUICoreStandaloneMinimap.SpecOverrides_BmPageLocked()
    end
    if not bmLock and not off and EUICoreStandaloneMinimap.SpecOverrides_DmPageLocked then
        bmLock = EUICoreStandaloneMinimap.SpecOverrides_DmPageLocked()
    end
...
function EUICoreStandaloneMinimap.SpecOverrides_PulseButton()
    if not specBtn then return end
    ...
    if not p then
        ...
        EUICoreStandaloneMinimap.MakeBorder(p, 1, 0.82, 0.30, 0.9)
    ...
    if not tip then
        ...
        local lbl = EUICoreStandaloneMinimap.MakeFont(tip, 12, nil, 1, 0.82, 0.30, 1)
        lbl:SetPoint("CENTER")
        lbl:SetText(L("New Overrides System"))
        local w = (lbl:GetStringWidth() or 120) + 20
        tip:SetSize(w, 24)
        tip:SetPoint("TOP", specBtn, "BOTTOM", 0, -8)
        local tbg = EUICoreStandaloneMinimap.SolidTex(tip, "BACKGROUND", 0.05, 0.06, 0.08, 0.95)
        EUICoreStandaloneMinimap.MakeBorder(tip, 1, 0.82, 0.30, 0.85)
...
function EUICoreStandaloneMinimap.SpecOverrides_SetupButton(btn)
    specBtn = btn
    ...
    btn:SetScript("OnEnter", function(self)
        if self._ovPageDisabled then
            if EUICoreStandaloneMinimap.ShowWidgetTooltip then
                EUICoreStandaloneMinimap.ShowWidgetTooltip(self, L("This page is excluded from overrides."))
            end
            return
        end
        if self._ovBmLocked then
            if EUICoreStandaloneMinimap.ShowWidgetTooltip then
                EUICoreStandaloneMinimap.ShowWidgetTooltip(self, L("Overrides are locked: this page's custom Buff Manager is active. Leave the Buff Manager tab to manage overrides."))
            end
            return
        end
        self:SetAlpha(1)
        if EUICoreStandaloneMinimap.ShowWidgetTooltip then
            EUICoreStandaloneMinimap.ShowWidgetTooltip(self, L("Settings Overrides"))
        end
    end)
    ...
    btn:SetScript("OnClick", function(self)
        ...
        EUICoreStandaloneMinimap.SpecOverrides_ToggleCardsPopup(self)
    end)
    UpdateIndicator()
    EUICoreStandaloneMinimap._specOvBtnPageState()
end
...
local function ShowNameIconPopup(specIDs, editing)
    if not nameIconPopup then
        ...
        EUICoreStandaloneMinimap.MakeBorder(p, 1, 1, 1, 0.15)

        local title = EUICoreStandaloneMinimap.MakeFont(p, 14, nil, ACCENT_R, ACCENT_G, ACCENT_B, 1)
        title:SetPoint("TOP", p, "TOP", 0, -14)
        title:SetText(L("New Spec Group"))
        p._title = title

        local nameLbl = EUICoreStandaloneMinimap.MakeFont(p, 12, nil, 1, 1, 1, 0.6)
        ...
        nameBox:SetFont(EUICoreStandaloneMinimap.EXPRESSWAY or "Fonts\\FRIZQT__.TTF", 13, "")
        nameBox:SetTextColor(1, 1, 1, 1)
        nameBox:SetTextInsets(8, 8, 0, 0)
        nameBox:SetMaxLetters(24)
        EUICoreStandaloneMinimap.SolidTex(nameBox, "BACKGROUND", 0.10, 0.10, 0.11, 0.9)
        EUICoreStandaloneMinimap.MakeBorder(nameBox, 1, 1, 1, 0.12)
        nameBox:SetScript("OnEscapePressed", function(self) self:ClearFocus() end)
        nameBox:SetScript("OnEnterPressed", function(self) self:ClearFocus() end)
        p._nameBox = nameBox

        local iconLbl = EUICoreStandaloneMinimap.MakeFont(p, 12, nil, 1, 1, 1, 0.6)
        ...
        for i, def in ipairs(defs) do
            ...
            local brd = EUICoreStandaloneMinimap.MakeBorder(b, 1, 1, 1, 0.10)
            b._def = def
            b._brd = brd
        ...
        EUICoreStandaloneMinimap.SolidTex(create, "BACKGROUND", 0.10, 0.10, 0.11, 0.9)
        local cbrd = EUICoreStandaloneMinimap.MakeBorder(create, ACCENT_R, ACCENT_G, ACCENT_B, 0.5)
        local clbl = EUICoreStandaloneMinimap.MakeFont(create, 12, nil, ACCENT_R, ACCENT_G, ACCENT_B, 1)
        ...
        create:SetScript("OnClick", function()
            ...
            if editing then
                ...
                if UpdateIndicator then UpdateIndicator() end
                if RefreshCardsPopup then RefreshCardsPopup() end
                local ap = EUICoreStandaloneMinimap.GetActivePage and EUICoreStandaloneMinimap:GetActivePage()
                if ap == LIST_PAGE then EUICoreStandaloneMinimap:RefreshPage(true) end
                return
            end
        ...
        EUICoreStandaloneMinimap.SolidTex(cancel, "BACKGROUND", 0.10, 0.10, 0.11, 0.9)
        local xbrd = EUICoreStandaloneMinimap.MakeBorder(cancel, 1, 1, 1, 0.22)
        local xlbl = EUICoreStandaloneMinimap.MakeFont(cancel, 12, nil, 1, 1, 1, 0.7)
        ...
        nameIconPopup = p
        EUICoreStandaloneMinimap._specOvNamePopup = p   -- panel-hide hook closes it (lexical: local declared below the hook)
    end
...
local function StartGroupCreation()
    if not EUICoreStandaloneMinimap.ShowSpecAssignPopup then return end
...
local function BuildCardRow(parent, y, opts)
    ...
    local brd = EUICoreStandaloneMinimap.MakeBorder(row, 1, 1, 1, opts.active and 0 or 0.08)
    ...
    local name = EUICoreStandaloneMinimap.MakeFont(row, 13, nil, 1, 1, 1, opts.dim and 0.55 or 0.9)
    name:SetPoint("LEFT", icon, "RIGHT", 10, 0)
    name:SetText(opts.name)
    if opts.tooltip then
        row:SetScript("OnEnter", function(self)
            if EUICoreStandaloneMinimap.ShowWidgetTooltip then EUICoreStandaloneMinimap.ShowWidgetTooltip(self, opts.tooltip) end
        end)
    ...
    if opts.locked then
        ...
        row:SetScript("OnClick", function(self)
            if EUICoreStandaloneMinimap.ShowWidgetTooltip then
                EUICoreStandaloneMinimap.ShowWidgetTooltip(self, L("The active override's Buff Manager is bound to the open page. Leave the Buff Manager tab to switch overrides."))
            end
        end)
    else
    ...
    if opts.deletable then
        ...
        del:SetScript("OnEnter", function(self)
            ...
            if EUICoreStandaloneMinimap.ShowWidgetTooltip then
                EUICoreStandaloneMinimap.ShowWidgetTooltip(self, L("Delete Override Group"))
            end
        end)
    ...
    if opts.onEdit then
        ...
        ico:SetAllPoints()
        if ico.SetSnapToPixelGrid then ico:SetSnapToPixelGrid(false); ico:SetTexelSnappingBias(0) end
        ico:SetTexture("Interface\\AddOns\\EUIStandaloneMinimap\\media\\icons\\eui-edit.png")
        ed:SetAlpha(0.75)
        ed:SetScript("OnEnter", function(self)
            self:SetAlpha(1)
            if EUICoreStandaloneMinimap.ShowWidgetTooltip then
                EUICoreStandaloneMinimap.ShowWidgetTooltip(self, L("Edit Override Group"))
            end
        end)
    ...
    if opts.onUnlock then
        ...
        ico:SetAllPoints()
        if ico.SetSnapToPixelGrid then ico:SetSnapToPixelGrid(false); ico:SetTexelSnappingBias(0) end
        ico:SetTexture("Interface\\AddOns\\EUIStandaloneMinimap\\media\\icons\\eui-unlocked-small.png")
...
local function SetGroupSpecs(g, newSpecs)
    ...
    if EUICoreStandaloneMinimap.SpecOverrides_Apply then
        EUICoreStandaloneMinimap.SpecOverrides_Apply(_activeSpec or CurrentSpecID())
    end
    UpdateIndicator()
    RequestGoldWalk()
    RefreshCardsPopup()
...
RefreshCardsPopup = function()
    ...
    if EUICoreStandaloneMinimap.SpecOverrides_BmPageLockInfo then
        bmLockKind, bmLockGid = EUICoreStandaloneMinimap.SpecOverrides_BmPageLockInfo()
    end
    if not bmLockKind and EUICoreStandaloneMinimap.SpecOverrides_DmPageLockInfo then
        bmLockKind, bmLockGid = EUICoreStandaloneMinimap.SpecOverrides_DmPageLockInfo()
    end
            ...
            onDelete = function()
                EUICoreStandaloneMinimap:ShowConfirmPopup({
                    ...
                    onConfirm = function()
                        ...
                        if EUICoreStandaloneMinimap.SpecOverrides_RemoveUnlockLayout then
                            EUICoreStandaloneMinimap.SpecOverrides_RemoveUnlockLayout(g.id)
                        end
                        ...
                        if EUICoreStandaloneMinimap.SpecOverrides_RemoveBmLayout then
                            EUICoreStandaloneMinimap.SpecOverrides_RemoveBmLayout(g.id)
                        end
                        if EUICoreStandaloneMinimap.SpecOverrides_RemoveDmLayout then
                            EUICoreStandaloneMinimap.SpecOverrides_RemoveDmLayout(g.id)
                        end
                        RebuildFKeyIndex()
                        RequestGoldWalk()
                        UpdateIndicator()   -- current spec may have been a member
                        RefreshCardsPopup()
                        if EUICoreStandaloneMinimap.GetActivePage and EUICoreStandaloneMinimap:GetActivePage() == LIST_PAGE then
    ...
    do
        ...
        local hl = EUICoreStandaloneMinimap.MakeFont(hdr, 13, nil, ACCENT_R, ACCENT_G, ACCENT_B, 1)
        hl:SetPoint("BOTTOM", hdr, "BOTTOM", 0, 4)
        hl:SetText(L("Conditional Overrides"))
        local sub = EUICoreStandaloneMinimap.MakeFont(hdr, 12, nil, 0.55, 0.55, 0.55, 1)
    ...
    for _, g in ipairs(EUICoreStandaloneMinimap.Conditions_GetGroups and EUICoreStandaloneMinimap.Conditions_GetGroups() or {}) do
        add(BuildCardRow(p, y, {
            ...
            onDelete = function()
                EUICoreStandaloneMinimap:ShowConfirmPopup({
                    ...
                    onConfirm = function()
                        ...
                        local groups = EUICoreStandaloneMinimap.Conditions_GetGroups()
                        if groups then
                            for i, gg in ipairs(groups) do
                                if gg == g then table.remove(groups, i); break end
                            end
                        ...
                        Cond.RebuildIndex()
                        if EUICoreStandaloneMinimap.Conditions_RemoveUnlockLayout then
                            EUICoreStandaloneMinimap.Conditions_RemoveUnlockLayout(g.id)
                        end
                        ...
                        if EUICoreStandaloneMinimap.Conditions_RemoveBmLayout then
                            EUICoreStandaloneMinimap.Conditions_RemoveBmLayout(g.id)
                        end
                        if EUICoreStandaloneMinimap.Conditions_RemoveDmLayout then
                            EUICoreStandaloneMinimap.Conditions_RemoveDmLayout(g.id)
                        end
                        if EUICoreStandaloneMinimap.Conditions_RebuildKeyBindings then EUICoreStandaloneMinimap.Conditions_RebuildKeyBindings() end
                        if EUICoreStandaloneMinimap.Conditions_Recheck then EUICoreStandaloneMinimap.Conditions_Recheck() end
                        RequestGoldWalk()
                        RefreshCardsPopup()
    ...
    local ll = EUICoreStandaloneMinimap.MakeFont(link, 12, nil, ACCENT_R, ACCENT_G, ACCENT_B, 0.9)
    ll:SetPoint("CENTER")
    ...
    link:SetScript("OnClick", function()
        ...
        EUICoreStandaloneMinimap:SelectModule(PROFILES_MODULE)
        if EUICoreStandaloneMinimap.SelectPage then EUICoreStandaloneMinimap:SelectPage(LIST_PAGE) end
    end)
...
function EUICoreStandaloneMinimap.SpecOverrides_ToggleCardsPopup(anchorBtn)
    if cardsPopup and cardsPopup:IsShown() then
        cardsPopup:Hide()
        return
    end
    if not cardsPopup then
        local p = CreateFrame("Frame", nil, _G.EUICoreStandaloneMinimapFrame or UIParent)
        ...
        EUICoreStandaloneMinimap.MakeBorder(p, 1, 1, 1, 0.15)
        local title = EUICoreStandaloneMinimap.MakeFont(p, 13, nil, ACCENT_R, ACCENT_G, ACCENT_B, 1)
        ...
        cardsPopup = p
        EUICoreStandaloneMinimap._specOvCardsPopup = p
    end
...
function Cond.CondLabel(id)
    for _, def in ipairs(EUICoreStandaloneMinimap.CONDITIONS or {}) do
...
function Cond.ShowNameIconPopup(conds, keyStr, existing)
    ...
    if not p then
        ...
        p._iconBtns = {}
        local defs = {}
        for _, cdef in ipairs(EUICoreStandaloneMinimap.CONDITIONS or {}) do
        ...
        create:SetScript("OnClick", function()
            ...
            if p._existing then
                ...
                local groups = EUICoreStandaloneMinimap.Conditions_GetGroups(true)
                if not groups then p:Hide(); return end
                local id = EUICoreStandaloneMinimap.Conditions_NewGroupId()
...
function Cond.ShowPickerPopup(existing)
    ...
    if not p then
        ...
        title:SetPoint("TOP", p, "TOP", 0, -14)
        p._title = title
        local sub = EUICoreStandaloneMinimap.MakeFont(p, 11, nil, 1, 1, 1, 0.5)
        sub:SetPoint("TOP", p, "TOP", 0, -32)
        sub:SetText(L("Select the conditions that activate this group:"))

        p._rows = {}
        local y = -52
        for _, def in ipairs(EUICoreStandaloneMinimap.CONDITIONS) do
            ...
            check:Hide()
            local lbl = EUICoreStandaloneMinimap.MakeFont(row, 12, nil, 1, 1, 1, def.comingSoon and 0.35 or 0.85)
            ...
            if def.comingSoon then
                row:SetScript("OnEnter", function(self)
                    if EUICoreStandaloneMinimap.ShowWidgetTooltip then
                        EUICoreStandaloneMinimap.ShowWidgetTooltip(self, L("This condition is coming in a future update."))
                    end
                end)
                row:SetScript("OnLeave", function()
                    if EUICoreStandaloneMinimap.HideWidgetTooltip then EUICoreStandaloneMinimap.HideWidgetTooltip() end
            ...
            elseif def.requires then
                ...
                row:SetScript("OnEnter", function(self)
                    if not p._staged[self._condID] and not self._reqFn()
                       and def.requiresHint and EUICoreStandaloneMinimap.ShowWidgetTooltip then
                        EUICoreStandaloneMinimap.ShowWidgetTooltip(self, L(def.requiresHint))
                    end
                end)
                row:SetScript("OnLeave", function()
                    if EUICoreStandaloneMinimap.HideWidgetTooltip then EUICoreStandaloneMinimap.HideWidgetTooltip() end
        ...
        local keyLbl = EUICoreStandaloneMinimap.MakeFont(keyRow, 12, nil, 1, 1, 1, 0.6)
        ...
        EUICoreStandaloneMinimap.SolidTex(keyBtn, "BACKGROUND", 0.10, 0.10, 0.11, 0.9)
        local kbrd = EUICoreStandaloneMinimap.MakeBorder(keyBtn, 1, 1, 1, 0.22)
        local kl = EUICoreStandaloneMinimap.MakeFont(keyBtn, 11, nil, 1, 1, 1, 0.85)
        kl:SetPoint("CENTER")
        ...
        local kx = EUICoreStandaloneMinimap.MakeFont(keyClear, 13, nil, 1, 1, 1, 0.6)
        kx:SetPoint("CENTER")
        kx:SetText("x")
        ...
        EUICoreStandaloneMinimap.SolidTex(nextBtn, "BACKGROUND", 0.10, 0.10, 0.11, 0.9)
        local nbrd = EUICoreStandaloneMinimap.MakeBorder(nextBtn, ACCENT_R, ACCENT_G, ACCENT_B, 0.5)
        local nlbl = EUICoreStandaloneMinimap.MakeFont(nextBtn, 12, nil, ACCENT_R, ACCENT_G, ACCENT_B, 1)
        ...
        nextBtn:SetScript("OnClick", function()
            ...
            if p._staged.keybind and not p._stagedKey then
                if EUICoreStandaloneMinimap.ShowWidgetTooltip then
                    EUICoreStandaloneMinimap.ShowWidgetTooltip(nextBtn, L("Set the toggle key first (or uncheck Keybind)."))
                end
                return
            end
            local conds = {}
        ...
        EUICoreStandaloneMinimap.SolidTex(cancel, "BACKGROUND", 0.10, 0.10, 0.11, 0.9)
        local xbrd2 = EUICoreStandaloneMinimap.MakeBorder(cancel, 1, 1, 1, 0.22)
        local xlbl2 = EUICoreStandaloneMinimap.MakeFont(cancel, 12, nil, 1, 1, 1, 0.7)
...
function EUICoreStandaloneMinimap.Conditions_ToggleCardsPopup(anchorBtn)
    EUICoreStandaloneMinimap.SpecOverrides_ToggleCardsPopup(anchorBtn)
end
-- ---- toolbar button ---------------------------------------------------------
...
EUICoreStandaloneMinimap.Conditions_UpdateButton = Cond.UpdateButton

function EUICoreStandaloneMinimap.Conditions_SetupButton(btn)
    Cond._btn = btn
    local tex = btn:CreateTexture(nil, "OVERLAY")
    tex:SetAllPoints()
    btn._tex = tex
    ...
    btn:SetScript("OnEnter", function(self)
        self:SetAlpha(1)
        if EUICoreStandaloneMinimap.ShowWidgetTooltip then
            EUICoreStandaloneMinimap.ShowWidgetTooltip(self, L("Conditional Overrides: override settings by condition (dungeon, raid, keybind...)"))
        end
    end)
    btn:SetScript("OnLeave", function(self)
        self:SetAlpha(0.9)
    ...
    btn:SetScript("OnClick", function(self)
        EUICoreStandaloneMinimap.Conditions_ToggleCardsPopup(self)
    end)
    Cond.UpdateButton()
end
...
local function BuildListRow(parent, y, entry)
    local CONTENT_PAD = EUICoreStandaloneMinimap.CONTENT_PAD or 40
    local row = CreateFrame("Frame", nil, parent)
    row:SetSize(parent:GetWidth() - CONTENT_PAD * 2, 36)
    row:SetPoint("TOPLEFT", parent, "TOPLEFT", CONTENT_PAD, y)

    local name = EUICoreStandaloneMinimap.MakeFont(row, 13, nil, 1, 1, 1, 0.9)
    name:SetPoint("LEFT", row, "LEFT", 20, 0)
    name:SetText(L(entry.label or "?"))

    local crumb = EUICoreStandaloneMinimap.MakeFont(row, 11, nil, 1, 1, 1, 0.3)
    ...
    local function MakeBtn(text, xOff, w)
        ...
        EUICoreStandaloneMinimap.SolidTex(b, "BACKGROUND", 0.10, 0.10, 0.11, 0.9)
        local brd = EUICoreStandaloneMinimap.MakeBorder(b, 1, 1, 1, 0.22)
        local lbl = EUICoreStandaloneMinimap.MakeFont(b, 11, nil, 1, 1, 1, 0.8)
        lbl:SetPoint("CENTER")
        lbl:SetText(L(text))
        return b, brd
    end
    ...
    if isCondEntry then
        ...
        if ownerEntry then
            ...
            local warn = EUICoreStandaloneMinimap.MakeFont(row, 11, nil, 1, 0.45, 0.45, 0.85)
            ...
            hit:SetScript("OnEnter", function(self)
                if EUICoreStandaloneMinimap.ShowWidgetTooltip then
                    EUICoreStandaloneMinimap.ShowWidgetTooltip(self,
    ...
    go:SetScript("OnClick", function()
        ...
        if mod and EUICoreStandaloneMinimap.GetModuleTitle and EUICoreStandaloneMinimap:GetModuleTitle(mod) then
            EUICoreStandaloneMinimap:SelectModule(mod)
            if entry.page and EUICoreStandaloneMinimap.SelectPage then
                EUICoreStandaloneMinimap:SelectPage(entry.page)
            end
        end
    end)
...
local function BuildStrandedRow(parent, y, entry, specID)
    ...
    EUICoreStandaloneMinimap.SolidTex(rm, "BACKGROUND", 0.10, 0.10, 0.11, 0.9)
    local rmBrd = EUICoreStandaloneMinimap.MakeBorder(rm, 1, 1, 1, 0.22)
    local rmLbl = EUICoreStandaloneMinimap.MakeFont(rm, 11, nil, 1, 1, 1, 0.8)
...
local function BuildUnlockLayoutRow(parent, y, g, opts)
    ...
    local removeFn = opts.removeFn or EUICoreStandaloneMinimap.SpecOverrides_RemoveUnlockLayout
    ...
    local crumb = EUICoreStandaloneMinimap.MakeFont(row, 11, nil, GOLD_R, GOLD_G, GOLD_B, 0.75)
    crumb:SetPoint("LEFT", name, "RIGHT", 10, 0)
    ...
    if opts.editKind and opts.editPage then
        ...
        EUICoreStandaloneMinimap.SolidTex(e, "BACKGROUND", 0.10, 0.10, 0.11, 0.9)
        local ebrd = EUICoreStandaloneMinimap.MakeBorder(e, 1, 1, 1, 0.22)
        local elbl = EUICoreStandaloneMinimap.MakeFont(e, 11, nil, 1, 1, 1, 0.8)
        ...
        e:SetScript("OnClick", function()
            ...
            if not (EUICoreStandaloneMinimap.GetModuleTitle and EUICoreStandaloneMinimap:GetModuleTitle("EUICoreStandaloneMinimapRaidFrames")) then return end
            ...
            EUICoreStandaloneMinimap:SelectModule("EUICoreStandaloneMinimapRaidFrames")
            if EUICoreStandaloneMinimap.SelectPage then
                EUICoreStandaloneMinimap:SelectPage(opts.editPage)
            end
        end)
    end
...
local BM_ROW_SPEC = {
    ...
    removeFn = function(gid) EUICoreStandaloneMinimap.SpecOverrides_RemoveBmLayout(gid) end,
...
local BM_ROW_COND = {
    ...
    removeFn = function(gid) EUICoreStandaloneMinimap.Conditions_RemoveBmLayout(gid) end,
...
local DM_ROW_SPEC = {
    ...
    removeFn = function(gid) EUICoreStandaloneMinimap.SpecOverrides_RemoveDmLayout(gid) end,
...
local DM_ROW_COND = {
    ...
    removeFn = function(gid) EUICoreStandaloneMinimap.Conditions_RemoveDmLayout(gid) end,
...
local function PromoteGroupToProfile(g)
    ...
    if EUICoreStandaloneMinimap.SpecOverrides_FlushUnlock then
        pcall(EUICoreStandaloneMinimap.SpecOverrides_FlushUnlock)
    end
    ReloadUI()
end
...
function EUICoreStandaloneMinimap.SpecOverrides_BuildListPage(parent, startY)
    ...
    if not EUICoreStandaloneMinimap._prebuilding then PruneOrphanEntries() end
    local store = GetStore()
    ...
    if (not store or #store == 0) and not layoutGroups and not bmGroups and not dmGroups then
        ...
        local fs = EUICoreStandaloneMinimap.MakeFont(hint, 13, nil, 1, 1, 1, 0.5)
    ...
    if stranded then
        ...
        local CONTENT_PAD = EUICoreStandaloneMinimap.CONTENT_PAD or 40
        local hint = EUICoreStandaloneMinimap.MakeFont(parent, 11, nil, 1, 1, 1, 0.45)
                ...
                for fkey in pairs(entry.values and entry.values.default or {}) do
                    if not BlacklistedFKey(fkey) and not FKeyLoaded(fkey) then
                        ...
                        if folder and not (missingSet and missingSet[folder]) then
                            ...
                            missingList[#missingList + 1] = folder:gsub("^EUICoreStandaloneMinimap", "")
                        end
                    end
                end
            end
...
function EUICoreStandaloneMinimap.Conditions_BuildListPage(parent, startY)
    local W = EUICoreStandaloneMinimap.Widgets
    local y = startY
    Cond.PruneEntries()
    local store = Cond.GetStore()
    local groups = EUICoreStandaloneMinimap.Conditions_GetGroups() or {}
        ...
        for _, g in ipairs(layoutGroups) do
            ...
            b:SetScript("OnClick", function()
                EUICoreStandaloneMinimap:ShowConfirmPopup({
                    ...
                    onConfirm = function()
                        if EUICoreStandaloneMinimap.Conditions_RemoveUnlockLayout then
                            EUICoreStandaloneMinimap.Conditions_RemoveUnlockLayout(gid)
                        end
                        EUICoreStandaloneMinimap:RefreshPage(true)
                    end,
                })
...
if EUICoreStandaloneMinimap.SelectModule then
    hooksecurefunc(EUICoreStandaloneMinimap, "SelectModule", OnPageRebuilt)
end
if EUICoreStandaloneMinimap.SelectPage then
    hooksecurefunc(EUICoreStandaloneMinimap, "SelectPage", OnPageRebuilt)
end
if EUICoreStandaloneMinimap.RefreshPage then
    hooksecurefunc(EUICoreStandaloneMinimap, "RefreshPage", function(_, force)
        if force then
            _watchResync = true
        end
        RequestGoldWalk()
...
evFrame:SetScript("OnEvent", function(_, event)
    if event == "PLAYER_LOGIN" then
        ...
        DeAlias(GetStore())
        if EUICoreStandaloneMinimap._CondOv then DeAlias(EUICoreStandaloneMinimap._CondOv.GetStore()) end
        -- One-time tidy: drop orphaned entries plus fkeys no group holds a value for.
        PruneOrphanEntries()
        PruneRedundantValues()
        if EUICoreStandaloneMinimap._CondOv then
            EUICoreStandaloneMinimap._CondOv.PruneEntries()
            EUICoreStandaloneMinimap._CondOv.PruneRedundant()
        end
