...
function MS:CreateGUI(TabToOpen)
    ...
    function MS:CreateGeneralOptions(Container)
        ...
        GlobalContainer:SetTitle("Global")
    ...
    function MS:CreateTooltipOptions(Container)
        ...
        local PositionContainer = AG:Create("InlineGroup")
        PositionContainer:SetTitle("Tooltip Position")
        PositionContainer:SetLayout("Flow")
        PositionContainer:SetFullWidth(true)
        ScrollFrame:AddChild(PositionContainer)

        local TooltipInfo = CreateInfoTag("Anchors are relative to the |cFF8080FFMinimap|r.")
        PositionContainer:AddChild(TooltipInfo)

        local AnchorFromDropdown = AG:Create("Dropdown")
        AnchorFromDropdown:SetLabel("Anchor From")
        AnchorFromDropdown:SetList({ ["TOP"] = "Top", ["BOTTOM"] = "Bottom", ["LEFT"] = "Left", ["RIGHT"] = "Right", ["TOPLEFT"] = "Top Left", ["TOPRIGHT"] = "Top Right", ["BOTTOMLEFT"] = "Bottom Left", ["BOTTOMRIGHT"] = "Bottom Right", ["CENTER"] = "Center" },
        { "TOP", "BOTTOM", "LEFT", "RIGHT", "TOPLEFT", "TOPRIGHT", "BOTTOMLEFT", "BOTTOMRIGHT", "CENTER" })
        AnchorFromDropdown:SetValue(DB.Tooltip.Position.AnchorFrom)
        AnchorFromDropdown:SetRelativeWidth(0.5)
        AnchorFromDropdown:SetCallback("OnValueChanged", function(_, _, value) DB.Tooltip.Position.AnchorFrom = value MS:UpdateAll() end)
        PositionContainer:AddChild(AnchorFromDropdown)

        local AnchorToDropdown = AG:Create("Dropdown")
        AnchorToDropdown:SetLabel("Anchor To")
        AnchorToDropdown:SetList({ ["TOP"] = "Top", ["BOTTOM"] = "Bottom", ["LEFT"] = "Left", ["RIGHT"] = "Right", ["TOPLEFT"] = "Top Left", ["TOPRIGHT"] = "Top Right", ["BOTTOMLEFT"] = "Bottom Left", ["BOTTOMRIGHT"] = "Bottom Right", ["CENTER"] = "Center" },
        { "TOP", "BOTTOM", "LEFT", "RIGHT", "TOPLEFT", "TOPRIGHT", "BOTTOMLEFT", "BOTTOMRIGHT", "CENTER" })
        AnchorToDropdown:SetValue(DB.Tooltip.Position.AnchorTo)
        AnchorToDropdown:SetRelativeWidth(0.5)
        AnchorToDropdown:SetCallback("OnValueChanged", function(_, _, value) DB.Tooltip.Position.AnchorTo = value MS:UpdateAll() end)
        PositionContainer:AddChild(AnchorToDropdown)

        local OffsetXSlider = AG:Create("Slider")
        OffsetXSlider:SetLabel("X Offset")
        OffsetXSlider:SetValue(DB.Tooltip.Position.OffsetX)
        OffsetXSlider:SetSliderValues(-200, 200, 1)
        OffsetXSlider:SetRelativeWidth(0.5)
        OffsetXSlider:SetCallback("OnValueChanged", function(_, _, value) DB.Tooltip.Position.OffsetX = value MS:UpdateAll() end)
        PositionContainer:AddChild(OffsetXSlider)

        local OffsetYSlider = AG:Create("Slider")
        OffsetYSlider:SetLabel("Y Offset")
        OffsetYSlider:SetValue(DB.Tooltip.Position.OffsetY)
        OffsetYSlider:SetSliderValues(-200, 200, 1)
        OffsetYSlider:SetRelativeWidth(0.5)
        OffsetYSlider:SetCallback("OnValueChanged", function(_, _, value) DB.Tooltip.Position.OffsetY = value MS:UpdateAll() end)
        PositionContainer:AddChild(OffsetYSlider)
