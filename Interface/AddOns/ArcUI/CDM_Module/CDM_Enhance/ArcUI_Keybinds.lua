...
local DEFAULT_SETTINGS = {
    ...
    replaceFindText = "",     -- Comma-separated text to find (e.g. "META,CTRL")
    replaceWithText = "",     -- Comma-separated replacements (e.g. "M,C")
}
...
local cachedReplacements = nil   -- Parsed replacement table
...
-- CUSTOM REPLACEMENTS
-- ===================================================================
-- Parse the custom replacements from two comma-separated lists
-- Find: "META,CTRL,ALT" + Replace: "M,C,A" → {META="M", CTRL="C", ALT="A"}
local function GetCustomReplacements()
    if cachedReplacements then return cachedReplacements end
    
    local settings = GetSettings()
    local findStr = settings.replaceFindText or ""
    local replaceStr = settings.replaceWithText or ""
    
    cachedReplacements = {}
    
    if findStr == "" then return cachedReplacements end
    
    -- Split find text by comma
    local findItems = {}
    for item in findStr:gmatch("[^,]+") do
        local trimmed = item:match("^%s*(.-)%s*$"):upper()
        if trimmed ~= "" then
            table.insert(findItems, trimmed)
        end
    end
    
    -- Split replace text by comma
    local replaceItems = {}
    for item in replaceStr:gmatch("[^,]+") do
        local trimmed = item:match("^%s*(.-)%s*$")
        table.insert(replaceItems, trimmed)
    end
    
    -- Match by position
    for i, findText in ipairs(findItems) do
        local replaceText = replaceItems[i] or ""
        cachedReplacements[findText] = replaceText
    end
    
    return cachedReplacements
end

-- Apply custom replacements to a keybind string
local function ApplyCustomReplacements(text)
    local replacements = GetCustomReplacements()
    if not next(replacements) then return text end
    
    -- Apply each replacement (longest first to avoid partial matches)
    for from, to in pairs(replacements) do
        text = text:gsub(from .. "[%-+]", to)  -- META- or META+ becomes M
        text = text:gsub(from, to)              -- Standalone META becomes M
    end
...
local function FormatKeybind(raw)
    ...
    -- Apply custom replacements FIRST (before standard processing)
