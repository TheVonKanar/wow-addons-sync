...
if C_ChatInfo then C_ChatInfo.RegisterAddonMessagePrefix(pre) end
...
D4:OnEvent(f, function(sel, event, ...)
    ...
    elseif event == "CHAT_MSG_ADDON" then
        ...
        if pref == pre and msg then
            ...
            if a and name and v and ver and a == "A" and v == "V" and AddonName == name and not D4:FoundHigher(name) then D4:CheckVersion(name, ver) end
        end
        ...
        D4:After(3, function()
            if C_ChatInfo and D4.VersionTab[string.lower(AddonName)] then
                ...
                D4:After(2, function()
                    ...
                    D4:After(id * 0.1, function()
                        local ver = D4:GetVersion()
                        if ver == nil then D4:MSG(AddonName, 0, "|cffff0000MISSING VERSION", AddonName) end
                        if ver then
                            local chatType
                            ...
                            if chatType then C_ChatInfo.SendAddonMessage(pre, string.format("A;%s;V;%s", AddonName, ver), chatType) end
                        end
