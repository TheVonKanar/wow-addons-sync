...
function MoveAny:InitMAVehicleSeatIndicator()
	...
	if VehicleSeatIndicator and MoveAny:IsEnabled("VEHICLESEATINDICATOR", false) then
		...
		hooksecurefunc(MAVehicleSeatIndicator, "SetScale", function(sel, scale)
			...
			if scale and type(scale) == "number" then VehicleSeatIndicator:SetScale(scale) end
		end)

		hooksecurefunc(MAVehicleSeatIndicator, "SetAlpha", function(sel, alpha) VehicleSeatIndicator:SetAlpha(alpha) end)
