...
-- We check desaturation and alpha since Blizzard uses these to show cooldown state
local function GetIconVisualState(icon)
    if not icon then return true end  -- Default to "ready" if no icon
    
    local isReady = true
    
    -- Check icon texture desaturation
    pcall(function()
    ...
    -- Check alpha (icons on cooldown often have reduced alpha)
    pcall(function()
        if icon:GetAlpha() < 0.5 then
            isReady = false
        end
    end)
...
local function GetSlotInfo(trackerKey, slotIndex)
    local icons = CollectIcons(trackerKey)
...
local function UpdateHighlightFrame(trackerKey, slotIndex)
    ...
    -- Method 2: C_Spell.GetSpellCooldownDuration with spellID (if it exists)
    if not cooldownApplied and spellID and C_Spell then
    ...
    -- Method 3: Use C_Spell.GetSpellCooldown and extract start/duration (legacy approach)
    ...
    -- Method 4: Direct copy from source cooldown using GetCooldownTimes (pre-Midnight fallback)
    if not cooldownApplied and sourceCooldown and sourceCooldown.GetCooldownTimes then
        pcall(function()
            local start, duration = sourceCooldown:GetCooldownTimes()
            if start and duration then
                -- GetCooldownTimes returns milliseconds, SetCooldown expects seconds
