...
-- Spell Cooldown Duration Object APIs (Midnight Beta 4+)
local HAS_SPELL_COOLDOWN_DURATION = C_Spell and C_Spell.GetSpellCooldownDuration ~= nil
local HAS_SPELL_CHARGES_DURATION = C_Spell and C_Spell.GetSpellChargesCooldownDuration ~= nil
...
local CUSTOM_TRACKER_UPDATE_INTERVAL = 0.05  -- Update cooldowns 20x per second
...
local function ApplyGridLayout(viewer, trackerKey)
    ...
    if useCustomLayout and #customRowSizes > 0 then
        ...
        local function StyleIcon(icon)
            ...
            elseif icon._TUI_TooltipDisabled then
        ...
        if maxRows > 0 and row >= maxRows then
            ...
            if alignment == "CENTER" or alignment == "RIGHT" then
                ...
                elseif primaryIsHorizontal then
                    ...
                    -- Vertical primary: calculate how many columns have items in this row
                    -- With n icons filling rows first: 
                    -- - fullColumns = floor(n / effectiveRows) are completely filled
                    -- - remainder = n % effectiveRows rows have one extra icon
                    local effectiveRows = numRows > 0 and numRows or math.ceil(#icons / columns)
                    local fullColumns = math.floor(#icons / effectiveRows)
                    local remainder = #icons % effectiveRows
                    if row < remainder then
                        iconsInThisRow = fullColumns + 1
                    else
                        iconsInThisRow = fullColumns
                    end
                    iconsInThisRow = math.max(0, math.min(iconsInThisRow, columns))
                end
            ...
            elseif icon._TUI_TooltipDisabled then
                -- Re-enable tooltips: restore original OnEnter
                if icon._TUI_OriginalOnEnter then
                    icon:SetScript("OnEnter", icon._TUI_OriginalOnEnter)
                end
                icon._TUI_TooltipDisabled = false
...
local BUFF_UPDATE_INTERVAL = 0.05  -- Check buff states 20 times per second
...
local function UpdateCustomTrackerCooldown(iconFrame)
    ...
    elseif trackType == "spell" then
        -- =====================================================================
        -- SPELL COOLDOWN (v1.2.5 - Duration Object priority)
        -- =====================================================================
        local cooldownSet = false
        
        -- Method 1: Duration Object API (Midnight Beta 4+)
        if HAS_SPELL_COOLDOWN_DURATION then
        ...
        -- Method 2: Traditional API (fallback)
...
function Cooldowns:CreateTrackerPanel(trackerKey)
    ...
    local function BuildHighlightsTab(parent)
        ...
        -- Controls panel
        local controlsPanel = CreateFrame("Frame", nil, parent, "BackdropTemplate")
        controlsPanel:SetPoint("TOPLEFT", 5, y)
        controlsPanel:SetSize(PANEL_WIDTH - 60, 330)
        ...
        -- Position button
        controls.positionBtn = CreateFrame("Button", nil, controlsPanel, "UIPanelButtonTemplate")
        controls.positionBtn:SetPoint("TOPLEFT", 10, -270)
        controls.positionBtn:SetSize(130, 24)
        controls.positionBtn:SetText("Open Layout Mode")
        controls.positionBtn:Hide()
        ...
        local function UpdateControlsForSlot(slotIndex)
            ...
            -- Get slot info for icon preview
            local viewer = _G["BuffIconCooldownViewer"]
            if viewer then
                local icons = CollectIcons(viewer)
            ...
            controls.positionBtn:SetScript("OnClick", function()
        ...
        refreshBtn:SetPoint("TOPLEFT", 5, y)
    ...
    local function BuildCooldownHighlightsTab(parent)
        ...
        local controlsPanel = CreateFrame("Frame", nil, parent, "BackdropTemplate")
        controlsPanel:SetPoint("TOPLEFT", 5, y)
        controlsPanel:SetSize(PANEL_WIDTH - 60, 480)
        controlsPanel:SetBackdrop({
            bgFile = "Interface\\Buttons\\WHITE8x8",
            edgeFile = "Interface\\Buttons\\WHITE8x8",
            edgeSize = 1,
        ...
        local function UpdateControlsForSlot(slotIndex)
            ...
            -- Get slot info for icon preview
            local slotInfo = CooldownHighlights:GetSlotInfo(trackerKey, slotIndex)
            if slotInfo and slotInfo.texture then
                controls.iconPreview:SetTexture(slotInfo.texture)
            end
...
function Cooldowns:CreateCustomTrackersPanel()
    ...
    local function BuildPerIconTab(parent)
        ...
        local function UpdateControlsForSlot(slotIndex)
            ...
            local slotInfo = CooldownHighlights:GetSlotInfo(customTrackerKey, slotIndex)
            if slotInfo and slotInfo.texture then
                controls.iconPreview:SetTexture(slotInfo.texture)
            end
