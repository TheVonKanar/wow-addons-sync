...
-- Minimap skins (Leatrix Plus's square map for one) hide the compass ring,
-- and a hidden ring stops turning, so in the delve it is kept shown but
-- fully transparent while the facing is wanted, and put back on the way out
local compassLent

local function lendCompass()
    if compassLent or MinimapCompassTexture:IsShown() or not AZT.InDelve() then
        return
    end
    compassLent = MinimapCompassTexture:GetAlpha()
...
local function returnCompass()
    if compassLent then
        ...
        MinimapCompassTexture:SetAlpha(compassLent)
        compassLent = nil
    end
end

function Safe.RingFacing()
    if GetCVar("rotateMinimap") ~= "1" or not MinimapCompassTexture then
        return nil
    end
    lendCompass()
    ...
    elseif rot ~= ringRot then
        ringRot, ringSince, ringLive = rot, now, true
    elseif not ringLive and now - ringSince > RING_PROVE then
        if not ringWarned and AZT.InDelve() then
            ringWarned = true
            AZT.chat("your minimap compass ring is not turning, so nothing here can tell which way you face")
            AZT.chat(
                "a minimap addon or skin that replaces the compass is the usual cause, "
                    .. "try turning its minimap module off"
            )
            AZT.chat("or set Recording to By hand in the options, or /azt manual, and answer each wave yourself")
        end
        return nil
    end
    return -rot % TAU
...
-- autoRotate remembers they left in automatic with rotation on, and the next
-- entry turns it on again without asking. rotateLent lives in the saved
-- variables so a reload or a crash inside the delve still puts it back
function AZT.LendRotation()
    ...
    AztarecHelperDB.autoRotate = true
end

function Safe.RotateSync()
    if AZT.InDelve() then
        if Safe.IsAuto() and AztarecHelperDB.autoRotate then
            AZT.LendRotation()
        end
        return
    end
    returnCompass()
    -- decided before the return, since a lent rotation is on right now too
    AztarecHelperDB.autoRotate = Safe.IsAuto() and GetCVar("rotateMinimap") == "1"
...
function AZT.SetManualMode(v)
    ...
    elseif GetCVar("rotateMinimap") ~= "1" then
        -- automatic with a still minimap records nothing, so the choice
        -- comes with the fix on offer
        AZT.chat("automatic recording is on, but it needs your minimap set to rotate to work")
        AZT.ShowRotateOffer()
    else
