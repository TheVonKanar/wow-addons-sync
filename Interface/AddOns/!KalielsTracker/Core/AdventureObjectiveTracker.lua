...
KT_AdventureObjectiveTrackerMixin = CreateFromMixins(KT_ObjectiveTrackerModuleMixin, settings);

function KT_AdventureObjectiveTrackerMixin:InitModule()
	-- POIButtonOwnerTemplate
	self:Init();
end

function KT_AdventureObjectiveTrackerMixin:OnEvent(event, ...)
...
function KT_AdventureObjectiveTrackerMixin:OnBlockHeaderClick(block, mouseButton)
...
function KT_AdventureObjectiveTrackerMixin:OnBlockHeaderEnter(block)
	if block.trackableType == Enum.ContentTrackingType.Appearance then
...
function KT_AdventureObjectiveTrackerMixin:OnBlockHeaderLeave(block)
...
function KT_AdventureObjectiveTrackerMixin:GetDebugReportInfo(block)
	return { debugType = "AdventureTracked", trackableType = block.trackableType, id = block.trackableID, };
end

function KT_AdventureObjectiveTrackerMixin:ClickProfessionTarget(recipeID)
...
function KT_AdventureObjectiveTrackerMixin:OpenToAppearance(appearanceID)
	TransmogUtil.OpenCollectionToItem(appearanceID);
end

function KT_AdventureObjectiveTrackerMixin:Untrack(trackableType, id)
	C_ContentTracking.StopTracking(trackableType, id, Enum.ContentTrackingStopType.Manual);
end

function KT_AdventureObjectiveTrackerMixin:ProcessTrackingEntry(trackableType, trackableID)
	...
	if targetType then
		...
		if objectiveText then
			block.objective = block:AddObjective(1, objectiveText, LINE_TYPE_ANIM, true, KT_OBJECTIVE_DASH_STYLE_SHOW, KT_OBJECTIVE_TRACKER_COLOR["Normal"]);
		else
			block.objective = block:AddObjective(1, CONTENT_TRACKING_RETRIEVING_INFO, LINE_TYPE_ANIM, nil, KT_OBJECTIVE_DASH_STYLE_HIDE, KT_OBJECTIVE_TRACKER_COLOR["Normal"]);
		end
		...
		if NavigableContentTrackingTargets[targetType] then
			...
			if objectiveText and (trackingResult ~= Enum.ContentTrackingResult.DataPending) then
				if not block.endLocationUIMap then
					block:AddObjective(2, CONTENT_TRACKING_LOCATION_UNAVAILABLE, nil, nil, KT_OBJECTIVE_DASH_STYLE_HIDE, KT_OBJECTIVE_TRACKER_COLOR["Normal"]);
				else
					...
					if (navigableTrackingResult == Enum.ContentTrackingResult.Failure) or
						...
						block:AddObjective(2, CONTENT_TRACKING_ROUTE_UNAVAILABLE, nil, nil, KT_OBJECTIVE_DASH_STYLE_HIDE, KT_OBJECTIVE_TRACKER_COLOR["Normal"]);
					else
						...
						if (trackableType == superTrackedType) and (trackableID == superTrackedID) then
							...
							if waypointText then
								...
								block:AddObjective(2, formattedText, nil, nil, KT_OBJECTIVE_DASH_STYLE_SHOW, KT_OBJECTIVE_TRACKER_COLOR["Normal"]);
							end
						end
					end
				end
...
function KT_AdventureObjectiveTrackerMixin:OnFreeBlock(block)
...
function KT_AdventureObjectiveTrackerMixin:EnumerateTrackables(callback)
...
function KT_AdventureObjectiveTrackerMixin:StopTrackingCollectedItems()
...
function KT_AdventureObjectiveTrackerMixin:OnTrackableItemCollected(trackableType, trackableID)
	...
	if block then
		if block.objective then
			...
			block.objective:SetState(KT_ObjectiveTrackerAnimLineState.Completing);
		end
...
function KT_AdventureObjectiveTrackerMixin:OnShowRewardsToastDone(block)
	self:RemoveBlockFromCache(block);
end

function KT_AdventureObjectiveTrackerMixin:LayoutContents()
	-- POIButtonOwnerTemplate
	self:ResetUsage();
