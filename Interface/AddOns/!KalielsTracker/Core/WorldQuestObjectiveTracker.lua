...
local settings = {
	...
	events = { "QUEST_TURNED_IN", "QUEST_LOG_UPDATE", "QUEST_WATCH_LIST_CHANGED", "QUEST_ACCEPTED", "SUPER_TRACKING_CHANGED" },  -- MSA
	-- for this module
...
function KT_WorldQuestObjectiveTrackerMixin:TryAddingExpirationWarningLine(block, questID)
	if QuestUtils_ShouldDisplayExpirationWarning(questID) then
		...
		local line = block:AddObjective("TimeLeft", text, nil, nil, KT_OBJECTIVE_DASH_STYLE_HIDE, KT_OBJECTIVE_TRACKER_COLOR["TimeLeft"], true);
		line.Icon:Hide();
	end
end
...
function KT_WorldQuestObjectiveTrackerMixin:GetSortedWorldQuests()
	local sortedQuests = {};
...
function KT_WorldQuestObjectiveTrackerMixin:LayoutContents()
	if self.ticker then
		self.ticker:Cancel();
		self.ticker = nil;
	end
