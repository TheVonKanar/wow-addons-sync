...
KT_AchievementObjectiveTrackerMixin = CreateFromMixins(KT_ObjectiveTrackerModuleMixin, settings);

local ARENA_CATEGORY = 165;
local MAX_CRITERIA_PER_ACHIEVEMENT = 5;
local TIMED_CRITERIA = { };

function KT_AchievementObjectiveTrackerMixin:OnEvent(event, ...)
...
function KT_AchievementObjectiveTrackerMixin:OnBlockHeaderClick(block, mouseButton)
...
function KT_AchievementObjectiveTrackerMixin:UntrackAchievement(achievementID)
...
function KT_AchievementObjectiveTrackerMixin:LayoutContents()
...
function KT_AchievementObjectiveTrackerMixin:AddAchievement(achievementID, achievementName, description)
	...
	if numCriteria > 0 then
		...
		for criteriaIndex = 1, numCriteria do
			...
			elseif numShownCriteria == MAX_CRITERIA_PER_ACHIEVEMENT and numCriteria > (MAX_CRITERIA_PER_ACHIEVEMENT + 1) then
				...
				block:AddObjective("Extra", "...", nil, nil, KT_OBJECTIVE_DASH_STYLE_HIDE);
