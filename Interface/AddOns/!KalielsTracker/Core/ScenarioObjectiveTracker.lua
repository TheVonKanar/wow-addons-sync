...
local function AcknowledgeEmberCourtHelpTip()
	if showingEmberCourtHelpTip then
		...
		EventRegistry:UnregisterCallback("WorldMapOnShow", KT_ScenarioObjectiveTracker);
	end
end
...
local function CheckEmberCourtHelpTip(widgetSetID)
	if widgetSetID == EMBER_COURT_TUTORIAL_WIDGET_SET_ID then
		if HelpTip:Show(UIParent, emberCourtMapHelpTipInfo) then
			...
			EventRegistry:RegisterCallback("WorldMapOnShow", AcknowledgeEmberCourtHelpTip, KT_ScenarioObjectiveTracker);
		end
	else
...
local settings = {
	...
	progressBarTemplate = "KT_ScenarioProgressBarTemplate",
	-- for this module
...
KT_ScenarioObjectiveTrackerMixin = CreateFromMixins(KT_ObjectiveTrackerModuleMixin, settings);

function KT_ScenarioObjectiveTrackerMixin:InitModule()
...
function KT_ScenarioObjectiveTrackerMixin:OnEvent(event, ...)
	if event == "UNIT_AURA" then
		...
		if KT.Safe.ShouldShowMawBuffs() ~= isShowingMawBuffs then  -- MSA
			self:MarkDirty();
		end
...
function KT_ScenarioObjectiveTrackerMixin:ShouldShowCriteria()
	return self.shouldShowCriteria;
end

function KT_ScenarioObjectiveTrackerMixin:SetShouldShowCriteria(shouldShow)
...
function KT_ScenarioObjectiveTrackerMixin:CanUpdate()
...
function KT_ScenarioObjectiveTrackerMixin:SetHasNewStage(hasNewStage)
	self.hasNewStage = hasNewStage;
end

-- override
function KT_ScenarioObjectiveTrackerMixin:MarkBlocksUnused()
	for i, block in ipairs(self.FixedBlocks) do
		block.used = false;
	end
...
-- override
function KT_ScenarioObjectiveTrackerMixin:FreeUnusedBlocks()
	for i, block in ipairs(self.FixedBlocks) do
...
function KT_ScenarioObjectiveTrackerMixin:LayoutContents()
	...
	local isInScenario = numStages > 0;
	local shouldShowMawBuffs = KT.Safe.ShouldShowMawBuffs();  -- MSA
	...
	if slidingState == KT_ObjectiveTrackerSlidingState.SlideOut then
	...
	if slidingState == KT_ObjectiveTrackerSlidingState.SlideIn and self:IsShown() then
		stageBlock:SetupStageTransition(hasNewStage, scenarioCompleted);
		self:SlideInContents();
	end
end

function KT_ScenarioObjectiveTrackerMixin:LayoutWidgetBlock(block)
...
function KT_ScenarioObjectiveTrackerMixin:SlideInContents()
...
function KT_ScenarioObjectiveTrackerMixin:UpdateCriteria(numCriteria)
	...
	for criteriaIndex = 1, numCriteria do
		...
		if criteriaInfo then
			...
			if criteriaInfo.completed then
				...
				line = objectivesBlock:AddObjective(criteriaIndex, criteriaString, nil, nil, KT_OBJECTIVE_DASH_STYLE_HIDE, KT_OBJECTIVE_TRACKER_COLOR["Complete"]);
				line.Icon:Show();
				...
				line = objectivesBlock:AddObjective(criteriaIndex, criteriaString, nil, nil, KT_OBJECTIVE_DASH_STYLE_HIDE);
				line.Icon:Show();
...
function KT_ScenarioObjectiveTrackerMixin:AddWeightedProgressObjective(stageDescription)
	if not self:ShouldShowCriteria() then
		return;
	end
...
function KT_ScenarioObjectiveTrackerMixin:AddSpells(allSpellInfo)
...
function KT_ScenarioObjectiveTrackerMixin:UpdateSpellCooldowns()
...
KT_ScenarioObjectiveTrackerStageMixin = { };

function KT_ScenarioObjectiveTrackerStageMixin:OnEnter()
...
function KT_ScenarioObjectiveTrackerStageMixin:GetBGAtlases(scenarioType, textureKit)
...
function KT_ScenarioObjectiveTrackerStageMixin:UpdateFindGroupButton(scenarioID)
	local hasButton = C_LFGList.CanCreateScenarioGroup(scenarioID);
	if hasButton then
		if not self.findGroupButton then
			self.findGroupButton = CreateFrame("BUTTON", nil, self, "KT_ScenarioObjectiveTrackerFindGroupButtonTemplate");
...
function KT_ScenarioObjectiveTrackerStageMixin:SetupStageTransition(hasNewStage, scenarioCompleted)
...
KT_ScenarioTimerMixin = { };

function KT_ScenarioTimerMixin:OnLoad()
...
function KT_ScenarioTimerMixin:OnEvent(event, ...)
...
function KT_ScenarioTimerMixin:StopTimer(timerID)
...
function KT_ScenarioTimerMixin:CheckTimers(...)
	...
	for i = 1, select("#", ...) do
		...
		if type == Enum.WorldElapsedTimerTypes.ChallengeMode then
			...
			if mapID then
				...
				KT_ScenarioObjectiveTracker.ChallengeModeBlock:Activate(timerID, elapsedTime, timeLimit);
				return;
			end
		elseif type == Enum.WorldElapsedTimerTypes.ProvingGround then
			local diffID, currWave, maxWave, duration = C_Scenario.GetProvingGroundsInfo()
			if duration > 0 then
				KT_ScenarioObjectiveTracker.ProvingGroundsBlock:Activate(timerID, elapsedTime, duration, diffID, currWave, maxWave);
				return;
			end
		end
	end
...
KT_ScenarioObjectiveTrackerChallengeModeMixin = { };

function KT_ScenarioObjectiveTrackerChallengeModeMixin:OnLoad()
	...
	self.affixPool = CreateFramePool("FRAME", self, "KT_ScenarioChallengeModeAffixTemplate");
end

function KT_ScenarioObjectiveTrackerChallengeModeMixin:OnEvent(event)
	if event == "CHALLENGE_MODE_DEATH_COUNT_UPDATED" then
		self:UpdateDeathCount();
	end
end

function KT_ScenarioObjectiveTrackerChallengeModeMixin:UpdateTime(elapsedTime)
...
function KT_ScenarioObjectiveTrackerChallengeModeMixin:IsActive()
	return not not self.timerID;
end

function KT_ScenarioObjectiveTrackerChallengeModeMixin:Activate(timerID, elapsedTime, timeLimit)
...
function KT_ScenarioObjectiveTrackerChallengeModeMixin:SetUpAffixes(affixes)
...
KT_ScenarioObjectiveTrackerProvingGroundsMixin = { };

function KT_ScenarioObjectiveTrackerProvingGroundsMixin:OnLoad()
...
function KT_ScenarioObjectiveTrackerProvingGroundsMixin:IsActive()
	return not not self.timerID;
end

local PROVING_GROUNDS_ENDLESS_INDEX = 4;
function KT_ScenarioObjectiveTrackerProvingGroundsMixin:Activate(timerID, elapsedTime, duration, medalIndex, currWave, maxWave)
	local statusBar = self.StatusBar;
...
function KT_ScenarioObjectiveTrackerProvingGroundsMixin:OnAnimFinished()
...
KT_ScenarioRewardsFrameMixin = { };

function KT_ScenarioRewardsFrameMixin:OnLoad()
	self:SetScale(0.9);
	self.Anim:SetScript("OnFinished", GenerateClosure(self.OnAnimFinished, self));
end

function KT_ScenarioRewardsFrameMixin:AddReward(label, texture, font)
...
function KT_ScenarioRewardsFrameMixin:DisplayRewards(xp, money)
...
function KT_ScenarioRewardsFrameMixin:OnAnimFinished()
	self:Hide();
end
...
KT_ScenarioTrackerProgressBarMixin = { };

function KT_ScenarioTrackerProgressBarMixin:OnGet(isNew, criteriaIndex)
	self.Bar.Icon:Hide();
...
function KT_ScenarioTrackerProgressBarMixin:SetValue(percentage)
	self.Bar:SetValue(percentage);
	self.Bar.Label:SetFormattedText(PERCENTAGE_STRING, percentage);
	self.percentage = percentage;
end

function KT_ScenarioTrackerProgressBarMixin:PlayFlareAnim(oldPercentage)
...
KT_ScenarioSpellButtonMixin = { };

function KT_ScenarioSpellButtonMixin:OnEnter()
	GameTooltip:SetOwner(self, "ANCHOR_RIGHT");
	GameTooltip:SetSpellByID(self.spellID);
end

function KT_ScenarioSpellButtonMixin:OnClick()
	CastSpellByID(self.spellID);
end

function KT_ScenarioSpellButtonMixin:SetSpell(spellInfo)
...
KT_ScenarioObjectiveTrackerFindGroupButtonMixin = { };

function KT_ScenarioObjectiveTrackerFindGroupButtonMixin:OnMouseDown()
...
function KT_ScenarioObjectiveTrackerFindGroupButtonMixin:OnMouseUp()
	if self:IsEnabled() then
...
function KT_ScenarioObjectiveTrackerFindGroupButtonMixin:OnEnter()
...
function KT_ScenarioObjectiveTrackerFindGroupButtonMixin:OnLeave()
	GameTooltip:Hide();
end

function KT_ScenarioObjectiveTrackerFindGroupButtonMixin:OnClick()
	local shouldShowCreateGroupButton = true;
	LFGListUtil_FindScenarioGroup(self.scenarioID, shouldShowCreateGroupButton);
end

function KT_ScenarioObjectiveTrackerFindGroupButtonMixin:SetScenarioID(scenarioID)
	self.scenarioID = scenarioID;
end