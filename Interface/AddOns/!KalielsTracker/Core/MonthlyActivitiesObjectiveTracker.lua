...
KT_MonthlyActivitiesObjectiveTrackerMixin = CreateFromMixins(KT_ObjectiveTrackerModuleMixin, settings);

function KT_MonthlyActivitiesObjectiveTrackerMixin:OnEvent(event, ...)
...
function KT_MonthlyActivitiesObjectiveTrackerMixin:OnBlockHeaderClick(block, mouseButton)
...
function KT_MonthlyActivitiesObjectiveTrackerMixin:OpenFrameToActivity(activityID)
	if not EncounterJournal then
		EncounterJournal_LoadUI();
	end
...
function KT_MonthlyActivitiesObjectiveTrackerMixin:UntrackPerksActivity(activityID)
	C_PerksActivities.RemoveTrackedPerksActivity(activityID);
end

function KT_MonthlyActivitiesObjectiveTrackerMixin:LayoutContents()
	local trackedActivities = C_PerksActivities.GetTrackedPerksActivities().trackedIDs;
...
function KT_MonthlyActivitiesObjectiveTrackerMixin:AddActivity(activityInfo)
