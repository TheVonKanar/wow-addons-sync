...
local settings = {
	...
	-- settings
	blockTemplate = "KT2_ObjectiveTrackerBlockTemplate",  -- MSA
	lineTemplate = "KT_ObjectiveTrackerLineTemplate",
	progressBarTemplate = "KT_ObjectiveTrackerProgressBarTemplate",
	...
	-- dynamic
	state = KT_ObjectiveTrackerModuleState.Skipped,
...
KT_ObjectiveTrackerModuleMixin = CreateFromMixins(KT_ObjectiveTrackerSlidingMixin, settings);

function KT_ObjectiveTrackerModuleMixin:OnLoad()
...
function KT_ObjectiveTrackerModuleMixin:OnEvent(event, ...)
	-- override in your mixin
end

function KT_ObjectiveTrackerModuleMixin:OnHide()
	self:EndSlide();
end

function KT_ObjectiveTrackerModuleMixin:SetContainer(container)
...
function KT_ObjectiveTrackerModuleMixin:InitModule()
...
function KT_ObjectiveTrackerModuleMixin:IsDirty()
	return self.isDirty;
end

function KT_ObjectiveTrackerModuleMixin:HasContents()
	return self.hasContents;
end

function KT_ObjectiveTrackerModuleMixin:IsDisplayable()
	return self.state >= KT_ObjectiveTrackerModuleState.ShownPartially;
end

function KT_ObjectiveTrackerModuleMixin:IsFullyDisplayable()
	return self.state == KT_ObjectiveTrackerModuleState.ShownFully;
end

function KT_ObjectiveTrackerModuleMixin:IsComplete()
	return self.state == KT_ObjectiveTrackerModuleState.NoObjectives or self:IsFullyDisplayable();
end

function KT_ObjectiveTrackerModuleMixin:IsTruncated()
	return self.state == KT_ObjectiveTrackerModuleState.NotShown or self.state == KT_ObjectiveTrackerModuleState.ShownPartially;
end

function KT_ObjectiveTrackerModuleMixin:GetContentsHeight()
	return self:IsDisplayable() and self.contentsHeight or 0;
end

function KT_ObjectiveTrackerModuleMixin:SetHeader(text)
	...
	if availableHeight > 0 then
		...
		if self.hasContents then
			if self.hasSkippedBlocks and not self:IsCollapsed() then
				self.state = KT_ObjectiveTrackerModuleState.ShownPartially;
			else
				self.state = KT_ObjectiveTrackerModuleState.ShownFully;
			end
		else
			if self.hasTriedBlocks then
				self.state = KT_ObjectiveTrackerModuleState.NotShown;
			else
				self.state = KT_ObjectiveTrackerModuleState.NoObjectives;
			end
		end
	end
...
function KT_ObjectiveTrackerModuleMixin:BeginLayout()
	...
	self.state = KT_ObjectiveTrackerModuleState.Skipped;
end

function KT_ObjectiveTrackerModuleMixin:CanUpdate()
	-- override in your mixin
	return true;
end

function KT_ObjectiveTrackerModuleMixin:LayoutContents()
	-- override in your mixin
	-- must return soon as a LayoutBlock fails
end

function KT_ObjectiveTrackerModuleMixin:EndLayout()
...
function KT_ObjectiveTrackerModuleMixin:HasSkippedBlocks()
	return self.hasSkippedBlocks;
end

function KT_ObjectiveTrackerModuleMixin:UpdateHeight()
...
function KT_ObjectiveTrackerModuleMixin:SetHeightModifier(key, height)
...
function KT_ObjectiveTrackerModuleMixin:ClearHeightModifier(key)
	if self.heightModifiers then
...
function KT_ObjectiveTrackerModuleMixin:GetBlock(id, optTemplate)
...
function KT_ObjectiveTrackerModuleMixin:MarkBlocksUnused()
	for template, blocks in pairs(self.usedBlocks) do
		for blockID, block in pairs(blocks) do
			block.used = nil;
		end
...
function KT_ObjectiveTrackerModuleMixin:FreeUnusedBlocks()
	for template, blocks in pairs(self.usedBlocks) do
		for blockID, block in pairs(blocks) do
...
function KT_ObjectiveTrackerModuleMixin:FreeBlock(block)
...
function KT_ObjectiveTrackerModuleMixin:OnFreeBlock(block)
	-- override in your mixin
end

function KT_ObjectiveTrackerModuleMixin:ForceRemoveBlock(block)
	block.used = nil;
	self:FreeBlock(block);
	self:MarkDirty();
end
...
function KT_ObjectiveTrackerModuleMixin:GetNextBlockAnchoring()
...
function KT_ObjectiveTrackerModuleMixin:LayoutBlock(block)
...
function KT_ObjectiveTrackerModuleMixin:AddBlock(block)
...
function KT_ObjectiveTrackerModuleMixin:CanFitBlock(block)
...
function KT_ObjectiveTrackerModuleMixin:InternalAddBlock(block)
...
function KT_ObjectiveTrackerModuleMixin:AnchorBlock(block)
...
function KT_ObjectiveTrackerModuleMixin:GetLastBlock()
	return self.lastBlock or self.Header;
end

function KT_ObjectiveTrackerModuleMixin:OnBlockHeaderClick(block, mouseButton)
	-- override in your mixin
end
...
function KT_ObjectiveTrackerModuleMixin:ToggleCollapsed()
	self:SetCollapsed(not self:IsCollapsed());
end

function KT_ObjectiveTrackerModuleMixin:SetCollapsed(collapsed)
...
function KT_ObjectiveTrackerModuleMixin:IsCollapsed()
...
function KT_ObjectiveTrackerModuleMixin:GetTimerBar(key)
	...
	if not timerBar then
		timerBar = self:AcquireFrame("KT_ObjectiveTrackerTimerBarTemplate");
...
function KT_ObjectiveTrackerModuleMixin:MarkTimerBarsUnused()
...
function KT_ObjectiveTrackerModuleMixin:FreeUnusedTimerBars()
	if self.usedTimerBars then
		for key, timerBar in pairs(self.usedTimerBars) do
...
function KT_ObjectiveTrackerModuleMixin:GetProgressBar(key, ...)
...
function KT_ObjectiveTrackerModuleMixin:MarkProgressBarsUnused()
...
function KT_ObjectiveTrackerModuleMixin:FreeUnusedProgressBars()
	if self.usedProgressBars then
		for key, progressBar in pairs(self.usedProgressBars) do
...
function KT_ObjectiveTrackerModuleMixin:GetRightEdgeFrame(settings, identifier)
...
function KT_ObjectiveTrackerModuleMixin:MarkRightEdgeFramesUnused()
...
function KT_ObjectiveTrackerModuleMixin:FreeUnusedRightEdgeFrames()
	if self.usedRightEdgeFrames then
		for key, frame in pairs(self.usedRightEdgeFrames) do
...
function KT_ObjectiveTrackerModuleMixin:AdjustSlideAnchor(offsetY)
...
function KT_ObjectiveTrackerModuleMixin:SetNeedsFanfare(key)
	if key then
...
function KT_ObjectiveTrackerModuleMixin:NeedsFanfare(key)
	return self.fanfares and self.fanfares[key];
end

function KT_ObjectiveTrackerModuleMixin:ClearFanfares()
	self.fanfares = nil;
end

function KT_ObjectiveTrackerModuleMixin:ForceExpand()
	if self:IsCollapsed() then
...
function KT_ObjectiveTrackerModuleMixin:AddBlockToCache(block)
	if not block.cached then
		block.cached = true;
...
function KT_ObjectiveTrackerModuleMixin:RemoveBlockFromCache(block, fromFreeBlock)
...
function KT_ObjectiveTrackerModuleMixin:UpdateCachedOrderList()
...
function KT_ObjectiveTrackerModuleMixin:CheckCachedBlocks(upcomingBlock)
...
KT_ObjectiveTrackerModuleHeaderMixin = {};

function KT_ObjectiveTrackerModuleHeaderMixin:OnLoad()
	self.MinimizeButton:SetScript("OnClick", GenerateClosure(self.OnToggle, self));
	local collapsed = false;
	self:SetCollapsed(collapsed);
end

function KT_ObjectiveTrackerModuleHeaderMixin:PlayAddAnimation()
	self.AddAnim:Restart();
end

function KT_ObjectiveTrackerModuleHeaderMixin:OnToggle()
...
function KT_ObjectiveTrackerModuleHeaderMixin:SetCollapsed(collapsed)
