...
do
    local widgetType = "MSA_LSM30_Sound"
    ...
    -- MSA
    local function GetSoundChannel(widget)
        local channel = widget.userdata.option.soundChannel or "Master"
        return type(channel) == "function" and channel() or channel
    end

    local function ContentSpeakerOnClick(this, button)
        local self = this.frame.obj
        local sound = this.frame.text:GetText()
        PlaySoundFile(self.list[sound] ~= sound and self.list[sound] or Media:Fetch('sound',sound), GetSoundChannel(self))  -- MSA
    end
