...
function M.Open(pageKey)
    ...
    ApplyMenuFramePriority(f)
    f:Show()
    local targetPage = pageKey or M.lastPage or "home"
    if M.SelectPage(targetPage) == false and targetPage ~= "home" then
        M.SelectPage("home")
    end
    return true
end
...
_G.MSUF_OpenStandaloneOptionsWindow = function(pageKey) M.Open(pageKey) end
_G.MSUF_ShowStandaloneOptionsWindow = function(pageKey) M.Open(pageKey) end
_G.MSUF_HideStandaloneOptionsWindow = function()
    HideSlashMenuAndMinibar(M.frame)
end
_G.MSUF_OpenOptionsMenu = function() M.Open() end
