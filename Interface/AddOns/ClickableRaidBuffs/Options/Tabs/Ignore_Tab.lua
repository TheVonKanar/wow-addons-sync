...
local runeforgingName
do
  local deathGateID = 50977
  local runeforgeID = 53428

  local deathGateName
  local runeforgeName

  if C_Spell and C_Spell.GetSpellInfo then
    local dgInfo = C_Spell.GetSpellInfo(deathGateID)
    deathGateName = dgInfo and dgInfo.name
    if not dgInfo and C_Spell.RequestLoadSpellData then
      C_Spell.RequestLoadSpellData(deathGateID)
    end

    local rfInfo = C_Spell.GetSpellInfo(runeforgeID)
    runeforgeName = rfInfo and rfInfo.name
    if not rfInfo and C_Spell.RequestLoadSpellData then
      C_Spell.RequestLoadSpellData(runeforgeID)
    end
  end

  deathGateName = deathGateName or L["Death Gate"]
  runeforgeName = runeforgeName or L["Runeforging"]


  runeforgingName = deathGateName .. " (" .. runeforgeName .. ")"
end
...
local function GetItemNameLocalized(itemID, fallbackName)
  if not itemID then
  ...
  return fallbackName or ("Item " .. tostring(itemID))
end
  ...
  for id, v in pairs(src) do
    if type(id) == "number" then
      ...
      if not ns.IsExpansionEnabled or ns.IsExpansionEnabled(expansionId) then
        ...
        local fallbackName = (type(v) == "table" and v.name) or tostring(v)
        ...
        items[#items + 1] = {
          id = id,
          isItem = true,
          name = itemName or fallbackName,
          rank = (type(v) == "table" and v.rank) or nil,
          icon = select(10, C_Item.GetItemInfo(id)) or select(1, GetItemIconSafe(id)),
    ...
    if type(keyID) == "number" then
      ...
      if C_Spell and C_Spell.GetSpellInfo then
        ...
        name = (info and info.name)
          or (type(v) == "table" and (v.name or v.label))
          or tostring(spellID)
        if not info and C_Spell.RequestLoadSpellData then
          C_Spell.RequestLoadSpellData(spellID)
          requested = true
        end
      else
...
local function collectClassAbilitiesByClass()
  ...
  then
    for _, classKey in ipairs(root.RAID_BUFF_CLASS_ORDER) do
      ...
      if #items > 0 then
        ...
        local classID = classIDFromKey(classKey)
        local classInfo = C_CreatureInfo and C_CreatureInfo.GetClassInfo and C_CreatureInfo.GetClassInfo(classID)
        local localizedName = classInfo and classInfo.className

        tmp[#tmp + 1] = {
          cid = classID or 99,
          block = {
            label = localizedName or root.RAID_BUFF_CLASS_LABELS[classKey] or tostring(classKey),
            items = items
          }
        }
  ...
  elseif type(byClass) == "table" and next(byClass) then
    for classID = 1, 13 do
      ...
      if #items > 0 then
        tmp[#tmp + 1] =
          { cid = classID, block = { label = CLASS_NAMES[classID] or (L["Class"] .. " " .. tostring(classID)), items = items } }
      end
    end
  else
    ...
    if #flat > 0 then
      outBlocks[#outBlocks + 1] = { label = L["All Class Abilities"], items = flat }
    end
  ...
  if type(root.ALL_TRINKETS) == "table" and next(root.ALL_TRINKETS) then
    for id, info in pairs(root.ALL_TRINKETS) do
      ...
      if type(itemID) == "number" then
        ...
        out[#out + 1] = { id = itemID, isItem = true, name = (C_Item.GetItemInfo(itemID)) or ((type(info) == "table" and info.name) or ""), icon = icon }
      end
    end
    sortItems(out)
    return { { label = L["Trinkets"], items = out } }
  end
...
local function PreloadIgnoreTabItems()
  local seen = {}
  ...
  local function scanTable(tbl)
    ...
    for id, v in pairs(tbl) do
      if type(id) == "number" then
        request(id)
        if type(v) == "table" and type(v.itemID) == "number" then
          request(v.itemID)
        end
      end
    end
  ...
  for catKey in pairs(SPLIT_SOURCES) do
    scanTable(getCategorySource(catKey))
  end

  local root = _G.ClickableRaidData or {}

  if type(root.ALL_TRINKETS) == "table" then
    for id, info in pairs(root.ALL_TRINKETS) do
      local itemID = (type(info) == "table" and info.itemID) or id
      request(itemID)
    end
  ...
  for classID = 1, 13 do
    ...
    if type(tbl) == "table" then
      for id, info in pairs(tbl) do
        if type(id) == "number" then
          request(info and info.itemID or id)
        end
      end
    end
  end
...
O.RegisterSection(function(AddSection)
  AddSection(IGNORE, function(content, Row)
    ...
    local function makeHeader(y, text, itemsForBlock, activeKeyForList)
      ...
      local btnEnable = MakeActionButton(holder, L["Show All"])
      btnEnable:SetPoint("RIGHT", holder, "RIGHT", -2, 0)

      local btnIgnore = MakeActionButton(holder, L["Hide All"])
    ...
    local TAB_DEFS = {
      { key = "FOOD", label = L["Food"] },
      { key = "FOOD_HEARTY", label = L["Hearty Food"] },
      { key = "FLASK", label = L["Flasks"] },
      { key = "AUGMENT_RUNE", label = L["Augment Runes"] },
      { key = "WEAPON_ENCHANTS", label = L["Weapon Enchants"] },

      { key = "CLASS_ABILITIES", label = L["Class Abilities"] },
      { key = "PETS", label = L["Pets"] },
      --{ key="TRINKET_RB",      label="Trinkets" },
      { key = "UTILITY", label = L["Utility"] },
      { key = "COSMETICS", label = L["Cosmetics"] },
    ...
    local function collectForTab(tabKey)
      if tabKey == "FOOD" or tabKey == "FOOD_HEARTY" then
        ...
        for id, v in pairs(src) do
          if type(id) == "number" then
            local entry = {
              id = id,
              isItem = true,
              name = (C_Item.GetItemInfo(id)) or ((type(v) == "table" and v.name) or tostring(v)),
              rank = (type(v) == "table" and v.rank) or nil,
        ...
        sortItems(hearty)
        if tabKey == "FOOD" then
          return { { label = L["Food"], items = plain } }

        end
        if tabKey == "FOOD_HEARTY" then
          return { { label = L["Hearty Food"], items = hearty } }
      ...
      elseif tabKey == "FLASK" then
        ...
        for id, v in pairs(src) do
          if type(id) == "number" then
            local name = (C_Item.GetItemInfo(id)) or ((type(v) == "table" and v.name) or tostring(v))
            local rank = (type(v) == "table" and v.rank) or nil
            local icon = select(10, C_Item.GetItemInfo(id)) or select(1, GetItemIconSafe(id))
            local expansionId = (type(v) == "table" and tonumber(v.expansionId)) or nil
            if not ns.IsExpansionEnabled or ns.IsExpansionEnabled(expansionId) then
        ...
        local blocks = {}
        if #normal > 0 then
          blocks[#blocks + 1] = { label = L["Flasks"], items = normal }

        end
        if #fleeting > 0 then
          blocks[#blocks + 1] = { label = L["Fleeting Flasks"], items = fleeting }

        end
        return blocks
      elseif tabKey == "AUGMENT_RUNE" then
        local src = getCategorySource("AUGMENT_RUNE") or {}
        return { { label = L["Augment Runes"], items = flattenItems(src) } }
      ...
      elseif tabKey == "WEAPON_ENCHANTS" then
        ...
        return { { label = L["Weapon Enchants"], items = combined } }
      ...
      elseif tabKey == "PETS" then
          ...
          items = {
            {
              ...
              isItem = false,
              spellID = -9104,
              name = L["Pet is on Passive Alert"],
              icon = 132311,
              noTooltip = true,
            },
          },
        ...
        local hunterClassName
        local info = C_CreatureInfo and C_CreatureInfo.GetClassInfo and C_CreatureInfo.GetClassInfo(3)
        if info then
          hunterClassName = info.className
        end


        local hunterItems = {
          { id = -7001, isItem = false, name = PETS .. " (" .. hunterClassName .. ")", icon = 132161, tipSpellID = 883 },
          {
            id = 982,
            isItem = false,
            spellID = 982,
        ...
        blocks[#blocks + 1] = { label = L["Hunter"], items = hunterItems }
        ...
        do
          ...
          sortItems(dkItems)
          blocks[#blocks + 1] = { label = L["Death Knight"], items = dkItems }
        ...
        sortItems(wlItems)
        blocks[#blocks + 1] = { label = L["Warlock"], items = wlItems }
      ...
      elseif tabKey == "UTILITY" then
        ...
        local items = {
          ...
          { id = -9103, isItem = false, name = runeforgingName, icon = 135766, spellID = 50977 },
          --{ id = 1231411, isItem = false, name = "Recuperate", icon = 136074, spellID = 1231411 },
        }
        sortItems(items)
        return { { label = L["Utility"], items = items } }
      ...
      elseif tabKey == "COSMETICS" then
        ...
        return { { label = L["Cosmetics"], items = items } }
        ...
        for i = 1, #blocks do
          ...
          if #midnight > 0 then
            midnightBlocks[#midnightBlocks + 1] = {
              label = L["Midnight"] .. " " .. (block.label or ""),
          ...
          if #tww > 0 then
            twwBlocks[#twwBlocks + 1] = {
              label = L["The War Within"] .. " " .. (block.label or ""),
