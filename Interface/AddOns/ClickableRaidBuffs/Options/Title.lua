...
O.RegisterSection(function(AddSection)
  AddSection("", function(content, Row)
    ...
    do
      ...
      title:SetText(L["|cff00ccffClickable Raid Buffs|r"])
      title:SetTextColor(0.92, 0.94, 1, 1)
    end

    local author = canvas:CreateFontString(nil, "ARTWORK", "GameFontHighlight")
    do
      local anchor = anchorFrameByTag(
        CFG.Author.anchorTo,
        { box = box, canvas = canvas, title = title }
      )
    ...
    do
      local anchor = anchorFrameByTag(CFG.Version.anchorTo, { box = box, canvas = canvas, title = title, author = author, logo = logo })
      version:SetText(GAME_VERSION_LABEL ..":  |cFF00ccff" .. ns.VERSION .. "|r")
      version:SetFont(O.GetFontPathByName(CFG.Version.fontName), CFG.Version.fontSize, "")
      version:SetPoint(CFG.Version.point, anchor, CFG.Version.relPoint, CFG.Version.x, CFG.Version.y)
    ...
    local col1 = MakeCopyColumn(inputs, L["Website"], "https://funki.gg")
    local col2 =
      MakeCopyColumn(inputs, L["Report Bugs and Get Support"], "https://funki.gg/discord")
