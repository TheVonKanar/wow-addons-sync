...
local function ConfirmReset(run, what)
  local msg = (L["Reset %s to default?"]):format(what or L["setting"])
...
local function buildTextColorCell(parent, args)
  ...
  lab:SetText(args.label or args.colorKey or L["Color"])
...
local function buildFontPickerCell(parent, args)
  ...
  lab:SetFont(THEME.fontPath(), THEME.sizeLabel(), "")
  lab:SetText((args and args.label) or key or L["Font"])
  ...
  local function rebuildMenu()
    ...
    for i = 1, total do
      ...
      if not b then
        ...
        b:SetBackdropColor(0.06, 0.07, 0.10, 1)
        b:SetBackdropBorderColor(0.18, 0.20, 0.26, 1)
        b:SetHeight(itemH)

        b._txt = b:CreateFontString(nil, "ARTWORK", "GameFontHighlight")
        b._txt:SetPoint("LEFT", b, "LEFT", 8, 0)

        b:SetScript("OnEnter", function(self)
          self:SetBackdropColor(0.12, 0.16, 0.22, 1)
          self:SetBackdropBorderColor(0.35, 0.60, 1.00, 1)
          self._txt:SetTextColor(1, 1, 1, 1)
        end)

        b:SetScript("OnLeave", function(self)
          self:SetBackdropColor(0.06, 0.07, 0.10, 1)
          self:SetBackdropBorderColor(0.18, 0.20, 0.26, 1)
          self._txt:SetTextColor(0.85, 0.90, 1.00, 1)
        end)
  ...
  local function openMenu()
    ...
    local optionsFrame = ddBtn:GetParent()
    while optionsFrame and not optionsFrame:IsToplevel() do
      optionsFrame = optionsFrame:GetParent()
    end

    local baseLevel = 0
    if optionsFrame and optionsFrame.GetFrameLevel then
      baseLevel = optionsFrame:GetFrameLevel()
    end

    menu:SetFrameStrata("TOOLTIP")
    menu:SetFrameLevel(baseLevel + 50)
