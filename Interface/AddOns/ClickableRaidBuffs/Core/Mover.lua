...
function ns.ToggleMover(show)
  ...
  if show then
    if not ns._mover then
      ...
      local lockBtn = makeTextButton(nil, L["Lock"], function()
      ...
      local centerHBtn = makeTextButton(lockBtn, L["Center Horizontal"], function()

        local pos = GetPos()
        TeleportMover(0, pos.y or 0)
      end)

      local centerVBtn = makeTextButton(centerHBtn, L["Center Vertical"], function()
      ...
      resetBtn:SetScript("OnClick", function()
        ...
        if not StaticPopupDialogs[KEY] then
          StaticPopupDialogs[KEY] = {
            text = L["Reset position to default?"],
