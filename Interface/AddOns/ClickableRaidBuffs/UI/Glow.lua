...
local function stopAllGlows(btn)
  if not GlowLib or not btn then
    return
  end
  GlowLib.PixelGlow_Stop(btn)
  GlowLib.AutoCastGlow_Stop(btn)
  GlowLib.ButtonGlow_Stop(btn)
...
local function ensureGlow(btn, shouldEnable, color, size)
  ...
  local d = DB()
  local style = d.glowStyle or "PIXEL"
  local rgba = { color.r, color.g, color.b, color.a or 1 }
  size = size or 50

  if btn._crb_glow_enabled then
    if btn._crb_glow_style ~= style
      or not sameRGBA(btn._crb_glow_rgba, rgba)
  ...
  elseif style == "AUTOCAST" then
    ...
    local scale = size / 50
    GlowLib.AutoCastGlow_Start(btn, rgba, N, frequency, scale)

  elseif style == "BUTTON" then
    local frequency = 0.125
    GlowLib.ButtonGlow_Start(btn, rgba, frequency)
  end
