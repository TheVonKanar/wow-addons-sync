...
local function ensureGlow(btn, shouldEnable, color, size)
  ...
  local style = (ns.GetDB() and ns.GetDB().glowStyle) or "PIXEL"
  local rgba = { color.r, color.g, color.b, color.a or 1 }

  if not shouldEnable then
    if btn._crb_glow_enabled then
      if btn._crb_glow_style == "BUTTON" and btn._crb_buttonGlowFrame then
        Glow.ButtonGlow_Stop(btn._crb_buttonGlowFrame)
      elseif btn._crb_glow_style == "AUTOCAST" then
        Glow.AutoCastGlow_Stop(btn)
      else
        Glow.PixelGlow_Stop(btn)
      end
    end
    btn._crb_glow_enabled = false
    btn._crb_glow_style = nil
  ...
  local needsRestart =
    (not btn._crb_glow_enabled)
    or (btn._crb_glow_style ~= style)
    or (btn._crb_glow_size ~= size)
    or (not btn._crb_glow_rgba)
    or (btn._crb_glow_rgba[1] ~= rgba[1])
    or (btn._crb_glow_rgba[2] ~= rgba[2])
    or (btn._crb_glow_rgba[3] ~= rgba[3])
    or ((btn._crb_glow_rgba[4] or 1) ~= rgba[4])
  ...
  elseif style == "AUTOCAST" then
    Glow.AutoCastGlow_Start(btn, rgba)

  elseif style == "BUTTON" then
    if not btn._crb_buttonGlowFrame then
      local f = CreateFrame("Frame", nil, btn)
      f:SetPoint("CENTER", btn, "CENTER")
      btn._crb_buttonGlowFrame = f
    end

    local pad = size * 0.10
    btn._crb_buttonGlowFrame:SetSize(size + pad, size + pad)

    Glow.ButtonGlow_Start(btn._crb_buttonGlowFrame, rgba)
  end

  btn._crb_glow_enabled = true
  btn._crb_glow_style = style
  ...
  for _, b in ipairs(ns.RenderFrames) do
    if b:IsShown() then
      if b._crb_glow_enabled then
        if b._crb_glow_style == "BUTTON" and b._crb_buttonGlowFrame then
          Glow.ButtonGlow_Stop(b._crb_buttonGlowFrame)
        elseif b._crb_glow_style == "AUTOCAST" then
          Glow.AutoCastGlow_Stop(b)
        else
          Glow.PixelGlow_Stop(b)
        end
        b._crb_glow_enabled = false
        b._crb_glow_style = nil
        b._crb_glow_rgba = nil
        b._crb_glow_size = nil
      end
  ...
  for idx, entry in ipairs(items) do
    ...
    if not (btn and btn:IsShown()) then
      ...
      if not btn then
        ...
        btn.textFrame:SetFrameLevel(btn:GetFrameLevel() + 30)
        ...
        btn.topText = btn.textFrame:CreateFontString(nil, "OVERLAY")
        ...
        btn.bottomText = btn.textFrame:CreateFontString(nil, "OVERLAY")
        ...
        btn.centerText = btn.textFrame:CreateFontString(nil, "OVERLAY")
        ...
        btn.cornerText = btn.textFrame:CreateFontString(nil, "OVERLAY")
        ...
        btn.timerText = btn.textFrame:CreateFontString(nil, "OVERLAY")
        ...
        local initColor = db.bottomTextColor or { r = 1, g = 1, b = 1, a = 1 }

        btn._crb_timer_font_size = db.bottomSize
        btn._crb_timer_font_path = ResolveFontPath(db.fontName)
        btn._crb_timer_outline = (db.bottomOutline ~= false) and "OUTLINE" or ""
        btn._crb_timer_color = {
          r = initColor.r,
          g = initColor.g,
          b = initColor.b,
          a = initColor.a
    ...
    local topFontPath = centerFontPath
    local topSizeScaled = math.max(1, math.floor(((db.topSize or 14) / 50) * (db.iconSize or 50) + 0.5))
    local topOutline = (db.topOutline ~= false) and "OUTLINE" or ""

    if
      btn2._crb_top_font_size ~= topSizeScaled
      or btn2._crb_top_font_path ~= topFontPath
      or btn2._crb_top_outline ~= topOutline
    then
      btn2.topText:SetFont(topFontPath, topSizeScaled, topOutline)
      btn2._crb_top_font_size = topSizeScaled
      btn2._crb_top_font_path = topFontPath
      btn2._crb_top_outline = topOutline
    end

    local topColor = db.topTextColor or { r = 1, g = 1, b = 1, a = 1 }

    if
      not btn2._crb_top_color
      or btn2._crb_top_color.r ~= topColor.r
      or btn2._crb_top_color.g ~= topColor.g
      or btn2._crb_top_color.b ~= topColor.b
      or (btn2._crb_top_color.a or 1) ~= (topColor.a or 1)
    then
      btn2.topText:SetTextColor(topColor.r, topColor.g, topColor.b, topColor.a or 1)
      btn2._crb_top_color = {
        r = topColor.r,
        g = topColor.g,
        b = topColor.b,
    ...
    local bottomFontPath = centerFontPath
    local bottomSizeScaled = math.max(1, math.floor(((db.bottomSize or 14) / 50) * (db.iconSize or 50) + 0.5))
    local bottomOutline = (db.bottomOutline ~= false) and "OUTLINE" or ""

    if
      btn2._crb_bottom_font_size ~= bottomSizeScaled
      or btn2._crb_bottom_font_path ~= bottomFontPath
      or btn2._crb_bottom_outline ~= bottomOutline
    then
      btn2.bottomText:SetFont(bottomFontPath, bottomSizeScaled, bottomOutline)
      btn2._crb_bottom_font_size = bottomSizeScaled
      btn2._crb_bottom_font_path = bottomFontPath
      btn2._crb_bottom_outline = bottomOutline
    end

    local bottomColor = db.bottomTextColor or { r = 1, g = 1, b = 1, a = 1 }

    if
      not btn2._crb_bottom_color
      or btn2._crb_bottom_color.r ~= bottomColor.r
      or btn2._crb_bottom_color.g ~= bottomColor.g
      or btn2._crb_bottom_color.b ~= bottomColor.b
      or (btn2._crb_bottom_color.a or 1) ~= (bottomColor.a or 1)
    then
      btn2.bottomText:SetTextColor(bottomColor.r, bottomColor.g, bottomColor.b, bottomColor.a or 1)
      btn2._crb_bottom_color = {
        r = bottomColor.r,
        g = bottomColor.g,
        b = bottomColor.b,
        a = bottomColor.a
    ...
    if
      btn2._crb_timer_font_size ~= bottomSizeScaled
      or btn2._crb_timer_font_path ~= bottomFontPath
      or btn2._crb_timer_outline ~= bottomOutline
    then
      btn2.timerText:SetFont(bottomFontPath, bottomSizeScaled, bottomOutline)
      btn2._crb_timer_font_size = bottomSizeScaled
      btn2._crb_timer_font_path = bottomFontPath
      btn2._crb_timer_outline = bottomOutline
    end

    if
      not btn2._crb_timer_color
      or btn2._crb_timer_color.r ~= bottomColor.r
      or btn2._crb_timer_color.g ~= bottomColor.g
      or btn2._crb_timer_color.b ~= bottomColor.b
      or (btn2._crb_timer_color.a or 1) ~= (bottomColor.a or 1)
    then
      btn2.timerText:SetTextColor(
        bottomColor.r,
        bottomColor.g,
        bottomColor.b,
        bottomColor.a or 1
      )
      btn2._crb_timer_color = {
        r = bottomColor.r,
        g = bottomColor.g,
        b = bottomColor.b,
        a = bottomColor.a
