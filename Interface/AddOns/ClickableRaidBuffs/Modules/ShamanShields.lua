...
local function Build()
  ...
  local out = ensureCat()
  local db = (ns.GetDB and ns.GetDB()) or _G.ClickableRaidBuffsDB or {}
  local esOpt = db.earthShieldOverride or {}
  local showPlayer = (esOpt.showPlayerIcon ~= false)
  local showTarget = (esOpt.showTargetIcon ~= false)
  local disableTargetWhenSolo = (esOpt.disableTargetWhenSolo == true)
  wipe(out)
...
if ES and knowSpell(ES.spellID) then
  ...
  if hasOrbit then
    ...
    local selfESid = ES.buffOnSelf or ES.buffOnOthers
  ...
  if needShow then
    ...
    if showPlayer then
      local selfESid = ES.buffOnSelf or ES.buffOnOthers
      local selfRem = selfESid and auraRem("player", selfESid, mineOnlyES)

      if not selfRem or selfRem <= tSec then
        local e = ns.copyItemData(ES)
        e.category = CAT
        e.spellID = ES.spellID
        e.id = "ES_SELF"
        e.macro = "/use [@player] " .. spellName
        e.btmLbl = PLAYER
        e.orderHint = 5

        if selfRem and selfRem > 0 and selfRem <= tSec then
          e.expireTime = GetTime() + selfRem
        end
    ...
    if showTarget then
      local suppressSolo = disableTargetWhenSolo and not (IsInRaid() or IsInGroup())

      if not suppressSolo then
