...
function MT:CreateCopyFrame()
    ...
    local function updatechars(dd)
        ...
        if MacroToolkitDB.char then
            for charName, charDb in pairs(MacroToolkitDB.char) do
                if charName ~= pname then
                    local displayName = charName
                    if charDb.classFile then
                        displayName = string.format('|Tinterface/icons/classicon_%s:16|t %s', charDb.classFile, charName)
                    end
                    ...
                    if charDb.macros then chars[charName] = displayName end
                end
            end
        end
        local order = {}
        for k in pairs(chars) do table.insert(order, k) end
        table.sort(order, function(a, b)
            -- sort to have playerClass on top, then sorted by class, then by name
            local aClass = MacroToolkitDB.char[a] and MacroToolkitDB.char[a].classFile or "Z"
            local bClass = MacroToolkitDB.char[b] and MacroToolkitDB.char[b].classFile or "Z"
            if aClass == bClass then
                return a < b
            elseif aClass == classFile then
                return true
            elseif bClass == classFile then
                return false
            else
                return aClass < bClass
            end
        end)
        dd:SetList(chars, order)
    end
